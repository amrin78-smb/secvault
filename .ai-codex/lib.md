...
`feedState` also returns **`blocked`** (v2.152.0) — the feed RAN and the publisher REFUSED it, distinct from `skipped` (which is SecVault's own decision). Excluded from the worst-state reduction like `skipped`/`missing`, NAMED in every title, and `FEEDS BLOCKED` when every feed is in that state. `FEED_STATUS_ORDER.blocked = 5`.
