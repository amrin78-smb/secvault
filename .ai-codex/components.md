...
components sharing a `base()` helper, each taking props `p` (no domain-specific props). ⛔ Never add
an icon LIBRARY; a new nav destination gets a new hand-rolled path here, because every sidebar entry
must keep a distinct glyph. Latest: `IconApplications` (a three-quarter box, v2.124.0) — the near
candidates `IconGrid`/`IconDevices`/`IconTopology` were all taken and all confusable at 16px.

## ui/CoverageBar.js

(v2.114.0) One stacked bar answering "how much of this fleet does the number above cover?" —
`segments:[{key,label,count,tone,title}]`, tone `measured|partial|gap`. ⛔ ONLY a real measurement
gets a solid fill; `gap` is the hueless hatch, because a coverage gap is neither good news (green)
nor a fault on the customer network (red). ⛔ Counts are printed beside the bar, never percentages
alone — on a 16-device fleet 12 vs 13 is visually nothing and operationally a whole firewall.
⛔ A zero total renders a sentence, NOT a full bar: painting 100%% from no data is the
failed-read-as-a-fact bug in pixel form.

## ui/Disclosure.js

(v2.114.0) Native `<details>`, so it works in a SERVER component with no client JS. ⛔ ONLY FOR
MECHANISM. The test before moving any sentence in here: if a reader who never opens it would draw a
WRONG CONCLUSION from the numbers on screen, the sentence is load-bearing and must stay outside.
If they would merely not know WHY, it belongs here. Pinned for VpnTunnelHealth by
tests/tunnelAnswer.test.js.
...
StatCard  label, value, sub, color, className, compact, icon, iconColor, iconBg, delta, layout, textColor, evidence — colored-border stat tile. `evidence` (v2.107.0) is a descriptor from lib/evidence.js; when present an EvidenceMark renders in the card's BOTTOM-RIGHT CORNER (authored inside the label element, positioned by app/globals.css). ⛔ Inline beside the label was tried first and MEASURED as a 9px height regression on all six dashboard tiles (96->105): it wrapped "Security Score" to two lines and the grid stretches every tile to the tallest. Shrinking the mark does not fix it — probed live at 15/13/12/11px, all wrapped, because the compact row label has zero headroom beside the value. The top-right corner is taken in both layouts (IconChip in stack, the number in row).
(c) Evidence (EvidenceProvider, useEvidence, EvidenceMark)  evidence, subject — THE interaction the evidence-grade rework rests on (v2.107.0): every headline figure can be asked how it knows. One GLOBAL drawer mounted once in app/(dashboard)/layout.js, not a tooltip per tile — a tooltip is sized for a sentence and cannot hold a formula, an input table and a list of measurement gaps, and could not be reached from a table cell. ⛔ EvidenceMark renders NOTHING unless isRenderableEvidence() passes, checked here rather than trusted from the call site: a mark that opens an empty drawer teaches the operator the affordance is decorative, after which they stop clicking the ones that work. ⛔ Violet (--evidence), and nothing else in the app is violet — evidence is a different AXIS from severity, not a point on it. Drawer is inert + aria-hidden when closed (a transform-offscreen panel is still focusable) and returns focus to its opener on close.
...
(c) Sidebar  version, capabilities — collapsible nav sidebar, four groups (Monitor / Inventory /
Risk / Access) plus a pinned Settings. ⛔ Every entry must keep a DISTINCT GLYPH — that, not colour,
is the per-item wayfinding cue; the active chip is always the brand accent. `/applications`
(v2.124.0) sits in **Risk, beside Segmentation**: both are declared intent re-checked against the
...
DownloadButton  href, className, style, wrapperStyle, title, fallbackName, preparingLabel, children (v2.181.0) — a download link that says something while the server is still building the file. ⛔ Renders a REAL `<a href>` and only intercepts a plain left click, so ctrl/cmd/middle-click still work and the no-JS path is unchanged. ⛔ NO PROGRESS BAR OR PERCENTAGE EVER: the server generates the whole file before sending a byte, so there is no Content-Length to measure and any bar would be a fabricated number. ⛔ The in-flight guard is a REF, not state — a fast double-click lands before React re-renders, and each export writes an audit row. ⛔ It REFUSES to save a `text/html` response: `fetch` follows redirects, and `/api/logs/export` answers a non-JSON request with a 303 to `/logs`, so without the guard the browser saves the error PAGE as `export.csv` — a corrupt file that downloads successfully is worse than a refusal. Judgement lives in `lib/downloadState.js`. Used by LogResults, ReportWorkspace, /compliance and QuickActions.

LogSearchForm  params, devices, options — a plain GET `<form>` posting to `/logs`. No client JS: the URL is the query. Fields cover time window, device, log class, action, source/destination address (bare or CIDR), port, user, rule, country, application, threat, website, raw-text contains, plus **source country** and **VPN login** (`authOutcome`, v2.179.0). ⛔ Every param the page accepts needs a field here: the form carries no hidden inputs, so one without a control is dropped by the first re-submit — which WIDENS the search while looking like the same one.
LogResults  result, deviceNames, searchParams, exportError — the results table plus the raw line behind a `<details>` toggle, since the raw message is the evidence and exists even for events nothing could parse. Carries the **Export CSV** link (v2.179.0) to `/api/logs/export`, built from the same `searchParams` the page was rendered from so the file and the screen cannot describe different searches. ⛔ A plain `<a download>`, NOT `next/link`: that prefetches, and the route writes an audit row, so a hover would record an export nobody made. `page`/`limit` are stripped. Offered on an empty result too — a header-only CSV is a real answer, and hiding the button exactly when the search was most conclusive would read as broken.
...
(c) DeviceScopeEditor  user, devices, onClose, onSaved — per-user firewall access (Settings → Users → Firewalls). ⛔ States the consequence of the CURRENT selection before saving, and confirms ONLY when saving empty, because empty means EVERY firewall. Added v2.169.0.
(c) RunAuditButton  deviceId — triggers on-demand compliance audit run
(c) ExceptionsPanel  deviceId, exceptions, availableChecks, summary, expiringWindowDays, canWrite — accepted-risk panel on the per-device compliance page (added 2026-09-22). Descriptors are computed SERVER-SIDE by `lib/engines/complianceExceptions.js` and passed in already serialised (ISO strings, no functions — the v2.120.0 blank-/reports lesson), so the state on screen is the state the server evaluated and the client cannot re-derive a different one from its own clock. ⛔ It STATES IN WORDS that the compliance score above it is computed as if none of these existed, and reports COUNTS — never a second percentage: two compliance percentages on one page, differing by a set of hand-typed labels, is the artefact this feature must not create. ⛔ FOUR STATES, FOUR TREATMENTS, and none may look like another or like an absence: `accepted` success tint + days remaining · `expiring` warn tint + "expires in N days" (its own VISIBLE state, 30-day window, so a lapse is never a surprise) · `expired` DANGER tint + "Lapsed N days ago" (a failing check with nothing covering it is a real actionable fact, not a grey footnote) · `revoked` muted, in a separate `<details>` History block naming who withdrew it. ⛔ A FIFTH, HUELESS treatment (`UnmeasuredChip`, `--unmeasured`, dashed) for what could not be measured: an unreadable expiry (state falls CLOSED to expired and says why, never red and never green), an exception over a check SecVault is not evaluating on this device, and a missing compensating control. ⛔ The form offers only checks that are ACTUALLY FAILING with no live exception — the server re-checks anyway; the list is the convenience, the route is the guarantee. ⛔ No acceptor field, in the UI or the body: `accepted_by` comes from the session. `canWrite` is `can(session, OPERATE)`, the SAME capability both routes check — a UI gate must never be stricter than the API it fronts. Sub-components (`UnmeasuredChip`/`StateChip`/`CoverageNotes`/`ExceptionItem`/`SummaryLine`) are all at MODULE TOP LEVEL; one defined inside the panel would remount on every keystroke and lose focus in the form that is its whole job.
...
(c) ApplicationBoard  initial, initialError — the whole `/applications` surface: answer sentence,
coverage summary, declare-application form, and one `ApplicationCard` per declared application.
Re-fetches `/api/applications` after every mutation rather than patching local state — a verdict is
a function of the rulebase, so a locally-patched row would be a stored verdict by the back door.
(c) ApplicationCard  entry, busy, onAddFlow, onRemoveFlow, onRemoveApp — one application: its state
chip, its flow table (PERMITTED / USED / finding as SEPARATE columns), and the add-flow form.
(c) ApplicationForms  — `DeclareApplicationForm`, `AddFlowForm`.
(c) CoverageSummary  orphans, coverage, windowDays — ⛔ renders the orphan figure as COVERAGE, never
as a finding and never with the word "unused".
(c) FlowChips  — `Chip`, `PermittedCell`, `UsedCell`, `FindingCell`, `AppStateChip`.
flowVocabulary.js  — `PERMITTED_CHIP`/`USED_CHIP`/`FINDING_CHIP`/`APP_STATE_CHIP` + label helpers.
⛔ ONE vocabulary for the whole feature: two files deciding what `partially_permitted` looks like
would eventually disagree. ⛔ Every unmeasurable state is HUELESS + `--hatch` (`UNMEASURED`), never
a reassuring grey and never a severity hue, and `UNKNOWN_CHIP` catches a state the vocabulary does
not recognise rather than rendering it as nothing.
applicationsAnswer.js  — `rollUp`, `buildApplicationsAnswer`, `applicationsEvidence`. ⛔ Refuses
`tone:'ok'` while anything is unverified, invalid or unevaluated.
