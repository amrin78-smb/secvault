...
StatCard  label, value, sub, color, className, compact, icon, iconColor, iconBg, delta, layout, textColor, evidence — colored-border stat tile. `evidence` (v2.107.0) is a descriptor from lib/evidence.js; when present an EvidenceMark renders BESIDE THE LABEL in both layouts. ⛔ Beside the label deliberately, not the value or the corner: the corner is the IconChip in stack layout and the NUMBER in row layout, so the label is the only anchor both layouts share and the affordance lands in the same relative place on every tile.
