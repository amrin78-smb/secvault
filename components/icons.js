// Inline SVG icons (no external icon dependency) — matches the NocVault suite's
// shared Feather-compatible convention: viewBox 0 0 24 24, stroke currentColor,
// strokeWidth 2, round caps/joins. Every icon accepts and spreads extra props
// (width/height/style/className) so callers can size/color them per use site.
...
export function IconChevronDown(p) {
  return (
    <svg {...base(p)} strokeWidth={p?.strokeWidth ?? 1.5} viewBox="0 0 12 12">
      <path d="M2 4l4 4 4-4" />
    </svg>
  );
}
...
// Network/nodes glyph for the Topology page (sidebar nav + any future use) --
// one central node with lines out to three satellite nodes.
export function IconTopology(p) {
  return (
    <svg {...base(p)}>
      ...
      <line x1="6" y1="6.3" x2="10.3" y2="10.3" />
      <line x1="18" y1="6.3" x2="13.7" y2="10.3" />
