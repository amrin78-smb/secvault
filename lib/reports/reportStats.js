  ...
  try {
    const { rows } = await pool.query(`
      SELECT
        ...
        (SELECT count(*)::int FROM segmentation_intents)                                  AS seg_intents,
        (SELECT count(*)::int FROM device_licenses
          WHERE expires_at IS NOT NULL AND expires_at <= now() + interval '60 days')       AS lic_expiring,
        -- ⛔ "unknown expiry" EXCLUDES 'Never' (perpetual) and 'n/a' (an
        -- entitlement the device does not hold). Counting those as unknown
        -- would put 44 live Fortinet rows into a renewal worklist that has no
        -- renewals in it.
        (SELECT count(*)::int FROM device_licenses
          WHERE expires_at IS NULL
            AND (expires_raw IS NULL OR expires_raw NOT IN ('Never', 'n/a')))              AS lic_unknown,
        (SELECT count(*)::int FROM config_diffs
          WHERE detected_at > now() - interval '30 days')                                  AS diffs_30d,
        (SELECT count(*)::int FROM config_diffs
          WHERE detected_at > now() - interval '30 days' AND acknowledged_at IS NULL)       AS diffs_unack,
        (SELECT count(*)::int FROM vpn_sessions
          WHERE last_seen_at > now() - interval '90 days')                                 AS vpn_sessions_90d,
        (SELECT count(DISTINCT username)::int FROM vpn_sessions
          WHERE last_seen_at > now() - interval '90 days')                                 AS vpn_users_90d
    `);
    return rows[0] || null;
  } catch (_err) {
    return null;
...
function tilesFor(reportId, s) {
  ...
  switch (reportId) {
    ...
    case 'segmentation-posture':
      return [
        { label: 'Boundaries declared', value: dash(s.seg_intents) },
        { label: 'Firewalls', value: dash(s.devices) },
        // ⛔ The rules that cannot report usage are what make a boundary
        // UNVERIFIED rather than clean, so this tile is the report's real
        // subject, not a footnote — and it is hueless because a coverage gap
        // is neither good news nor bad.
        { label: 'Rules with no usage data', value: dash(s.rules_unmeasured), tone: 'unmeasured' },
      ];
    case 'fleet-lifecycle':
      return [
        { label: 'Expiring within 60 days', value: dash(s.lic_expiring), tone: s.lic_expiring > 0 ? 'warn' : 'ok' },
        { label: 'Unknown expiry', value: dash(s.lic_unknown), tone: 'unmeasured' },
        { label: 'Firewalls', value: dash(s.devices) },
      ];
    case 'change-audit':
      return [
        { label: 'Changes in 30 days', value: dash(s.diffs_30d) },
        { label: 'Unacknowledged', value: dash(s.diffs_unack), tone: s.diffs_unack > 0 ? 'warn' : 'ok' },
        { label: 'Firewalls', value: dash(s.devices) },
      ];
    case 'vpn-access-review':
      return [
        { label: 'Sessions in 90 days', value: dash(s.vpn_sessions_90d) },
        { label: 'Distinct users', value: dash(s.vpn_users_90d) },
        { label: 'Firewalls', value: dash(s.devices) },
      ];
    default:
      return null;
