...
async function buildChangeAuditData(pool, options = {}) {
  ...
  let activityTotal = 0;
  try {
    // ⛔ `count(*) OVER ()` IS THE UNCAPPED TOTAL, AND IT IS NOT A NICETY.
    // A window function is evaluated over the whole result of the WHERE before
    // LIMIT is applied, so this is the number of operator actions in the window
    // rather than the number this query was allowed to return. `activityTotal`
    // used to be `activityRows.length`, i.e. the CAP — so the section heading
    // printed "(120)" over a list of 120 and no truncation note could ever
    // fire, because the shown count and the "total" were the same number by
    // construction. Live on the reference fleet at the catalogue's own 365-day
    // window that read 120 where the truth is 208: a truncated list presented
    // as complete, which is the one failure mode a capped table has. Every
    // other table in this file already discloses its cap.
    ...
    // ⛔ The fallback can only ever be the rows actually read, never fewer — an
    // unreadable total must not be able to UNDERSTATE the population and
    // manufacture the "nothing was dropped" case.
    const reportedTotal = activityRows.length > 0 ? Number(activityRows[0].total_in_window) : 0;
    activityTotal = Number.isFinite(reportedTotal) && reportedTotal >= activityRows.length
      ? reportedTotal
      : activityRows.length;
  } catch (err) {
    sectionErrors.push({
...
function renderReviewTrail(doc, layout, data) {
  ...
  // ⛔ NEVER A SILENT CAP, here as everywhere else in this document. This table
  // is the evidence about the REVIEWERS on a change-control audit; a reader who
  // works it to the bottom and believes they have seen every operator action has
  // been misled, not merely shown less.
  const note = truncationNote(activity.length, totals.activityRows, 'operator actions');
  if (note) paragraph(doc, layout, note, MUTED);
