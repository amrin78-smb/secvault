...
async function buildVpnAccessReviewData(pool, options = {}) {
  ...
  // ⛔ NO `active` FILTER HERE, AND THAT IS DELIBERATE - unlike every sibling
  // report. An access review asks who connected during a window; a gateway
  // decommissioned part-way through that window is exactly the one whose
  // sessions an auditor wants to see, and dropping it would silently shorten
  // the review. Inactive gateways are LABELLED instead ("This firewall is
  // currently inactive in inventory", buildGatewayTable) rather than omitted.
  // Recorded because the absence of the filter reads as an oversight beside
  // the other reports, and was raised as one during the 2026-09-15 sweep.
