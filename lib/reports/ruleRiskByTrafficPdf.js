...
function renderRuleRiskPdf(d) {
  ...
  drawCover(doc, {
    ...
    summary: [
      {
        value: d.concentration.rules === 0 ? NOT_MEASURED : num(d.concentration.rules),
        label: 'Rules carry most traffic',
        color: d.concentration.rules === 0 ? UNMEASURED : undefined,
      },
      {
        // ⛔ HUELESS WHEN NOTHING WAS RANKED. A green 0 here announced "none of
        // your busiest rules has a problem" on a report where nothing could be
        // measured at all — an all-clear assembled from an absence of evidence.
        value: d.concentration.rules === 0 ? NOT_MEASURED : num(d.busiestWithRisk.length),
        label: 'of those, with a finding',
        color: d.concentration.rules === 0
          ? UNMEASURED
          : (d.busiestWithRisk.length > 0 ? STATUS_RED : GREEN),
      },
      {
  ...
  if (d.busiest.length === 0) {
    ...
    drawBarChart(doc, layout, d.busiest.slice(0, 15).map((r) => ({
      ...
      labelW: 210,
      max: 15,
      // ⛔ SAY WHEN THE CHART IS SHORTER THAN THE COUNT BESIDE IT. The cover
      // chip and the headline both say 21; the chart drew 15 and looked
      // complete, and the donut under it was computed over all 21 — so the two
      // pictures did not reconcile with each other or with the text.
      caption: `Logged hits over ${d.windowDays} days`
        + (d.busiest.length > 15 ? `, for the top 15 of ${d.busiest.length} rules shown here` : '')
        + '. Red bars are rules that ALSO carry a rule-hygiene finding \u2014 those are the ones worth '
        + 'looking at first, because whatever is wrong with them is wrong on the busiest paths in '
        + 'the estate.',
    });
  ...
  if (d.cleanupMeasurable === 0) {
    ...
    drawTable(doc, {
      columns: [
        ...
        { key: 'f', label: 'Finding', width: 105 },
        { key: 'v', label: 'Firewall counter', width: 100 },
      ],
      rows: d.cleanupCandidates.slice(0, 40).map((r) => ({
        r: r.name,
        d: r.deviceName,
        f: `${r.worst.type} (${r.worst.severity})`,
        // ⛔ THREE STATES, BECAUSE TWO OF THEM ARE OPPOSITE FACTS. This used to
        // print an em dash for BOTH "the firewall gave no counter" and "the
        // firewall's counter says millions of hits" — live, 349 of 565 rows had
        // a non-zero lifetime counter and row one of the printed table was a
        // rule reporting 11,272,804 hits, shown as a dash on the page whose
        // next step is deletion. An operator reading that dash as "no data"
        // deletes a rule the device itself says is busy.
        v: r.lifetimeHits === null
          ? NOT_MEASURED
          : (r.lifetimeHits === 0 ? 'agrees: 0' : `SAYS ${num(r.lifetimeHits)}`),
      })),
    }, layout, { continueOnPage: true });
    ...
    paragraph(doc, layout,
      'These carried no traffic in this window AND carry a risk finding, which makes them the '
      + 'strongest removal candidates in the estate. SecVault does not remove anything: raise a '
      + 'change request and it will confirm against the next collected ruleset whether the rule '
      + 'actually went.', MUTED);
    // ⛔ THE LAST COLUMN IS A CONTRADICTION, NOT A FOOTNOTE. A rule can show no
    // logged traffic in the window while the firewall's own lifetime counter
    // reports millions — the two measure different spans, and the disagreement
    // is the single most useful signal on this page.
    paragraph(doc, layout,
      'The last column is the firewall\u2019s OWN counter, which covers everything since that counter '
      + 'was last reset \u2014 a different and much longer span than this window. "agrees: 0" is the '
      + 'strongest case for removal. A number there means the firewall has seen this rule used at '
      + 'some point and it should be investigated before anything is removed. An em dash means the '
      + 'firewall supplies no counter at all, so there is no second opinion.', MUTED);
  }
