...
const REPORTS = [
  ...
  },
  {
    ...
    icon: 'IconDocument',
    contents: [
      'Score across PCI DSS, ISO 27001, CIS v8, NIST and SANS',
      'Per-firewall compliance scores (fleet scope)',
      'Failing and warning findings appendix',
      'Checks SecVault cannot ask of a device, listed but not scored',
    ],
    name: 'Compliance Report',
    summary:
      'Compliance across PCI DSS, ISO 27001, CIS v8, NIST and SANS — for one firewall or '
      + 'the whole fleet, optionally narrowed to a single standard. Checks SecVault cannot '
      + 'ask of a device are excluded from the score and listed separately.',
    scope: SCOPES.FLEET,
    // ⛔ ONE FIREWALL OR THE WHOLE FLEET, from the same entry — the same call
    // Traffic Activity makes, whose own comment says it scopes by device
    // "exactly as the compliance report treats a standard". The pattern went
    // one way and never came back; this is the other half.
    //
    // ⛔ Scoping to a device is NOT a filter applied to the fleet document. The
    // engine re-reads every coverage denominator at that scope — in particular
    // `applicable` becomes "checks that can run on THIS vendor", since a
    // Fortinet-only check can never run on a Palo Alto. Reusing the fleet
    // denominator under one firewall's name would put back exactly the
    // overclaim v2.163.0 removed.
    optionalDevice: true,
    capability: OPERATE,
    formats: ['pdf'],
