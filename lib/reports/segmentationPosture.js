...
/**
 * The colour of one of the cover's two VIOLATION chips.
 *
 * ⛔ GREEN MEANS "NO DECLARED BOUNDARY IS CROSSED AT ALL", NEVER "NOT THIS
 * BUCKET". There are THREE violation verdicts and the cover has room for two
 * chips, so a zero in one of them painted green is an all-clear over the third.
 * Measured on the reference fleet: one `violation_permitted` breach, and a
 * cover carrying two green zeros with the violation mentioned nowhere on it,
 * while page two led with it. `violation_permitted` is the one an operator
 * should go and close, so hiding it behind green is the wrong direction twice
 * over.
 *
 * Pure and exported so the rule is pinned by a test rather than inspected by
 * eye in a rendered PDF.
 */
function violationChipColor(count, totalViolations, hotColor) {
  if (Number(count) > 0) return hotColor;
  return Number(totalViolations) > 0 ? UNMEASURED : GREEN;
}
    ...
    doc,
    {
      ...
      meta: [
        ...
        ['Zone pairs declared', scope === 'device'
          ...
          : num(totals.intents)],
        // ⛔ THE HEADLINE FINDING, ON THE COVER. The two violation chips below
        // cover only the `in use` and `not measurable` verdicts; a fleet whose
        // only breaches are `permitted, no traffic recorded` produced a cover
        // that never mentioned a violation at all while page two led with one.
        // This row is the total across all three verdicts, so the cover can no
        // longer be read as a clean estate when it is not.
        ['Declared boundaries a rule permits', num(totals.violations)],
      ...
      summary: [
        { label: 'Zone pairs declared', value: num(totals.intents), color: NAVY },
        {
          ...
          // See violationChipColor(): a zero in one violation bucket is not a
          // clean estate, and the cover is the only page guaranteed to be read.
          color: violationChipColor(totals.violationsActive, totals.violations, STATUS_RED),
        },
        {
          label: 'Violations - usage not measurable',
          value: num(totals.violationsUnverified),
          color: violationChipColor(totals.violationsUnverified, totals.violations, ORANGE),
        },
