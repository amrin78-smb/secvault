...
async function buildRuleRiskData(pool, options = {}) {
  ...
  const cleanupCandidates = active
    .filter((r) => r.windowHits === 0 && r.evidence === 'measured-zero' && r.findings.length > 0)
    .map((r) => ({
      ...r,
      worst: r.findings.slice().sort((a, b) => rankOf(a.severity) - rankOf(b.severity))[0],
  ...
  const covRows = [...coverageMap.values()];
  ...
  const windowCoverage = {
    devicesCovered,
    devicesWithLogs: covRows.length,
    historyHours,
    windowHours,
...
function headlineSentence(d) {
  ...
  const parts = [
    `${c.rules} rule${c.rules === 1 ? '' : 's'} carry `
  ...
  parts.push(
    d.busiestWithRisk.length > 0
      ? `${d.busiestWithRisk.length} of them carry a hygiene finding`
