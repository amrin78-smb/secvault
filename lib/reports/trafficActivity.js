...
async function buildTrafficActivityData(pool, options = {}) {
  ...
  const [timeline, actions, hosts, apps, protocols, blocked, rules, coverage] = await Promise.all([
    ...
    // ⛔ null, NOT []. An empty array is a real answer ("no active firewalls"),
    // and coverageSentence turned that into a confident affirmative printed
    // above five million events. A failed read has no answer at all.
    guard('fleet coverage', () => tw.windowCoverage(pool, window), null),
  ]);
  ...
  // A failed coverage read is carried forward as null all the way to the
  // sentence, rather than being flattened into "nothing to report on".
  const coverageFailed = coverage === null;
  const coverageRows = coverage || [];
  const scoped = deviceId ? coverageRows.filter((c) => c.deviceId === deviceId) : coverageRows;
  ...
  return {
    ...
    // is arbitrary.
    // ⛔ ANCHOR ON THE LAST HOUR THE QUERY ACTUALLY RETURNED, NOT ON `to`.
    // The window is half-open [from, to), so the bucket AT `to` is deliberately
    // excluded — but fillHourlyGaps anchors on floor(anchor/hour), so passing
    // `to` made its last bar that excluded bucket and silently dropped the
    // first real one. Live that cost 1.03M events (4.3% of a 24h window): the
    // chart no longer reconciled with the Events figure one page earlier, and
    // the caption asserted "1 of 24 hours produced no rows at all — that is
    // missing data, not quiet traffic" about an hour that was never requested.
    // One millisecond back lands on the last INCLUDED bucket.
    timeline: fillHourlyGaps(timeline, window.hours, window.to.getTime() - 1),
...
function coverageSentence(d) {
  const c = d.coverage;
  // ⛔ UNREADABLE IS NOT EMPTY. These are opposite facts and the second one is
  // an all-clear: "nothing to report on" was printed above 5,000,000 counted
  // events when the coverage query threw.
  if (c.failed) {
    return 'The list of firewalls could not be read, so this report cannot say how much of the '
      + 'estate the figures below cover. They are counted from whatever did reach SecVault and '
      + 'may describe a fraction of the fleet.';
  }
...
function renderTrafficActivityPdf(d) {
  ...
  drawBarChart(doc, layout, d.rules.slice(0, 10).map((r) => ({
    ...
    value: r.hits,
    // ⛔ classifyAction, NOT A REGEX. This was a FIFTH deny list in the file
    // whose own header names the cost of the previous four, and it was wrong on
    // this fleet today: `reset-both` is a Palo Alto BLOCK and matched none of
    // deny/drop/block, so 735,000 blocked hits a day — including three rules
    // inside the charted top 15, one at 562,195 hits — were painted in the
    // ALLOW colour under a caption reading "Red bars are deny rules".
    color: classifyAction(r.action) === 'blocked' ? STATUS_RED : ACCENT,
