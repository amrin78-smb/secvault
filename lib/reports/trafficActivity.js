...
async function buildTrafficActivityData(pool, options = {}) {
  ...
  const [timeline, actions, hosts, apps, protocols, blocked, rules, coverage] = await Promise.all([
    guard('log volume', () => tw.timeline(pool, window, deviceId), []),
    guard('session outcomes', () => tw.actions(pool, window, deviceId), []),
    guard('top sources', () => tw.topHosts(pool, window, deviceId, 15), []),
    guard('top applications', () => tw.topApplications(pool, window, deviceId, 15), []),
    guard('protocols', () => tw.protocols(pool, window, deviceId), []),
    guard('blocked destinations', () => tw.topBlocked(pool, window, deviceId, 15), []),
    guard('busiest rules', () => tw.topRules(pool, window, deviceId, 15), []),
    guard('fleet coverage', () => tw.coverage(pool, window), []),
  ]);
