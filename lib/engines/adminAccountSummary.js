...
// ---------------------------------------------------------------------------
// Check Point — configParsed.administrators, added 2026-07-30 by
// lib/adapters/checkpoint/index.js's getConfig() via parser.js's
// parseAdministratorObjects() (show-administrators Management API call).
// Doc-derived (no live management server), field names cross-referenced from
// two independently-agreeing public sources -- see that function's own
// header comment. `permissions_profile` is a profile NAME (e.g. Check
// Point's built-in "Super User" profile), passed straight through as
// `privilege` -- KNOWN GAP: SUPERUSER_PATTERN below requires "super" and
// "admin"/"user" with no space between them (superadmin/super_admin/
// superuser), so a literal "Super User" profile name will NOT be counted by
// countSuperusers() today. Not fixed here deliberately -- widening the
// shared pattern to guess at a value that has never been confirmed against
// a real management server would compound one unverified assumption on top
// of another; left as an honest, documented limitation instead (matches this
// file's own header comment: the heuristic "won't catch every vendor's own
// naming... and isn't meant to").
// ---------------------------------------------------------------------------
function summarizeCheckPoint(configParsed) {
  const admins = configParsed && Array.isArray(configParsed.administrators) ? configParsed.administrators : null;
  if (!admins) return { accounts: [] };
  ...
  const accounts = admins.map((entry) => {
    ...
    return {
      username: e.name || null,
      privilege: typeof e.permissions_profile === 'string' ? e.permissions_profile : null,
      // Not modeled: show-administrators' documented field set (see
      // parseAdministratorObjects()) has no 2FA/MFA or source-IP-restriction
      // field -- "not modeled here", not "confirmed disabled". Never coerce
      // an unmodeled fact to false (CLAUDE.md's tri-state discipline).
      twoFactorEnabled: null,
      sourceRestricted: null,
    };
  });
...
// ---------------------------------------------------------------------------
// Forcepoint — configParsed.smc_administrators, added 2026-07-30 by
// lib/adapters/forcepoint/index.js's getConfig() via parser.js's
// summarizeAdministrators() (SMC-managed, server-wide administrator list,
// doc-derived from the smc-python client's AdminUser wrapper, never
// live-verified). Unlike Check Point's profile-NAME privilege field, SMC's
// `superuser` field is already a plain boolean -- mapped straight to the
// literal strings 'superuser'/'standard' so SUPERUSER_PATTERN's existing
// `/^super(_?admin|user)$/i` matches it with no widening needed.
// ---------------------------------------------------------------------------
function summarizeForcepoint(configParsed) {
  const admins = configParsed && Array.isArray(configParsed.smc_administrators) ? configParsed.smc_administrators : null;
  if (!admins) return { accounts: [] };
  ...
  const accounts = admins.map((entry) => {
    ...
    return {
      username: e.name || null,
      privilege: e.superuser === true ? 'superuser' : e.superuser === false ? 'standard' : null,
      twoFactorEnabled: null,
      sourceRestricted: null,
    };
  });
...
// ---------------------------------------------------------------------------
// Dispatch — Sangfor collects no admin/user account data at all today
// (researched 2026-07-30: no documented CLI surface could be confirmed with
// reasonable confidence — see .ai-codex/connectors.md's Sangfor section).
// Falls through to `supported: false` below, same as any vendor not listed
// here -- distinct from `supported: true, accounts: []` ("this vendor IS
// modeled, this device just has none collected yet"), which every other
// vendor below can still legitimately return.
// ---------------------------------------------------------------------------
const SUMMARIZERS = {
  fortinet: summarizeFortinet,
  paloalto: summarizePaloAlto,
  cisco_asa: summarizeCiscoAsa,
  checkpoint: summarizeCheckPoint,
