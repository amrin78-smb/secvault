...
// Extracts the per-entry NAMES from a PAN-OS schema node that holds a set of
// named entries, tolerant of BOTH transports' shapes: XML/API renders repeated
// `<entry name="...">` as `{ entry: [{'@_name':...}] }` (or a single object);
// the SSH brace tree renders the same node as `{ <name>: {...sub-block...} }`
// (the block key IS the name — same convention ruleFromBraceEntry() relies on).
// Best-effort: returns [] for anything it can't confidently read as named
// entries. NOT a config-shape guess that could mislabel — an empty list just
// means "no names surfaced", and the caller degrades to the prior display.
function extractEntryNames(node) {
  if (!node || typeof node !== 'object') return [];
  if (node.entry !== undefined) {
    const arr = Array.isArray(node.entry) ? node.entry : [node.entry];
    return arr
      .map((e) => (e && typeof e === 'object' ? e['@_name'] || e.name : null))
      .filter((n) => typeof n === 'string' && n.length > 0);
  }
  // SSH brace shape: object keys are the entry names (only count keys whose
  // value is itself a sub-object, i.e. a named block, not a scalar setting).
  return Object.keys(node).filter((k) => node[k] && typeof node[k] === 'object' && !Array.isArray(node[k]));
}
...
function summarizePaloAlto(configParsed) {
  ...
  if (found) {
    // Best-effort inventory: surface the configured GlobalProtect GATEWAY and
    // PORTAL names (what an admin actually recognizes), instead of only the
    // opaque "found in config at ..." path. Searched WITHIN the matched
    // GlobalProtect node for child keys containing 'gateway'/'portal', then
    // that child's entry names — both transports via extractEntryNames(). Empty
    // arrays when nothing surfaces (the UI then just shows the plain
    // Configured state, exactly as before).
    const gwNode = deepFindKeyByPattern(found.value, /gateway/i, '', 0);
    const portalNode = deepFindKeyByPattern(found.value, /portal/i, '', 0);
    return {
      supported: true,
      hasConfig: true,
      foundAt: found.path,
      gateways: gwNode ? extractEntryNames(gwNode.value) : [],
      portals: portalNode ? extractEntryNames(portalNode.value) : [],
      fields: found.value,
    };
  }
  return { supported: true, hasConfig: false, fields: {} };
...
// ---------------------------------------------------------------------------
// Forcepoint — configParsed.smc_vpn_gateway_configured is a TRI-STATE
// (true/false/null), added 2026-07-30 by lib/adapters/forcepoint/index.js's
// getConfig() via parser.js's detectVpnGatewayConfigured() (reads the SMC
// engine element's `internal_gateway` field — doc-derived, never
// live-verified, same standing caveat as every other Forcepoint field
// mapping). The key is entirely ABSENT on any config_parsed row collected
// before this feature existed (old rows never had this field computed at
// all) — distinct from the key being PRESENT with value `null` ("collected
// under the new code, but undetected"). Checking `=== undefined` is what
// tells these two apart; never coerce either one to `false`.
// ---------------------------------------------------------------------------
function summarizeForcepoint(configParsed) {
  const configured = configParsed ? configParsed.smc_vpn_gateway_configured : undefined;
  ...
  return {
    supported: true,
    hasConfig: true,
    enabled: configured, // true | false | null
    fields: { smc_vpn_gateway_configured: configured },
    lowConfidence: true, // always flagged for this vendor -- doc-derived, never live-verified (see header comment)
  };
}

// ---------------------------------------------------------------------------
// Dispatch — any vendor not listed here (e.g. Check Point — researched
// 2026-07-30, its Management API has no VPN-community schema this codebase
// could verify field names for) has no VPN config collection at all yet:
// `supported: false` renders distinctly in the UI from `supported: true,
// hasConfig: false` ("collected, and it's genuinely empty") -- the two are
...
const SUMMARIZERS = {
  ...
  forcepoint: summarizeForcepoint,
};
