...
const { parseCidrOrIp, rangeContains, cidrToRange } = require('./cidrUtils');
...
/**
 * Parses one device_interfaces.ip_address ("10.1.1.1/24", or a bare literal)
 * into BOTH halves of what an interface address actually is: the SUBNET it
 * sits on, and the HOST address it holds on that subnet.
 *
 * ⛔ parseCidrOrIp() masks host bits off by design (see its own comment), so
 * parsing once throws the host address away. buildAdjacencyGraph() used to do
 * exactly that and then compare only the masked ranges, which is why an
 * interface could be declared adjacent to another holding the IDENTICAL
 * address -- an IP CONFLICT, physically impossible on one link, and yet the
 * strongest "evidence" the old test could see. Both values are kept here so
 * the predicate below can require a shared subnet AND two distinct hosts.
 *
 * ⛔ `null` covers "this vendor reported no address", which on the live fleet
 * is the LITERAL STRING 'N/A' in 95 of 241 rows (39%, 10 devices) -- a
 * "no address" answer recorded as if it were an address. It must never become
 * an adjacency input. parseCidrOrIp already rejects it (and stays silent: it
 * is not IP-shaped, so no warning is emitted), so no special case is needed
 * here -- but the collector-side fix, storing NULL instead of a sentinel
 * string, still belongs upstream.
 *
 * @param {*} rawValue - device_interfaces.ip_address (TEXT, nullable).
 * @returns {{network:number, prefixLen:number, host:number} | null}
 */
function parseInterfaceAddress(rawValue) {
  if (typeof rawValue !== 'string') return null;
  const trimmed = rawValue.trim();
  if (trimmed === '') return null;

  // Parse the WHOLE value first: if it is malformed, cidrUtils warns exactly
  // once here, and the bare-address parse below is never reached with the
  // same bad input (which would warn a second time about one row).
  const cidr = parseCidrOrIp(trimmed);
  if (cidr === null) return null;

  const slash = trimmed.indexOf('/');
  const bare = slash === -1 ? trimmed : trimmed.slice(0, slash);
  const hostOnly = parseCidrOrIp(bare); // guaranteed to parse -- `trimmed` did
  if (hostOnly === null || hostOnly.prefixLen !== 32) return null;

  return { network: cidr.network, prefixLen: cidr.prefixLen, host: hostOnly.network };
}

/**
 * The adjacency predicate: two interfaces on DIFFERENT devices are on the
 * same link when they agree on the subnet and disagree on the host.
 *
 *   different device  AND  same masked network  AND  equal prefixLen
 *                     AND  different host address
 *
 * ⛔ Each clause removes a class of link that was measured FALSE on the live
 * fleet (68 cross-device interface pairs, of which only 11 were plausible):
 *
 *  - equal prefixLen  -- a /30 point-to-point link nested inside an unrelated
 *    device's /24 overlaps it, but the two devices are not on one link. 47 of
 *    the 68 pairs were this shape.
 *  - different host   -- `10.254.254.1/24` was held SIMULTANEOUSLY by four
 *    devices (IDC FW, SMT, TUM(TUTH1), TUFF(TUTH3)) and `192.168.100.254/24`
 *    by two more. That is an IP conflict or, far more often, the same default
 *    template deployed to unconnected branches. 10 of the 68 pairs were this.
 *
 * The cost of getting this wrong is not cosmetic: simulateMultiHopPath() takes
 * neighbors[0], so a phantom neighbour makes the simulator evaluate the WRONG
 * FIREWALL'S RULESET and return a confident allow/deny for a hop that does not
 * exist -- and the Fleet Map's first-wins pair dedupe let a phantom edge claim
 * the pair key and DISPLACE the real tunnel link from the diagram.
 *
 * ⛔ Symmetric with the other direction of error: this must not be tightened
 * further (e.g. by demanding a /30, or by rejecting a subnet with more than
 * two speakers). Deleting a real edge silently truncates a path query, which
 * is the same wrong answer wearing different clothes.
 */
function interfacesAreAdjacent(a, b) {
  if (a.deviceId === b.deviceId) return false; // only cross-device links count
...
/**
 * Two interfaces on DIFFERENT devices that sit on the SAME subnet holding
 * DIFFERENT host addresses are "adjacent" -- they share a broadcast
 * domain/link, so traffic can cross from one device to the other there.
 * Built once per query over every device's device_interfaces rows. See
 * interfacesAreAdjacent() above for the exact predicate and why each clause
 * of it exists.
 *
...
function buildAdjacencyGraph(interfacesByDevice) {
  ...
  for (const [deviceId, interfaces] of toDeviceMap(interfacesByDevice)) {
    for (const iface of Array.isArray(interfaces) ? interfaces : []) {
      ...
      const addr = parseInterfaceAddress(iface.ip_address);
      if (addr === null) continue; // unparseable/missing/'N/A' -- skip, never throw (per spec)
      entries.push({
        deviceId,
  ...
  for (let i = 0; i < entries.length; i++) {
    for (let j = i + 1; j < entries.length; j++) {
      const a = entries[i];
      const b = entries[j];
      if (!interfacesAreAdjacent(a, b)) continue;
      link(a, b);
      link(b, a);
    }
  }
...
function buildVpnEdges(deviceList, vpnTunnelsByDevice, ifacesByDevice, seenPairs) {
  ...
  for (const d of deviceList) {
    ...
    for (const iface of Array.isArray(ifaces) ? ifaces : []) {
      // Match buildAdjacencyGraph()'s rule: a disabled interface carries no live
      // traffic, so a tunnel whose peer resolves to a shut-down interface is not
      // a real link and must not draw an edge.
