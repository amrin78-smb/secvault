 ...
 *   edges: {sourceDeviceId:string, sourceInterface:string, targetDeviceId:string, targetInterface:string}[],
 * }}
 */
function buildFleetTopologyGraph(devices, interfacesByDevice) {
  const ifacesByDevice = toDeviceMap(interfacesByDevice);
  const graph = buildAdjacencyGraph(ifacesByDevice);

  const nodes = (Array.isArray(devices) ? devices : []).map((d) => {
  ...
  for (const [key, neighbors] of graph) {
    ...
    for (const neighbor of neighbors) {
      // Canonical pair key (sorted device ids) so A<->B collapses to one
      // edge regardless of which direction buildAdjacencyGraph recorded
      // first.
      const pairKey = [deviceId, neighbor.deviceId].sort().join('|');
