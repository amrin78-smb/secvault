...
//
// ─────────────────────────────────────────────────────────────────────────
// WHAT CHANGED UNDER YOUR FEET (2026-09-09): device_configs rows are now
// DEDUPED AT WRITE TIME by lib/adapters/index.js's storeConfigSnapshot(). An
// unchanged pull no longer inserts a row — it UPDATEs the existing one, moving
// `collected_at` forward, leaving `first_collected_at` alone and incrementing
// `observation_count`. Read that function's header before changing anything
// here. Three consequences for this engine, none of which weakens a protection:
//
//   - THE AGE TEST GOT MORE PROTECTIVE, NOT LESS. Every query here ranks and
//     ages rows by `collected_at`, which still means "most recently observed".
//     A configuration that is still live on the device therefore keeps
//     refreshing its own timestamp and can never age out from under a device
//     that still has it. Previously it aged out and was re-created daily.
//
//   - PROTECTION 3 (min-keep) GOT DEEPER. `MIN_KEEP_CONFIGS` = 10 used to mean
//     "the last 10 pulls", which on this fleet was ~10 near-identical copies of
//     one configuration, i.e. ten days of history. It now means the last 10
//     DISTINCT configurations — genuinely ten changes of history, which is a
//     much stronger floor for the same row count. The number was NOT retuned:
//     it is a safety floor, and the floor just got higher on its own.
//
//   - ONE DELETED ROW CAN NOW DISCARD MANY OBSERVATIONS. A row carries
//     `observation_count`, so "deleted 7 rows" may mean 200 collections' worth
//     of evidence. This engine's whole purpose is to let an operator tell
//     retention from data loss, so the summary reports observations alongside
//     rows rather than letting the row count quietly understate the loss.
//
// Protections 1, 2 and 4 are untouched: is_baseline is never deleted (and is
// also never the dedupe target, so it never absorbs observations), the newest
// row per device is never deleted, and config_backups is not deduped at all.
//
...
function emptyTableSummary(retentionDays, minKeepPerDevice) {
  return {
    ...
    // ⛔ null, not 0: "this table does not carry an observation count" is not
    // "zero observations". config_backups genuinely has no such column (it is
    // not deduped), and a pre-migration device_configs would not either. Same
    // unmeasured-vs-zero discipline as hit_count.
    observations: null,
    perDevice: [],
    error: null,
  };
...
const CLASSIFY_DEVICE_CONFIGS_SQL = `
  ...
  SELECT
    ...
    END AS bucket,
    count(*)::int AS rows,
    -- Rows are deduped at write time, so one row can stand for many successful
    -- collections. Reporting rows alone would understate what a removal costs.
    -- (Prose in a CLASSIFY query must avoid the bare destructive verb:
    -- tests/configRetention.test.js proves dryRun issues no statement matching
    -- it, and that assertion reads comments too.)
...
const DELETE_DEVICE_CONFIGS_SQL = `
  ...
  RETURNING t.device_id, t.observation_count`;
...
async function retainTable(pool, opts) {
  ...
  try {
    ...
    // Observations are only reported when the query actually returned them —
    // an absent column leaves `observations` null (unknown) rather than 0.
    const obs = { total: 0, wouldDelete: 0, deleted: null };
    let sawObservations = false;
    for (const row of classified.rows) {
      ...
      // pg returns bigint as a string; Number() is safe at these magnitudes.
      const o = Number(row.observations);
      if (Number.isFinite(o)) {
        sawObservations = true;
        obs.total += o;
        if (row.bucket === 'deletable') obs.wouldDelete += o;
      }
    }
    if (sawObservations) summary.observations = obs;
    summary.kept.total =
    ...
    let everyRowCarriedACount = true;
    for (const row of deleted.rows) {
      byDevice.set(row.device_id, (byDevice.get(row.device_id) || 0) + 1);
      const o = Number(row.observation_count);
      if (Number.isFinite(o)) deletedObservations += o;
      else everyRowCarriedACount = false;
    }
    // Only claimed when EVERY removed row accounted for itself — both that the
    // RETURNING set covers the whole rowCount, and that each of its rows carried
    // a count. A partial tally would UNDERSTATE what was removed, and on the one
    // engine in this app that destroys data that is the wrong direction to be
    // wrong. Nothing removed reports a truthful 0, not "unknown".
    if (summary.observations && everyRowCarriedACount && deleted.rows.length === summary.deleted) {
      summary.observations.deleted = deletedObservations;
    }
...
async function runConfigRetention(pool, options = {}) {
  ...
  const opts = options || {};
  // ⛔ A request for a dry run must NEVER be misread as permission to delete.
  // `opts.dryRun === true` alone meant the string 'true' -- which is the only
  // thing an env var or a JSON config can ever hold -- ran the job FOR REAL.
  // For a destructive job that is the wrong way to be wrong: an operator who
  // explicitly asked for a rehearsal would have got a live deletion. Any
  // recognisable affirmative disarms it; anything unrecognised leaves the job
  // armed, which is the correct default for a scheduled cleanup.
  const dryRun = isAffirmative(opts.dryRun);
 ...
 */
// True for `true` and for the string spellings a config file or env var can
// produce. Case- and whitespace-insensitive. Everything else is false.
...
function formatRetentionSummary(summary) {
  ...
  const describe = (label, t, protectedLabel) => {
    if (!t) return;
    if (t.error) {
      // ⛔ Report the count, never assume zero. `summary.deleted` is assigned
      // from rowCount BEFORE the RETURNING rows are tallied, so a failure
      // after the DELETE has already committed would previously print
      // "nothing deleted from this table" while rows really were removed.
      // On the one engine in this app that destroys data, understating what
      // it did is the worst possible direction for a log line to be wrong.
      const removed = Number(t.deleted) || 0;
      const what =
        removed > 0
          ? `${removed} row(s) had already been deleted before it failed`
          : 'nothing deleted from this table';
      lines.push(`${label}: FAILED — ${t.error} (${what}).`);
      return;
    }
    ...
    // Rows understate the loss once rows are deduped: one row can stand for many
    // successful collections. Appended, never substituted, so the row counts an
    // operator already reads stay exactly where they were.
    let observed = '';
    if (t.observations) {
      const n = summary.dryRun ? t.observations.wouldDelete : t.observations.deleted;
      if (Number.isFinite(n)) {
        observed = ` [${n} of ${t.observations.total} collection observation(s)]`;
      }
    }
    lines.push(
      `${label}: ${action}${capped}${observed}; kept ${t.kept.total} of ${t.totalRows} ` +
