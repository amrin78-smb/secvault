...
// Whitelist mapping ?sort= to a safe ORDER BY fragment. The raw query param is
// NEVER interpolated into SQL — only used as a key lookup here.
const SORT_OPTIONS = {
  name: 'd.name ASC',
  score: 'security_sort ASC NULLS LAST, d.name ASC',
  risk: 'risk_sort DESC NULLS LAST, d.name ASC',
  cve_count: 'patch_now_count DESC, scheduled_count DESC, monitor_count DESC, d.name ASC',
  rules: 'rule_count DESC, d.name ASC',
  last_collected: 'd.last_collected_at DESC NULLS LAST',
  vendor: 'd.vendor ASC, d.name ASC',
  site: 'd.site ASC NULLS LAST, d.name ASC',
};
