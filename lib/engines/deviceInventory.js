...
async function getDeviceRows(pool) {
  const { rows } = await pool.query(
    `SELECT d.id, d.name, d.vendor, d.site, d.mgmt_ip, d.smc_host, d.asset_criticality,
            ...
            -- ⛔ THE OTHER FOUR HA FACTS, added 2026-09-09. deviceHealth.js's
            -- (no backticks in this comment: it sits inside a template literal)
            -- haStatus() derives its degraded verdict from FIVE inputs; only
            -- peer_connection_status was projected, so HaCell on /devices was
            -- structurally incapable of seeing the other four and reddened on
            -- one condition. Live, 5 of 6 HA pairs were degraded — four on
            -- version_compat_ok=false, two also reporting "Link down" — and
            -- /devices drew all five in ordinary body text, indistinguishable
            -- from PAKFood, the only genuinely healthy pair. /lifecycle
            -- rendered the SAME rows with a red Degraded badge, because it
            -- calls the engine. Two pages, one dataset, opposite verdicts, and
            -- the fleet-overview page was the reassuring one.
            --
            -- ⛔ version_compat_ok is TRI-STATE: NULL means the device reported
            -- no compatibility block at all and must not be read as either a
            -- match or a mismatch.
            ha.version_compat_ok      AS ha_version_compat_ok,
            ha.config_sync_state      AS ha_config_sync_state,
            ha.peer_state             AS ha_peer_state,
            ha.last_nonfunctional_reason AS ha_last_nonfunctional_reason,
...
function decorate(row) {
  ...
  // ⛔ NEVER ASSESSED IS NOT "NO VULNERABILITIES FOUND". Found live 2026-09-09:
  // OKF(F2), the one device in the fleet that had never been collected at all —
  // no version, no rules, no config, no CVE assessment — rendered 100/100 with
  // the label "Not analysed". Hygiene correctly reported itself unmeasurable
  // (empty deviceRiskScores) and compliance correctly passed null, so the
  // vulnerability component was the ONLY measurable one and the composite
  // became whatever it said. It said 100, because both band counts are
  // COALESCEd to 0 in the query above and "0 of 1 devices carry a patch_now
  // finding" is a perfect score. A confident all-clear for the single device
  // SecVault knew nothing about — the most reassuring number in the worst
  // place, and CLAUDE.md's failed-read-as-a-fact rule one layer up from the SQL.
  //
  // ⛔ THE GATE IS EVIDENCE OF A RUN, NOT THE COUNTS. A MEASURED zero must keep
  // scoring 100: "assessed, and no advisory currently matches this firmware" is
  // a real, good result and making it pessimistic would be the same bug pointed
  // the other way. Only the absence of any evidence that a match ever ran makes
  // the component unmeasurable.
  //
  // Same two signals, ORed, as computeTiles' cveNotAssessed and CveCell — see
  // the ⛔ blocks on both. last_cve_assessed_at is the authoritative proof of a
  // run but is NULL on every already-deployed row until the matcher next runs;
  // assessment_count is independent proof (rows exist only because a match
  // produced them) and covers a device holding nothing but monitor-band rows.
  // Requiring both would report the whole fleet as unmeasured on day one.
  //
  // ⛔ null, not 0, and not a new parameter: vulnerabilitySubscore already
  // returns null for a null activeDevices through its existing finiteOrNull
  // contract, and computeSecurityScore then DROPS the 40% weight from the
  // denominator rather than scoring it 0. Passing 0 happens to work today
  // (active <= 0 is also null) but reads as a measured count of zero devices,
  // which is not what is being said.
  const cveAssessed = Boolean(row.last_cve_assessed_at) || (row.assessment_count || 0) > 0;

  const security = computeSecurityScore({
    activeDevices: cveAssessed ? 1 : null,
