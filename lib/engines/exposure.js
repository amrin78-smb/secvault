...
function scoreExposure(path) {
  ...
  if (path.sourceIsAny) {
    score += 40;
    reasons.push('Source is ANY — reachable from the entire internet.');
  } else {
    score += 15;
  ...
  if (path.observation === 'observed') {
    score += 20;
    reasons.push(
      `Traffic from ${path.evidence.sources} distinct public source(s) was ALLOWED across this path ` +
        `(${path.evidence.events} events, last ${path.evidence.lastSeen}).`
    );
    ...
    reasons.push(
      'No syslog coverage for this device in the window — this path is UNMEASURED, not unused.'
    );
  } else {
    reasons.push(
...
function buildExposurePaths({ rules, objects, natRules, interfaces }) {
  ...
  for (const rule of Array.isArray(rules) ? rules : []) {
    ...
    for (const face of faces.values()) {
      ...
      const internal = nats.length > 0 ? nats.flatMap((n) => n.internal || []) : null;
      ...
      paths.push({
        ...
        internal: internal && internal.length > 0 ? internal : null,
  ...
  return { paths, publicIps, unresolvedRules };
}
