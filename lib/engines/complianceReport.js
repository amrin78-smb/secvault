...
async function generateReportPdf(pool) {
  ...
  // dumpio: true (temporary diagnostic, 2026-08-02) -- --no-sandbox alone
  // did NOT fix the "Code: 1002" launch failure seen live (identical error
  // before and after), so the Session-0-sandbox theory was wrong or
  // incomplete. Piping the browser process's OWN stdout/stderr into this
  // process's (captured in app.log/app-error.log for SecVault-App, per
  // CLAUDE.md's NSSM logging setup) turns puppeteer's generic wrapper
  // message into the browser's real startup error -- confirmed working
  // (msedge.exe launches fine as both firmans0 interactively and via the
  // exact same flags) everywhere EXCEPT from inside the actual service
  // process, so something concrete must differ there that a generic
  // "Code: 1002, empty stderr" message isn't surfacing.
