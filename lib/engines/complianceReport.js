 ...
 */
async function libraryCoverage(pool) {
  try {
    const { rows } = await pool.query(
      `SELECT s AS standard,
              count(*)::int AS mapped,
              count(*) FILTER (
                WHERE ac.vendor IS NULL
                   OR ac.vendor IN (SELECT vendor FROM devices WHERE active = true)
              )::int AS applicable,
 ...
 */
async function fleetCheckCoverage(pool) {
  try {
    const { rows } = await pool.query(
      `SELECT s AS standard,
...
async function buildReportData(pool, options = {}) {
  ...
  const [fleet, perDevice, findingsAppendix, coverage, library, checkCoverage] = await Promise.all([
    ...
    libraryCoverage(pool),
    fleetCheckCoverage(pool),
  ]);
  const coverageIndex = buildCoverageIndex({ library, checkCoverage, perDevice });
  return {
    fleet,
    perDevice,
    findingsAppendix,
...
function renderReportBody(doc, data, layout) {
  ...
  if (!findingsAppendix.length) {
    ...
    const clear = standard
      ...
      : 'No failing or warning findings across the fleet.';
    ...
    const line = blind.length === 0
      ...
      : `${clear} This is NOT an all-clear: ${blind.length} of ${perDevice.length} `
        + `firewalls have no ${standard ? `${scopeName} ` : ''}findings recorded at all, so `
