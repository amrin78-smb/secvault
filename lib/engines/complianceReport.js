...
const { PRODUCT_TAGLINE } = require('../branding');
const {
...
// The old name, kept so the ~30 existing call sites do not all have to move
// in the same change. ⛔ It is no longer red; do not "fix" the name back.
const RED = ACCENT;
...
function drawCover(doc, o, layout) {
  ...
  meta.forEach(([k, v]) => {
    ...
    doc.fillColor('#0D131C').font('Helvetica').text(v, left + 130, my, { width: contentW - 130 });
    my += 22;
  });
...
function drawTable(doc, tbl, layout, o2 = {}) {
  ...
  rows.forEach((r, idx) => {
    ...
    columns.forEach((c, i) => {
      const color = typeof c.color === 'function' ? c.color(r) || '#0D131C' : c.color || '#0D131C';
      doc
        .fillColor(color)
        .font('Helvetica')
        .fontSize(8)
...
module.exports = { buildReportData, generateReportPdf, dispatchMonthlyReport, reportingPeriod };
