// lib/engines/complianceReport.js
// CommonJS ONLY — required by services/engine-worker.js's
// runComplianceReportJob() and app/api/compliance/report/**. See
// lib/schema.sql's compliance_report_log comment for the full design
// rationale (fleet-wide PDF, on-demand + monthly-scheduled delivery).

'use strict';

const { computeFleetComplianceScores } = require('./dashboardSnapshot');
const { PRODUCT_NAME } = require('../branding');
// ⛔ The shared PDF chassis. These SEVEN helpers used to live here AND in
// lib/engines/ruleChangeRequestReport.js, and the two copies had drifted — the
// other one grew an `ensureSpace` guard against pdfkit orphaning a table header
// across five pages, and this file never received it. See lib/reports/chassis.js.
const {
  ACCENT, NAVY, MUTED, LIGHT, BORDER, GREEN,
  pdfSafe, installPdfSafeText, fmtStamp,
  drawCover, sectionTitle, drawTable, stampHeadersFooters,
} = require('../reports/chassis');
// ⛔ The coverage judgement is SHARED with /compliance and the All Checks page,
// not reworded here. A report and a screen disagreeing about what a percentage
// is computed over is worse than either saying nothing.
const {
  COVERAGE_CLAIM,
  buildCoverageSet,
} = require('./complianceCoverage');
// ⛔ The freshness judgement is SHARED with the two compliance pages, not
// reworded here — a document and a screen disagreeing about whether a firewall
// is behind is worse than either saying nothing. Same call as COVERAGE_CLAIM.
const {
  complianceFreshness,
  freshnessNote,
  summariseFreshness,
} = require('./complianceFreshness');
const {
  listEnabledChannelsWithSecrets,
  recordChannelSuccess,
  recordChannelError,
} = require('../notificationChannels');
const { dispatchNotification } = require('../notify');

// Mirrors components/compliance/ComplianceMatrix.js's STANDARDS/scoreColor/
// SCORE_COLOR_VAR exactly (same 5 keys, same label text, same >80/>=60/else
// score-band thresholds) — duplicated, not imported. ComplianceMatrix.js is
// an ES module (`export const`); this file is required directly by
// services/engine-worker.js under plain `node`, which cannot parse ESM
// export syntax. Same ESM/CJS-boundary convention CLAUDE.md already
// documents for components/devices/vendorMeta.js <-> lib/adapters/index.js
// ("Two registries, deliberately duplicated").
const STANDARDS = [
  { key: 'PCI_DSS', label: 'PCI DSS' },
  { key: 'ISO_27001', label: 'ISO 27001' },
  { key: 'CIS_V8', label: 'CIS v8' },
  { key: 'NIST', label: 'NIST' },
  { key: 'SANS', label: 'SANS' },
];

const STANDARD_KEYS = STANDARDS.map((s) => s.key);
const STANDARD_LABEL = new Map(STANDARDS.map((s) => [s.key, s.label]));

/**
 * Resolves the optional `standard` scope.
 *
 * ⛔ AN UNKNOWN STANDARD THROWS; it does not fall back to the whole fleet. A
 * report silently widening from "PCI DSS" to "everything" while still being
 * titled and filed as a PCI document is a mislabelled audit artefact, which is
 * the one output this product must never produce. The route also allow-lists
 * the value against the catalogue's own declared choices, so this is the second
 * of two independent gates rather than the only one.
 */
function resolveStandard(standard) {
  if (standard === null || standard === undefined || standard === '') return null;
  if (!STANDARD_KEYS.includes(standard)) {
    throw new Error(`Unknown compliance standard: ${standard}`);
  }
  return standard;
}

function emptyStandardStats() {
  const stats = {};
  for (const s of STANDARDS) stats[s.key] = { pass: 0, fail: 0, warning: 0, na: 0, total: 0, scorePct: null };
  return stats;
}

function finalizeScorePct(stats) {
  for (const s of STANDARDS) {
    const c = stats[s.key];
    const measurable = c.pass + c.fail + c.warning;
    c.scorePct = measurable > 0 ? Math.round((c.pass / measurable) * 100) : null;
  }
}

// ⛔ Deliberate 5th instance of this app's already-self-documented duplicated
// scoring formula (see app/api/compliance/fleet/route.js's own "BUG FIXED
// 2026-07-18... kept as a literal array, not an import, per this file's own
// established 'duplicated query/shape, not shared' convention" comment) —
// NOT unified with the other 4 sites in this change, to avoid touching
// already-working, already-tested compliance pages while adding this
// feature. The fleet-wide aggregate above reuses
// lib/engines/dashboardSnapshot.js's computeFleetComplianceScores() instead
// of a 6th duplicate of THAT formula, since that function already has
// exactly one other caller and extending it was zero-risk; this per-device
// breakdown and the findings query below have no reusable function to
// extend, so they're new, acknowledged duplicates instead.
async function buildPerDeviceStandards(pool, deviceId = null) {
  // ⛔ ONE FIREWALL IS A NARROWER SCOPE, NOT A FILTERED FLEET. Every query
  // below is scoped in SQL rather than by filtering the fleet result in JS:
  // the coverage denominators are computed from these same reads, and a
  // JS-side filter would leave them counted over all sixteen firewalls while
  // the scores came from one. That is the overclaim v2.163.0 removed, and it
  // would come straight back wearing a device filter.
  const deviceClause = deviceId ? ' AND d.id = $1' : '';
  const deviceParams = deviceId ? [deviceId] : [];

  // No alias here: there is no join to disambiguate, and the unaliased form is
  // what every other reader of this table uses.
  const { rows: devices } = await pool.query(
    `SELECT id, name, vendor FROM devices WHERE active = true${
      deviceId ? ' AND id = $1' : ''} ORDER BY name ASC`,
    deviceParams
  );

  const { rows: findingRows } = await pool.query(
    `SELECT af.device_id, af.status, ac.standards
     FROM audit_findings af
     JOIN audit_checks ac ON ac.id = af.check_id
     JOIN devices d ON d.id = af.device_id
     WHERE d.active = true${deviceClause}`,
    deviceParams
  );

  const statsByDevice = new Map();
  for (const device of devices) statsByDevice.set(device.id, emptyStandardStats());

  for (const row of findingRows) {
    const stats = statsByDevice.get(row.device_id);
    if (!stats) continue;
    const standardsForRow = Array.isArray(row.standards) ? row.standards : [];
    for (const key of standardsForRow) {
      if (!stats[key]) continue;
      stats[key].total += 1;
      if (row.status === 'pass' || row.status === 'fail' || row.status === 'warning' || row.status === 'na') {
        stats[key][row.status] += 1;
      }
    }
  }

  return devices.map((device) => {
    const stats = statsByDevice.get(device.id);
    finalizeScorePct(stats);
    return { deviceId: device.id, deviceName: device.name, vendor: device.vendor, standards: stats };
  });
}

// Only 'fail' and 'warning' findings, across every active device — 'pass'/
// 'na' are excluded deliberately (see the module-level design note in the
// approved plan: a monthly auditor report showing every passing check on
// every device would be hundreds of redundant rows on a fleet this size).
// 'warning' is included alongside 'fail', NOT fail-only — CLAUDE.md's own
// standing rule ("`unknown` must never silently default to `no`") applies
// here by the same logic: a warning is an unresolved predicate, not a
// pass, and a document titled "Compliance Report" omitting it would
// misrepresent the fleet's real posture to whoever reads it.
async function buildFindingsAppendix(pool, standard = null, deviceId = null) {
  // ⛔ PARAMETERISED, and the value has already been allow-listed twice. The
  // `standards` column is a TEXT[], so membership is `@>` against a
  // single-element array — never string interpolation, never a LIKE.
  const params = [];
  let standardClause = '';
  if (standard) {
    params.push([standard]);
    standardClause = ` AND ac.standards @> $${params.length}::text[]`;
  }
  let deviceClause = '';
  if (deviceId) {
    params.push(deviceId);
    deviceClause = ` AND d.id = $${params.length}`;
  }
  const { rows } = await pool.query(
    `SELECT af.device_id, d.name AS device_name, ac.name AS check_name, ac.severity,
            af.status, af.detail, ac.remediation_guidance
     FROM audit_findings af
     JOIN audit_checks ac ON ac.id = af.check_id
     JOIN devices d ON d.id = af.device_id
     WHERE d.active = true AND af.status IN ('fail', 'warning')${standardClause}${deviceClause}
     ORDER BY
       d.name ASC,
       CASE af.status WHEN 'fail' THEN 0 WHEN 'warning' THEN 1 ELSE 2 END,
       CASE ac.severity WHEN 'critical' THEN 0 WHEN 'high' THEN 1 WHEN 'medium' THEN 2 WHEN 'low' THEN 3 ELSE 4 END,
       ac.name ASC`,
    params
  );

  const byDevice = new Map();
  for (const row of rows) {
    if (!byDevice.has(row.device_id)) byDevice.set(row.device_id, { deviceName: row.device_name, findings: [] });
    byDevice.get(row.device_id).findings.push({
      checkName: row.check_name,
      severity: row.severity,
      status: row.status,
      detail: row.detail,
      remediationGuidance: row.remediation_guidance,
    });
  }
  return Array.from(byDevice.values());
}

/**
 * How much of the check library the chosen standard actually covers.
 *
 * ⛔ THIS IS THE SENTENCE THAT KEEPS A SCOPED REPORT HONEST. A check carries a
 * `standards` ARRAY — one check commonly maps to several frameworks — so
 * narrowing to PCI DSS does not filter a fleet, it changes the DENOMINATOR. A
 * PCI score of 62% and an overall score of 51% are both correct and are
 * answers to different questions. Printed side by side in two documents with
 * no explanation, they read as a bug in one of them, and the reader has no way
 * to tell which. So the scoped report states its own coverage on the cover.
 *
 * Returns null on a read failure rather than a zero — "0 of 45 checks map to
 * PCI DSS" is a claim, and a false one.
 */
async function standardCoverage(pool, standard) {
  if (!standard) return null;
  try {
    const { rows } = await pool.query(
      `SELECT count(*)::int AS total,
              count(*) FILTER (WHERE standards @> $1::text[])::int AS mapped
         FROM audit_checks`,
      [[standard]]
    );
    const r = rows[0];
    if (!r || typeof r.total !== 'number') return null;
    return { standard, label: STANDARD_LABEL.get(standard) || standard, ...r };
  } catch (_err) {
    return null;
  }
}

/**
 * The whole library's per-standard mapping, for EVERY standard — the
 * denominators the unscoped report was printing five percentages without.
 *
 * ⛔ THIS IS THE DEFECT THIS FUNCTION EXISTS FOR. `standardCoverage` above
 * already qualified a SCOPED report, but the fleet-wide document — the one the
 * monthly job emails to auditors — printed "PCI DSS 49% · NIST 42%" and so on
 * with no denominator at all. Measured live 2026-09-22: 45 checks, mapping
 * CIS_V8 44 / ISO_27001 35 / PCI_DSS 21 / SANS 12 / NIST 7. A NIST percentage
 * computed over seven checks, three of them vendor-scoped, read in an audit
 * document as a NIST posture.
 *
 * `applicable` is counted against the vendors actually in the inventory,
 * because a check scoped to a vendor nobody owns can never run.
 *
 * ⛔ SCOPED TO ONE FIREWALL, `applicable` MEANS THAT FIREWALL'S VENDOR, not
 * the fleet's. A Fortinet-only check counts as applicable on a fleet holding
 * one Fortinet, and is INAPPLICABLE on a Palo Alto — leaving the fleet
 * denominator in place for a single-device report would credit that device with
 * checks that can never run on it. The per-device page already reads the library
 * this way (`getCheckLibraryCoverage(pool, [selected.vendor])`); this is the
 * same call from the document.
 *
 * ⛔ Returns null on a read failure, never a zeroed map — the same call
 * standardCoverage() makes, for the same reason: "0 of 45 checks carry this
 * mapping" is a claim, and a false one.
 */
async function libraryCoverage(pool, vendors = null) {
  try {
    // ⛔ The fleet form keeps its own subquery rather than reading the vendor
    // list in JS and passing it back down: an inventory read that returned
    // nothing would otherwise silently become "no vendor applies to anything".
    const scoped = Array.isArray(vendors);
    const applicableClause = scoped
      ? 'ac.vendor IS NULL OR ac.vendor = ANY($1::text[])'
      : 'ac.vendor IS NULL OR ac.vendor IN (SELECT vendor FROM devices WHERE active = true)';
    const { rows } = await pool.query(
      `SELECT s AS standard,
              count(*)::int AS mapped,
              count(*) FILTER (WHERE ${applicableClause})::int AS applicable,
              (SELECT count(*)::int FROM audit_checks) AS library_total
       FROM audit_checks ac, unnest(ac.standards) s
       GROUP BY s`,
      scoped ? [vendors.filter(Boolean)] : []
    );
    const byStandard = {};
    let libraryTotal = null;
    for (const r of rows || []) {
      if (typeof r.standard !== 'string' || !Number.isFinite(Number(r.mapped))) continue;
      byStandard[r.standard] = { mapped: Number(r.mapped), applicable: Number(r.applicable) };
      if (Number.isFinite(Number(r.library_total))) libraryTotal = Number(r.library_total);
    }
    if (libraryTotal === null) return null;
    return { libraryTotal, byStandard };
  } catch (_err) {
    return null;
  }
}

/**
 * How many DISTINCT library checks actually produced a result fleet-wide, and
 * how many produced a GRADEABLE one.
 *
 * ⛔ DISTINCT CHECKS, NOT FINDING ROWS, and the difference is an order of
 * magnitude. A finding is a (device, check) PAIR, so NIST's 91 live finding
 * rows come from at most seven distinct questions. Reporting the row count as
 * the evidence behind the figure would overstate the library thirteenfold —
 * the same two-units trap lib/evidence.js's CVE builder documents at length.
 *
 * ⛔ An `na` row counts as EVALUATED but not as ANSWERED: the question was put
 * and SecVault could not answer it. That is a different fact from a check that
 * never ran, and both differ from a pass or a fail.
 *
 * ⛔ null on a read failure, so the coverage statement reports the evidence
 * grade as UNKNOWN rather than as `none` — "no check produced an answer" would
 * be a claim about the fleet made from a failed read of our own tables.
 */
async function fleetCheckCoverage(pool, deviceId = null) {
  try {
    const { rows } = await pool.query(
      `SELECT s AS standard,
              count(DISTINCT ac.id)::int AS evaluated_checks,
              count(DISTINCT ac.id) FILTER (WHERE af.status <> 'na')::int AS answered_checks
       FROM audit_findings af
       JOIN audit_checks ac ON ac.id = af.check_id
       JOIN devices d ON d.id = af.device_id,
            unnest(ac.standards) s
       WHERE d.active = true${deviceId ? ' AND d.id = $1' : ''}
       GROUP BY s`,
      deviceId ? [deviceId] : []
    );
    const byStandard = {};
    let sawOne = false;
    for (const r of rows || []) {
      if (typeof r.standard !== 'string' || !Number.isFinite(Number(r.evaluated_checks))) continue;
      byStandard[r.standard] = {
        evaluatedChecks: Number(r.evaluated_checks),
        answeredChecks: Number(r.answered_checks),
      };
      sawOne = true;
    }
    // ⛔ An EMPTY result is a real answer (nothing has been audited yet) and is
    // returned as an empty map; an unrecognisable one is not, and returns null.
    return sawOne || (rows && rows.length === 0) ? byStandard : null;
  } catch (_err) {
    return null;
  }
}

/**
 * When each firewall in scope was last COLLECTED FROM, and when its checks were
 * last evaluated.
 *
 * ⛔ THIS DOCUMENT HAD NO FRESHNESS STATEMENT AT ALL. It is the artefact an
 * auditor files and re-reads months later, and it printed a fleet score with no
 * statement of how old the evidence behind it was. A compliance audit runs
 * inside `collectAndStore` gated on `result.configCollected`, so a firewall that
 * stops being collectable simply stops being audited and its numbers FREEZE —
 * they do not disappear, they do not go red, they keep printing. Measured on the
 * live fleet 2026-09-22: TSR_EKC's evidence was 1,116h old and TSR-TL's 252h,
 * printed beside fourteen ~12h-old ones with nothing to tell them apart.
 *
 * ⛔ THE AGE IS THE EVIDENCE'S, NOT THE EVALUATION'S. Both timestamps are read
 * because they genuinely diverge — see lib/engines/complianceFreshness.js — and
 * grading on the audit time launders a stale configuration into a fresh-looking
 * score.
 *
 * ⛔ null ON A FAILED OR UNRECOGNISABLE READ, never an empty list. "No firewall
 * has a collection time" would print as a fleet nothing has ever been collected
 * from, which is a claim; a failed read is not. The caller prints the unknown
 * explicitly rather than omitting the sentence.
 */
async function collectionAges(pool, deviceId = null) {
  try {
    const { rows } = await pool.query(
      `SELECT d.id   AS device_id,
              d.name AS device_name,
              (SELECT max(dc.collected_at) FROM device_configs dc WHERE dc.device_id = d.id)
                AS collected_at,
              (SELECT max(af.detected_at) FROM audit_findings af WHERE af.device_id = d.id)
                AS evaluated_at
         FROM devices d
        WHERE d.active = true${deviceId ? ' AND d.id = $1' : ''}
        ORDER BY d.name ASC`,
      deviceId ? [deviceId] : []
    );
    if (!Array.isArray(rows)) return null;
    // ⛔ SHAPE-CHECKED, not trusted. A row set that does not carry the columns
    // this query asked for did not come from this query, and reading an absent
    // `collected_at` as "never collected" would turn a wrong answer into a
    // confident alarming one.
    for (const r of rows) {
      if (!r || typeof r !== 'object'
        || !Object.prototype.hasOwnProperty.call(r, 'device_name')
        || !Object.prototype.hasOwnProperty.call(r, 'collected_at')) {
        return null;
      }
    }
    return rows.map((r) => ({
      deviceId: r.device_id,
      deviceName: r.device_name,
      collectedAt: r.collected_at,
      evaluatedAt: r.evaluated_at,
    }));
  } catch (_err) {
    return null;
  }
}

/**
 * The sentence this report prints about how current its evidence is. PURE —
 * takes the rows, the scope and a clock.
 *
 * ⛔ IT NEVER ADJUSTS A SCORE, and it never withholds one either: a stale result
 * is real evidence about an old configuration, and wording it as garbage pushes
 * a reader to ignore the page rather than fix the collection.
 *
 * ⛔ IT NAMES THE FIREWALLS THAT ARE BEHIND. "2 firewalls are behind" sends
 * somebody hunting through sixteen rows of a printed table.
 */
function evidenceAgeStatement(rows, { device = null, now = new Date(), env = process.env } = {}) {
  if (rows === null || rows === undefined) {
    return 'SecVault could not read when these configurations were last collected, so how '
      + 'current this report is could not be established. That is a failed read on SecVault\'s '
      + 'side, not a statement about the firewalls.';
  }
  const list = Array.isArray(rows) ? rows : [];
  if (list.length === 0) {
    return 'No active firewall is in scope, so there is no collected configuration behind these '
      + 'figures.';
  }

  if (device) {
    const r = list.find((x) => x.deviceId === device.id) || list[0];
    const f = complianceFreshness(
      { evidenceAt: r.collectedAt, evaluatedAt: r.evaluatedAt }, now, env
    );
    const lagNote = f.evaluatedAgainstOldConfig
      ? ' The checks were last run against a configuration that was already older than the '
        + 'expected collection cadence when they ran, so the audit is more recent than the '
        + 'evidence behind it.'
      : '';
    return freshnessNote(f, device.name) + lagNote;
  }

  const s = summariseFreshness(
    list.map((r) => ({ deviceName: r.deviceName, lastRunAt: r.collectedAt })), now, env
  );
  if (s.behind.length === 0) {
    return `Every firewall in scope was collected from within the expected cadence, so these `
      + 'figures describe the fleet as it is now.';
  }
  // ⛔ The named list is capped for the page, and the cap DISCLOSES itself —
  // a truncated list reads as complete, which is the more insidious failure.
  const shown = s.behind.slice(0, 8);
  const more = s.behind.length - shown.length;
  return `${s.behind.length} of ${s.total} firewalls were last collected from longer ago than the `
    + `expected cadence, so their checks describe an older configuration: ${shown.join(', ')}`
    + `${more > 0 ? `, and ${more} more` : ''}. Their scores are real evidence about that older `
    + 'configuration, not a current one, and a firewall that stopped being collected from also '
    + 'stopped being assessed — its findings are frozen, not clean.';
}

/**
 * The coverage statement per standard, built from the three reads above plus
 * the per-device counts already computed. PURE — delegates every judgement to
 * lib/engines/complianceCoverage.js so this document and the two pages state
 * the same thing.
 */
function buildCoverageIndex(data) {
  const library = data.library || null;
  const checks = data.checkCoverage || null;
  const perDevice = data.perDevice || [];
  return buildCoverageSet(
    STANDARDS,
    (key) => {
      const lib = library && library.byStandard[key] ? library.byStandard[key] : null;
      const ck = checks && checks[key] ? checks[key] : null;
      const f = fleetForStandard(perDevice, key);
      return {
        libraryTotal: library ? library.libraryTotal : null,
        mapped: lib ? lib.mapped : null,
        applicable: lib ? lib.applicable : null,
        evaluatedChecks: ck ? ck.evaluatedChecks : null,
        answeredChecks: ck ? ck.answeredChecks : null,
        findings: { pass: f.pass, fail: f.fail, warning: f.warning, na: f.na },
        scorePct: f.scorePct,
      };
    },
    // ⛔ 'device' is not cosmetic: it drops the "across N firewalls" clause
    // the fleet wording carries, which on a one-firewall report would read as
    // a fleet claim. The per-device page already builds its coverage this way.
    data.device
      ? { scope: 'device', deviceCount: 1 }
      : { scope: 'fleet', deviceCount: perDevice.length }
  );
}

/**
 * Assembles every piece of data the report template needs. Best-effort per
 * query is NOT applied here (unlike most engine-worker jobs) — a report
 * with silently-missing data is worse than a failed report generation the
 * caller can see and retry; a thrown error here propagates straight up to
 * generateReportPdf()'s caller.
 * @param {import('pg').Pool} pool
 */
async function buildReportData(pool, options = {}) {
  const standard = resolveStandard(options.standard);
  // Resolved FIRST: the vendor it returns is what scopes the library read, and
  // an unresolvable device must stop the report rather than widen it.
  const device = options.deviceId ? await resolveReportDevice(pool, options.deviceId) : null;
  if (options.deviceId && !device) return null;
  const deviceId = device ? device.id : null;

  const [perDevice, findingsAppendix, coverage, library, checkCoverage, evidenceAges] =
    await Promise.all([
      buildPerDeviceStandards(pool, deviceId),
      buildFindingsAppendix(pool, standard, deviceId),
      standardCoverage(pool, standard),
      libraryCoverage(pool, device ? [device.vendor] : null),
      fleetCheckCoverage(pool, deviceId),
      collectionAges(pool, deviceId),
    ]);

  // ⛔ A DEVICE-SCOPED SUMMARY IS NEVER THE FLEET READ. computeFleetComplianceScores()
  // always counts every active firewall, so calling it here and printing the
  // result above a one-firewall table would put the fleet's score under that
  // firewall's name — the most quietly wrong thing this document could do.
  const fleet = device
    ? summaryFromPerDevice(perDevice)
    : await computeFleetComplianceScores(pool);

  const coverageIndex = buildCoverageIndex({ library, checkCoverage, perDevice, device });
  return {
    fleet,
    perDevice,
    findingsAppendix,
    coverage,
    library,
    checkCoverage,
    coverageIndex,
    evidenceAges,
    standard,
    device,
    generatedAt: new Date(),
  };
}

/**
 * The fleet figure for ONE standard, summed from the per-device stats that
 * were already computed.
 *
 * ⛔ Deliberately NOT a sixth copy of the scoring formula reaching into the
 * database again: these are the same findings over the same active devices, so
 * summing the per-device counts gives the identical numerator and denominator.
 * ⛔ Nothing measurable gives null, NEVER 0 — the same rule as `na` being
 * excluded from the denominator. A standard no device can be assessed against
 * must not print as total non-compliance.
 */
function fleetForStandard(perDevice, standard) {
  const c = { pass: 0, fail: 0, warning: 0, na: 0 };
  for (const d of perDevice) {
    const s = d.standards && d.standards[standard];
    if (!s) continue;
    c.pass += s.pass; c.fail += s.fail; c.warning += s.warning; c.na += s.na;
  }
  const measurable = c.pass + c.fail + c.warning;
  return { ...c, scorePct: measurable > 0 ? Math.round((c.pass / measurable) * 100) : null };
}

/**
 * The summary block for a scope, computed from the per-device counts already
 * read rather than from a second pass over the database.
 *
 * ⛔ IT REPRODUCES `computeFleetComplianceScores()` EXACTLY, INCLUDING ITS
 * `overall`. That figure sums the PER-STANDARD counts, so a check mapped to
 * three frameworks contributes to three of them — which is the definition
 * every score in this product already uses. Recomputing it over DISTINCT
 * findings here would be more defensible in the abstract and would print a
 * different number under the same word on one report out of eleven. A score
 * that changes meaning with the scope it was asked at is worse than one with a
 * debatable definition, so this stays identical.
 */
// ⛔ THE OPERAND ORDER IS LOAD-BEARING, NOT COSMETIC. `(100*pass)/measurable`
// and `(pass/measurable)*100` disagree at exact-.5 float boundaries — 23 pass
// of 40 is 57.5 one way and 57.49999999999999 the other, so 58 vs 57. This file
// used the first form while dashboardSnapshot.js and both compliance pages used
// the second, and the fleet PDF printed BOTH on the same page: "PCI DSS 57%" in
// the summary table with "the 58% rests on 40 checks" one line beneath it.
// Every copy now matches the form the pages use. tests/complianceScoreFormula
// .test.js cross-checks this file against computeFleetComplianceScores directly
// rather than against hand-written numbers, which is why the drift survived.
function summaryFromPerDevice(perDevice) {
  const byStandard = {};
  const byStandardCounts = {};
  let totalPass = 0;
  let totalMeasurable = 0;
  for (const key of STANDARD_KEYS) {
    const c = fleetForStandard(perDevice, key);
    const measurable = c.pass + c.fail + c.warning;
    byStandard[key] = measurable > 0 ? Math.round((c.pass / measurable) * 100) : null;
    byStandardCounts[key] = { pass: c.pass, fail: c.fail, warning: c.warning };
    totalPass += c.pass;
    totalMeasurable += measurable;
  }
  return {
    overall: totalMeasurable > 0 ? Math.round((totalPass / totalMeasurable) * 100) : null,
    byStandard,
    byStandardCounts,
  };
}

/**
 * The firewall a device-scoped report is about.
 *
 * ⛔ AN UNKNOWN OR INACTIVE DEVICE RETURNS null AND THE REPORT IS NOT
 * PRODUCED — the route turns that into a 404. Rendering a document titled
 * with an id nobody can resolve, over a fleet-wide body, would be an audit
 * artefact that claims to be about a firewall it never read.
 */
async function resolveReportDevice(pool, deviceId) {
  if (!deviceId) return null;
  const { rows } = await pool.query(
    'SELECT id, name, vendor FROM devices WHERE id = $1 AND active = true',
    [deviceId]
  );
  return rows.length ? rows[0] : null;
}

/**
 * The devices this document can say NOTHING about: no finding rows at all in
 * the scope being reported.
 *
 * ⛔ IT EXISTS TO STOP AN UNQUALIFIED ALL-CLEAR. "No failing or warning
 * findings across the fleet" is the single most reassuring sentence this report
 * can print, and on a fleet nothing has been assessed against it is also the
 * most wrong: a firewall with no findings contributes no failures, which makes
 * a total collection gap look exactly like a clean estate. CLAUDE.md forbids an
 * all-clear while coverage is incomplete; this is how that sentence learns to
 * qualify itself.
 *
 * A device is counted here only when EVERY standard in scope has no findings at
 * all — `na` rows count as assessed, because the device was asked and SecVault
 * could not answer, which is a different fact and is already reported as such.
 */
function unassessedDevices(perDevice, standard = null) {
  const keys = standard ? [standard] : STANDARD_KEYS;
  return (perDevice || []).filter((d) => keys.every((k) => {
    const s = (d && d.standards) ? d.standards[k] : null;
    return !s || !s.total;
  }));
}

// ⛔ Rewritten 2026-08-02: the original implementation rendered a self-contained
// HTML string and used puppeteer-core (headless Edge) to convert it to a PDF.
// That worked in every manual test (locally, and even a raw msedge.exe launch
// on the real production server) but consistently failed with an opaque
// "Failed to launch the browser process: Code: 1002" / empty-stderr error
// SPECIFICALLY when launched from inside the SecVault-App/SecVault-Engine NSSM
// Windows services -- root cause never conclusively identified after several
// live diagnostic passes (ruled out: missing browser, wrong path, Session-0
// sandbox restrictions, GPU, active Defender/ASR blocking). Per the sibling
// NocVault suite apps (logvault/ddivault/spanvault), the established,
// battle-tested pattern for exactly this "generate a branded PDF report from
// a Node service" need is `pdfkit` -- a pure-JS vector PDF library with no
// child process, no browser, no launch step at all. Ported directly from
// spanvault/api/reportsPdf.js's helpers (drawCover/drawKpiTiles/drawTable/
// sectionTitle/stampHeadersFooters/pdfSafe/installPdfSafeText), same brand
// hex palette (confirmed identical to this app's own app/globals.css tokens
// -- the suite shares one design system, see CLAUDE.md's Design System
// section), same Buffer-returning generateReportPdf(pool) contract so both
// the on-demand route and the scheduled job need zero further changes.
const PDFDocument = require('pdfkit');

// The PDF's palette, mirroring app/globals.css's LIGHT tokens by hand.
//
// ⛔ THESE MUST BE UPDATED WITH THE TOKENS, and nothing enforces it. pdfkit
// draws to a page, not to a DOM, so there is no var() to resolve and no
// stylesheet to inherit -- these literals are the only palette the report
// has. When the tokens moved in v2.87.0 this file was left behind for a
// while, which is exactly the drift to watch for: the product on screen and
// the report a customer receives are the same brand or they are not.
//
// ⛔ Always the LIGHT theme values. A report is printed and emailed; there is
// no viewer preference to read, and a dark-theme PDF is unreadable on paper.
//
// ⛔ ACCENT vs STATUS_RED is the distinction this file must not lose. In
// v2.87.0 the brand moved OFF red precisely so that red means danger and
// nothing else -- so cover chrome and accent bars are now TEAL, and red
// appears in this document only where something is actually wrong. A report
// whose header bar is the same colour as its critical findings trains the
// reader to ignore the colour, on paper just as on screen.
const YELLOW = '#B7791F';      // --yellow / --sev-med
const BLUE = '#2F6FE0';        // --blue
const STATUS_RED = '#D4353B';  // --red / --sev-crit
// ⛔ No hue, by design -- the printed form of --unmeasured. A compliance
// standard with nothing measurable must not print as a 0% or as a pass.
const UNMEASURED = '#6D7784';


function scoreColorHex(pct) {
  // ⛔ A null score is "nothing was measurable", not a low score and not a
  // muted label -- CLAUDE.md excludes `na` from the denominator entirely.
  // It gets the dedicated unmeasured grey so it cannot be read as a value.
  if (pct == null) return UNMEASURED;
  if (pct > 80) return GREEN;
  if (pct >= 60) return YELLOW;
  return STATUS_RED;
}

const STATUS_LABEL = {
  fail: { label: 'Fail', color: STATUS_RED },
  warning: { label: 'Warning', color: YELLOW },
};

const SEVERITY_LABEL = {
  critical: { label: 'Critical', color: STATUS_RED },
  high: { label: 'High', color: YELLOW },
  medium: { label: 'Medium', color: BLUE },
  low: { label: 'Low', color: MUTED },
  info: { label: 'Info', color: MUTED },
};

// Builds the three drawTable() row/column shapes from buildReportData()'s
// output — pure data transforms, no pdfkit calls, so they're easy to reason
// about independently of layout.
function buildFleetSummaryTable(fleet, standard = null, coverageIndex = null) {
  // ⛔ A scoped report shows ONLY its own standard. Listing the other four
  // beside it — greyed, or worse, populated — invites the reader to compare
  // numbers computed over different denominators, which is exactly the
  // confusion the coverage note above exists to prevent.
  const shown = standard ? STANDARDS.filter((s) => s.key === standard) : STANDARDS;
  return {
    columns: [
      { key: 'standard', label: 'Standard', width: 140 },
      { key: 'score', label: 'Score', width: 60, color: (r) => r._color },
      { key: 'pass', label: 'Pass', width: 50 },
      { key: 'fail', label: 'Fail', width: 50 },
      { key: 'warning', label: 'Warning', width: 60 },
      // ⛔ THE COLUMNS THAT STOP THE PERCENTAGE STANDING ALONE. A score with no
      // denominator beside it reads as a framework posture; these two say what
      // it was actually computed over. Both drawn in the unmeasured grey —
      // hueless, because how well a figure is known is a different axis from
      // whether the news is good.
      { key: 'checks', label: 'SecVault checks', width: 100, color: () => UNMEASURED },
      { key: 'evidence', label: 'Evidence behind it', width: 95, color: () => UNMEASURED },
    ],
    rows: shown.map((s) => {
      const pct = fleet.byStandard[s.key];
      const counts = fleet.byStandardCounts[s.key] || { pass: 0, fail: 0, warning: 0 };
      const cov = coverageIndex ? coverageIndex[s.key] : null;
      return {
        standard: s.label,
        score: pct == null ? '-' : `${pct}%`,
        pass: counts.pass,
        fail: counts.fail,
        warning: counts.warning,
        // ⛔ '-' when the library read failed, never '0 of 45' and never a
        // blank cell: a blank reads as "nothing to report".
        checks: cov ? cov.cell : '-',
        evidence: cov ? cov.gradeLabel : 'Evidence not known',
        _color: scoreColorHex(pct),
      };
    }),
  };
}

function buildPerDeviceTable(perDevice, standard = null, coverageIndex = null) {
  // ⛔ THE SCOPED TABLE IS A DIFFERENT SHAPE, not the same table with four
  // columns hidden. With only one score left, a Device/Vendor/percentage row
  // is three cells of mostly whitespace and says less than the five-standard
  // version it replaced. The counts behind the percentage are what a scoped
  // reader actually needs — and showing `na` explicitly is what stops a
  // device scoring 100% off two measurable checks from looking fully assessed.
  if (standard) {
    return {
      columns: [
        { key: 'device', label: 'Device', width: 130 },
        { key: 'vendor', label: 'Vendor', width: 80 },
        { key: 'score', label: 'Score', width: 60, color: (r) => r._color },
        { key: 'pass', label: 'Pass', width: 50 },
        { key: 'fail', label: 'Fail', width: 50 },
        { key: 'warning', label: 'Warning', width: 60 },
        { key: 'na', label: 'Not assessable', width: 80, color: () => UNMEASURED },
      ],
      rows: perDevice.map((d) => {
        const c = d.standards[standard] || { pass: 0, fail: 0, warning: 0, na: 0, scorePct: null };
        return {
          device: d.deviceName,
          vendor: d.vendor,
          score: c.scorePct == null ? '-' : `${c.scorePct}%`,
          pass: c.pass,
          fail: c.fail,
          warning: c.warning,
          na: c.na,
          _color: scoreColorHex(c.scorePct),
        };
      }),
    };
  }
  // ⛔ THE DENOMINATOR TRAVELS IN THE COLUMN HEADER. This is a matrix of
  // twenty-odd bare percentages under five framework names, and the header is
  // the only place each framework is named — so "NIST (7)" is where the count
  // behind every figure in that column has to appear. The legend under the
  // table says what the number in brackets is. A header left bare means every
  // cell below it reads as a framework score, which is the misreading this
  // whole change exists to stop.
  return {
    columns: [
      { key: 'device', label: 'Device', width: 100 },
      { key: 'vendor', label: 'Vendor', width: 70 },
      ...STANDARDS.map((s) => {
        const cov = coverageIndex ? coverageIndex[s.key] : null;
        return {
          key: s.key,
          // ⛔ No brackets at all when the count could not be read — never
          // "(0)", which would say SecVault has no checks for the standard.
          label: cov && cov.mapped !== null ? `${s.label} (${cov.mapped})` : s.label,
          width: 71,
          color: (r) => r[`_${s.key}Color`],
        };
      }),
    ],
    rows: perDevice.map((d) => {
      const row = { device: d.deviceName, vendor: d.vendor };
      STANDARDS.forEach((s) => {
        const pct = d.standards[s.key].scorePct;
        row[s.key] = pct == null ? '-' : `${pct}%`;
        row[`_${s.key}Color`] = scoreColorHex(pct);
      });
      return row;
    }),
  };
}

function buildFindingsTable(group) {
  return {
    columns: [
      { key: 'checkName', label: 'Check Name', width: 130 },
      { key: 'severity', label: 'Severity', width: 55, color: (r) => r._sevColor },
      { key: 'status', label: 'Status', width: 55, color: (r) => r._stColor },
      { key: 'detail', label: 'Detail', width: 150 },
      { key: 'remediation', label: 'Remediation', width: 130 },
    ],
    rows: group.findings.map((f) => {
      const sev = SEVERITY_LABEL[f.severity] || SEVERITY_LABEL.info;
      const st = STATUS_LABEL[f.status] || STATUS_LABEL.fail;
      return {
        checkName: f.checkName,
        severity: sev.label,
        status: st.label,
        detail: f.detail || '-',
        remediation: f.remediationGuidance || '-',
        _sevColor: sev.color,
        _stColor: st.color,
      };
    }),
  };
}

// Draws the full report body onto an already-covered doc: fleet summary ->
// per-device scores -> one findings-appendix table per device group (only
// devices with fail/warning findings appear — matches the design decision
// to omit pass/na rows for brevity while never omitting warning, per
// CLAUDE.md's "unknown must never silently default to no" spirit).
function renderReportBody(doc, data, layout) {
  const { fleet, perDevice, findingsAppendix, standard, coverage } = data;
  const device = data.device || null;
  // ⛔ EVERY "fleet" NOUN IN THIS DOCUMENT IS SCOPE-DEPENDENT. The sentences
  // below were written when there was only one scope, and "across the fleet"
  // printed over a single firewall is not a wording slip — it is a claim
  // about fifteen firewalls this document never read.
  const estate = device ? `on ${device.name}` : 'across the fleet';
  const coverageIndex = data.coverageIndex || null;

  // Small helper so the three coverage paragraphs below share one rhythm.
  const note = (text, color = MUTED, font = 'Helvetica', size = 9) => {
    doc.fillColor(color).fontSize(size).font(font)
      .text(text, layout.left, doc.y, { width: layout.contentW });
    doc.y += 8;
  };

  // ⛔ The scoped headline is the standard's OWN score, not the fleet overall.
  // Printing the overall figure under a "PCI DSS Compliance Report" heading
  // would attribute a number to a framework it was not computed for.
  const headline = standard
    ? fleetForStandard(perDevice, standard).scorePct
    : fleet.overall;
  const standardName = standard ? (STANDARD_LABEL.get(standard) || standard) : null;
  const scopeName = standardName || (device ? device.name : 'Fleet');

  sectionTitle(doc, layout, `${scopeName} Summary — ${headline == null ? '-' : `${headline}%`}`);

  if (standard) {
    const scoped = coverageIndex ? coverageIndex[standard] : null;
    const comparability =
      ' This score is computed over those checks only, so it is not comparable with the overall '
      + 'compliance score or with another standard’s score — the denominators differ. Checks '
      + 'SecVault cannot ask of a device are excluded from the score entirely and are counted '
      + 'separately as not assessable.';
    // ⛔ ONE SOURCE FOR THE COUNT WHEN THERE IS ONE. standardCoverage() reads
    // the same mapped/total independently and stays as the fallback, but printing
    // both would let the two reads disagree inside one paragraph.
    const line = scoped && scoped.available
      ? scoped.basis + comparability
      : coverage
        ? `${coverage.mapped} of ${coverage.total} checks in SecVault's library carry a ${coverage.label} mapping.`
          + comparability
        // ⛔ Both reads failed. Say that, rather than printing the score with
        // no caveat at all - an unqualified scoped score is the misreading this
        // whole note exists to prevent.
        : 'The number of library checks mapped to this standard could not be read. This score covers only '
          + 'the checks that carry this mapping, so it is not comparable with the overall compliance score.';
    doc.fillColor(MUTED).fontSize(9).font('Helvetica')
      .text(line, layout.left, doc.y, { width: layout.contentW });
    doc.y += 10;
  }

  // ⛔ THE SENTENCE THIS DOCUMENT WAS MISSING, ON BOTH FORMS OF IT. The scoped
  // note above has existed since v2.121.0, but it only ever said the score was
  // not COMPARABLE with the overall one — never that it is not a measure of the
  // framework. And the fleet-wide document, the one the monthly job emails to
  // auditors, printed five framework percentages with no denominator anywhere.
  // Drawn in the unmeasured grey rather than body colour because it is a
  // statement about what is NOT known.
  note(COVERAGE_CLAIM, UNMEASURED);

  // ⛔ HOW OLD THE EVIDENCE IS, PRINTED — the other sentence this document was
  // missing. A score with no date beside it reads as current, and on the one
  // artefact an auditor files and re-reads months later that is the most
  // quietly wrong thing on the page. Drawn in the unmeasured grey for the same
  // reason as the coverage claim: it is a statement about what is NOT known,
  // not a finding. It NEVER moves a number.
  note(
    evidenceAgeStatement(data.evidenceAges, { device, now: data.generatedAt }),
    UNMEASURED
  );

  drawTable(doc, buildFleetSummaryTable(fleet, standard, coverageIndex), layout, { continueOnPage: true });

  // One line per standard in scope, stating the whole basis: how many library
  // checks carry the mapping, how many apply to the vendors on this fleet, how
  // many ran, how many could not be assessed, and how many the figure rests on.
  if (coverageIndex) {
    doc.y += 4;
    // Scoped reports already printed their one basis line above.
    const inScope = standard ? [] : STANDARD_KEYS;
    for (const key of inScope) {
      const cov = coverageIndex[key];
      if (cov) note(cov.basis, UNMEASURED, 'Helvetica', 8);
    }
  }

  // ⛔ OMITTED ENTIRELY ON A ONE-FIREWALL REPORT, not printed as a
  // single-row table. Scoped to one device this table restates the summary
  // above cell for cell — same five numbers, same five denominators — and a
  // reader meeting the same figure twice under two headings reasonably assumes
  // they are two different measurements.
  if (!device) {
    sectionTitle(doc, layout, 'Per-Device Scores');
    drawTable(doc, buildPerDeviceTable(perDevice, standard, coverageIndex), layout, { continueOnPage: true });
  }
  if (!device && !standard && coverageIndex) {
    doc.y += 4;
    // ⛔ Names what the bracketed number in each column header is. Without
    // this the header reads as part of the standard's name.
    note(
      'The number beside each standard is how many of SecVault’s library checks carry that '
      + 'mapping. A check commonly carries several, so the five columns do not sum to the library. '
      + 'Each cell is the share of that standard’s mapped checks the firewall passes; checks '
      + 'SecVault cannot ask of a device are excluded from it entirely.',
      UNMEASURED, 'Helvetica', 8
    );
  }

  sectionTitle(doc, layout, 'Findings Requiring Attention');
  doc
    .fillColor(MUTED)
    .fontSize(9)
    .font('Helvetica')
    .text(standard
      ? `Failing and warning checks mapped to ${standardName}, grouped by device. Passing/not-applicable checks are omitted for brevity.`
      : 'Failing and warning checks only, grouped by device. Passing/not-applicable checks are omitted for brevity.', layout.left, doc.y, {
      width: layout.contentW,
    });
  doc.y += 10;

  if (!findingsAppendix.length) {
    const blind = unassessedDevices(perDevice, standard);
    const clear = standard
      ? `No failing or warning findings mapped to ${standardName} ${estate}.`
      : `No failing or warning findings ${estate}.`;
    // ⛔ AN ALL-CLEAR IS ONLY PRINTED AS ONE WHEN THE COVERAGE BEHIND IT IS
    // COMPLETE. Otherwise the same sentence carries the gap that produced it,
    // in the unmeasured grey rather than in ordinary body colour — a fleet
    // nobody has assessed must not read as a fleet with nothing wrong.
    // ⛔ ONE FIREWALL GETS ITS OWN SENTENCE. "1 of 1 firewalls have no
    // findings" is arithmetically true and reads as a statistic about an
    // estate; scoped to one device the fact is simply that nothing was
    // assessed on it, and that has to be said plainly.
    const scopedTo = standard ? `${standardName} ` : '';
    const line = blind.length === 0
      ? clear
      : device
        ? `${clear} This is NOT an all-clear: ${device.name} has no ${scopedTo}findings recorded `
          + 'at all, so nothing could be assessed on it. Absence of a finding is not evidence '
          + 'of compliance.'
        : `${clear} This is NOT an all-clear: ${blind.length} of ${perDevice.length} `
          + `firewalls have no ${scopedTo}findings recorded at all, so `
          + 'nothing could be assessed on them. Absence of a finding is not evidence of compliance.';
    doc.fillColor(blind.length === 0 ? MUTED : UNMEASURED)
      .fontSize(10).font('Helvetica-Oblique')
      .text(line, layout.left, doc.y, { width: layout.contentW });
  } else {
    findingsAppendix.forEach((group) => {
      sectionTitle(doc, layout, group.deviceName);
      drawTable(doc, buildFindingsTable(group), layout, { continueOnPage: true });
    });
  }
}

// Pure-JS vector PDF generation via pdfkit — no browser, no child process,
// no service-account launch permissions to worry about. Same convention as
// every sibling NocVault suite app (logvault/ddivault/spanvault all use
// pdfkit for exactly this "generate a branded PDF from a Node service"
// need) — see this file's top-of-file comment for why puppeteer-core was
// abandoned. bufferPages:true is required for stampHeadersFooters()'s final
// switchToPage() pass to see every page already drawn.
async function generateReportPdf(pool, options = {}) {
  const data = await buildReportData(pool, options);
  // ⛔ null, NOT an empty fleet report. buildReportData() returns null only
  // when a deviceId was asked for and could not be resolved; the route turns
  // that into a 404 rather than serving a document about a different scope.
  if (!data) return null;
  const standardLabel = data.standard
    ? (STANDARD_LABEL.get(data.standard) || data.standard)
    : null;
  // ⛔ THE FIREWALL'S NAME GOES IN THE TITLE, not only in a table cell. This
  // document is filed and re-read months later, and "Compliance Report" over a
  // one-firewall body is how a device-scoped PDF gets mistaken for the fleet's.
  // ⛔ PREFIXED, so the two existing titles are unchanged byte for byte:
  // "Compliance Report" and "PCI DSS Compliance Report" are what every already-
  // filed copy is named, and renaming them to add a separator nobody asked for
  // would make this change look like a different report.
  const baseTitle = standardLabel ? `${standardLabel} Compliance Report` : 'Compliance Report';
  const title = data.device ? `${data.device.name} — ${baseTitle}` : baseTitle;

  const doc = installPdfSafeText(
    new PDFDocument({ size: 'A4', layout: 'portrait', margin: 36, bufferPages: true })
  );
  const layout = {
    pageW: doc.page.width,
    pageH: doc.page.height,
    left: doc.page.margins.left,
    right: doc.page.width - doc.page.margins.right,
    contentW: doc.page.width - doc.page.margins.left - doc.page.margins.right,
  };
  const generatedAt = fmtStamp(data.generatedAt);

  drawCover(
    doc,
    {
      title,
      company: PRODUCT_NAME,
      generatedAt,
      // ⛔ Reproduces THIS report's pre-chassis cover exactly: 28pt title at a
      // fixed y, accent rule at 238, 11pt/120px/22px metadata rhythm. Unifying
      // the two covers is a design decision, not a refactor side effect.
      fixedGeometry: true,
      titleSize: 28,
      summary: [
        data.standard
          ? (() => {
            const f = fleetForStandard(data.perDevice, data.standard);
            return { label: `${standardLabel} Score`, value: f.scorePct == null ? '-' : `${f.scorePct}%`, color: scoreColorHex(f.scorePct) };
          })()
          : { label: 'Fleet Score', value: data.fleet.overall == null ? '-' : `${data.fleet.overall}%`, color: scoreColorHex(data.fleet.overall) },
        // ⛔ "Devices: 1" is a tile that answers nothing. Scoped to one
        // firewall the useful fact is WHICH, and its vendor.
        data.device
          ? { label: 'Firewall', value: data.device.name, color: NAVY }
          : { label: 'Devices', value: data.perDevice.length, color: NAVY },
        { label: 'Open Findings', value: data.findingsAppendix.reduce((n, g) => n + g.findings.length, 0), color: STATUS_RED },
      ],
    },
    layout
  );

  renderReportBody(doc, data, layout);
  stampHeadersFooters(doc, {
    title: standardLabel ? `SecVault ${standardLabel} Compliance Report` : 'SecVault Compliance Report',
    company: 'SecVault',
    generatedAt,
  });

  return new Promise((resolve, reject) => {
    const chunks = [];
    doc.on('data', (c) => chunks.push(c));
    doc.on('end', () => resolve(Buffer.concat(chunks)));
    doc.on('error', reject);
    doc.end();
  });
}

/**
 * The reporting period: **the calendar month that just ended**, as `YYYY-MM`,
 * derived from the SERVER'S LOCAL clock — the same clock the monthly cron in
 * services/engine-worker.js fires on (node-cron is registered there with no
 * `timezone` option, so it fires in server-local time; see CLAUDE.md's Engine
 * Worker note that every "fixed HH:MM" job in this codebase is server-local,
 * not UTC).
 *
 * ⛔ It used to be `getUTCFullYear()/getUTCMonth()` — the CURRENT month, on a
 * DIFFERENT clock from the cron. On the reference deployment (Asia/Bangkok,
 * UTC+7, confirmed by `current_setting('TimeZone')`) `0 6 1 * *` fires at
 * 06:00 ICT on the 1st, which is 23:00 UTC on the LAST DAY OF THE PREVIOUS
 * MONTH. The two clocks disagreeing across a month boundary broke the
 * once-a-month guarantee in both directions, arithmetically:
 *
 *   1 Oct 06:00 ICT  → UTC month Sep → period 2026-09 → sent, 'success' row for 2026-09
 *   engine restart later that same day (the job also runs at startup)
 *                    → UTC month Oct → period 2026-10 → the SAME report emailed AGAIN
 *   1 Nov 06:00 ICT  → UTC month Oct → period 2026-10 → 'success' row already exists
 *                    → the scheduled tick is SKIPPED, and stays skipped every month after
 *
 * So: double-send in month one, then silence forever. Not yet observed in data
 * (compliance_report_log is empty — no channel is configured for
 * 'compliance_report' on this deployment yet), so this is arithmetic-confirmed,
 * not data-confirmed.
 *
 * Deriving "the month that just ended" from the same clock the cron fires on
 * fixes both halves at once: every tick and every restart ANYWHERE within local
 * October answers 2026-09, so the idempotency row means what it says, and the
 * 1 Nov tick asks a genuinely new question (2026-10).
 *
 * ⛔ CLAUDE.md's rule: the cron's timezone and this derivation are ONE decision.
 * If a `timezone` option is ever added to that cron.schedule() call, this
 * function must move to the same zone in the same commit — changing either
 * alone silently re-opens the bug above.
 */
function reportingPeriod(date = new Date()) {
  // Local getters throughout. Day 1 of (this month - 1); the Date constructor
  // normalises month -1 into December of the previous year on its own.
  const previous = new Date(date.getFullYear(), date.getMonth() - 1, 1);
  return `${previous.getFullYear()}-${String(previous.getMonth() + 1).padStart(2, '0')}`;
}

/**
 * Shared orchestration for BOTH the scheduled monthly job
 * (services/engine-worker.js's runComplianceReportJob) and the manual
 * POST /api/compliance/report/generate route — one code path, so "generate
 * now" and "the cron tick" can never drift out of step. Idempotent per
 * calendar month via compliance_report_log's partial unique index (see that
 * table's own comment in lib/schema.sql): skips entirely, no PDF generated,
 * no email sent, if a 'success' row already exists for this period.
 *
 * status='success' requires at least one channel to have actually received
 * the report — if every matching channel's send fails (e.g. every
 * configured SMTP relay is down), this logs 'error' instead of 'success'
 * with recipient_count=0, specifically so the partial unique index does
 * NOT block a retry later that same month. Per-channel failures are
 * separately visible on that channel's own last_error (recordChannelError),
 * same as every other alert type.
 *
 * `options.period` ('YYYY-MM') lets the caller state the period EXPLICITLY
 * rather than have it inferred here — services/engine-worker.js's cron job
 * passes it, so the tick and the period it claims are computed from one clock
 * at one call site instead of two functions independently guessing. Omitted
 * (the manual POST /api/compliance/report/generate route), it falls back to the
 * same reportingPeriod() default, so both callers still agree.
 * @param {import('pg').Pool} pool
 * @param {{period?: string}} [options]
 * @returns {Promise<{skipped: boolean, reason?: string, period: string, sent?: number}>}
 */
async function dispatchMonthlyReport(pool, options = {}) {
  const period =
    options && typeof options.period === 'string' && /^\d{4}-\d{2}$/.test(options.period)
      ? options.period
      : reportingPeriod();

  const already = await pool.query(
    `SELECT id FROM compliance_report_log WHERE period = $1 AND status = 'success'`,
    [period]
  );
  if (already.rows.length > 0) {
    return { skipped: true, reason: 'already sent this period', period };
  }

  const channels = await listEnabledChannelsWithSecrets(pool);
  const targets = channels.filter(
    (c) => c.channelType === 'email' && Array.isArray(c.alertTypes) && c.alertTypes.includes('compliance_report')
  );
  if (targets.length === 0) {
    return { skipped: true, reason: 'no channel configured for compliance_report', period };
  }

  const startedAt = new Date();
  try {
    const pdfBuffer = await generateReportPdf(pool);
    const message = {
      alertType: 'compliance_report',
      title: `SecVault Monthly Compliance Report — ${period}`,
      summary: `The fleet-wide compliance report for ${period} is attached.`,
      attachments: [
        {
          filename: `secvault-compliance-report-${period}.pdf`,
          content: pdfBuffer,
          contentType: 'application/pdf',
        },
      ],
    };

    let sent = 0;
    const errors = [];
    for (const channel of targets) {
      try {
        await dispatchNotification(channel, message);
        await recordChannelSuccess(channel.id, pool);
        sent += 1;
      } catch (err) {
        errors.push(`${channel.name}: ${err.message}`);
        await recordChannelError(channel.id, err.message, pool);
      }
    }

    if (sent === 0) {
      await pool.query(
        `INSERT INTO compliance_report_log (period, status, recipient_count, error, started_at, finished_at)
         VALUES ($1, 'error', 0, $2, $3, now())`,
        [period, `All ${targets.length} channel(s) failed to send: ${errors.join('; ')}`, startedAt]
      );
      return { skipped: false, period, sent: 0 };
    }

    await pool.query(
      `INSERT INTO compliance_report_log (period, status, recipient_count, started_at, finished_at)
       VALUES ($1, 'success', $2, $3, now())`,
      [period, sent, startedAt]
    );
    return { skipped: false, period, sent };
  } catch (err) {
    await pool.query(
      `INSERT INTO compliance_report_log (period, status, recipient_count, error, started_at, finished_at)
       VALUES ($1, 'error', 0, $2, $3, now())`,
      [period, err.message, startedAt]
    );
    throw err;
  }
}

module.exports = {
  STANDARDS,
  STANDARD_KEYS,
  summaryFromPerDevice,
  resolveStandard,
  standardCoverage,
  libraryCoverage,
  fleetCheckCoverage,
  collectionAges,
  evidenceAgeStatement,
  buildCoverageIndex,
  fleetForStandard,
  unassessedDevices,
  buildFleetSummaryTable,
  buildPerDeviceTable,
  buildReportData,
  generateReportPdf,
  dispatchMonthlyReport,
  reportingPeriod,
};
