...
function vulnerabilitySubscore({ activeDevices, devicesWithPatchNow, devicesWithScheduled }) {
  if (!Number.isFinite(activeDevices) || activeDevices <= 0) return null;
  const patchNow = Math.max(0, Number(devicesWithPatchNow) || 0);
  const scheduled = Math.max(0, Number(devicesWithScheduled) || 0);
  const exposure = (patchNow + scheduled * SCHEDULED_EXPOSURE_FACTOR) / activeDevices;
...
function complianceSubscore(fleetCompliancePct) {
  if (fleetCompliancePct === null || fleetCompliancePct === undefined) return null;
  return clampScore(Number(fleetCompliancePct));
}
