 ...
 * @returns {Date|null}
 */
// Raw keys that carry an UNAMBIGUOUS absolute start time (epoch seconds),
// alongside the ambiguous display string. PAN-OS GlobalProtect reports BOTH:
//   "login-time":"Sep.10 09:03:53", "login-time-utc":"1789005833"
// Verified live 2026-09-10: that epoch is 2026-09-10T02:03:53Z, i.e. exactly
// the displayed 09:03:53 at +07.
const AUTHORITATIVE_EPOCH_KEYS = ['login-time-utc', 'login_time_utc', 'login-time-epoch'];

/**
 * ⛔ PREFER THE DEVICE'S OWN EPOCH OVER PARSING ITS DISPLAY STRING.
 *
 * parseLoginTime() below has to infer a YEAR (the string carries none) and
 * assume a ZONE (it carries none either). Both inferences are sound, and both
 * are unnecessary whenever the device already stated the answer — which
 * PAN-OS does, in the same object, on every session. Guessing a value the
 * source supplied verbatim is the same mistake as recording a failed read as
 * a fact, just in a quieter register.
 *
 * Using the epoch also removes the ONLY case where a firewall configured in a
 * different timezone from the SecVault host would skew a duration: the epoch
 * is absolute, so nothing shifts.
 *
 * Falls back to the string parse when no epoch is present (every non-PAN-OS
 * vendor today), so behaviour is unchanged where there is nothing better.
 */
function resolveLoginTime(rec, now) {
  const raw = rec && rec.raw;
  if (raw && typeof raw === 'object' && !Array.isArray(raw)) {
    for (const key of AUTHORITATIVE_EPOCH_KEYS) {
      const v = raw[key];
  ...
  return parseLoginTime(rec ? rec.login_time : null, now);
}
...
async function storeVpnSessions(deviceId, sessions, pool, options = {}) {
  ...
  try {
    ...
    for (const s of list) {
      ...
      const loginAt = resolveLoginTime(rec, now);
      if (!username || !loginAt) {
        unsessionizable += 1;
        continue;
      }
