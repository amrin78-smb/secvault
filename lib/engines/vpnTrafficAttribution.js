...
//
// ── ⛔ FILTERS NARROW THE ANSWER, AND NOW ALSO THE REFUSALS ───────────────
// Attribution ALWAYS runs over the whole session set and the whole bucket set;
// `username`/`deviceId` are applied AFTERWARDS and that ordering is pinned by a
// test. Narrowing the SQL would let a filtered view make an ambiguous address
// look clean, which is the one thing this module exists to prevent.
//
// What filtering did NOT do, until v2.104.x, is narrow the UNATTRIBUTED
// figures. `unattributed`/`collisions` were fleet-wide totals that the UI drew
// verbatim under a filter, so `?utUser=alice` showed Alice's single row above
// 166,892 unnamed events and a collision table naming twenty other employees —
// presented as the answer to "what about Alice". That is a privacy leak in one
// direction and a wrong denominator in the other.
//
// `result.scope` is the fix: the SAME buckets, tied to the filtered subject
// where a tie genuinely exists, and per reason the tie is DIFFERENT and is
// stated rather than blurred:
//
//   * partial_hour → tied by SESSION: the subject's own session held the
//     address for part of that hour;
//   * collision    → tied by SESSION: the subject's own session was one of the
//     parties. Naming the counterparty is the point — they are WHY this hour is
//     missing from the subject's total;
//   * gap          → tied by ADDRESS only: nobody held the address then, so the
//     traffic is nobody's, and the strongest honest statement is "on an address
//     this subject held elsewhere in the window". The label says exactly that.
//
// ⛔ A bucket with NO tie to the subject is NOT dropped. It stays in the
// fleet-wide totals, which the UI renders beside the scoped ones and labels as
// fleet-wide. Scoping is a lens, never a deletion — a coverage gap that
// disappears when you filter is this codebase's dominant bug wearing a filter.
//
// ⛔ scope ⊆ fleet, always: every scoped bucket is counted once in the subject's
// tally and is also present in the fleet total, so "X of Y" is honest. Scoped
// tallies of two DIFFERENT subjects may overlap (both held an address that has
// a gap hour) and must never be added together.
//
...
// ⛔ AN ABSENT OPTION IS THE DEFAULT, NOT THE MINIMUM. Caught live: this read
// `Number(String(value ?? '').trim())`, and `Number('')` is 0 — finite — so an
// omitted `topUsers` clamped to 1 and the page silently drew ONE user out of
// 62 while `usersTotal` correctly said 62. Nothing threw, and the number on
// screen was a plausible table. Empty, blank and absent must all fall through
// to the fallback.
...
function attributeTraffic(input) {
  ...
  // ⛔ ONE RECORD PER UNATTRIBUTED BUCKET, kept so a filtered view can say which
  // of these refusals are the subject's own instead of reprinting the fleet's.
  // It carries the session ids involved (none for a gap — that is the whole
  // meaning of a gap) and never the events of an attributed bucket. Bounded by
  // the same MAX_BUCKETS ceiling the caller already applies and reports; a gap
  // deliberately stores only its address rather than every session that ever
  // held it, so a heavily recycled pool address cannot blow the record up.
  const unattributedBuckets = [];
  // Per-address bucket/event totals — the DENOMINATOR a scoped panel needs.
  // "27% of all traffic" computed against the fleet under a one-user filter is
  ...
  for (const b of buckets) {
    ...
    let ipTotal = ipTotals.get(ip);
    if (!ipTotal) {
      ipTotal = { buckets: 0, events: 0 };
      ipTotals.set(ip, ipTotal);
    }
    ipTotal.buckets += 1;
    ipTotal.events += events;
    ...
    if (overlapping.length === 0) {
      ...
      // No session ids: by definition nobody held this address in this hour. A
      // filtered view may tie it to a subject by ADDRESS only, and must say so.
      unattributedBuckets.push({
        reason: 'gap',
        ip,
        hourStart,
        events,
    ...
    if (overlapping.length > 1) {
      ...
      const sessionIds = overlapping.map((s) => s.id);
      ...
      collisions.push({
        ...
        events,
        // Carried so a filtered view can keep only the collisions the subject
        // was actually party to. Under a filter, another employee's name may
        // appear ONLY as the counterparty to the subject's own ambiguity.
        sessionIds,
        deviceIds: [...new Set(overlapping.map((s) => s.deviceId).filter(Boolean))],
      });
      unattributedBuckets.push({
        reason: 'collision',
        ip,
  ...
  return {
    ...
    collisions,
    totals,
    // Raw material for scoping, never rendered as-is.
    unattributedBuckets,
    ipTotals,
  };
}

// ⛔ SCOPE A REFUSAL TO THE FILTERED SUBJECT — without deleting anything.
//
// Called ONLY after attributeTraffic() has run over the complete session and
// bucket sets, so nothing here can make an ambiguous address look unambiguous:
// the ambiguity was already decided, this only decides what to SHOW.
//
// The tie is per reason and is deliberately unequal (see the header): session
// identity for partial_hour and collision, address only for gap. Whatever fails
// to tie stays where it was — in the fleet totals the caller also returns.
//
// @param {object} attributed  the return value of attributeTraffic()
// @param {Array}  subjectSessions  the sessions the filter selected
function scopeUnattributed(attributed, subjectSessions) {
  const sessions = Array.isArray(subjectSessions) ? subjectSessions : [];
  const ids = new Set(sessions.map((s) => s && s.id).filter((v) => v != null));
  const ips = new Set(
    sessions.map((s) => normalizeIp(s && s.assignedIp)).filter(Boolean)
  );
  ...
  const records = Array.isArray(attributed && attributed.unattributedBuckets)
    ? attributed.unattributedBuckets
    : [];
  for (const rec of records) {
    // A gap has no session ids — that IS the gap. Address membership is the
    // only tie available and the only one claimed.
    const tied = rec.sessionIds
      ? rec.sessionIds.some((id) => ids.has(id))
      : ips.has(rec.ip);
    if (!tied) continue;
    const slot = unattributed[rec.reason];
    if (!slot) continue;
    slot.buckets += 1;
  ...
  const ipTotals = attributed && attributed.ipTotals instanceof Map
    ? attributed.ipTotals
    : new Map();
  for (const ip of ips) {
    const t = ipTotals.get(ip);
    if (!t) continue;
    bucketsConsidered += t.buckets;
    eventsConsidered += t.events;
  }

  const allCollisions = Array.isArray(attributed && attributed.collisions)
    ? attributed.collisions
    : [];
  const collisions = allCollisions.filter(
    (c) => Array.isArray(c.sessionIds) && c.sessionIds.some((id) => ids.has(id))
  );
...
async function getVpnUserTraffic(pool, options = {}) {
  ...
  // ⛔ THE SCOPED VIEW. `result.unattributed`, `result.collisions` and
  // `result.totals` above stay FLEET-WIDE under every filter — deliberately, so
  // a caller that ignores `scope` under-claims (shows a bigger, honest number)
  // rather than over-claims. Everything narrowed to the subject lives here.
  if (wantUser || wantDevice) {
    const subjectSessions = sessions.filter(
      (s) =>
        (!wantUser || String(s.username || '').toLowerCase() === wantUser)
        && (!wantDevice || s.deviceId === wantDevice)
    );
    const scoped = scopeUnattributed(attributed, subjectSessions);
    const hoursAttributed = sessionRowsOut.reduce((a, s) => a + s.attributedHours, 0);
    const eventsAttributed = sessionRowsOut.reduce((a, s) => a + s.events, 0);
    const unnamedBuckets = UNATTRIBUTED_REASONS.reduce(
      (a, k) => a + scoped.unattributed[k].buckets, 0
    );
    const unnamedEvents = UNATTRIBUTED_REASONS.reduce(
      (a, k) => a + scoped.unattributed[k].events, 0
    );

    result.scope = {
      active: true,
      // Echo what was asked for, and separately what actually matched — an
      // operator who mistyped a username must be able to see both.
      requestedUsername: typeof opts.username === 'string' ? opts.username.trim() : null,
      requestedDeviceId: wantDevice,
      // Canonical casing as the gateway reported it, not as it was typed.
      username: subjectSessions.length > 0 && wantUser ? subjectSessions[0].username : null,
      deviceId: wantDevice,
      // Only when a GATEWAY was actually asked for. A username filter spans
      // whichever gateways that person used, and naming one of them here would
      // read as "they were only on this one".
      deviceName: wantDevice
        ? ((subjectSessions.find((s) => s.deviceName) || {}).deviceName || null)
        : null,
      gateways: [...new Set(subjectSessions.map((s) => s.deviceName).filter(Boolean))].sort(),
      matchedSessions: subjectSessions.length,
      addresses: scoped.addresses,
      unattributed: scoped.unattributed,
      unattributedBuckets: unnamedBuckets,
      unattributedEvents: unnamedEvents,
      collisions: scoped.collisions.slice(0, 50),
      collisionsTotal: scoped.collisionsTotal,
      // Denominator for "what share of THIS subject's address traffic went
      // unnamed" — the fleet denominator would be a wrong number under a filter.
      bucketsConsidered: scoped.bucketsConsidered,
      eventsConsidered: scoped.eventsConsidered,
      hoursAttributed,
      eventsAttributed,
      usersNamed: users.length,
      reason: null,
      reasonText: null,
    };

    // ⛔ FOUR DIFFERENT ZEROS, and which one it is IS the answer. Same
    // {reason, reasonText} shape as `coverage` above, on purpose: a caller that
    // already knows how to render one renders the other unchanged. Order is
    // narrowest-cause-first — a filter that matched nothing explains every
    // downstream zero, so it must be reported instead of them.
    if (subjectSessions.length === 0) {
      result.scope.reason = 'filter_matched_no_sessions';
      result.scope.reasonText =
        (wantUser && wantDevice
          ? 'No VPN session on this gateway carries this username in this window.'
          : wantUser
            ? 'No VPN session in this window carries this username. Either nobody by that '
              + 'name connected, or the name is spelled differently on the gateway.'
            : 'No VPN session was retained for this gateway in this window. Only Palo Alto '
              + 'gateways report per-session VPN detail at all.')
        + ' There is therefore no subject to scope anything to, and the figures on this page '
        + 'are NOT MEASURED for this filter rather than zero. Nothing below is this filter’s '
        + 'answer.';
    } else if (scoped.bucketsConsidered === 0) {
      result.scope.reason = 'no_traffic_on_subject_addresses';
      result.scope.reasonText =
        'The addresses these sessions held generated no rows at all in the traffic rollup '
        + 'for this window, so nothing about this subject’s traffic has been measured — '
        + 'named or unnamed.';
    } else if (hoursAttributed === 0) {
      result.scope.reason = 'no_attributable_traffic_for_subject';
      result.scope.reasonText =
        'Traffic was seen on the addresses these sessions held, but none of it fell in an '
        + 'hour one of those sessions demonstrably held on its own, so no name can be put to '
        + 'it. Every event of it is still counted — below if it ties to this subject, '
        + 'otherwise in the fleet-wide total.';
    } else if (unnamedBuckets === 0) {
      result.scope.reason = 'nothing_unattributed_for_subject';
      result.scope.reasonText =
        'Every address-hour that ties to this subject was attributable to exactly one of '
        + 'their sessions. The fleet-wide figure below is unchanged by this filter and is '
        + 'not this subject’s.';
    }
  }
...
function emptyResult(meta) {
  return {
    ...
    // ⛔ Null means NO FILTER IS ACTIVE — never "the filter found nothing".
    // That case is an object carrying its own reason, because an operator who
    // filtered to a name and an operator who filtered to nobody must not be
    // shown the same screen.
    scope: null,
