...
function pullIntervalHours(env = process.env) {
  ...
  // ⛔ CLAMPED TO AT LEAST ONE HOUR. `n > 0` ran BEFORE Math.trunc, so a
  // legitimate sub-hour cadence (CONFIG_PULL_INTERVAL_HOURS=0.5) passed the
  // guard and truncated to 0 — which made `intervals` null, so STALE and
  // AGEING became unreachable and every device read `fresh` for ever. An
  // operator tightening the pull interval silently switched the whole
  // staleness feature off, which is the "a typo must never disable a control"
  // rule this codebase applies to SESSION_IDLE_MINUTES.
  if (!Number.isFinite(n) || n <= 0) return DEFAULT_PULL_HOURS;
  return Math.min(Math.max(1, Math.trunc(n)), 24 * 30);
}
