...
function licenseStatus(row, now, warnDays = DEFAULT_LICENSE_WARN_DAYS) {
  ...
  if (!expiresAt) {
    // No parsed date. The raw string is what distinguishes a genuinely
    // perpetual licence from one whose expiry we simply could not read.
    const raw = typeof row.expires_raw === 'string' ? row.expires_raw : row.expiresRaw;
    if (typeof raw === 'string' && /^never$/i.test(raw.trim())) {
 ...
 */
const LICENSE_SEVERITY = { expired: 4, expiring: 3, unknown: 2, ok: 1, perpetual: 1 };
