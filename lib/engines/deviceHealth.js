...
function worstLicenseStatus(rows, now, warnDays = DEFAULT_LICENSE_WARN_DAYS) {
  ...
  for (const row of list) {
    ...
    if (
      ...
      // at the most urgent renewal rather than an arbitrary one. A null
      // daysRemaining (a perpetual licence, or an 'expired' verdict that came
      // from the device's own flag rather than a date) sorts AFTER any dated
      // row — requiring both to be numbers meant row order decided instead,
      // which could surface the undated one and hide the actionable date.
      (rank === worst.rank &&
        typeof s.daysRemaining === 'number' &&
        (typeof worst.daysRemaining !== 'number' || s.daysRemaining < worst.daysRemaining))
    ) {
