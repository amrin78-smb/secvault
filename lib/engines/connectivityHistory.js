...
// ⛔ 'vpn' WAS MISSING HERE until 2026-09-09, and the omission was silent and
// actively misleading. recordConnectivity falls back to 'collect' for an
// unrecognised source, so every VPN-poll observation — added 2026-08-25
// specifically so a failing VPN poll would stop reading as healthy — was filed
// under 'collect' instead. The tooltip then attributed a VPN failure to a
// collect that had never run, and OKF(F2) reported "collect 0/14" while its
// actual collect had just succeeded.
//
// ⛔ Any new poller MUST add its source here. The fallback exists so a bad
// source string cannot lose an observation entirely, but it buys that by
// misattributing it, which is worse when nobody checks. Keep this set in step
// with every recordConnectivity caller in services/engine-worker.js.
const VALID_SOURCES = new Set(['test', 'collect', 'metrics', 'vpn']);
  ...
  for (const r of rows) {
    if (!byDevice.has(r.device_id)) {
      byDevice.set(r.device_id, {
        ...
        sources: {},
        worstRate: null,
        // ⛔ WHICH source is worst, not just how bad. "0% of polls succeeding"
        // with no subject reads as "nothing about this device works" — it was
        // shown for OKF(F2) on 2026-09-09 while two of its three sources were
        // green and it had just been collected in full. A rate is only
        // actionable if the operator knows what it is a rate OF.
        worstSource: null,
        lastSuccessAt: null,
        lastError: null,
      });
    ...
    if (rate !== null && (d.worstRate === null || rate < d.worstRate)) {
      d.worstRate = rate;
      d.worstSource = r.source;
    }
