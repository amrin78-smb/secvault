// lib/engines/cloudApps.js
//
// Names an address or hostname against the published cloud catalogue: "this is
// Microsoft Teams", "this is inside AWS". Pure — takes catalogue rows in and
// returns a verdict. The fetching lives in lib/feeds/cloudApps.js.
//
// ⛔ WHY THIS EXISTS. Measured on the reference fleet: 510 distinct FQDN
// address objects sit in the rulebase, and 102 rules reference them. Today they
// are opaque strings, which is why flows touching them come back unverified and
// why the unaccounted-rule list is a wall rather than something readable. The
// catalogue turns a third of them into names.
//
// ⛔ THE THREE RULES THAT MAKE THIS HONEST RATHER THAN DECORATIVE:
//
// 1. AN EMPTY CATALOGUE IS "UNKNOWN", NEVER "NOT A CLOUD APP". This product
//    installs on segmented and air-gapped networks — that is its target
//    customer, not an edge case — where these feeds cannot be fetched at all.
//    A lookup with nothing to look in must say it could not answer. Reporting
//    "not a cloud app" there is the failed-read-as-a-fact bug in its most
//    plausible form, because the output looks like a real negative.
//
// 2. A PROVIDER IS NOT AN APPLICATION. An address inside AWS's ranges is AWS.
//    It is NOT Salesforce, or any other thing a customer happens to run on AWS.
//    Where a feed publishes no service breakdown, `service` stays null and the
//    label says only what was published.
//
// 3. THE FEED'S GRANULARITY IS OUR GRANULARITY. Microsoft publishes four
//    service areas, and 131 of the 152 live matches land in its catch-all
//    "Microsoft 365 Common and Office Online". We can say Teams. We cannot say
//    Word, and must not imply it.

'use strict';

// ⛔ The CIDR parser is BORROWED from applicationView.js, not rewritten. Two
// implementations of IPv4 range arithmetic in one codebase is how they drift,
// and the one that drifts is the one making a claim about a firewall rule.
const { parseCidr } = require('./applicationView');

const STATES = {
  MATCHED: 'matched',
  NO_MATCH: 'no_match',
  UNAVAILABLE: 'unavailable',
};

// How old the catalogue may be before a reader should be told. Not an expiry:
// ⛔ a stale catalogue is STALE, not WRONG. Published ranges change slowly, and
// refusing to answer from a two-week-old copy would be worse than answering
// with its age attached.
const STALE_AFTER_DAYS = 14;

const PROVIDER_LABEL = {
  microsoft_365: 'Microsoft 365',
  aws: 'AWS',
  google_cloud: 'Google Cloud',
  cloudflare: 'Cloudflare',
};

function providerLabel(provider) {
  // ⛔ An unknown provider prints its own raw key rather than a friendly guess.
  // A feed added later must show up as itself, not as blank or as "Other".
  return PROVIDER_LABEL[provider] || provider || 'Unknown provider';
}

// ── Hostnames ──────────────────────────────────────────────────────────────

function normaliseHost(h) {
  if (typeof h !== 'string') return null;
  const s = h.trim().toLowerCase().replace(/\.$/, '');
  return s || null;
}

/**
 * Does a catalogue pattern cover this host?
 *
 * ⛔ A WILDCARD MATCHES SUBDOMAINS ONLY, NEVER THE APEX. `*.office.com` does
 * not name `office.com` here. That is how firewalls themselves generally treat
 * the form, and it is the safe direction for a NAMING feature: under-claiming
 * leaves something unlabelled, which is visible and harmless, while
 * over-claiming puts a confident wrong name on a rule. Microsoft lists an apex
 * separately when it means it.
 */
function hostMatches(pattern, host) {
  const p = normaliseHost(pattern);
  const h = normaliseHost(host);
  if (!p || !h) return false;
  if (p.startsWith('*.')) {
    const suffix = p.slice(1); // '.office.com'
    return h.endsWith(suffix) && h.length > suffix.length;
  }
  return h === p;
}

/**
 * How specific a host pattern is, for choosing between two matches.
 * An exact name always beats a wildcard; among wildcards, more labels wins.
 */
function hostSpecificity(pattern) {
  const p = normaliseHost(pattern) || '';
  const labels = p.split('.').filter(Boolean).length;
  return p.startsWith('*.') ? labels : labels + 100;
}

// ── Addresses ──────────────────────────────────────────────────────────────

function ipToUint32(ip) {
  const r = parseCidr(ip);
  // parseCidr treats a bare address as a /32. Anything wider is not a single
  // address and is refused rather than silently taking the network address.
  if (!r || r.start !== r.end) return null;
  return r.start;
}

/** A catalogue CIDR to inclusive bounds. Returns null for anything unparseable. */
function cidrBounds(cidr) {
  const r = parseCidr(cidr);
  if (!r) return null;
  return { start: r.start, end: r.end };
}

// ── The address index ──────────────────────────────────────────────────────
//
// ⛔ A LINEAR SCAN IS NOT VIABLE HERE, measured rather than assumed: checking
// the reference fleet's 7,122 address objects against 11,766 catalogue entries
// took 18.4 SECONDS unindexed — 84 million comparisons for a page section.
// Bucketing by first octet cuts each lookup to the handful of ranges that could
// possibly contain the address.
//
// ⛔ A RANGE SPANNING SEVERAL /8s IS FILED UNDER EVERY ONE IT TOUCHES. Filing
// it only under its start would make addresses in its tail invisible to the
// index — a silent under-match, which on a naming feature means quietly
// reporting "not a cloud service" for something that is one.

const OCTET = 16777216; // 2^24

/**
 * A stored bound to a usable number, or null.
 *
 * ⛔ Number(null) IS 0, AND 0 IS FINITE. A bare
 * `Number.isFinite(Number(row.range_start))` therefore ACCEPTS a row whose
 * bounds were never populated and files it at 0.0.0.0 — where it silently
 * claims addresses in 0.0.0.0/8. Caught by a test probing that exact octet
 * rather than a convenient one; the same mistake was live in matchIp's own
 * loop, where the existing test happened to probe 1.1.1.1 and missed it.
 */
function boundOrNull(v) {
  if (v === null || v === undefined || v === '') return null;
  const n = Number(v);
  return Number.isFinite(n) ? n : null;
}

function buildIpIndex(entries) {
  const byOctet = new Map();
  let usable = 0;
  for (const e of Array.isArray(entries) ? entries : []) {
    const start = boundOrNull(e.range_start);
    const end = boundOrNull(e.range_end);
    if (start === null || end === null) continue;
    usable += 1;
    const first = Math.floor(start / OCTET);
    const last = Math.floor(end / OCTET);
    for (let o = first; o <= last; o += 1) {
      if (!byOctet.has(o)) byOctet.set(o, []);
      byOctet.get(o).push(e);
    }
  }
  return { __ipIndex: true, byOctet, size: usable };
}

/** How many entries are available, whether given an index or a plain array. */
function entryCount(entries) {
  if (entries && entries.__ipIndex) return entries.size;
  return Array.isArray(entries) ? entries.length : 0;
}

/** The entries that could possibly cover [startN, endN]. */
function candidates(entries, startN, endN) {
  if (!entries || !entries.__ipIndex) return Array.isArray(entries) ? entries : [];
  const first = Math.floor(startN / OCTET);
  const last = Math.floor(endN / OCTET);
  if (first === last) return entries.byOctet.get(first) || [];
  const seen = new Set();
  for (let o = first; o <= last; o += 1) {
    for (const e of entries.byOctet.get(o) || []) seen.add(e);
  }
  return [...seen];
}

// ── Choosing between matches ───────────────────────────────────────────────

/**
 * The identity a match CLAIMS: a provider AND the service that provider
 * published under it.
 *
 * ⛔ THE PROVIDER ALONE IS NOT THE CLAIM, and treating it as one was a live
 * bug. AWS publishes the SAME prefix once per service — measured on the live
 * catalogue, 2,297 distinct AWS ranges carry more than one `service`, and
 * `1.178.4.0/24` is listed under AMAZON, EC2 and S3 — while Microsoft lists
 * `*.sharepointonline.com` under both Common and SharePoint. Those rows tie on
 * size/specificity, so the winner was whichever the database happened to return
 * first, from a SELECT with no ORDER BY over a heap every sync rewrites. The
 * label therefore flipped between refreshes, and `ambiguous` said `false` with
 * no alternatives — an ambiguity resolved silently, which is the one thing
 * matched() promises not to do.
 */
function claimKey(entry) {
  return `${entry.provider || ''}::${entry.service_display || entry.service || ''}`;
}

/**
 * Pick one winner from equally-good matches and report the rest.
 *
 * @param {{entry:object, rank:number}[]} scored  LOWER rank is a better match
 *   (a smaller published range, a more specific hostname pattern).
 *
 * ⛔ THE WINNER IS DETERMINISTIC. Among rows of equal rank the smallest
 * `claimKey` wins — an ARBITRARY rule, deliberately, because there is no
 * meaningful way to prefer "AWS — EC2" over "AWS — S3" when the publisher
 * states both. What matters is that it is STABLE: an arbitrary-but-fixed choice
 * plus a disclosed alternatives list is an honest answer, while an
 * order-dependent one renames somebody's firewall rule between two page loads
 * with neither name wrong and no way to tell which.
 */
function pickBest(scored) {
  let best = null;
  for (const m of scored) {
    if (best === null
      || m.rank < best.rank
      || (m.rank === best.rank && claimKey(m.entry) < claimKey(best.entry))) {
      best = m;
    }
  }
  if (!best) return null;

  // Every OTHER distinct claim that matched just as well. Same-claim duplicates
  // (the same provider and service listed twice) are not alternatives — they
  // are one statement made twice.
  const bestKey = claimKey(best.entry);
  const rivals = new Map();
  for (const m of scored) {
    if (m.rank !== best.rank) continue;
    const k = claimKey(m.entry);
    if (k === bestKey || rivals.has(k)) continue;
    rivals.set(k, m.entry);
  }
  return { entry: best.entry, alternatives: [...rivals.values()] };
}

// ── Matching ───────────────────────────────────────────────────────────────

function unavailable(reason) {
  return {
    state: STATES.UNAVAILABLE,
    reason,
    provider: null,
    service: null,
    label: null,
    // ⛔ Present and explicitly null so a caller destructuring this cannot
    // accidentally read a missing key as false.
    ambiguous: null,
  };
}

function noMatch() {
  return {
    state: STATES.NO_MATCH, reason: null, provider: null, service: null, label: null, ambiguous: false,
  };
}

function matched(entry, alternatives) {
  const service = entry.service_display || entry.service || null;
  return {
    state: STATES.MATCHED,
    provider: entry.provider,
    providerLabel: providerLabel(entry.provider),
    service,
    // ⛔ The label says exactly what was published and no more. With no service
    // breakdown it is the provider alone, which is the honest answer.
    label: service ? `${providerLabel(entry.provider)} — ${service}` : providerLabel(entry.provider),
    value: entry.value,
    category: entry.category || null,
    sourceVersion: entry.source_version || null,
    // ⛔ AMBIGUITY IS REPORTED, NOT RESOLVED SILENTLY. Two providers can
    // legitimately publish overlapping space (a SaaS fronted by Cloudflare, a
    // service inside a hyperscaler range). Picking one and hiding the other
    // would make a coincidence look like a fact.
    // ⛔ AND ONE PROVIDER CAN DO IT TO ITSELF — the case this originally missed.
    // AWS lists the same prefix under AMAZON, EC2 and S3; Microsoft lists
    // `*.sharepointonline.com` under both Common and SharePoint. A rival CLAIM
    // is a (provider, service) pair, not a provider. See claimKey/pickBest.
    ambiguous: alternatives.length > 0,
    alternatives: alternatives.map((a) => ({
      provider: a.provider,
      providerLabel: providerLabel(a.provider),
      service: a.service_display || a.service || null,
      value: a.value,
    })),
    reason: null,
  };
}

/**
 * @param {string} host
 * @param {object[]|null} entries catalogue rows with kind='host'
 * @param {{count?:number}} [meta] total catalogue size, to tell "empty" from "no match"
 */
function matchHost(host, entries, meta = {}) {
  if (!Array.isArray(entries) || entries.length === 0) {
    return unavailable(
      Number(meta.count) > 0
        ? 'No hostname entries in the catalogue.'
        : 'The cloud catalogue is empty — the published feeds have not been fetched on this install.'
    );
  }
  const h = normaliseHost(host);
  if (!h) return noMatch();

  const scored = [];
  for (const e of entries) {
    if (!hostMatches(e.value, h)) continue;
    // Negated so LOWER is better, the one convention pickBest understands.
    scored.push({ entry: e, rank: -hostSpecificity(e.value) });
  }
  const picked = pickBest(scored);
  if (!picked) return noMatch();
  return matched(picked.entry, picked.alternatives);
}

/**
 * @param {string} ip a single IPv4 address
 * @param {object[]|null} entries catalogue rows with kind='ip' and numeric bounds
 */
function matchIp(ip, entries, meta = {}) {
  if (entryCount(entries) === 0) {
    return unavailable(
      Number(meta.count) > 0
        ? 'No address entries in the catalogue.'
        : 'The cloud catalogue is empty — the published feeds have not been fetched on this install.'
    );
  }
  const n = ipToUint32(ip);
  if (n === null) return noMatch();

  const scored = [];
  for (const e of candidates(entries, n, n)) {
    // ⛔ boundOrNull, NOT Number(). node-pg returns BIGINT as a STRING
    // ('50596868'), so a coercion is required — but Number(null) is 0, which
    // would accept an unpopulated row and match it against 0.0.0.0. See the
    // helper.
    const start = boundOrNull(e.range_start);
    const end = boundOrNull(e.range_end);
    if (start === null || end === null) continue;
    if (n < start || n > end) continue;
    // ⛔ SMALLEST RANGE WINS — rank IS the size, so lower is better. A /32
    // published for Teams beats the /8 the same address also sits inside; the
    // specific statement is the informative one. Equal sizes go to pickBest,
    // which reports them rather than picking one quietly.
    scored.push({ entry: e, rank: end - start });
  }
  const picked = pickBest(scored);
  if (!picked) return noMatch();
  return matched(picked.entry, picked.alternatives);
}

/**
 * Does a whole declared RANGE sit inside published provider space?
 *
 * ⛔ FULL CONTAINMENT, NEVER OVERLAP. A /16 that merely clips the edge of a
 * provider's /24 is not "pinned to that provider" — it is a big range that
 * happens to touch one, and reporting it as the former would put a confident
 * wrong label on somebody's internal supernet. Only a range entirely inside
 * published space is a statement about what that object is for.
 */
function matchRange(cidr, entries, meta = {}) {
  if (entryCount(entries) === 0) {
    return unavailable(
      Number(meta.count) > 0
        ? 'No address entries in the catalogue.'
        : 'The cloud catalogue is empty — the published feeds have not been fetched on this install.'
    );
  }
  const b = cidrBounds(cidr);
  if (!b) return noMatch();

  const scored = [];
  for (const e of candidates(entries, b.start, b.end)) {
    const start = boundOrNull(e.range_start);
    const end = boundOrNull(e.range_end);
    if (start === null || end === null) continue;
    if (b.start < start || b.end > end) continue; // must be wholly inside
    scored.push({ entry: e, rank: end - start });
  }
  const picked = pickBest(scored);
  if (!picked) return noMatch();
  return matched(picked.entry, picked.alternatives);
}

// ── Catalogue health ───────────────────────────────────────────────────────

/**
 * Is the catalogue usable, and how old is it?
 *
 * ⛔ THREE STATES, NOT TWO. `empty` is not a kind of `stale` and neither is a
 * kind of `ok`: an install that has never fetched is a different situation from
 * one whose copy is a month behind, and only the second can answer anything at
 * all. Collapsing them would tell an air-gapped operator their fleet uses no
 * cloud services.
 */
function catalogueStatus(summary, now = new Date()) {
  const count = Number(summary && summary.count) || 0;
  if (count === 0) {
    return {
      state: 'empty',
      count: 0,
      ageDays: null,
      usable: false,
      message: 'No cloud catalogue has been fetched on this install, so nothing can be named. '
        + 'That is expected on a network with no outbound access.',
    };
  }
  const last = summary && summary.lastSeenAt ? new Date(summary.lastSeenAt) : null;
  const ageDays = last && !Number.isNaN(last.getTime())
    ? Math.floor((now.getTime() - last.getTime()) / 86400000)
    : null;
  if (ageDays === null) {
    return {
      state: 'unknown_age',
      count,
      ageDays: null,
      usable: true,
      message: `${count.toLocaleString('en-US')} catalogue entries, but when they were last refreshed could not be read.`,
    };
  }
  if (ageDays > STALE_AFTER_DAYS) {
    return {
      state: 'stale',
      count,
      ageDays,
      // ⛔ STILL USABLE. Published ranges change slowly; refusing to name
      // anything from a copy two weeks old would be worse than saying its age.
      usable: true,
      message: `${count.toLocaleString('en-US')} catalogue entries, last refreshed ${ageDays} days ago. `
        + 'Names below may lag recent changes by the publishers.',
    };
  }
  return {
    state: 'ok',
    count,
    ageDays,
    usable: true,
    message: `${count.toLocaleString('en-US')} catalogue entries, refreshed ${ageDays === 0 ? 'today' : `${ageDays} day${ageDays === 1 ? '' : 's'} ago`}.`,
  };
}

/**
 * Group a set of matched hostnames into candidate applications, for the
 * "declare this?" suggestion.
 *
 * ⛔ A SUGGESTION IS A HYPOTHESIS AND MUST BE LABELLED ONE. This never creates
 * a declaration. An auto-created application is a declaration with nobody
 * behind it, which is worse than the stale-but-owned map the competing products
 * ship — at least someone once meant theirs. The operator accepts, or does not.
 */
function suggestApplications(matches) {
  const byKey = new Map();
  for (const m of matches || []) {
    // A caller handing over an entry with no match at all is a bug, not a
    // reason to take the whole page down with a TypeError.
    if (!m || !m.match || m.match.state !== STATES.MATCHED) continue;
    const key = `${m.match.provider}::${m.match.service || ''}`;
    if (!byKey.has(key)) {
      byKey.set(key, {
        provider: m.match.provider,
        providerLabel: m.match.providerLabel,
        service: m.match.service,
        label: m.match.label,
        hosts: [],
        deviceIds: new Set(),
      });
    }
    const g = byKey.get(key);
    g.hosts.push(m.host);
    if (m.deviceId) g.deviceIds.add(m.deviceId);
  }
  return [...byKey.values()]
    .map((g) => ({
      provider: g.provider,
      providerLabel: g.providerLabel,
      service: g.service,
      label: g.label,
      hostCount: new Set(g.hosts).size,
      hosts: [...new Set(g.hosts)].sort(),
      deviceCount: g.deviceIds.size,
      isSuggestion: true,
    }))
    .sort((a, b) => b.hostCount - a.hostCount);
}

module.exports = {
  STATES,
  STALE_AFTER_DAYS,
  PROVIDER_LABEL,
  providerLabel,
  normaliseHost,
  hostMatches,
  hostSpecificity,
  claimKey,
  pickBest,
  ipToUint32,
  cidrBounds,
  boundOrNull,
  buildIpIndex,
  entryCount,
  matchHost,
  matchIp,
  matchRange,
  catalogueStatus,
  suggestApplications,
};
