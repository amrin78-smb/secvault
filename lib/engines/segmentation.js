...
function trafficEvidence(rules) {
  ...
  for (const r of rules) {
    ...
    // ⛔ A NAME-GRADE ZERO IS NOT A ZERO HERE (A3, 2026-09-25). It becomes
    // `violation_permitted` — whose own hover text calls it "the safest kind to
    // close" — or `unused_permission`. Both are licences to delete a rule, and
    // a name-grade zero cannot carry one: every Palo Alto's logs identify rules
    // by NAME only (80,203 rollup rows, rule id NULL on all of them), so a rule
    // RENAMED during the window is absent under its new name while passing
    // traffic under its old one. Downgrading it to unmeasured yields
    // `violation_unverified`, which this file already says must be "assumed
    // live" — the correct reading.
    //
    // ⛔ ONLY THE ZERO. A name-grade HIT stays a hit: at worst a name collision
    // attributes another rule's traffic here, which produces `violation_active`
    // or `ok_in_use` — "do not close this" — and erring toward not deleting is
    // the safe direction. The asymmetry is the point, not an oversight.
    if (Number(hits) === 0 && r.usageGrade === 'log-name') {
      unmeasured += 1;
      reasons.add('name-grade-zero');
      continue;
    }
    measured += 1;
...
function exampleOf(r) {
  return {
    ...
    // ⛔ WHAT THE NUMBER RESTS ON travels with the number. Without this the UI
    // can render "0 hits" from a rule NAME match exactly as it renders one from
    // the device's own counter, and this board's verdicts are read as deletion
    // advice.
    usageGrade: r.usageGrade || null,
    deletionEvidence: r.deletionEvidence === true,
  };
}
