...
const { getCoverageRegister } = require('./coverageRegisterData');
...
// ⛔ 11. Evidence SecVault does not have on a firewall it CAN reach ──────
//
// ⛔ THIS IS NOT `collection_gap`, AND THE TWO MUST NEVER DOUBLE-LIST A
// FIREWALL. `collection_gap` answers "we cannot reach this device at all" and
// its action is "check reachability and credentials". This source answers the
// harder question: we collect from this firewall SUCCESSFULLY, every day, and
// a specific class of evidence still never arrives — so a whole set of answers
// about it is unmeasured while every page renders it as assessed.
//
// Measured on the reference fleet 2026-09-25, all three live:
//   PAKFood     collected fine, 0 syslog buckets, yet carrying 17 CVE
//               assessments and 24 audit findings.
//   TSR_EKM     collected fine, 78 of 78 rules with NO hit count, while being
//               one of the fleet's busiest loggers.
//   5 Fortinets  the same, fleet-wide — `unused` can never fire on any of them.
//
// ⛔ DEVICES ALREADY REPORTED BY `collection_gap` ARE EXCLUDED, using that
// source's own COLLECTION_STALE_DAYS rather than a second copy of the number.
// An uncollectable firewall has one problem, not two, and its coverage gaps are
// a SYMPTOM of the collection failure — listing both would put the same device
// in the queue twice with two different first steps. The full register (stale
// findings included) is on /coverage, which every item here links to.
//
// ⛔ ALWAYS `unmeasured`, SO IT CAN NEVER REACH `act_now`. The claim is "we
// cannot see this", never "this firewall is broken" — exactly the distinction
// the third band exists to hold.
async function gatherCoverageGaps(pool, coverageResult) {
  const register = coverageResult || await getCoverageRegister(pool, {});

  // ⛔ AN INCOMPLETE REGISTER THROWS. getCoverageRegister reports a failed read
  // in `failures` and returns an EMPTY `entries` — which is indistinguishable
  // from a fleet with no blind spots by the summary alone. Letting that through
  // would contribute zero items silently, and this source's whole subject is
  // that silence. runSource() banners it instead.
  const failures = Array.isArray(register.failures) ? register.failures : [];

  // ⛔ TWO SEVERITIES, AND THEY MUST NOT SHARE A FATE (A3, 2026-09-25).
  //
  // A failure of the register's OWN query means `entries` is empty and a fleet
  // with blind spots is indistinguishable from a fleet without any — so it
  // throws, and `runSource` banners the whole source. That is the rule this
  // gather was built on.
  //
  // `rule_log_evidence` is different in kind. It is an ENRICHMENT that shrinks
  // the `ruleUsage` gap when a device's logs can answer what its counters
  // cannot. If it fails, every device is still present, every cell still has a
  // state, and the affected cells fall back to the DEVICE-ONLY reading — which
  // is the WORSE one — and are marked `certain: false` so the page says "not
  // checked". Nothing is overstated and nothing is hidden.
  //
  // Treating that as fatal would discard ~14 real, still-open items of security
  // work to protest a cosmetically pessimistic gap number. Silence for a cycle
  // is recoverable, but so is a slightly overstated gap; losing the queue is the
  // more expensive of the two, and it is the failure mode this file's own header
  // warns about from the other direction.
  const fatal = failures.filter((f) => !String(f && f.source).startsWith('rule_log_evidence'));
  if (fatal.length > 0) {
    throw new Error(fatal.map((f) => `${f.source}: ${f.error}`).join('; '));
  }

  // The devices `collection_gap` already owns. A failed read here must not
  // silently widen this source into that one's territory, so it throws.
  const owned = new Set();
  const r = await pool.query(`
    SELECT d.id
      FROM devices d
     WHERE d.active
       AND (d.last_collected_at IS NULL
            OR d.last_collected_at < now() - ($1 || ' days')::interval)`,
  [String(COLLECTION_STALE_DAYS)]);
  for (const x of rows(r)) owned.add(x.id);

  const items = [];
  for (const e of (Array.isArray(register.entries) ? register.entries : [])) {
    if (!e || e.fullyCovered) continue;
    if (owned.has(e.deviceId)) continue;

    const gaps = Array.isArray(e.gaps) ? e.gaps : [];
    // ⛔ A cell we could not even CHECK is not a confirmed gap, and an item
    // whose every gap is unchecked would assert a blind spot we have not
    // established. Those are visible on /coverage; they do not become work.
    const confirmed = gaps.filter((g) => g && g.certain !== false);
    if (confirmed.length === 0 && !e.staleFindings) continue;

    const blocked = Array.isArray(e.blockedEngines) ? e.blockedEngines : [];
    const parts = confirmed.map((g) => g.detail).filter(Boolean);

    // ⛔ Stale findings are the loudest state the register has: an old answer
    // is ACTED ON, where a missing one is not.
    if (e.staleFindings) parts.unshift(e.staleFindings.detail);

    const severity = e.staleFindings ? 'high'
      : (e.answersWithheld >= 6 ? 'medium' : 'low');

    items.push({
      type: 'coverage_gap',
      key: `coverage:${e.deviceId}`,
      title: e.staleFindings
        ? `${e.deviceName}: findings are ${e.staleFindings.ageDays} days old and shown as current`
        : `${e.deviceName}: ${confirmed.length} evidence source`
          + `${confirmed.length === 1 ? '' : 's'} missing on a firewall SecVault can reach`,
      severity,
      urgency: 'soon',
      // ⛔ NEVER anything else. See the header — this is a claim about what we
      // cannot see, which is the definition of unmeasured.
      evidence: 'unmeasured',
      why: `${parts.join(' ')} SecVault collects from this firewall successfully, so it appears `
        + 'fully assessed everywhere else in the product — it contributes findings, scores and '
        + 'counts computed without this evidence, and nothing on those pages says so.'
        + (blocked.length
          ? ` Currently unanswerable for it: ${blocked.slice(0, 4).join(', ')}`
            + `${blocked.length > 4 ? `, and ${blocked.length - 4} more` : ''}.`
          : ''),
      affects: [e.deviceName],
      deviceIds: [e.deviceId],
      action: e.staleFindings
        ? 'Re-run collection and rule analysis for this firewall so its findings reflect its '
          + 'current ruleset, then re-check the ones that survive.'
        : 'Supply the missing evidence — point the firewall’s syslog at SecVault, switch it to a '
          + 'transport that reports hit counts, or collect its objects — or accept that these '
          + 'answers stay unmeasured for it.',
      // ⛔ NO "MARK AS DONE". The item clears because the evidence starts
      // arriving, which SecVault observes for itself on the next collection.
      done: 'The item clears when the missing evidence actually arrives — a syslog bucket, a '
        + 'measured hit count, a resolved object — never because someone ticked it off.',
      href: '/coverage',
      count: confirmed.length,
      magnitude: magnitudeOf([e.deviceId]),
    });
  }

  // ⛔ The whole register is already in memory, so the pre-cap length IS the
  // true total — same shape as the segmentation and application sources.
  return withCap(items, () => ({ rows: [{ n: items.length }] }));
}
 ...
 * @param {object} [opts.coverage] an already-computed getCoverageRegister() result.
...
async function gatherWorkQueue(pool, opts = {}) {
  const results = await Promise.all([
    ...
    runSource('coverage', () => gatherCoverageGaps(pool, opts.coverage)),
  ]);
