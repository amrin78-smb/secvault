...
// ── Headline strip ───────────────────────────────────────────────────────
//
// Four figures for the top of the page, each with the same figure from the
// PREVIOUS window of equal length so a reader can see direction.
//
// ⛔ TWO QUERIES, AND THE SPLIT IS NOT AN OPTIMISATION — IT IS THE WHOLE
// CORRECTNESS OF IT. `syslog_vpn_auth_hourly.usernames` is an ARRAY. Counting
// distinct usernames needs `unnest`, which MULTIPLIES each row by the length of
// its array; `sum(event_count)` in that same statement then counts every bucket
// once per username it carried. Measured live on 2026-09-25: the joined form
// reports 309,994 failed events where the true figure is 46,498 — inflated 6.7x,
// and plausible enough that nothing would have questioned it. The event totals
// are counted WITHOUT the join; only the username count uses it.
const HEADLINE_SQL = `
  SELECT
    sum(event_count) FILTER (WHERE auth_outcome = 'failure' AND bucket_hour > $1::timestamptz)  AS cur_failed,
    sum(event_count) FILTER (WHERE auth_outcome = 'failure' AND bucket_hour <= $1::timestamptz) AS prev_failed,
    count(DISTINCT src_ip) FILTER (WHERE auth_outcome = 'failure' AND bucket_hour > $1::timestamptz)  AS cur_sources,
    count(DISTINCT src_ip) FILTER (WHERE auth_outcome = 'failure' AND bucket_hour <= $1::timestamptz) AS prev_sources,
    count(DISTINCT device_id) FILTER (WHERE bucket_hour > $1::timestamptz)  AS cur_devices,
    bool_or(usernames_truncated) FILTER (WHERE bucket_hour > $1::timestamptz) AS cur_truncated
  FROM syslog_vpn_auth_hourly
  WHERE bucket_hour > $2::timestamptz`;

const HEADLINE_USERNAMES_SQL = `
  SELECT
    count(DISTINCT u) FILTER (WHERE bucket_hour > $1::timestamptz)  AS cur_usernames,
    count(DISTINCT u) FILTER (WHERE bucket_hour <= $1::timestamptz) AS prev_usernames
  FROM syslog_vpn_auth_hourly, LATERAL unnest(usernames) AS u
  WHERE bucket_hour > $2::timestamptz AND auth_outcome = 'failure'`;

/**
 * Assemble the headline strip. Pure — takes the two query rows and the baseline.
 *
 * ⛔ A PREVIOUS WINDOW WE DO NOT HOLD IS `null`, NEVER ZERO. If the retained
 * history begins after the previous window would have started, we did not
 * observe a quieter period — we observed nothing. Returning 0 there would
 * render as "up 46,498 since yesterday" on the day a customer installs the
 * product. `DeltaBadge` renders nothing at all for a null previous, which is
 * the correct output: no claim.
 *
 * ⛔ AND THE USERNAME COUNT IS A FLOOR WHEN ANY BUCKET WAS CAPPED.
 * `usernames_truncated` means the rollup stopped recording names for that hour,
 * so the distinct count is "at least this many". Live today that flag is false
 * across the window, which is exactly when a floor is cheapest to carry and
 * easiest to forget.
 */
function buildHeadline(row, userRow, baseline, windowStart, prevStart) {
  const r = row || {};
  const u = userRow || {};
  // Did we actually hold data for the whole of the previous window?
  const first = toDate(baseline && baseline.firstBucketAt);
  const covered = Boolean(first) && first.getTime() <= prevStart.getTime();
  const prev = (v) => (covered ? num(v) : null);

  return {
    windowStart,
    previousWindowStart: prevStart,
    previousWindowCovered: covered,
    usernamesIsFloor: Boolean(r.cur_truncated),
    figures: [
      {
        key: 'failed_events',
        // ⛔ "authentications", not "attempts". One failed authentication event
        // is what the firewall logged; an attempt may produce several.
        label: 'Failed VPN authentications',
        current: num(r.cur_failed),
        previous: prev(r.prev_failed),
        goodDirection: 'down',
      },
      {
        key: 'source_addresses',
        // ⛔ FAILURE-ONLY, and the label says so. `coverage.sourcesSeen` counts
        // every source with ANY outcome including successful staff logins, so
        // it is not "attacking addresses" and must not be labelled as though it
        // were. This figure is its own query for that reason.
        label: 'Addresses with failures',
        current: num(r.cur_sources),
        previous: prev(r.prev_sources),
        goodDirection: 'down',
      },
      {
        key: 'usernames_targeted',
        label: 'Usernames targeted',
        current: num(u.cur_usernames),
        previous: prev(u.prev_usernames),
        goodDirection: 'down',
      },
      {
        key: 'firewalls_reporting',
        // ⛔ "reporting", not "affected". The rollup tells us which firewalls
        // SENT VPN authentication logs — not which were attacked, and not which
        // were compromised. A firewall missing from this count may be under
        // attack and silent, which is the more worrying case and the one the
        // reporting-gap banner below exists to name.
        label: 'Firewalls reporting',
        current: num(r.cur_devices),
        previous: null,
        goodDirection: 'up',
      },
    ],
...
async function getVpnDetections(pool, opts = {}) {
  ...
  // The previous window of equal length, for the headline deltas.
  const prevStart = new Date(windowStart.getTime() - hours * 3600 * 1000);
  const ps = prevStart.toISOString();

  const [history, coverage, attribution, breadth, brute, targeted, successRows, hourRows, locations,
    headlineRow, headlineUsers] =
    await Promise.all([
      ...
      // Both windows in ONE scan each — see HEADLINE_SQL for why these are two
      // statements and not one.
      pool.query(HEADLINE_SQL, [ws, ps]),
      pool.query(HEADLINE_USERNAMES_SQL, [ws, ps]),
    ]);
  ...
  return {
    ...
    headline: buildHeadline(
      headlineRow.rows[0], headlineUsers.rows[0], baseline, windowStart, prevStart
    ),
    coverage: {
