...
function buildNewCountryDetection({ successRows, baseline, windowStart }) {
  ...
  for (const r of Array.isArray(successRows) ? successRows : []) {
    ...
    if (!at) continue;
    const country = r.src_country || null;
  ...
  return {
    ...
    method:
      `A successful authentication from a country absent from that user's own prior history, where `
      + `the user has at least ${NEW_COUNTRY_MIN_USER_DAYS} days of history and the fleet has at `
      + `least ${NEW_COUNTRY_MIN_BASELINE_DAYS}.`,
...
function buildOffHoursDetection({ hourRows, successRows, baseline, windowStart }) {
  ...
  const profile = [...perHour.values()].map((s) => ({
    ...
    events: s.events,
    days: s.days.size,
    meanPerDay: s.days.size > 0 ? s.events / s.days.size : null,
  }));
