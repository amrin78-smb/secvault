  ...
  for (const rule of ruleList) {
    ...
    if (zeroFromDevice || loggedZero) {
      ...
      findings.push({
        ...
        severity: 'medium',
        detail: viaLogs
          ? `Rule ${ruleLabel(rule)} matched no traffic in ${opts.unusedDays} days of firewall logs ` +
            `(the device reports no hit counters; evidence is from logs, with ` +
            `${Math.round((rule.logCoverageRatio || 0) * 100)}% log coverage of the window).`
