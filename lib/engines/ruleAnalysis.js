...
const { correlateDeviceRules } = require('./ruleHitCorrelation');
...
const DEFAULT_OPTIONS = {
  unusedDays: 90,
  // Window for the LOG-derived usage evidence (Phase 8b). Deliberately SEPARATE
  // from unusedDays: it is bounded by SYSLOG_RETENTION_DAYS, not by how long an
  // operator considers a rule stale. Conflating the two made the `unused`
  // finding claim "no traffic in 90 days of firewall logs" for a measurement
  // that spanned at most 30.
  logEvidenceDays: 30,
  expiryWindowDays: 14,
...
// ⛔ The finding types produced ONLY inside analyzeRules()'s O(n^2) pairwise block, i.e. the
// ones that are NOT COMPUTED AT ALL when a ruleset is above maxRulesForShadow. Declared here,
// exported, and consumed by lib/engines/configAuditor.js, because "we skipped this pass" is a
// fact a downstream consumer cannot recover from the findings table: a skipped pass and a pass
// that ran and found nothing both look like ZERO ROWS.
//
// That indistinguishability was a live bug (fixed 2026-09-09): configAuditor's rule_scan
// evaluator reported PASS -- "no rules matched" -- for `rule-no-shadowed-rules` and
// `rule-no-redundant-rules` on any device with more than 1000 rules, and CREDITED that pass in
// the compliance denominator, for an analysis SecVault deliberately never ran. Same class as
// hit_count's old DEFAULT 0: our own coverage limit recorded as an affirmative fact about the
// device. Largest live ruleset is IDC FW at 706 rules, so it is latent, not active.
const PAIRWISE_FINDING_TYPES = Object.freeze([
  'shadow',
  'redundant',
  'correlation',
  'generalization',
...
/**
 * Which finding types were NOT computed for a ruleset of this size. Pure; the single source of
 * truth for the cap comparison, so the producer (runAnalysisForDevice) and the consumer
 * (configAuditor's rule_scan checks) can never drift on the boundary condition.
 *
 * @param {number} ruleCount
 * @param {number} [maxRulesForShadow] - defaults to DEFAULT_OPTIONS.maxRulesForShadow
 * @returns {string[]} empty when every pass ran
 */
function skippedPairwiseFindingTypes(ruleCount, maxRulesForShadow) {
  const cap =
    Number.isFinite(maxRulesForShadow) && maxRulesForShadow > 0
      ? maxRulesForShadow
      : DEFAULT_OPTIONS.maxRulesForShadow;
  return Number(ruleCount) > cap ? PAIRWISE_FINDING_TYPES.slice() : [];
}
...
function isStrictlyEarlier(sRule, rRule) {
  ...
  // VDOM-aware (fixed 2026-07-30): rules live in independent evaluation contexts
  // per VDOM on a multi-VDOM Fortinet device -- `sequence_number` is contiguous
  // ACROSS vdoms (see fortinet/parser.js's parsePolicies sequenceStart), so two
  // identical rules in different VDOMs used to false-positive as shadow/
  // redundant/correlation/generalization/reorder_candidate against each other.
  // `vdom` is null for every non-VDOM vendor/device, so null !== null is false
  // and this comparison is a no-op there -- existing behavior is unchanged.
  if ((sRule.vdom || null) !== (rRule.vdom || null)) return false;
...
async function analyzeRules(rules, options) {
  ...
  for (const rule of ruleList) {
    ...
    // ⛔ MEASURED zero only. `hit_count` is tri-state (see lib/schema.sql):
    // NULL means the vendor/transport could not read hit counts at all, and
    // "we could not measure it" is not evidence of "it is unused". Until
    // 2026-08-25 this read `Number(rule.hit_count) === 0`, and Number(null)
    // is 0 -- so every rule on every device reporting no hit data at all
    // (Fortinet SSH, Sangfor SSH, and, because of a PAN-OS command that was
    // being rejected outright, EVERY Palo Alto) was reported as unused.
    // Those findings were fabricated from missing data, and at fleet scale
    // they were the single largest source of analysis noise.
    const measuredHits =
      rule.hit_count === null || rule.hit_count === undefined ? null : Number(rule.hit_count);

    // Phase 8b: LOG EVIDENCE is a second, independent way to measure a zero,
    // for the vendors/transports whose API cannot report hit counts at all
    // (Fortinet SSH, Palo Alto SSH, Sangfor -- 164 rules on this fleet).
    // ⛔ Only `measured-zero` counts. ruleHitCorrelation.js sets that ONLY when
    // the device was logging throughout the window AND the rule has logging
    // enabled AND the window is long enough. Every other state leaves
    // loggedHits null, because a rule that cannot appear in a log, or a device
    // that was not sending, measures the collector rather than the rule.
    // Accepting any weaker signal here would rebuild the fabricated-`unused`
    // bug that this same block was rewritten to remove.
    const loggedZero = rule.logEvidence === 'measured-zero' && rule.loggedHits === 0;
    const zeroFromDevice = measuredHits !== null && Number.isFinite(measuredHits) && measuredHits === 0;

    // ⛔ ADDED 2026-09-09. `loggedZero` was already careful, but `zeroFromDevice` looked ONLY
    // at hit_count and ignored the log evidence sitting on the same rule object -- so a
    // device-reported 0 that SecVault's OWN syslog directly contradicts still emitted `unused`.
    // Measured live: 6 rules on TSR_EKC carry hit_count = 0 alongside up to 126,300 logged hits,
    // and the stored finding says "has zero recorded hits". (Those particular rows are also
    // stale-data -- last collected before the tri-state fix -- but the code gap is independent:
    // nothing suppressed the finding even on a fresh run, and a Palo Alto counter reset with
    // live logged traffic reproduces it exactly.)
    //
    // A counter is a claim; a log line is an observation. When they disagree, the observation
    // wins -- and the honest output is NO finding, not an inverted one. Only positive evidence
    // ('hits' with a real count) contradicts; every weaker/absent log state leaves this false so
    // a device-measured zero is still reported, exactly as before.
    const contradicted = rule.logEvidence === 'hits' && rule.loggedHits > 0;

    if (!contradicted && (zeroFromDevice || loggedZero)) {
      const viaLogs = !zeroFromDevice && loggedZero;
      findings.push({
        rule_id: rule.id,
        finding_type: 'unused',
        severity: 'medium',
        // ⛔ Report the window the EVIDENCE covers, not opts.unusedDays. Those
        // are different numbers (90 vs 30 by default) and the log window can
        // never exceed SYSLOG_RETENTION_DAYS regardless.
        detail: viaLogs
          ? `Rule ${ruleLabel(rule)} matched no traffic in ${rule.logWindowDays || opts.unusedDays} ` +
            `days of firewall logs (the device reports no hit counters; evidence is from logs, ` +
            `with ${Math.round((rule.logCoverageRatio || 0) * 100)}% log coverage of the window)` +
            // ⛔ A NAME-GRADE ABSENCE STATES ITS OWN WEAKNESS IN THE FINDING
            // ITSELF. This device's logs identify rules only by NAME (every
            // Palo Alto on this fleet: 80,203 rollup rows, rule id NULL on all
            // of them), and a name is neither unique nor stable across a config
            // change — so a rule RENAMED since these logs were written reads as
            // having had no traffic while it is passing some. The finding is
            // still worth showing; it is not worth deleting a rule over, and
            // `getCleanupCandidates` refuses it separately.
            (rule.usageGrade === 'log-name'
              ? '. This firewall\'s logs identify rules by name only, so this rests on the rule '
                + 'name being unchanged over the window — it is not sufficient evidence to remove '
                + 'the rule.'
              : '.')
          : `Rule ${ruleLabel(rule)} has zero recorded hits.`,
        affected_rule_ids: [],
        remediation: viaLogs
          ? (rule.usageGrade === 'log-name'
            ? `Confirm the rule has not been renamed during the `
              + `${rule.logWindowDays || opts.unusedDays}-day window, then verify it is still `
              + `required before disabling it.`
            : `Verify the rule is still required; if it has genuinely had no traffic for the `
              + `${rule.logWindowDays || opts.unusedDays}-day log window, disable and then remove it.`)
          : `Verify the rule is still required; if it has genuinely had no traffic for over ${opts.unusedDays} days, disable and then remove it.`,
      });
    }
  ...
  // ── Pairwise checks — every finding type in PAIRWISE_FINDING_TYPES ──
  // Skipped entirely above the O(n^2) cap. ⛔ Keep PAIRWISE_FINDING_TYPES in step with the
  // finding_types emitted inside this block: configAuditor.js relies on that list to tell
  // "this pass found nothing" from "this pass never ran", and a type missing from it would be
  // silently scored as a compliance PASS on any ruleset above the cap.
  if (ruleList.length <= opts.maxRulesForShadow) {
    ...
    for (let i = 0; i < ruleList.length; i++) {
      ...
      for (let j = 0; j < i; j++) {
        ...
        // ⛔ Guards added: without these, the same rule pair can be
        // double-reported as both `generalization` and `shadow`/
        // `correlation` with contradictory recommendations (confirmed via
        // direct construction: mutual CIDR-based coverage produces both a
        // `shadow` finding on r AND a `generalization` finding on s for the
        // identical pair). Mirrors the equivalent guards already present in
        // the `correlation` loop above.
        if (ruleCovers(s, r)) continue; // mutual coverage — already claimed by shadow
        if (shadowPairs.has(`${r.id}|${s.id}`)) continue;
        if (correlationPairs.has(`${r.id}|${s.id}`)) continue;
...
async function runAnalysisForDevice(deviceId, pool) {
  ...
  // ⛔ RECORDED, not just logged (2026-09-09). Per CLAUDE.md "Rule Shadow Analysis" the O(n^2)
  // passes are capped -- but until now the ONLY trace of that was a console.warn, so every
  // consumer of rule_analysis_results saw "zero shadow findings" and had no way to tell that
  // from "we never looked". configAuditor.js turned exactly that into a compliance PASS. The
  // skipped list is returned to callers and re-derived independently (same helper, same cap) by
  // configAuditor, which reads the table rather than this return value.
  const skippedPasses = skippedPairwiseFindingTypes(rules.length, options.maxRulesForShadow);
  if (skippedPasses.length > 0) {
    console.warn(
      `[ruleAnalysis] Device ${deviceId} has ${rules.length} rules (cap ${options.maxRulesForShadow}); ` +
        `NOT RUN for this ruleset: ${skippedPasses.join(', ')}. Zero findings of those types means ` +
        'not measured, not clean.'
    );
  }

  // ⛔ WIRING FIXED 2026-09-09. Phase 8b (v2.77.0) built the whole log-evidence
  // path — getDeviceLogCoverage / getLoggedRuleHits / enrichRulesWithLogEvidence
  // — and then never called it from anywhere. `rule.logEvidence` was therefore
  // permanently undefined, `loggedZero` permanently false, and the 164 rules
  // whose hit_count is NULL (vendors/transports that cannot report hits at all,
  // the entire reason the feature exists) still produced no usage finding. The
  // test suite passed because it only ever exercised the pure function.
  //
  // Best-effort by design: a syslog read failure must degrade to the
  // un-enriched ruleset, never block the rest of the analysis — same fail-safe
  // posture as the zone-classification load above. And an enrichment that
  // cannot prove coverage yields NO finding rather than a confident "unused":
  // an `unused` verdict still requires a MEASURED zero.
  let analysisRules = rules;
  try {
    analysisRules = await correlateDeviceRules(pool, deviceId, rules, options.logEvidenceDays);
  } catch (err) {
    console.warn(
      `[ruleAnalysis] Log-evidence correlation failed for device ${deviceId}; ` +
        `falling back to collected hit counts only: ${err.message}`
    );
  }

  const findings = await analyzeRules(analysisRules, options);
  ...
  return { findings: findings.length, byType, skippedPasses };
}
...
module.exports = {
  ...
  DEFAULT_RISKY_PORTS,
  DEFAULT_OPTIONS,
  // Consumed by lib/engines/configAuditor.js so a skipped O(n^2) pass can never be read as a
  // clean result -- see skippedPairwiseFindingTypes()'s own comment.
