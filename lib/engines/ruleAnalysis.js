...
// (ordered by sequence_number ASC) and emits findings for the 9 finding types
// stored in rule_analysis_results. runAnalysisForDevice() /
...
// in S, string equality, case-insensitive, trimmed). Provable coverage only —
// no CIDR math (address items are object names/strings).
function fieldCovers(sList, rList) {
  if (isAny(sList)) return true;
  if (isAny(rList)) return false;
  const sSet = new Set(normList(sList));
  return normList(rList).every((item) => sSet.has(item));
}
...
// S covers R when S's src_addresses, dst_addresses and services are each
// supersets of R's.
...
function analyzeRules(rules, options) {
  ...
  for (const rule of ruleList) {
    ...
    if (Number(rule.hit_count) === 0 && !rule.last_hit_at) {
      findings.push({
        rule_id: rule.id,
        finding_type: 'unused',
        severity: 'medium',
        detail: `Rule ${ruleLabel(rule)} has zero recorded hits and no last-hit timestamp (hit-count data may be unavailable from this vendor).`,
        affected_rule_ids: [],
