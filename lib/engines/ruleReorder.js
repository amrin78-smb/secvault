...
function computeRecommendedOrder(rules, findings) {
  ...
  // in-degree counting below isn't inflated by duplicates.
  ...
  // topological order (neither is part of a cycle).
  for (const e of edges) {
    if (resolvedSet.has(e.before) && resolvedSet.has(e.after)) resolvedFindingCount += 1;
