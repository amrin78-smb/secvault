...
// External zone directly to an explicitly-classified Internal one) belongs
// here too -- it's arguably the most on-point exposure-widening finding of
// all, and is already included alongside these same three types in
// OptimizationTab.js's own finding-type filter.
const EXPOSURE_FINDING_TYPES = ['any_any', 'overly_permissive', 'risky_service', 'external_exposure'];
...
async function getExposureCorrelationForDevice(deviceId, pool) {
  // Excludes rule findings and CVE assessments the operator has already
  // dismissed/acknowledged/actioned, on both sides of the join -- the UI
  // (OverviewExposureCard.js) explicitly frames this as "open" correlations,
  // and every other "open" query in this codebase (fetchPatchNow() in
  // app/api/events/route.js) treats only a bare/unset 'new' status as open,
  // not 'acknowledged' -- see that file's own 2026-07-18 bug-fix comment for
  // why the stricter definition was chosen over "not dismissed/actioned".
  const { rows: findings } = await pool.query(
    `SELECT rar.id, rar.rule_id, rar.finding_type, rar.severity, rar.detail
