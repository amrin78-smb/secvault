  ...
  for (const rule of enabled) {
    ...
    if (claimed > 0n) {
      ...
      const summary = {
        ...
        // ⛔ THE GRADE TRAVELS WITH THE COUNT (A3). A `log-name` answer came
        // from matching a rule NAME in the logs, which is neither unique nor
        // stable across a config change — so a renamed rule reads as carrying
        // no traffic. Without this field the card renders it identically to the
        // device's own counter, and "a rule permitting this flow is in use"
        // becomes a claim the evidence does not support.
        usageGrade: rule.usageGrade || null,
        deletionEvidence: rule.deletionEvidence === true,
