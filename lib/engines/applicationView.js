...
// end. Hitting this makes the flow `unverified`, which is a refusal to answer,
// not a partial answer dressed as a whole one.
...
function evaluateFlowOnDevice(normalised, rules, objects, opts = {}) {
  ...
  let verdict;
  if (truncated) verdict = VERDICTS.UNSPECIFIED;
  else if (allowVolume >= total) verdict = VERDICTS.PERMITTED;
...
function aggregateFlow(perDevice, coverage = {}) {
  ...
  const unverified = anyUnverified || devicesWithoutRules > 0;
