...
//
// ⛔ THE COLUMN IS TRI-STATE (2026-09-09) and this file is the only producer:
//   true   measured, and the service was reached (all four conditions)
//   false  MEASURED, and not reached
//   NULL   NOT MEASURED -- no syslog coverage in the window, or no collected
//          device_interfaces rows
// Nothing about the DETECTION changes with that: `true` still means exactly
// what CLAUDE.md's "What log_hit MEANS" says, and the rejected "a threat
// signature fired on this device" definition stays rejected. Only the way an
// UNMEASURED result is stored changed -- it used to be indistinguishable from
// a measured `false`, because the column was NOT NULL DEFAULT false.

'use strict';

const { updatePrioritiesForDevice } = require('./prioritization');

// ⛔ THE ACTION VOCABULARY IS NOT DEFINED HERE. It lives in
// lib/syslog/actions.js, which exists precisely because this codebase once had
// FOUR deny lists and the narrowest one drove every dashboard number.
//
// This file kept a fifth copy until 2026-09-09, and it had ALREADY DRIFTED:
// actions.js carried `drop-packet` (1,737 live PAN threat events/24h) and this
// copy did not, so `classifyAction('drop-packet')` answered 'blocked' in one
// file and 'unknown' in the other. It also still carried `timeout` in the
// blocked set after that verb was proven to be a FortiOS session-TEARDOWN —
// which matters more here than anywhere else, because the rollup this engine
// reads was setting `allowed = false` for a service that was genuinely
// reached, so rule 2 could never fire on it.
//
// ⛔ Do not reintroduce a local list. Import it, or the drift comes back.
const {
  classifyAction,
  ALLOWED_ACTIONS,
  DENIED_ACTIONS,
} = require('../syslog/actions');

// Historical alias: this module exported the deny set as `BLOCKED_ACTIONS`.
// Kept as a re-export of the SAME Set object so no caller can be reading a
// second, divergent copy.
const BLOCKED_ACTIONS = DENIED_ACTIONS;
...
/**
 * Persist one assessment's `log_hit`, TRI-STATE.
 *
 * ⛔ `value` is `true` | `false` | `null`, and `null` is a REAL INTENT — "this
 * run could not measure this device" — not an absence of one. Until 2026-09-09
 * the column was `NOT NULL DEFAULT false`, so the guard here read
 *
 *     if (value === (r.log_hit === true)) continue;
 *
 * which collapsed the stored value to a boolean and therefore treated a
 * NULL intent as "already false" and skipped the write. That is the whole bug:
 * "never measured" and "measured, not reached" were the same stored value, so
 * a device that fell out of syslog coverage kept asserting a stale measurement
 * forever.
 *
 * The comparison is now a strict three-way identity against a NORMALISED
 * current value, so every one of the nine (current, intended) pairs is
 * handled explicitly:
 *   null -> false   is a real transition (we started measuring)
 *   true -> null    is a real transition (we STOPPED being able to measure,
 *                   and the stale escalation must be withdrawn)
 *   x    -> x       writes nothing.
 *
 * @returns {boolean} true if a row was actually written.
 */
async function persistLogHit(pool, row, value) {
  // Normalise both sides. pg gives `null` for SQL NULL, but a caller-built row
  // could carry `undefined`; both mean UNMEASURED, and neither means false.
  const current = row.log_hit === true ? true : row.log_hit === false ? false : null;
  const intended = value === true ? true : value === false ? false : null;
  if (current === intended) return false;
...
/**
 * Mark every assessment for a device UNMEASURED.
 *
 * ⛔ Called from the two skip paths, which CLAUDE.md requires to "write NOTHING
 * rather than false". Before the column was nullable, "write nothing" had to
 * mean "issue no UPDATE" — which was only half right: it stopped a fabricated
 * `false` being written, but it also left whatever the previous run had
 * concluded standing as if it were still true today. `log_hit = NULL` is what
 * actually expresses the intent, and withdrawing a stale `true` matters most,
 * because rule 2 outranks CVSS 9.0.
 */
async function markDeviceUnmeasured(pool, rows, summary, touchedDevices, deviceId) {
  for (const r of rows) {
    if (await persistLogHit(pool, r, null)) {
      summary.setUnmeasured++;
...
async function runLogHitCorrelation(pool, opts = {}) {
  ...
  const summary = {
    ...
    setTrue: 0,
    setFalse: 0,
    // Rows moved TO unmeasured (NULL) by this run — a device that fell out of
    // syslog coverage, or whose interface collection was lost. Counted
    // separately from setFalse on purpose: they are opposite claims.
    setUnmeasured: 0,
    reprioritized: 0,
    hits: [],
    errors: [],
    ...
    for (const [deviceId, rows] of byDevice) {
      try {
        ...
        if (!ips || ips.size === 0) {
          ...
          // this device from traffic THROUGH it. Store NULL (UNMEASURED)
          // rather than a false that would read as "not reached".
          summary.devicesSkippedNoInterfaces++;
          await markDeviceUnmeasured(pool, rows, summary, touchedDevices, deviceId);
          continue;
        }
        ...
        // failed-read-as-a-fact bug this codebase keeps rediscovering. NULL is
        // the state that says so, and since 2026-09-09 the column can hold it.
        //
        ...
        for (const r of rows) {
          ...
          // We got here past BOTH gates: the device has its own addresses and
          // it was sending syslog over the window. Whatever we conclude now is
          // a genuine MEASUREMENT, so the value is a real boolean, never null.
          const value = hitPort !== undefined;
          if (!(await persistLogHit(pool, r, value))) continue;
