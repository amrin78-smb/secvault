...
function zoneNamesWithRole(zoneRoleMap, role) {
  ...
  for (const [zone, r] of Object.entries(zoneRoleMap || {})) {
    if (String(r).toLowerCase() === String(role).toLowerCase()) out.add(String(zone).toLowerCase());
  }
  return out;
}
...
function ruleZoneList(rule, field) {
  ...
  if (Array.isArray(raw)) return raw.map((z) => String(z).toLowerCase());
...
function evaluateRulePropertyCheck(check, rules, zoneRoleMap) {
  ...
  const appliesTo = (cfg.applies_to && typeof cfg.applies_to === 'object') ? cfg.applies_to : {};
  ...
  for (const rule of rules) {
    ...
    if (requiredZones) {
      ...
      const isAny = zones.some((z) => z === 'any' || z === 'all');
      if (!isAny && !zones.some((z) => requiredZones.has(z))) continue;
    }
    applicable += 1;
    ...
    if (!paths.some((p) => isSetValue(rawRuleValue(raw, p)))) violations.push(rule);
  }
  ...
  if (violations.length > 0) {
    return {
      ...
      detail: `"${check.name}" failed — ${violations.length} of ${applicable} applicable rule(s) `
        + `have no ${subject}: ${nameList(violations)}${violations.length > 3 ? ', …' : ''}.`
        + undecidableNote,
