...
// A field is "any" when null/undefined, an empty array, or contains a
// recognized wildcard entry — same semantics as ruleAnalysis.js's isAny(),
// duplicated rather than imported (see header comment above).
function isAnyField(list) {
  if (list === null || list === undefined) return true;
  if (!Array.isArray(list)) return false;
  if (list.length === 0) return true;
  return list.some((item) => ANY_ALIASES.has(String(item).trim().toLowerCase()));
}
 ...
 * CURRENT firewall_rules rows. No pass_when (same fixed-polarity reasoning
 * as evaluateRuleScanCheck() — every property here is "this should exist",
 * so finding it is always PASS). An unrecognized `property` value is a
 * curated-data problem (same 'warning' treatment as this file's other two
 * evaluators' malformed-config-guards), never a silent false PASS or FAIL.
 *
 * @param {object} check
 * @param {Array<object>} rules - this device's current firewall_rules rows
 * @returns {{status: 'pass'|'fail'|'warning', detail: string}}
 */
function evaluateRulesetPropertyCheck(check, rules) {
  const predicateConfig = check.predicate_config || {};
...
async function runComplianceAuditForDevice(deviceId, pool) {
  ...
  // services), not just Phase 5 finding buckets — loaded once, shared by
  // every ruleset_property check, same "one query, not per-check"
  // convention as ruleFindingsByType above. Only fetched when there's
  // actually a rule to look at (ruleCount > 0), same guard.
  let rulesForPropertyChecks = [];
  if (ruleCount > 0) {
    const { rows } = await pool.query(
      'SELECT action, src_addresses, dst_addresses, services, enabled FROM firewall_rules WHERE device_id = $1',
      [deviceId]
    );
  ...
  const evaluated = checks.map((check) => {
    ...
    if (predicateType === 'ruleset_property') {
      ...
      const { status, detail } = evaluateRulesetPropertyCheck(check, rulesForPropertyChecks);
      return { check, status, detail, matchedRuleIds: [] };
    }
