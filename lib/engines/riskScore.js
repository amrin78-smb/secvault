...
// ⛔ Bug fixed 2026-07-23, found via a live UI audit + confirmed against the
...
// saturate the total on its own. Verified against the real fleet before
// shipping (not synthetic data): distribution went from 13 critical/1 high
// (useless -- everything looked the same) to 7 critical/7 high, with the
// split landing exactly on "has a real critical/high finding" vs. "zero
// critical, just a medium-severity backlog" -- the intended differentiation
// restored. Same bands, same 0-100 scale, same documented "a single critical
// finding lands in 'medium', not 'high'" edge case -- unchanged (verified:
// 1 critical alone still contributes min(10,60)=10, still bands as
// 'medium').
//
// ⛔ Follow-up fix, same day, found by adversarial re-verification of the fix
// above: the first cut of these caps (critical: 40) was still BELOW the
// 'high'->'critical' band boundary (59, see BANDS below) -- so a device
// dominated purely by critical findings could never score above 40 and was
// permanently stuck in the 'high' band, no matter how many critical findings
// it accumulated (e.g. 50 critical findings -> min(500,40)=40 -> 'high',
// identical to a device with 50 high-severity findings). Fixed by raising
// TIER_CAPS.critical to 60 -- above the 'high' band's own ceiling -- so a
// critical-heavy device (6+ criticals) can reach 'critical' on that tier
// alone, while low counts (1-3 criticals) still land exactly where the
// documented edge cases above say they should. TIER_CAPS.info was also
// dropped entirely: SEVERITY_WEIGHTS.info is 0, so an info cap could never
// bind regardless of its value -- it was dead weight that made the "caps sum
// to MAX_SCORE" claim look intentional when it was actually just coincidence
// with the OLD numbers. With critical raised to 60, the caps below (60+30+20
// = 110) now deliberately exceed MAX_SCORE -- Math.min(MAX_SCORE, ...) in
// computeRiskScoreFromCounts is genuinely load-bearing now, not a defensive
// backstop the way the original comment claimed.
const TIER_CAPS = {
  critical: 60,
  high: 30,
  medium: 20,
...
function computeRiskScoreFromCounts(counts = {}) {
  ...
  // which is exactly what a SQL LEFT JOIN with no matching row, or a JSON
  // body with an absent object, hands you -- would reach the property reads
  // below and throw. This engine feeds securityScore.js from inside a
  // scheduled job, so it resolves to the CLEAN end (0) rather than throwing.
  //
  // ⛔ 0 here means "measured, and clean", NOT "not measurable". The
  // not-measurable decision belongs to the CALLER: deviceInventory.js does
  // `analysed ? risk.score : null` and passes [] rather than [0] into
  // securityScore, so hygiene drops out of the denominator instead of
  // scoring a fake perfect. Do not move that null in here.
  const safeCounts = counts && typeof counts === 'object' ? counts : {};
  const n = (key) => Number(safeCounts[key]) || 0;
  ...
  // above is unrelated/unchanged and was never the bug). info has no cap
  // here: SEVERITY_WEIGHTS.info is 0, so its contribution is always 0
  // regardless of count -- capping it would be dead code.
