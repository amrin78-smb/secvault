...
async function getVpnTunnelHealth(pool, options = {}) {
  ...
  // ⛔ FLEET-WIDE, deliberately, and it must stay that way. It is the denominator
  //    for devicesWithInterfaceData, whose numerator comes from the fleet-wide
  //    ifaceRows query above. Using the SCOPED device list instead rendered
  //    "collected from 15 of 1 active firewalls" whenever the page was filtered to
  //    one firewall (/vpn?thDevice=<uuid>).
  const { rows: fleetCountRows } = await pool.query(
    `SELECT count(*)::int AS total FROM devices WHERE active = true`
  );
  const fleetDeviceTotal = fleetCountRows[0] ? fleetCountRows[0].total : 0;
  ...
  return assembleTunnelHealth({
    ...
    pollRows,
    fleetDeviceTotal,
    scopedToOneDevice: Boolean(deviceId),
    now,
    staleAfterMinutes,
    lookbackDays,
  });
 ...
 */
function assembleTunnelHealth({
  deviceRows, ifaceRows, pollRows, fleetDeviceTotal, scopedToOneDevice,
  now, staleAfterMinutes, lookbackDays,
}) {
  ...
  for (const d of byDevice.values()) {
    ...
    for (const r of d.rawTunnels) {
      ...
      const tunnel = {
        ...
        downSince: null,
        // ⛔ CUSTOMER-FACING STRING. It names what SecVault does, not how it is
        // stored: no table name, no file path. A reader of this sentence is an
        // operator asking "why can't you tell me how long it has been down",
        // and an internal identifier answers a question they did not ask while
        // failing the one they did.
        downSinceReason:
          'Not derivable. SecVault keeps only the latest snapshot of a firewall’s tunnels — each '
          + 'poll replaces the previous one — so there is no state-change history to measure against.',
      };
      tunnels.push(tunnel);
    ...
    // ⛔ PER-DEVICE DOWN-OBSERVABILITY, because a COLUMN is read differently
    // from a bar. The fleet-level caveat (downObservability, below) landed in
    // the answer sentence and the coverage bar and fixed the headline — but the
    // per-device coverage table still printed a bare "0" in its Down column for
    // every firewall structurally incapable of reporting a down tunnel, next to
    // a teal "Reporting" badge. Measured live 2026-09-14: 10 of 16 active
    // firewalls (all Palo Alto, 127 tunnels between them, IDC FW alone holding
    // 101) were fresh, down-blind, and drawn as a confident "Down: 0". An
    // operator scanning that column never reads the bar above it, so the
    // fleet-level fix did not reach them. These three fields let the renderer
    // give that cell the hueless not-measured treatment WITH a reason, which is
    // the same failed-read-as-a-fact rule this whole file exists to enforce.
    const canReportDown = downObservable(d.vendor, d.mgmtMethod);
    // ⛔ THE `counts.down > 0` ESCAPE HATCH IS DELIBERATE AND MUST STAY. If a
    // firewall we believe cannot report a down tunnel reports one anyway, the
    // evidence outranks the map: suppressing a real reported outage because a
    // hard-coded table says it is impossible would hide the exact fact this
    // screen exists to surface, and would also hide the drift in the map.
    const downCountMeasured = snapshotFreshness === 'fresh'
      && (canReportDown === true || counts.down > 0);
    ...
    devices.push({
      ...
      canReportDown,
      downCountMeasured,
      // Why the down count is not a measurement. null when it IS one — a
      // present reason string is the renderer's signal to draw the gap.
      downCountReason: downCountMeasured
        ? null
        : (snapshotFreshness === 'fresh'
          ? DOWN_COUNT_REASON[canReportDown === null ? 'unknown' : 'blind']
  ...
  // ⛔ Which devices CANNOT report a down tunnel AT ALL, so the caller can say so
  // rather than presenting their 0-down as a clean result. This lives INSIDE
  // fleet.tunnels, beside the counts it qualifies — it was briefly a sibling of
  // , which meant the component reading fleet.tunnels.downObservability
  // got undefined and the caveat never rendered at all. 143 of 152 tunnels were
  // drawn with a hard "Down: 0" and no qualification for a full deploy cycle.
  const downObservability = (() => {
    ...
    // ⛔ THE FRESH SUBSETS ARE SEPARATE NUMBERS AND THE CALLER NEEDS THEM.
    //
    // `blindDevices` counts EVERY device that cannot report a down tunnel,
    // including ones that are stale or returned no rows at all. A caller
    // building a coverage bar over the FRESH population cannot subtract this
    // from `reportingFresh` — the two are different populations, and clamping
    // one against the other (Math.min) silently double-counts.
    //
    // Measured live when this was wrong: 16 devices, 11 blind (10 of them
    // fresh, 1 with no rows), 12 reportingFresh. The bar computed
    // min(11,12) = 11 blind-and-fresh and rendered "1 fully readable,
    // 11 cannot show a tunnel as down". The truth was 2 and 10 — the
    // no-rows Palo Alto was counted both as blind AND in the no-rows segment,
    // and it consumed one of the two genuinely readable Fortinets. The
    // segments still summed to 16 because the two errors cancelled, which is
    // exactly why it would never have looked wrong.
    const isFresh = (d) => d.coverage === 'reporting' && d.snapshotFreshness === 'fresh';
    return {
      ...
      // The only two numbers a coverage bar over the fresh population may use.
      blindAndFresh: blind.filter(isFresh).length,
      unknownAndFresh: unknown.filter(isFresh).length,
    };
  })();
  ...
  return {
    ...
    fleet: {
      ...
      tunnels: { ...fleetTunnels, downObservability },
      peering: {
        ...peering,
        // ⛔ BOTH HALVES OF THIS RATIO MUST HAVE THE SAME SCOPE. `ifaceRows` is
        // deliberately FLEET-WIDE (peer matching has to see every device’s
        // addresses), while `devices` honours options.deviceId — so when the page
        // was scoped to one firewall this rendered "collected from 15 of 1 active
        // firewalls". Report the fleet denominator, and say plainly that peer
        // matching stays fleet-wide even when the table is scoped.
        devicesWithInterfaceData: devicesWithInterfaceData.size,
        // Falls back to the scoped count only when a direct caller (a test) omits it.
        devicesTotal: Number.isFinite(fleetDeviceTotal) ? fleetDeviceTotal : totalDevices,
        scopedToOneDevice: Boolean(scopedToOneDevice),
      },
    },
    devices,
    down,
...
const COVERAGE_REASON = {
  ...
  // ⛔ DO NOT SAY "most likely it has none configured". That wording was here and it
  // was demonstrably WRONG on the live fleet: OKF(F2) is classified no_rows_polled
  // while its own syslog — in this same database — carries IPsec tunnel-stats and
  // "IPsec SA installed" rows, i.e. an established tunnel. Ranking one branch of an
  // ambiguity as "most likely" turned a per-device collection failure into a
  // statement about the customer’s network.
  no_rows_polled:
    'The VPN poll reached this device recently and this vendor can report tunnels, but no '
    + 'tunnel rows are stored. SecVault cannot tell which of two things happened: the device '
    + 'has no tunnels configured, or the tunnel command failed on a device that was otherwise '
    + 'reachable. Nothing records a successful TUNNEL pull, so both look identical here. '
    + 'Firewall logs may settle it — an IPsec tunnel-stats or SA-installed event from this '
    + 'device means it has tunnels this view is missing.',
  no_rows_unconfirmed:
...
// ⛔ Why a Down count is not a measurement, for the two cases where the snapshot
// itself is perfectly current. Both are facts about SECVAULT and about the
// vendor's answer format — never about the customer's network — and both are
// worded so a firewall administrator reading the tooltip learns why no number
// can be shown rather than what class of bug produced a dash.
const DOWN_COUNT_REASON = {
  blind:
    'This vendor and access method answer the tunnel query with currently-ESTABLISHED tunnels '
    + 'only. A tunnel that is down is simply absent from the answer rather than listed with a '
    + '“down” state, so SecVault cannot count down tunnels on this firewall. A 0 here would be a '
    + 'reading it never took. The Up count beside it is real: those tunnels were established when '
    + 'the snapshot was taken.',
  unknown:
    'SecVault does not know whether this vendor and access method can report a tunnel as down, so '
    + 'no down count can be claimed for this firewall. Unknown is not zero.',
};
...
const NOTES = {
  ...
  downSince:
    'How long a tunnel has been down is NOT derivable today: SecVault keeps only the latest '
    + 'snapshot of each firewall’s tunnels, so there is no state history to measure against. '
    + 'Answering it would take either an append-only record of every tunnel state-change, written '
    + 'at poll time, or correlation with the firewalls’ own syslog — the FortiOS '
    + '`action="tunnel-down"` event carries the phase-1 tunnel name only inside the raw message '
    + 'text, and no other vendor in this fleet emits an equivalent at all.',
  unknownStatus:
  ...
  peering:
    'A peer is matched to a managed firewall by comparing it against the interface addresses '
    + 'SecVault has collected from the fleet. That collection is itself partial, so “not matched” '
    + 'means SecVault could not match it — not that the far end is outside the fleet.',
};

// ⛔ EVERY STRING IN NOTES AND COVERAGE_REASON IS RENDERED TO THE CUSTOMER, so
// none of them may name a source file, a module, or a database table. This used
// to say “vpn_ipsec_tunnels holds only the latest snapshot” and “collected
// device_interfaces addresses”, which tells a firewall administrator to go and
// look at something they have no access to and cannot act on. Name the PRODUCT
// behaviour and the POLICY instead; the implementation detail belongs in a
// comment like this one, where the next reader of the code will find it.
// tests/vpnTunnelHealth.test.js asserts this over both objects.
