...
function isUnassessableAdvisory(advisory) {
  ...
  if (advisory.matchability === UNASSESSABLE_MATCHABILITY) return true;
  // ⛔ A NON-ARRAY `affected_version_ranges` IS ALSO UNASSESSABLE, and until
  // 2026-09-25 it CRASHED THE WHOLE RUN. The loop below does
  // `for (const range of advisory.affected_version_ranges || [])`, and `|| []`
  // does not help a truthy non-array: a jsonb object or a number threw
  // "ranges is not iterable", which aborts matching for EVERY remaining
  // advisory on EVERY remaining device — the fleet's CVE assessment silently
  // stops on one malformed row.
  //
  // That shape is not hypothetical here: cveHub.js's own guard records meeting
  // "a jsonb object, a string, an absent key" in this column.
  //
  // ⛔ Counted as unassessable rather than skipped quietly, because that is the
  // existing contract for "could not be evaluated" — the caller reports the
  // number, so a row we cannot read shows up as a coverage gap instead of
  // passing for "not affected". ⛔ NULL/undefined is deliberately NOT included:
  // that means "ingested before this column existed", which migrate.js
  // backfills, and treating it as unassessable would make every pre-column
  // advisory vanish from matching on the first deploy.
  const ranges = advisory.affected_version_ranges;
  if (ranges !== null && ranges !== undefined && !Array.isArray(ranges)) return true;
  return false;
}
...
function matchDeviceToAdvisories(device, deviceVersionTuple, advisories, recommendedReleases, applicability = null) {
  ...
  for (const advisory of advisories) {
    ...
    for (const range of ranges) {
      // ⛔ A NULL OR NON-OBJECT ENTRY IS SKIPPED, NOT DEREFERENCED. `[null]` is
      // a perfectly ordinary jsonb array and it threw "Cannot read properties
      // of null" — which, like the non-array case above, aborts matching for
      // every remaining advisory and device rather than costing one row.
      // Skipping the entry loses only that range; the advisory's other ranges
      // are still evaluated, so coverage degrades by one bound instead of
      // vanishing entirely.
    ...
    //
    // ⛔ AND FROM `exclude_fixed` WHEN THAT COLUMN IS EMPTY, because the fix
    // version is often already in the row and was simply never surfaced.
    // `exclude_fixed: true` means the range is "vulnerable up to but NOT
    // INCLUDING max" — which is NVD's `versionEndExcluding`, and by its own
    // definition `max` IS the first fixed release. This is a DEFINITION, not an
    // inference: nothing is guessed and no version is invented.
    //
    // Measured 2026-09-25: the fleet's only three `patch_now` rows were
    // CVE-2026-24858 (KEV, CVSS 9.8 at NVD) on three FortiGates running 7.4.9,
    // all with `fixed_in = NULL` — an urgent finding nobody could act on. The
    // hub's feed carried `{min: 7.4.0, max: 7.4.11, exclude_fixed: true}` the
    // whole time. Across that feed only 10 of 955 advisories populate
    // `fixed_in_versions`, while many carry the boundary in a range.
    //
    // ⛔ DERIVED ONLY WHEN THE PUBLISHED LIST IS EMPTY. A vendor that states
    // its fixed releases outranks a boundary we read off a range: Palo Alto's
    // PSIRT lists several per advisory, and a single range max would narrow
    // that to one branch.
    const published = Array.isArray(advisory.fixed_in_versions)
      ? advisory.fixed_in_versions
      : [];
    const fixedInCandidates = published.length > 0
      ? published
      : (Array.isArray(advisory.affected_version_ranges) ? advisory.affected_version_ranges : [])
        .filter((r) => r && r.exclude_fixed === true && r.max)
        .map((r) => r.max);
    let fixedIn = null;
