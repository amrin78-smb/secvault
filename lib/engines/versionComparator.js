...
function parseForcepointVersion(versionString) {
  ...
  // ⛔ Low-severity noise fix, 2026-07-23, found while investigating repeat
  // "[versionComparator] Unparseable version segment" log spam: a trailing
  // (or doubled-trailing) dot in a stored version string -- e.g. "6.1.1."
  // from a raw advisory data-quality artifact -- produces one EMPTY split
  // segment. parseInt("") is NaN, so this used to warn and silently default
  // that segment to 0 -- the exact same practical outcome as just not having
  // that segment at all (a shorter tuple already pads with 0 in
  // compareVersions below), just with a pointless warning attached. A
  // TRAILING empty segment carries no information either way, so any
  // trailing empty segment(s) are dropped before parsing rather than warned
  // about and zeroed.
  // ⛔ Fixed 2026-07-23 (same investigation): the original fix here filtered
  // out EVERY empty segment anywhere in the string, not just a trailing one
  // -- an embedded/doubled dot (e.g. "6..21", NOT at the end of the string)
  // silently shifted every later real segment one position to the LEFT,
  // changing the tuple's meaning ("6..21" -> [6,21], i.e. major=6, minor=21,
  // instead of the correct [6,0,21], major=6, minor=0, patch=21) and
  // flipping compareVersions() results in either direction -- the opposite
  // of this fix's own "changes NO comparison outcome" claim. Only a
  // TRAILING run of empty segments is stripped now; an embedded empty
  // segment still zero-pads at its original position (with its warning),
  // exactly like before this noise fix ever existed.
  const rawParts = String(versionString).split('.');
...
function parseCheckpointVersion(versionString) {
  ...
  // ⛔ Bug fixed 2026-07-23: any dotted segment past major.minor (tuple[2])
  // used to be silently discarded whenever the string had no explicit
  // "Take N" text, defaulting `take` to 0 regardless of that segment's real
  // value. lib/feeds/nvd.js's branchRangeFromWildcardCriteria() widens a
  // wildcarded CPE version like "81.20.*" into {min: "81.20.0", max:
  // "81.20.999"} to mean "the whole R81.20 branch" -- but both of those
  // strings parsed to the IDENTICAL tuple ([81,20,0,0]) here, since neither
  // has "Take" text and the appended ".0"/".999" segment was thrown away,
  // silently collapsing the widened range back to a single point and
  // defeating the fix for this one vendor. Fall back to the parsed 3rd
  // segment (if present) as the take number whenever there's no explicit
  // "Take N" text, so an appended ".0"/".999" branch-range suffix actually
  // changes the parsed tuple, the same as it does for every other vendor's
  // parser. A real "R81.20 Take 41" string is unaffected -- its explicit
  // "Take" text still takes priority over this fallback.
  const take = takeMatch
    ? parseInt(takeMatch[1], 10)
    : tuple[2] !== undefined
      ? tuple[2]
      : 0;
 ...
 */
/** Do the first three components match, i.e. is this checkpoint THIS device’s train? */
function isSameTrain(deviceTuple, checkpointTuple) {
  for (let i = 0; i < 3; i++) {
...
function hotfixOf(tuple) {
  return tuple[3] !== undefined ? tuple[3] : 0;
}

function isSafeOnMatchingTrain(deviceTuple, checkpointTuple) {
  if (!isSameTrain(deviceTuple, checkpointTuple)) return false;
  return hotfixOf(deviceTuple) >= hotfixOf(checkpointTuple);
}
...
function isInRange(vendor, deviceTuple, rangeMin, rangeMax, maxExclusive, safeCheckpoints) {
  // ⛔ A NAMED TRAIN IS A DEFINITE ANSWER IN BOTH DIRECTIONS — it must short-circuit
  // the min/max range either way, and getting only half of this wrong produced a
  // silent FALSE NEGATIVE on a CVSS 9.2 unauthenticated RCE.
  //
  // Vendors publish per-hotfix-train fix points: PAN-OS CVE-2026-0310 names
  // 11.1.4-h36, 11.1.6-h38, 11.1.7-h10, 11.1.10-h33, 11.1.13-h12 and 11.1.16-h2,
  // while its top-level `lessThan` bound is the LOWEST of those (11.1.4-h36).
  // A device on 11.1.13-h5 is therefore ABOVE that coarse max and used to fall out
  // at the rangeMax check below — reported NOT AFFECTED — even though its own train
  // is named right there with a fix (h12) it has not reached. Measured live on this
  // fleet: 6 advisories × 11 firewalls = 66 assessments silently missing, topped by
  // a CVSS 9.2 RCE.
  //
  // So: if the device is on a train the source NAMES, that checkpoint IS this
  // device’s fix point and the answer is decided here.
  //   at or past it  -> NOT vulnerable (unchanged behaviour)
  //   behind it      -> VULNERABLE, regardless of the coarse range
  // A device on an UNLISTED train still falls through to the coarse min/max and
  // stays conservatively flagged, exactly as before.
  for (const checkpoint of safeCheckpoints || []) {
    const checkpointTuple = parseVersion(vendor, checkpoint);
    if (isSameTrain(deviceTuple, checkpointTuple)) {
      return hotfixOf(deviceTuple) < hotfixOf(checkpointTuple);
    }
  }
