...
function parseForcepointVersion(versionString) {
  ...
  // (or doubled) dot in a stored version string -- e.g. "6.1.1." from a raw
  // advisory data-quality artifact -- produces one EMPTY split segment.
  // parseInt("") is NaN, so this used to warn and silently default that
  // segment to 0 -- the exact same practical outcome as just not having that
  // segment at all (a shorter tuple already pads with 0 in compareVersions
  // below), just with a pointless warning attached. An empty segment carries
  // no information either way, so it's dropped before parsing rather than
  // warned about and zeroed -- this changes NO comparison outcome, only
  // removes noise for this one specific, harmless shape. A genuinely
  // unparseable NON-empty segment (letters, a wildcard, prose text) still
  // warns exactly as before -- this is not a general suppression of the
  // warning, only of the empty-segment case.
