 ...
 * `system_info.*`, `content-preview.*`) for this vendor AND its immediate
 * field name under that subtree root is NOT in the allowlist (an EMPTY
 * allowlist means every field under that root is volatile). A vendor with no
 * registry entry never excludes anything — this only ever narrows what's
 * reported for vendors that have been explicitly, individually opted in
 * above.
 */
