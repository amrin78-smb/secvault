...
function diffValue(oldVal, newVal, path, acc, vendor) {
  ...
  if (bothArrays) {
    // Scoped narrowly: ONLY when every element on BOTH sides is a plain
    // primitive does the LCS-based diff apply (see diffPrimitiveArrayLCS's
    // header comment above). An array containing any object/array element on
    // EITHER side (e.g. a rule-entry array) keeps the original positional
    // index-by-index comparison below, unchanged -- extending LCS to object
    // arrays would need an identity field to align by, which is out of scope
    // here and riskier than this narrowly-scoped fix.
...
const SECTION_LABELS = [
  ...
  // classifyPath() below), but sectionLabelFor()'s per-segment EXACT match
  // needs its own entries or an unresolvable-index path under either falls
  // through to the generic "Other (Pre Rulebase)"/"Other (Post Rulebase)"
  // fallback instead of this label.
  ['pre-rulebase', 'Rules (detail unavailable for this device)'],
  ['post-rulebase', 'Rules (detail unavailable for this device)'],
...
//
// Only two path shapes are recognized, both generic PAN-OS config-tree
// concepts, identified purely by SEGMENT NAME (same segment-matching
// approach sectionLabelFor() already uses: split on '.', strip `[N]` index
// suffixes, look for a recognizable segment) -- NOT tied to one transport's
// path prefix:
...
function friendlyDescriptionForLocalUserDatabase(segments, ludIdx, changeType, values) {
  ...
  if (rest[0].toLowerCase() === 'user-group') {
    ...
    if (changeType === 'modified') {
      ...
      // mixed-shape array ever lands here instead.
      return `User "${groupName}" membership changed: "${formatValueForSentence(values.old)}" → "${formatValueForSentence(values.new)}"`;
    }
    return null;
  }
 ...
 * See this section's header comment above for the two recognized shapes and
 * the frozen-contract requirements. Never throws -- any unexpected input or
 ...
 * @param {{added:object[],removed:object[],modified:object[]}} diff
 * @returns {{
 *   ruleChanges: {ruleName:string, field:string|null, changeType:'added'|'removed'|'modified', friendlyDescription:string|null, value?:*, old?:*, new?:*}[],
 *   sections: {label:string, addedCount:number, removedCount:number, modifiedCount:number, totalCount:number, entries:{changeType:string, path:string, friendlyDescription:string|null, value?:*, old?:*, new?:*}[]}[],
 * }}
 */
function classifyDiff(diff) {
  ...
  function pushRuleChange(ruleName, field, rest) {
    ruleChanges.push({
      ...
      // Rule-table rows never match either recognized friendlyDescription
      // shape (rulebase.security.rules.* paths can't also contain
      // local-user-database/address/service segments) -- always null here,
      // but still present so DiffViewer.js can rely on the field existing
      // uniformly across both classifyDiff() output shapes.
      friendlyDescription: null,
      ...rest,
    });
  }
