...
// Anchors each registered volatile-subtree root to its REAL known nesting,
// so findSubtreeRootIndex()/isRegisteredSubtreeRoot() below never match the
// bare segment name floating anywhere in the path. Without this, a PAN-OS
// object of ANY type can legitimately be given an arbitrary free-text name --
// a security rule literally named "content-preview" would otherwise have
// every field change on it silently treated as noise against the (EMPTY)
// content-preview allowlist and dropped from the diff entirely, with no
// operator-visible signal at all. Two anchor shapes, one per registered root:
//  - `parentSegment`: the root must be immediately preceded by this exact
//    segment -- content-preview's real nesting is always `shared.content-preview`
//    (confirmed via Palo Alto's own docs, see MEANINGFUL_SUBTREE_FIELDS_BY_VENDOR's
//    own comment above), regardless of how deep `shared` itself sits (SSH's
//    `tree.shared.*`, XML/API's `devices.entry.vsys.entry.shared.*`).
//  - `atIndex`: the root must sit at this exact segment index -- system_info
//    is merged in by both parsers at the very TOP of config_parsed (per that
//    entry's own comment above), so it never legitimately appears anywhere
//    else.
// A root with no entry here falls back to the original bare-anywhere scan.
const SUBTREE_ROOT_ANCHORS = {
  'content-preview': { parentSegment: 'shared' },
  system_info: { atIndex: 0 },
};

// True when segment `i` of `segments` (already known to match `root` by
// name) also satisfies that root's registered anchor, if any -- see
// SUBTREE_ROOT_ANCHORS above.
function matchesSubtreeAnchor(segments, i, root) {
  const anchor = SUBTREE_ROOT_ANCHORS[root];
  if (!anchor) return true;
  if (typeof anchor.atIndex === 'number' && i !== anchor.atIndex) return false;
  if (anchor.parentSegment && (i === 0 || segments[i - 1].replace(/\[\d+\]$/, '') !== anchor.parentSegment)) {
    return false;
  }
  return true;
}
...
// -- BUT only when that occurrence also satisfies the root's registered
// anchor (SUBTREE_ROOT_ANCHORS above), so an unrelated object that merely
// happens to share the root's name is never matched. `[N]` array-index
// suffixes are stripped before comparing, same convention `stripIndex()`
// uses elsewhere in this file. Returns the segment's index, or -1 if not
// found.
function findSubtreeRootIndex(path, root) {
  const segments = path.split('.');
  for (let i = 0; i < segments.length; i++) {
    if (segments[i].replace(/\[\d+\]$/, '') === root && matchesSubtreeAnchor(segments, i, root)) return i;
  }
  return -1;
}
 ...
 * the start). Also gated by SUBTREE_ROOT_ANCHORS above, same as
 * findSubtreeRootIndex() -- otherwise a whole object elsewhere in the tree
 * that merely happens to be NAMED the same as a registered root (e.g. a
 * security rule literally named "content-preview") would be wrongly
 * decomposed as if it were that noisy subtree.
 */
function isRegisteredSubtreeRoot(path, vendor) {
  ...
  const lastSegment = segments[lastIdx].replace(/\[\d+\]$/, '');
  if (!Object.prototype.hasOwnProperty.call(subtrees, lastSegment)) return false;
  return matchesSubtreeAnchor(segments, lastIdx, lastSegment);
}
...
function normalizeLeaf(v) {
  ...
  // ⛔ SCALAR TYPE COERCION IS NOT A CONFIG CHANGE.
  // A device config value that renders identically has not changed, even if
  // its JS type did. Numbers/booleans are compared as their string form so
  // that 8 vs "8", 3.5 vs "3.5" and true vs "true" are equal.
  //
  // Why this exists (2026-08-03): the Palo Alto XML parser was coercing
  // numeric-looking element text to JS numbers, which silently destroyed
  // leading zeros in serials (007051000188962 -> 7051000188962). Fixing that
  // (parseTagValue:false) flipped ~125 scalars per device from number to
  // string, and every one surfaced as a bogus "modified" entry rendering as
  // `8 → 8` / `514 → 514` — identical on screen, alarming in the alerts feed.
  // Normalizing here rather than one-off cleaning the stored rows makes the
  // differ permanently immune to this whole class of parser-shape change,
  // which will happen again on the next adapter or transport revision.
  //
  // null/undefined are deliberately NOT stringified — String(null) === 'null'
  // would make a real null-vs-"null" difference vanish.
 ...
 *
 * ⛔ fast-xml-parser single-<entry> collapse (found live against
 * config_diffs row a6d89bcb-faf7-48b2-b77e-d70d41d1abfb, device 79111004/
 * ITC-SLY): a PAN-OS XML/API rulebase/object array
 * (`rulebase.nat.rules.entry`, etc.) is only ever an ARRAY when fast-xml-parser
 * sees MORE THAN ONE `<entry>` element -- exactly one remaining `<entry>`
 * collapses to a bare OBJECT instead of a 1-element array, a quirk this
 * codebase already has to work around elsewhere (see lib/adapters/paloalto/
 * parser.js). Left uncorrected, a real 3-to-1 rule-count reduction lands here
 * as `Array.isArray(oldVal)===true` / `Array.isArray(newVal)===false`, so
 * `bothArrays` below is never true and the whole array vs. whole object falls
 * through to the mixed-type fallback as ONE opaque 'modified' blob -- neither
 * the LCS primitive-array fix nor the identity-alignment object-array fix
 * (both below) ever get a chance to run, and classifyPath()/
 * extractIndexedRuleEntry() can't recognize it as a rule change either (no
 * `[N]` index in the path). Normalizing the lone-object side into a
 * 1-element array here, before bothArrays is computed, lets the real array
 * branch (and its LCS/identity alignment) run every time, diffing it
 * item-by-item instead of dumping it whole.
 */
function diffValue(oldVal, newVal, path, acc, vendor) {
  let oldEff = oldVal;
  let newEff = newVal;
  if (Array.isArray(oldEff) && isPlainObject(newEff)) {
    newEff = [newEff];
  } else if (Array.isArray(newEff) && isPlainObject(oldEff)) {
    oldEff = [oldEff];
  }

  const bothObjects = isPlainObject(oldEff) && isPlainObject(newEff);
  const bothArrays = Array.isArray(oldEff) && Array.isArray(newEff);

  if (bothObjects) {
    const keys = new Set([...Object.keys(oldEff), ...Object.keys(newEff)]);
    for (const key of keys) {
      const p = childPath(path, key, false);
      const inOld = Object.prototype.hasOwnProperty.call(oldEff, key) && oldEff[key] !== undefined;
      const inNew = Object.prototype.hasOwnProperty.call(newEff, key) && newEff[key] !== undefined;
      if (inOld && !inNew) {
        ...
        if (isPlainObject(oldEff[key]) && isRegisteredSubtreeRoot(p, vendor)) {
          diffValue(oldEff[key], {}, p, acc, vendor);
        } else if (!isVolatilePath(p, vendor)) {
          acc.removed.push({ path: p, value: oldEff[key] });
        }
      } else if (!inOld && inNew) {
        if (isPlainObject(newEff[key]) && isRegisteredSubtreeRoot(p, vendor)) {
          diffValue({}, newEff[key], p, acc, vendor);
        } else if (!isVolatilePath(p, vendor)) {
          acc.added.push({ path: p, value: newEff[key] });
        }
      } else if (inOld && inNew) {
        diffValue(oldEff[key], newEff[key], p, acc, vendor);
      }
    }
    return;
  }
  ...
  if (bothArrays) {
    ...
    if (oldEff.every(isPrimitiveArrayElement) && newEff.every(isPrimitiveArrayElement)) {
      diffPrimitiveArrayLCS(oldEff, newEff, path, acc, vendor);
      return;
    }
    ...
    const identityKey = chooseArrayIdentityKey(oldEff, newEff);
    if (identityKey !== null) {
      diffObjectArrayByIdentity(oldEff, newEff, identityKey, path, acc, vendor);
      return;
    }
    ...
    //    arrays of arrays).
    const maxLen = Math.max(oldEff.length, newEff.length);
    for (let i = 0; i < maxLen; i++) {
      const p = childPath(path, i, true);
      if (i >= oldEff.length) {
        if (!isVolatilePath(p, vendor)) acc.added.push({ path: p, value: newEff[i] });
      } else if (i >= newEff.length) {
        if (!isVolatilePath(p, vendor)) acc.removed.push({ path: p, value: oldEff[i] });
      } else {
        diffValue(oldEff[i], newEff[i], p, acc, vendor);
      }
    }
    return;
  }
...
// ⛔ A SINGLE SPACE IS LEGAL IN A PAN-OS NAME AND IS NOT CORRUPTION.
// This was `/[\s{}]/` — ANY whitespace — which is wrong on this fleet and was
// throwing away real information: live rule names include `65.32 allow all`,
// `Batch Scan for 27.254.123.18` and `HR TO Jobsdb.`, and object names have
// spaces too. Every one of those rendered as
// "(unreadable path — see full diff for details)" on the Changes page, so a
// genuine, readable firewall change was reported as unreadable garbage — the
// failed-read-as-a-fact rule inverted: a GOOD read recorded as a failure.
//
// What actually proves a captured "path" is corrupted raw config text is a
// BRACE or a CONTROL character (the brace-grammar capture bleeding in), or a
// RUN of whitespace — never one space between two words. The length bound at
// each call site still catches a long blob that carries neither.
const PATH_SHAPE_VIOLATION = new RegExp('[{}\\r\\n\\t\\f\\v]|\\s{2,}');
...
function summarizeDiff(diff) {
  ...
  // ⛔ WHAT CHANGED, NOT WHERE IT LIVES IN THE CONFIG TREE. This line used to
  // read `12 added — e.g. devices.entry.vsys.entry.tag.entry[17],
  // devices.entry.vsys.entry.tag.entry[18], ...` — three internal PAN-OS
  // paths, truncated, carrying an array index nobody can look up. It told a
  // reader nothing they could act on and pushed the one useful fact (what KIND
  // of thing moved) off the end of the line.
  //
  // classifyDiff() already buckets every entry into the same sections the page
  // renders below, so the summary now states those: `12 added — 4 tags,
  // 4 address objects, 3 security rules, 1 service object`.
  //
  // ⛔ The path fallback is KEPT for a diff nothing could classify. An
  // unclassifiable change is still a change, and a summary that named only the
  // counts would hide which part of the config it touched — leaving the reader
  // with less than the ugly version gave them.
  const sectionPhrase = summarizeSections(diff);
  if (sectionPhrase) summary += ` — ${sectionPhrase}`;
  else if (examplePaths.length > 0) summary += ` — e.g. ${examplePaths.join(', ')}`;
  return summary;
}

// Section labels are written for a heading ("Address Objects", "Other (Tag)").
// Inside a sentence they need a noun that pluralises, and `Other (X)` has to
// shed its wrapper or the line reads `4 Other (Tag)`.
//
// ⛔ A WORD IS AN ACRONYM OR A PROPER NOUN BY ITS OWN SHAPE, NOT BY A LIST.
// This was a curated Set of acronyms, and the curation is what broke it: the
// trailing-`s` strip ran BEFORE the lookup, so "DNS" was cut to "DN", missed
// the list it was in, and rendered "1 dn". A shape test cannot go stale the
// next time a label is added — ALL CAPS is an acronym (NAT, DNS, SNMP), an
// internal capital is a proper noun (GlobalProtect, FortiGuard), and neither
// may be lowercased or pluralised. Measured on live labels, the old function
// also produced "global protects", "system infos", "password policys" and
// "zones/interface".
// ⛔ ONE TEST, NOT TWO. A capital anywhere but the first letter already covers
// both cases — `DNS`/`NAT`/`SNMP` and `GlobalProtect`/`FortiGuard` — and an
// extra all-caps branch beside it could never change an outcome, which is the
// guard-that-cannot-fire this codebase treats as a defect in its own right.
function isAcronymOrProperNoun(word) {
  return /[A-Z]/.test(word.slice(1));
}

// Singular of ONE ordinary word. `ss` is left alone (an "address" is not an
// "addres"), and every word of the label is singularised, not just the last —
// "Zones/Interfaces" reads "1 zone/interface", never "1 zones/interface".
...
// ⛔ NOT EVERY SECTION IS A COUNT NOUN, AND PLURALISING ONE THAT ISN'T IS HOW
// "GlobalProtect" BECAME "2 global protects". "DNS", "System Info" and
// "Password Policy" name a part of the configuration, not a countable thing,
// so they are reported as changes TO that part and the label is carried
// through VERBATIM — capitalisation intact, nothing invented.
function sectionNoun(label, count) {
  const other = /^Other \((.+)\)$/.exec(String(label || ''));
  const base = (other ? other[1] : String(label || '')).trim();
  if (!base) return null;

  const tokens = base.split(/(\s+|\/|-)/).filter((t) => t !== '');
  const words = tokens.filter((t) => /[A-Za-z0-9]/.test(t));
  const lastWord = words[words.length - 1] || '';
  const countable = /s$/i.test(lastWord) && !/ss$/i.test(lastWord) && !isAcronymOrProperNoun(lastWord);

  if (!countable) {
    return count === 1 ? `1 change to ${base}` : `${count} changes to ${base}`;
  }

  // ⛔ AN ACRONYM IS NEVER SINGULARISED EITHER. "DNS" ends in an `s`; cutting
  // it produced the "1 dn" this function shipped with.
  const render = (singular) => tokens
    .map((t) => {
      if (!/[A-Za-z0-9]/.test(t)) return t;
      if (isAcronymOrProperNoun(t)) return t;
      const w = t.toLowerCase();
      return singular ? singularWord(w) : w;
    })
    .join('');

  return count === 1 ? `1 ${render(true)}` : `${count} ${render(false)}`;
}

// ⛔ NEVER THROWS, AND RETURNS null RATHER THAN A PARTIAL PHRASE. summarizeDiff()
// is on the write path for every config pull; a classifier problem must degrade
// to the old path-based summary, not lose the row.
const MAX_SUMMARY_SECTIONS = 4;

function summarizeSections(diff) {
  try {
    const { sections } = classifyDiff(diff);
    if (!Array.isArray(sections) || sections.length === 0) return null;
    const named = sections
      .map((sec) => sectionNoun(sec.label, sec.totalCount))
      .filter(Boolean);
    if (named.length === 0) return null;
    if (named.length <= MAX_SUMMARY_SECTIONS) return named.join(', ');
    // ⛔ The remainder is COUNTED, never dropped. A list silently cut to four
    // reads as the whole story, which is the truncation-looks-complete failure
    // this codebase guards everywhere else.
    const shown = named.slice(0, MAX_SUMMARY_SECTIONS);
    return `${shown.join(', ')} and ${named.length - MAX_SUMMARY_SECTIONS} more`;
  } catch (_err) {
    return null;
  }
}
 ...
 */
/**
 * Retroactively strips scalar TYPE-COERCION entries from already-stored
 * config_diffs, and deletes any row left with nothing real in it.
 *
 * Added 2026-08-03. The differ itself is now immune to this (see
 * normalizeLeaf), but rows written BEFORE that fix are already in the table
 * and already surfacing in the alerts feed as nonsense like `8 → 8` and
 * `514 → 514` — identical on screen because only the JS type differed. A code
 * fix does not retroactively repair persisted rows, exactly like the
 * Palo Alto version-range backfills above.
 *
 * Only touches `modified` entries whose old/new are equal under normalizeLeaf.
 * added/removed are left untouched, and any entry representing a REAL value
 * change survives — so a row that mixed genuine changes with coercion noise
 * keeps the genuine ones.
 *
 * @returns {Promise<{checked:number, deleted:number, updated:number}>}
 */
async function cleanupTypeCoercionConfigDiffs(pool) {
  ...
  for (const row of rows) {
    ...
    const kept = modified.filter((e) => {
      if (!e || typeof e !== 'object') return true;
      // ⛔ The test is a TYPE difference, not value equality.
      //
      // Value equality alone deleted real security events. Secret-bearing paths
      // are stored with BOTH sides rewritten to the identical literal
      // '<redacted>' (see redactSecretEntries), so a genuine PSK rotation, SNMP
      // community change or admin password-hash change is persisted as
      // {old:'<redacted>', new:'<redacted>'} — which read as "unchanged" and was
      // dropped, deleting the whole config_diffs row when it was the only entry.
      // On a firewall security product that is precisely the audit record you
      // cannot lose, and migrate.js runs this on EVERY deploy, so it recurred.
      //
      // A coercion artefact is specifically the same value in a DIFFERENT JS
      // type (8 vs "8"). Identical type AND identical value is a redacted real
      // change, and is kept.
      if (typeof e.old === typeof e.new) return true;
      return normalizeLeaf(e.old) !== normalizeLeaf(e.new);
    });
    if (kept.length === modified.length) continue; // nothing coercion-only here

    const added = Array.isArray(diff.added) ? diff.added : [];
    const removed = Array.isArray(diff.removed) ? diff.removed : [];
    const next = { added, removed, modified: kept };
...
/**
 * Removes config_diffs rows that record a SYSTEM-INFO READ FAILURE as if the
 * device had changed.
 *
 * Both Palo Alto transports fetched `show system info` best-effort and, on a
 * failure, stored `system_info: {}` plus null hostname/model/sw_version. The
 * diff engine then correctly reported ~12 REMOVALS against the previous
 * populated snapshot, and ~12 ADDITIONS when the next read succeeded.
 *
 * Confirmed live on TUG: config_raw was byte-identical (151063 bytes) on both
 * the "removed" and the "added" day — the device changed nothing. Fixed going
 * forward by preserveSystemInfoOnReadFailure() in lib/adapters/index.js, but a
 * code fix does not repair rows already persisted and already in the alerts
 * feed, exactly like the type-coercion cleanup above.
 *
 * ⛔ Deliberately narrow, for the same reason that cleanup had to be narrowed
 * after it deleted real security events: an entry only qualifies when it is
 * system-info-derived AND its shape is the read-failure signature. A modified
 * entry qualifies ONLY if exactly one side is null/empty — a real model or
 * version change has BOTH sides populated and is kept. Anything else in the
 * row survives, and a row keeping any real entry is updated rather than
 * deleted.
 *
 * @returns {Promise<{checked:number, deleted:number, updated:number}>}
 */
const SYSTEM_INFO_SCALARS = new Set(['model', 'hostname', 'sw_version']);

function isSystemInfoPath(path) {
  if (typeof path !== 'string') return false;
  return path === 'system_info' || path.startsWith('system_info.') || SYSTEM_INFO_SCALARS.has(path);
}

function isBlankSide(v) {
  return v === null || v === undefined || v === '' ||
    (typeof v === 'object' && !Array.isArray(v) && Object.keys(v).length === 0);
}

async function cleanupSystemInfoReadFailureDiffs(pool) {
  const { rows } = await pool.query('SELECT id, diff FROM config_diffs');
  let deleted = 0;
  let updated = 0;
  ...
  for (const row of rows) {
    ...
    // A wholesale appearance/disappearance of system_info, never both at once.
    const sysAdded = added.filter((e) => e && isSystemInfoPath(e.path));
    const sysRemoved = removed.filter((e) => e && isSystemInfoPath(e.path));
    if (sysAdded.length > 0 && sysRemoved.length > 0) continue;
    if (sysAdded.length === 0 && sysRemoved.length === 0) continue;

    const keptAdded = added.filter((e) => !(e && isSystemInfoPath(e.path)));
    const keptRemoved = removed.filter((e) => !(e && isSystemInfoPath(e.path)));
    // ⛔ One side blank is the failure signature. Both sides populated is a
    // REAL model/version/hostname change and must survive.
    const keptModified = modified.filter((e) => {
      if (!e || typeof e !== 'object' || !isSystemInfoPath(e.path)) return true;
      return !(isBlankSide(e.old) !== isBlankSide(e.new));
    });

    const changedAnything =
      keptAdded.length !== added.length ||
      keptRemoved.length !== removed.length ||
      keptModified.length !== modified.length;
    if (!changedAnything) continue;

    const next = { added: keptAdded, removed: keptRemoved, modified: keptModified };
    if (isEmptyDiff(next)) {
      await pool.query('DELETE FROM config_diffs WHERE id = $1', [row.id]);
      deleted += 1;
    } else {
...
const SECTION_LABELS = [
  ...
  // ⛔ ADDED AFTER READING THE HEADINGS THE LIVE FLEET ACTUALLY PRODUCES.
  // Anything with no entry here falls through to `Other (Title Case)`, which is
  // a heading that tells the reader the classifier gave up. Measured over 30
  // days of real changes, these were the ones showing up that way:
  // "Other (Tag)" 10, "Other (Global Protect)" 32, "Other (Content Preview)" 9,
  // "Other (Model)"/"(Hostname)"/"(Sw Version)" 2 each, "Other (Profile Group)" 1.
  //
  // ⛔ The `Other (...)` FALLBACK STAYS, and must. It is how an unrecognised
  // vendor section still reaches the page instead of vanishing, and naming the
  // segment is more useful than a bucket called "Other". This list just means
  // the common ones no longer need it.
  ['tag', 'Tags'],
  ['global-protect', 'GlobalProtect'],
  ['profile-group', 'Security Profile Groups'],
  ['profiles', 'Security Profiles'],
  ['content-preview', 'Content Preview'],
  // Bare top-level device identity leaves, from the system_info capture shape.
  // They ARE system info; they just arrive without the wrapper segment.
  ['hostname', 'System Info'],
  ['model', 'System Info'],
  ['sw_version', 'System Info'],
  ['serial', 'System Info'],
  ['uptime', 'System Info'],
];
...
// ⛔ SOME SECTION KEYS ARE ALSO RULE FIELD NAMES, and a rule field must stay
// with its rule. `tag` names both an object namespace (`vsys.entry.tag.entry[N]`,
// a real tag) and a field on a security rule (`rules.entry[5].tag`, that rule's
// tags); `profiles` likewise appears inside `profile-setting.profiles`. Adding
// them to SECTION_LABELS without this moved 33 rule changes out of
// "Security Rules" and into "Tags" — splitting one rule edit across two
// headings, which is worse than the generic bucket the label was fixing.
//
// ⛔ THE TEST IS "IS IT A FIELD OF A RULE", NOT "IS IT LAST". A first attempt
// deferred whenever the key was the final segment, which also swallowed
// `devices.entry.vsys.entry.tag` — the whole tag collection captured as one
// entry, which IS the Tags section. So the key defers only when a RULEBASE
// ancestor is present, which is exactly the case where the row belongs to a
// rule's own table.
//
// ⛔ AND "FINAL SEGMENT" WAS NOT THAT TEST — IT MISSED EVERY REAL PAN-OS
// MULTI-VALUE FIELD. A rule's tags are `rules.entry[5].tag.member[0]` and its
// profiles `rules.entry[5].profile-setting.profiles.virus.member[0]`: the
// ambiguous key is NOT last in either, so the deferral never fired and one
// rule edit was split across "Security Rules", "Tags" and "Security Profiles"
// — three headings for a change an operator made once. `service` is here for
// exactly the same reason (`rules.entry[5].service.member[0]` was filed under
// "Service Objects"); a real service OBJECT never sits below a rulebase, so
// scoping it this way costs nothing.
//
// The condition is now positional: the key defers when it sits BELOW a
// rulebase segment, which is what "a field of a rule" actually means.
const FIELD_AMBIGUOUS_SECTION_KEYS = new Set(['tag', 'profiles', 'service']);

function sectionLabelFor(path) {
  const segments = path.split('.').map(stripIndex).filter(Boolean);
  const rulebaseIdx = segments.findIndex((x) => FALLBACK_RULEBASE_LABELS.has(String(x).toLowerCase()));
  for (let i = 0; i < segments.length; i++) {
    const lower = segments[i].toLowerCase();
    if (FIELD_AMBIGUOUS_SECTION_KEYS.has(lower) && rulebaseIdx !== -1 && i > rulebaseIdx) continue;
  ...
  return `Other (${sectionFallbackName(firstReal)})`;
}

// A real config segment is a short identifier. A CORRUPTED one is not: the
// brace-grammar redaction bug documented at MAX_EXAMPLE_PATH_LENGTH above can
// swallow thousands of characters of raw config TEXT — newlines and braces
// included — into what should have been one key, and `Other (<that blob>)`
// then became a section HEADING on the page and, since v2.172.0 put the
// section phrase in the summary, a multi-thousand-character multi-line
// `change_summary` as well. That is the exact defect sanitizeExamplePath()
// was written to close, re-opened one layer up.
//
// ⛔ BOUNDED AT THE SOURCE, NOT IN THE SUMMARY. Fixing only summarizeDiff()
// would leave the heading itself corrupted — config_diffs has several
// independent places a path can render and fixing one is not fixing the
// others, which this file already says out loud. Two unreadable paths do
// collapse into one bucket; their entries are all still there and still
// counted, and neither name was readable to begin with.
const MAX_SECTION_NAME_LENGTH = 40;

function sectionFallbackName(segment) {
  const str = String(segment);
  if (PATH_SHAPE_VIOLATION.test(str) || str.length > MAX_SECTION_NAME_LENGTH) return 'unreadable';
 ...
 */
// ⛔ A RULE NAME MAY CONTAIN DOTS, SO `parts[0]` IS NOT THE NAME.
// This took the first segment and called it the rule, which on the live fleet
// reported `Allow_URL_tfcc.fisheries.go.th` as a rule called
// `Allow_URL_tfcc` — a rule that does not exist, named confidently on the page
// an operator reads to find out what changed on a firewall. Measured:
// 47 of 1,780 live rules carry a dot in their name.
//
// The name is recovered the same way nameTreePath() recovers an object name:
// anchor on the FIELD, never on the separator. The name runs from the marker
// up to the first segment that is a real PAN-OS rule field (TREE_FIELD_TOKENS,
// the list the tree-path namer already uses), so nothing is guessed from
// punctuation.
//
// ⛔ The search starts at index 1, never 0: a rule may legitimately be NAMED
// after a field word, and the segment immediately after the marker is always
// the name. A name whose LATER segment collides with a field token
// (`Allow.source`) is genuinely ambiguous from the path alone and resolves in
// favour of the field — the same trade nameTreePath makes, and still strictly
// better than splitting every dotted name.
function classifyPath(path) {
  ...
  if (idx !== -1) {
    ...
    for (let i = 1; i < parts.length; i += 1) {
      if (TREE_FIELD_TOKENS.has(stripIndex(parts[i]))) { cut = i; break; }
    }
    const ruleNameRaw = parts.slice(0, cut).join('.');
    ...
    if (ruleNameRaw && !isUnresolvableIndex) {
      ...
      const field = parts.slice(cut).join('.') || null;
 ...
 */
// ⛔ The container segment decides the NOUN. Unmapped containers still get a
// name -- the segment humanised -- because "Schedule \"X\" was added" is worth
// far more than the raw path even when nobody has curated a label for it. The
// map exists to fix the ones where humanising the segment reads badly
// ("Address-group", "User", "Proxy-id").
const INDEXED_ENTRY_NOUNS = new Map([
  ['address', 'Address object'],
  ['address-group', 'Address group'],
  ['service', 'Service object'],
  ['service-group', 'Service group'],
  ...
  ['ipsec', 'IPsec tunnel'],
  ['ike', 'IKE gateway'],
  // Added after reading the descriptions this produced on 30 days of real
  // diffs: these rendered as a bare, redundant or lowercase noun.
  ['ethernet', 'Ethernet interface'],
  ['gateway', 'IKE gateway'],
  // ⛔ `units` IS EVERY INTERFACE TYPE'S SUB-INTERFACE CONTAINER, NOT TUNNELS'.
  // It was mapped to 'Tunnel interface', so
  // `network.interface.ethernet.entry[2].layer3.units.entry[5].ip` read
  // "Tunnel interface ip was added" — a false statement about WHICH interface
  // changed, on a page whose whole job is saying what changed. The tunnel
  // wording survives where it is actually true, scoped as
  // `interface.tunnel.units` in INDEXED_PARENT_OVERRIDES below.
  ['tunnel.units', 'Tunnel interface'],
  ['units', 'Interface unit'],
  // ⛔ AND `users` IS NOT ALWAYS AN ADMINISTRATOR. Unscoped, it turned
  // `ssl_vpn.users.entry[0]` into `Administrator "vpnuser1" was added` — a
  // privileged-account-creation claim about a VPN user. PAN-OS's real admin
  // container is `mgt-config.users`, which keeps the word, scoped.
...
// The name PAN-OS's XML/API shape puts on an array entry. `@_name` is what the
// parser emits (verified against live config_diffs rows); the others are
// accepted because the same idea appears under different spellings across the
// shapes this function already serves, and reading one that is not there costs
// nothing.
const ENTRY_NAME_KEYS = ['@_name', '@name', 'name'];

function entryOwnName(values) {
  for (const side of ['value', 'new', 'old']) {
    const v = values && values[side];
...
/**
 * `<Noun> "<name>" was added/removed/changed` for an INDEXED entry whose name
 * lives in its value rather than in its path.
 *
 * ⛔ REQUIRES BOTH an indexed final segment AND a name in the value. Without
 * the index check this would rename entries whose path already carries a name
 * and whose own builder described them better; without the name it would emit
 * `Address object "" was added`, which is worse than the path it replaced.
 * Returns null in either case, and the raw path is shown exactly as before.
 */
function friendlyDescriptionForIndexedNamedEntry(path, segments, changeType, values) {
  if (typeof path !== 'string') return null;
  const rawSegments = path.split('.');
  const last = rawSegments[rawSegments.length - 1];
  // Only the entry ITSELF, never a field underneath it: `...entry[5].log-end`
  // is a field change on a rule and is described by the rule grouper.
  if (!/^entry\[\d+\]$/.test(String(last || ''))) return null;

  const name = entryOwnName(values);
  if (name === null) return null;

  const container = segments.length >= 2 ? String(segments[segments.length - 2]).toLowerCase() : '';
  // The container's own PARENT, so a noun can be scoped where the bare word is
  // ambiguous -- `mgt-config.users` is an administrator, `ssl_vpn.users` is
  // not. Tried first; the bare container remains the fallback.
  const scoped = segments.length >= 3
    ? `${String(segments[segments.length - 3]).toLowerCase()}.${container}`
    : '';
  // ⛔ CAPITALISED. Without this the unmapped containers rendered mid-sentence
  // lowercase -- `users "awnadm" was added`, `gateway "Backup-SMT" was added`
  // -- which reads as a fragment rather than a sentence.
  const noun = (scoped ? INDEXED_ENTRY_NOUNS.get(scoped) : null)
    || INDEXED_ENTRY_NOUNS.get(container)
    || (container
      ? humanizeFieldForSentence(container).replace(/^./, (c) => c.toUpperCase())
      : 'Entry');

  const verb = changeType === 'added' ? 'was added'
...
// ⛔ A LIST MEMBERSHIP IS THE SINGLE BIGGEST SOURCE OF RAW PATHS ON SCREEN.
// Measured over the last 30 days of real diffs: 200 of 695 rendered entries
// were raw config paths, and 125 of those 200 were this one shape --
// `<container>.member[N]`, where the VALUE is the member and the path carries
// only an index. PAN-OS uses it for user-group members, address-group members,
// GlobalProtect split-tunnel routes, virtual-router interfaces and zone
// interfaces alike.
//
// ⛔ IT SAYS "member", NOT "was created". `user-group.entry[0].user.member[42]`
// is an existing account joining a group; describing it as a new user is the
// lie the indexed-entry guard already exists to prevent, and it would arrive
// here by a different road.
//
// ⛔ IT DOES NOT NAME WHICH CONTAINER, because the container is an INDEX in
// the path (`entry[0]`) and its name is nowhere in the diff entry. Inventing
// one would be worse than the path. The section heading above the row already
// says which kind of object it is, and the value says which member.
const MEMBER_PATH_CONNECTIVES = new Set([
  'entry', 'static', 'units', 'config', 'configs', 'member', 'list', 'members',
]);

// Where humanising the surrounding segments reads badly or loses the point.
const MEMBER_CONTAINER_OVERRIDES = new Map([
  ['user-group.user', 'User group member'],
  ['address-group', 'Address group member'],
  ['split-tunneling.access-route', 'Split-tunnel access route'],
  ['import.network.interface', 'Imported interface'],
  ['virtual-router.interface', 'Virtual router interface'],
  ['zone.network.layer3', 'Zone layer-3 interface'],
]);

function memberContainerLabel(segments, memberIdx) {
  const meaningful = [];
  for (let i = memberIdx - 1; i >= 0 && meaningful.length < 3; i--) {
    const seg = String(segments[i]).toLowerCase();
    if (MEMBER_PATH_CONNECTIVES.has(seg)) continue;
    meaningful.unshift(seg);
  }
  if (meaningful.length === 0) return null;
  for (let take = meaningful.length; take >= 1; take--) {
    const key = meaningful.slice(meaningful.length - take).join('.');
    if (MEMBER_CONTAINER_OVERRIDES.has(key)) return MEMBER_CONTAINER_OVERRIDES.get(key);
  }
  const last = meaningful[meaningful.length - 1];
  return `${humanizeFieldForSentence(last).replace(/^./, (c) => c.toUpperCase())} member`;
}

/**
 * `<Container> member "<value>" was added/removed` for a list element.
 *
 * ⛔ Requires the final segment to be `member` or `member[N]` and the value to
 * be a PRIMITIVE. A member whose value is an object is not a plain list entry
 * and is left to the shapes above, which may know more about it.
 */
function friendlyDescriptionForListMember(path, segments, changeType, values) {
  if (typeof path !== 'string') return null;
  const rawSegments = path.split('.');
  const last = String(rawSegments[rawSegments.length - 1] || '');
  if (!/^member(\[\d+\])?$/.test(last)) return null;
  ...
  // ⛔ An ARRAY is accepted because PAN-OS collapses a one-element list to a
  // bare string and expands it to an array on the second element -- so
  // `layer3.member` modified arrives as "ethernet1/1" -> [..., ...]. Refusing
  // arrays left exactly that change rendering as a raw path.
  const pick = (v) => {
  ...
  const value = changeType === 'modified'
    ? pick(values && values.new)
    : pick(values && values.value);
  if (value === null || value.trim() === '') return null;

  const memberIdx = segments.length - 1;
  const label = memberContainerLabel(segments, memberIdx);
  if (label === null) return null;

  if (changeType === 'modified') {
    const before = pick(values && values.old);
    if (before !== null) return `${label} changed from "${before}" to "${value}"`;
  }
  return `${label} "${value.trim()}" ${verb}`;
}

// ⛔ A FIELD UNDER AN INDEXED PARENT: the second-biggest raw shape once list
// members were named. `...tunnel.ipsec.entry[7].tunnel-monitor.destination-ip`
// is a real, readable change and was rendering as that string.
//
// ⛔ IT DOES NOT NAME WHICH PARENT, AND THAT LOSES NOTHING. The parent is an
// INDEX in the path and its name is nowhere in the diff entry -- the raw path
// did not identify it either. What this adds is the WHAT; the row already shows
// the value beside it. Naming a parent we cannot read would be the invention
// this file refuses everywhere else.
const INDEXED_PARENT_OVERRIDES = new Map([
  ['interface.tunnel.units', 'Tunnel interface'],
  ['tunnel.ipsec', 'IPsec tunnel'],
  ['ike.gateway', 'IKE gateway'],
  ['local-user-database.user', 'Local user'],
  ['virtual-router.routing-table.ip.static-route', 'Static route'],
  ['profiles.interface-management-profile', 'Interface management profile'],
  ['global-protect.global-protect-gateway', 'GlobalProtect gateway'],
]);

const PARENT_PATH_CONNECTIVES = new Set(['entry', 'devices', 'vsys', 'shared', 'network', 'config']);

function indexedParentLabel(segments, entryIdx) {
  const meaningful = [];
  for (let i = entryIdx - 1; i >= 0 && meaningful.length < 4; i--) {
    const seg = String(segments[i]).toLowerCase();
    if (PARENT_PATH_CONNECTIVES.has(seg)) continue;
    meaningful.unshift(seg);
  }
  if (meaningful.length === 0) return null;
  for (let take = meaningful.length; take >= 1; take--) {
    const key = meaningful.slice(meaningful.length - take).join('.');
    if (INDEXED_PARENT_OVERRIDES.has(key)) return INDEXED_PARENT_OVERRIDES.get(key);
    const single = INDEXED_ENTRY_NOUNS.get(key);
    if (single) return single;
  }
  const last = meaningful[meaningful.length - 1];
  return humanizeFieldForSentence(last).replace(/^./, (c) => c.toUpperCase());
}

/**
 * `<Parent> <field> was added/removed/changed` for a leaf under `entry[N]`.
 */
function friendlyDescriptionForIndexedParentField(path, segments, changeType, values) {
  if (typeof path !== 'string') return null;
  // ⛔ ONE INDEX SPACE, NOT TWO. `entryPos` is found here and then used to
  // index `segments`, which the caller built as
  // `split('.').map(stripIndex).filter(Boolean)` -- so a path carrying an
  // empty or index-only segment shifted the two apart and the label was read
  // off the wrong word. Filtering the raw array by the same rule keeps
  // `rawSegments.map(stripIndex)` identical to `segments`, position for
  // position.
  const rawSegments = path.split('.').filter((s) => stripIndex(s) !== '');
  // The LAST indexed `entry[N]`, so the tail is everything below it.
  let entryPos = -1;
  for (let i = rawSegments.length - 2; i >= 0; i--) {
    if (/^entry\[\d+\]$/.test(String(rawSegments[i]))) { entryPos = i; break; }
  }
  if (entryPos === -1) return null;

  const tail = rawSegments.slice(entryPos + 1).filter((x) => !/^\w+\[\d+\]$/.test(x));
  ...
  const label = indexedParentLabel(segments, entryPos);
  if (label === null) return null;
  const field = tail.map((t) => humanizeFieldForSentence(t)).join(' ');
  return `${label} ${field} ${verb}`;
}

/**
 * PAN-OS `deviceconfig` — the box's own settings (management address, DNS, NTP,
 * enabled services).
 *
 * ⛔ IT WAS LEFT RAW ON PURPOSE UNTIL NOW, and the reason has changed. The
 * original note said `deviceconfig` is a broad root with no confident one-level
 * identifier, which is true — so this does not try to invent one. It names the
 * SETTING, which is exactly what the path already spelled out and all a reader
 * needs: `System service disable-https was changed`. The alternative, until a
 * moment ago, was that the object-section loop matched the word `service`
 * inside it and called it `Service object "disable-https"` — naming the wrong
 * kind of thing entirely.
 */
function friendlyDescriptionForDeviceConfig(segments, changeType, values) {
  const idx = segments.findIndex((x) => String(x).toLowerCase() === 'deviceconfig');
  if (idx === -1) return null;
  const tail = segments.slice(idx + 1).filter(Boolean);
  ...
  const sentence = (words) => `${words.replace(/^./, (c) => c.toUpperCase())} ${verb}`;

  // ⛔ `entry` IS NOT A SETTING NAME, IT IS AN ARRAY SLOT. This function runs
  // BEFORE friendlyDescriptionForIndexedNamedEntry(), and it accepted any
  // 1-4 segment tail — so `deviceconfig.high-availability.group.entry[0]`,
  // carrying {"@_name":"HA-Group-1"}, described itself as "High availability
  // group entry was added" and, because a description now existed, the display
  // layer dropped the `@_name` row as a duplicate of a heading that never said
  // it. The name then appeared NOWHERE on the page. It is read from the value
  // here instead, which is the one place it exists.
  if (String(tail[tail.length - 1]).toLowerCase() === 'entry') {
    const name = entryOwnName(values);
    const words = tail.slice(0, -1).map((t) => humanizeFieldForSentence(t)).join(' ');
    if (name === null || words === '') return null; // no name to state -- the named-entry builder gets its turn
    return `${words.replace(/^./, (c) => c.toUpperCase())} "${name}" ${verb}`;
  }

  return sentence(tail.map((t) => humanizeFieldForSentence(t)).join(' '));
}

// ---------------------------------------------------------------------------
// friendlyDescription -- `tree.`-prefixed CLI/brace-grammar paths
// ---------------------------------------------------------------------------
//
// ⛔ THE NAME IS IN THE PATH AND CONTAINS DOTS AND SPACES, SO NOTHING THAT
// SPLITS ON '.' CAN READ IT. The PAN-OS CLI/brace capture
// (lib/adapters/paloalto/sshParser.js) uses the object's own name as a block
// key, and PAN-OS names legitimately contain both:
...
// Every builder above resolves a name by splitting on '.', so each of these
// either fragments into a name that does not exist or resolves nothing at
// all. Measured on the live fleet: 496 of 523 rendered entries on one
// firewall's Changes page were raw `tree.` paths, and the shape is current on
// 7 Palo Altos.
//
// ⛔ ANCHORED AT BOTH ENDS, NEVER SPLIT IN THE MIDDLE. The section PREFIX is
// known (it is PAN-OS schema, not user input) and a trailing FIELD is
// recognisable (lower-case/hyphen tokens, optionally indexed). Whatever is
// left between the two is the name, taken VERBATIM — dots, spaces and all.
// Nothing about it is parsed, because there is nothing in a PAN-OS name that
// can be parsed.
//
// Longest prefix first, so `address-group` wins over `address` and
// `local-user-database.user-group` over `local-user-database.user`.
const TREE_SECTIONS = [
  ['local-user-database.user-group', 'User group'],
  ['local-user-database.user', 'Local user'],
  ['rulebase.security.rules', 'Security rule'],
  ['rulebase.nat.rules', 'NAT rule'],
  ['rulebase.pbf.rules', 'Policy-based forwarding rule'],
  ['network.ike.gateway', 'IKE gateway'],
  ['network.tunnel.ipsec', 'IPsec tunnel'],
  ['network.interface', 'Interface'],
  ['address-group', 'Address group'],
  ['service-group', 'Service group'],
  ['application-filter', 'Application filter'],
  ['application-group', 'Application group'],
  ['profile-group', 'Security profile group'],
  ['address', 'Address object'],
  ['service', 'Service object'],
  ['tag', 'Tag'],
  ['zone', 'Zone'],
...
// A trailing field token: `.user[67]`, `.source[4]`, `.ip`,
// `.peer-address.ip`. ⛔ ONLY lower-case/hyphen tokens qualify, because a NAME
// on this fleet is mixed-case, numeric or spaced far more often than not
// (`SSL_GROUP`, `IDC-Salaya`, `192.168.2.0 TO WAN-Specific`) — a looser test
// would eat the tail of a name and report an object that does not exist.
const TREE_FIELD_TAIL = /((?:\.[a-z][a-z0-9-]*)(?:\[\d+\])?)+$/;

// ⛔ AND THE LOWER-CASE TEST ALONE IS NOT ENOUGH, BECAUSE AN FQDN ADDRESS
// OBJECT IS ALL LOWER CASE AND FULL OF DOTS. Live on this fleet:
// `tree.address.adobe.com`, `tree.address.m.platform.boomi.com`,
// `tree.address.tfcc.fisheries.go.th` — whole objects, value `{"fqdn": ...}`,
// whose names end in a token the tail pattern happily eats. Reading `com` as a
// field produced `Address object "adobe"'s com was added`: an object that is
// not on the device and a setting that does not exist, stated flatly.
//
// So a tail is only a FIELD when its first token is one PAN-OS actually has.
// Grounded, not guessed: every token below is either already named by this
// file (RULE_OBJECT_DETAIL_FIELDS, the NAT/PBF/address builders), by
// DiffViewer's RULE_FIELD_LABELS, or by lib/adapters/paloalto/sshParser.js's
// own extraction — or was read off the live fleet's own diff paths.
const TREE_FIELD_TOKENS = new Set([
  // rules (security / NAT / PBF)
  'from', 'to', 'source', 'destination', 'service', 'application', 'action',
  'category', 'schedule', 'disabled', 'description', 'tag', 'log-start',
  'log-end', 'log-setting', 'rule-type', 'source-user', 'source-hip',
  'destination-hip', 'hip-profiles', 'negate-source', 'negate-destination',
  'profile-setting', 'group-tag', 'target', 'uuid', 'option', 'qos',
  'to-interface', 'nat-type', 'source-translation', 'destination-translation',
  'dynamic-destination-translation', 'translate-to', 'enforce-symmetric-return',
  'active-active-device-binding', 'icmp-unreachable',
  // address / address-group / service / service-group / tag
  'ip-netmask', 'ip-range', 'ip-wildcard', 'fqdn', 'color', 'comments',
  'static', 'dynamic', 'members', 'protocol', 'disable-override',
  // users and groups
  'user', 'phash', 'password-hash', 'expires',
  // zones, interfaces, routing
  'network', 'layer2', 'layer3', 'units', 'ip', 'comment', 'interface',
  'enable-user-identification', 'user-acl', 'device-acl',
  'zone-protection-profile', 'link-state', 'aggregate-group', 'metric',
  'nexthop', 'destination-ip',
  // IKE / IPsec
  'peer-address', 'local-address', 'authentication', 'protocol-common',
  'local-id', 'peer-id', 'auto-key', 'tunnel-monitor', 'tunnel-interface',
  'anti-replay', 'copy-tos', 'proxy-id',
  // application filters / groups, profile groups
  'subcategory', 'technology', 'risk', 'evasive', 'pervasive',
  'excessive-bandwidth-use', 'used-by-malware', 'transfers-files',
  'has-known-vulnerabilities', 'tunnels-other-application', 'prone-to-misuse',
  'virus', 'spyware', 'vulnerability', 'url-filtering', 'file-blocking',
  'data-filtering', 'wildfire-analysis', 'member',
]);

// True when the entry's VALUE is a whole config NODE rather than a leaf. It
// is the second half of the FQDN decision above: an unrecognised tail on an
// OBJECT value means the path ends at a node, so every token belongs to the
// name (`tree.address.adobe.com` = `{"fqdn":"adobe.com"}`), while the same
// tail on a scalar means the path ends at a leaf, so the split stands. A
// KNOWN field token outranks both — `...ike.gateway.IDC-Salaya.protocol.
// ikev2.pq-ppk` carries an object and is still a field.
function treeEntryValueIsNode(changeType, values) {
  if (!values || typeof values !== 'object') return false;
  if (changeType === 'modified') return isPlainObject(values.old) || isPlainObject(values.new);
  return isPlainObject(values.value);
}

/**
 * `{noun, name, field, whole}` for a `tree.`-prefixed path, or null when no
 * registered section owns it. `name` is the raw remainder between the section
 * prefix and the field tail — never split, never trimmed of its own dots.
 *
 * @param {boolean} valueIsNode - see treeEntryValueIsNode() above.
 */
function nameTreePath(path, valueIsNode) {
  if (typeof path !== 'string' || !path.startsWith('tree.')) return null;
  const afterTree = path.slice('tree.'.length);
  // ⛔ `shared` IS A WRAPPER, NOT A SECTION -- exactly as WRAPPER_SEGMENTS
  // above already treats it, and for the same reason: PAN-OS nests the real,
  // type-specific objects directly under it (`tree.shared.address.<name>`,
  // `tree.shared.local-user-database.user.<name>`). Stripping it lets ONE
  // section list serve both scopes instead of two parallel copies that can
  // drift. Only this one wrapper, and only at the front: anything else is a
  // shape nobody has verified.
  const candidates = afterTree.startsWith('shared.')
    ? [afterTree, afterTree.slice('shared.'.length)]
    : [afterTree];
  for (const rest of candidates) {
    for (const [prefix, noun] of TREE_SECTIONS) {
      if (rest === prefix) return { noun, name: null, field: null, whole: true };
      if (!rest.startsWith(`${prefix}.`)) continue;
      let remainder = rest.slice(prefix.length + 1);
      let field = null;
      const m = TREE_FIELD_TAIL.exec(remainder);
      // Only a field tail if something is LEFT to be the name -- otherwise the
      // whole remainder is a lower-case name and calling it a field would drop
      // the only identifier the row has.
      if (m && m.index > 0) {
        const candidate = m[0].replace(/^\./, '');
        const firstToken = stripIndex(candidate.split('.')[0]);
        if (TREE_FIELD_TOKENS.has(firstToken) || !valueIsNode) {
          field = candidate;
          remainder = remainder.slice(0, m.index);
        }
        // else: an unrecognised tail on a whole config node -- the tokens are
        // part of the NAME (an FQDN address object), and the name is kept
        // whole rather than cut at a dot that means nothing.
      }
      if (remainder === '') return null;
      return { noun, name: remainder, field, whole: false };
    }
  }
  return null;
}

// `user[67]` -> `user`, `peer-address.ip` -> `peer address ip`.
function humanizeTreeField(field) {
  return String(field)
    .split('.')
    .map((seg) => humanizeFieldForSentence(stripIndex(seg)))
    .join(' ');
}

function friendlyDescriptionForTreePath(path, changeType, values) {
  const named = nameTreePath(path, treeEntryValueIsNode(changeType, values));
  ...
  // The whole section container appearing or disappearing at once. It names
  // the section rather than an object, because there is no object here to
  // name -- inventing one would be the fabrication this file refuses.
  if (named.whole) return `${named.noun} section ${verb}`;

  if (named.field === null) return `${named.noun} "${named.name}" ${verb}`;

  // A field UNDER the named object. The row already shows the value beside
  // this label, so the sentence states which object and which setting and
  // stops there.
  const field = humanizeTreeField(named.field);
  if (field === '') return `${named.noun} "${named.name}" ${verb}`;
  if (looksLikeSecretFieldName(named.field)) return `${named.noun} "${named.name}" ${verb}`;
  if (changeType === 'modified') return `${named.noun} "${named.name}"'s ${field} was changed`;
  return `${named.noun} "${named.name}"'s ${field} ${verb}`;
}

// ⛔ A NAME CARRYING A DOT OR A SPACE CANNOT SURVIVE ANY OTHER BUILDER.
// `tree.address.NW_GoogleCloud-10.120.0.0_20.ip-netmask` reached
// friendlyDescriptionForNetworkObject(), which read segments[2] as the name
// and reported `Address object "NW_GoogleCloud-10" was changed` — an object
// that does not exist on the device, stated with no hedge. That is worse than
// the raw path it replaced, so these resolve FIRST; every other `tree.` path
// keeps the more specific description its own builder already gives it and
// only falls back to the section namer at the end.
function treePathNeedsAnchoredName(path, changeType, values) {
  const named = nameTreePath(path, treeEntryValueIsNode(changeType, values));
  if (named === null || named.name === null) return false;
  return /[.\s]/.test(named.name);
}
...
function computeFriendlyDescription(path, changeType, values) {
  try {
    ...
    // ⛔ FIRST, AND ONLY FOR A NAME NO OTHER BUILDER CAN READ. See
    // treePathNeedsAnchoredName() above: a `tree.` name carrying a dot or a
    // space is fragmented by every builder below, and one of them states the
    // fragment as fact. Every other `tree.` path falls to the same namer at
    // the END of this function, after the specific builders have had it.
    if (treePathNeedsAnchoredName(path, changeType, safeValues)) {
      const anchored = friendlyDescriptionForTreePath(path, changeType, safeValues);
      if (anchored !== null) return anchored;
    }

    const ludIdx = segments.findIndex((s) => s.toLowerCase() === 'local-user-database');
    if (ludIdx !== -1) {
      const lud = friendlyDescriptionForLocalUserDatabase(segments, ludIdx, changeType, safeValues);
      // ⛔ FALLS THROUGH ON null INSTEAD OF RETURNING IT. This used to return
      // whatever that builder produced, including null — so an indexed PAN-OS
      // shape it cannot name (`shared.local-user-database.user.entry[530]`, 53
      // live occurrences) short-circuited past every later branch and rendered
      // as a raw path. The builder is unchanged; only its null is now allowed
      // to reach the named-entry fallback at the bottom.
      if (lud !== null) return lud;
    }

    // ⛔ `deviceconfig` IS NOT AN OBJECT NAMESPACE. It is PAN-OS's general
    // settings root, and `deviceconfig.system.service.disable-https` was being
    // read as an ADDRESS/SERVICE OBJECT called "disable-https" -- a system
    // toggle described as a firewall object. Pre-existing, and only visible
    // once the surrounding rows stopped being raw paths.
    //
    // ⛔ AND A RULE'S OWN FIELDS ARE NOT OBJECTS EITHER — the same failure, one
    // table over. `rules.entry[5].service.member[0] = "tcp-7903"` was read as a
    // SERVICE OBJECT whose name is the literal word `member`, because this loop
    // runs before the list-member builder and wins. A real address or service
    // object never sits below a rulebase segment, so anything that does is a
    // field of a rule and belongs to the builders further down, which name the
    // member itself. Same positional test sectionLabelFor() uses for the
    // heading, so a rule edit cannot be split between the two.
    const isDeviceConfig = segments.some((x) => String(x).toLowerCase() === 'deviceconfig');
    const rulebaseIdx = segments.findIndex((x) => FALLBACK_RULEBASE_LABELS.has(String(x).toLowerCase()));
    for (let i = 0; !isDeviceConfig && i < segments.length; i++) {
      if (rulebaseIdx !== -1 && i > rulebaseIdx) break;
      const kind = OBJECT_SECTION_KIND.get(segments[i].toLowerCase());
      if (kind) {
        const objDescription = friendlyDescriptionForNetworkObject(segments, i, kind, changeType, safeValues);
        // ⛔ FALLS THROUGH ON null, same correction as the local-user branch
        // above. friendlyDescriptionForNetworkObject() reads the name from the
        // PATH, which the XML/API shape does not carry -- it returns null for
        // `address.entry[1716]` and this `return` then skipped every later
        // branch, so the 101 live address objects and every service object
        // rendered as a raw path. The builder is untouched; only its null now
        // reaches the named-entry fallback.
        if (objDescription !== null) return objDescription;
        break;
      }
    }
    ...
    // ⛔ LAST RESORT: AN INDEXED ENTRY THAT CARRIES ITS OWN NAME.
    // Every builder above resolves a name from the PATH, which works on the
    // CLI/text shapes where the name IS a path segment (`address.<name>.
    // ip-netmask`). PAN-OS's XML/API shape does not name anything in the path
    // — it is `address.entry[1716]`, an array index — and the name sits in
    // the VALUE as `@_name`. Measured on the live fleet: 101 address objects,
    // 65 security rules, 53 local users, 41 static routes and 23 IPsec
    // proxy-ids all arrive this way, and every one of them was rendering as a
    // raw config path with an index nobody can look up.
    //
    // ⛔ IT RUNS LAST, so it can never override a more specific description.
    const deviceConfig = friendlyDescriptionForDeviceConfig(segments, changeType, safeValues);
    if (deviceConfig !== null) return deviceConfig;

    const listMember = friendlyDescriptionForListMember(path, segments, changeType, safeValues);
    if (listMember !== null) return listMember;

    const namedEntry = friendlyDescriptionForIndexedNamedEntry(path, segments, changeType, safeValues);
    if (namedEntry !== null) return namedEntry;

    const parentField = friendlyDescriptionForIndexedParentField(path, segments, changeType, safeValues);
    if (parentField !== null) return parentField;

    // ⛔ LAST RESORT FOR THE CLI/BRACE TRANSPORT. Everything above reads a
    // name out of one path SEGMENT; this reads it out of the SPAN between a
    // known section prefix and a recognisable field suffix, which is the only
    // way a PAN-OS name containing dots or spaces can be recovered. It runs
    // here, after every specific builder, so it can never override a richer
    // description -- the one exception being the dotted/spaced names handled
    // at the TOP of this function, which no other builder can read correctly
    // at all.
    const treeNamed = friendlyDescriptionForTreePath(path, changeType, safeValues);
    if (treeNamed !== null) return treeNamed;
...
function friendlyDescriptionForRuleObject(rawRule, changeType) {
  ...
  try {
    ...
    const zoneWord = (list) => (list.length > 1 ? 'zones' : 'zone');
    // `disabled` (PAN-OS's own field, same 'yes'/absent convention
    // ruleFromBraceEntry() in lib/adapters/paloalto/sshParser.js already
    // reads for its own `enabled` flag: `!(asScalar(a.disabled) === 'yes')`)
    // -- surfaced in the COLLAPSED one-line summary, not just the expanded
    // RuleDetailGrid, so a newly-added-but-inactive rule doesn't read
    // identically to a live one when an operator is scanning the default,
    // unexpanded accordion list.
    const disabledPrefix = rawRule.disabled === 'yes' ? '(disabled) ' : '';
    let sentence = `${disabledPrefix}${action} rule from ${zoneWord(fromZones)} "${fromZones.join(', ')}" to ${zoneWord(toZones)} "${toZones.join(', ')}" was ${changeType === 'added' ? 'added' : 'removed'}`;
...
// ---------------------------------------------------------------------------
// Historical OBJECT-array shift-cascade collapse (companion to the
// primitive-array fix above)
// ---------------------------------------------------------------------------
//
// ⛔ THE GAP: collapsePrimitiveArrayShifts() above only reconstructs a
// primitive-array shift -- ARRAY_INDEX_PATH_RE requires the path to end BARE
// in `[N]` (the whole array element IS the diffed value), which is what a
// scalar list produces. It never retroactively applies the SAME-DAY identity-
// alignment fix (diffObjectArrayByIdentity, live for every NEW diff) to an
// already-stored row for an OBJECT array (a PAN-OS XML/API rulebase). A
// positional diff of an object array instead recurses field-by-field into
// each overlapping index, so a shift cascade there shows up as dozens of
// `...entry[N].<field>` MODIFIED rows -- including fields that can never
// legitimately change on a real edit (`@_name`, `@_uuid`) -- a shape
// ARRAY_INDEX_PATH_RE can't match at all (verified live against config_diffs
// row c3f88e40-baac-495d-9f33-55f68bcfb193, device 79111004: "41 added, 42
// removed, 146 modified", the modified paths textbook shift artifacts from
// entry[3] through entry[24]+).
//
// Full reconstruction of the true field-level diff for a shifted rule is NOT
// possible from the stored data alone: a positional diff of old[N] vs new[N]
// only ever recorded a field when it DIFFERED between those two (different!)
// rules, so a field that happened to be identical on both sides of that
// (wrong) pairing was never captured either way -- there is no complete old
// or new rule object to recover for an index inside the overlapping range.
// Fabricating one from partial data would risk inventing a WRONG modified/
// removed entry, worse than leaving the noise in place. This does two
// conservative, evidence-only cleanups instead of a full reconstruction:
//
//  1. Boundary whole-object reconciliation: when the array lengths differ,
//     the original positional fallback records the non-overlapping TAIL as
//     whole-object `added`/`removed` entries (the WHOLE rule, not a
//     per-field fragment -- full data IS available here, same as a primitive
//     array's tail). If an identity (@_name/name, same ARRAY_IDENTITY_KEYS
//     diffObjectArrayByIdentity() uses) shows up in BOTH a removed and an
//     added whole-object entry on the same array path, that rule never
//     actually left the ruleset -- it just moved past the overlap boundary.
//     Re-diff the two FULL objects directly (mirrors diffObjectArrayByIdentity's
//     own matched-pair recursion) and replace the false add+remove pair with
//     the true result (frequently nothing at all).
//  2. Identity-field noise removal: inside a detected cascade run (a
//     CONTIGUOUS run of >= CASCADE_MIN_MODIFIED touched indices on one array
//     path -- the object-array analogue of collapsePrimitiveArrayShifts'
//     own per-slot gate, adapted to "how many different indices show churn"
//     since an object-array cascade's signature is many DIFFERENT indices
//     each showing a handful of changed fields, not many changes piled on
//     one slot), a modified `@_name`/`@_uuid` entry is DEFINITELY an
//     artifact -- these are read as the element's identity precisely
//     BECAUSE they don't change on a real edit (see ARRAY_IDENTITY_KEYS'
//     own comment above) -- and is dropped outright. The one piece of this
//     noise that can be removed with total confidence and zero
//     reconstruction risk.
//
// Everything else inside the overlap zone (a genuinely-differing non-identity
// field at a cascade index, or any discontiguous mismatch) is left exactly as
// stored -- same "no ruleset is safer than the wrong one" honesty this file
// already applies elsewhere, rather than guessing at a pairing this data
// can't prove.
const IDENTITY_LEAF_FIELDS = new Set(['@_name', '@_uuid', 'name']);

// Matches a path ending `<arrayPath>[N].<field...>` (field may itself be a
// dotted sub-path, e.g. `to.member`) -- the per-field shape a positional
// object-array diff produces for an overlapping index. Deliberately separate
// from ARRAY_INDEX_PATH_RE (which only matches the BARE `[N]` ending, used
// for a whole-element entry) rather than making that regex's tail optional,
// so each call site stays explicit about which shape it expects.
const OBJECT_ARRAY_INDEX_FIELD_RE = /^(.*)\[(\d+)\]\.(.+)$/;

// Parses one diff entry's path into {arrayPath, index, field}, where `field`
// is the remainder after `[N].` or `null` for a whole-element entry (path
// ends bare at `[N]`). Returns null for a path with no array-index segment
// at all. Shared by both cleanup steps below so a whole-element entry and a
// per-field entry for the SAME index group under the SAME key.
function parseArrayElementPath(path) {
  const str = typeof path === 'string' ? path : '';
  const bare = ARRAY_INDEX_PATH_RE.exec(str);
  if (bare) return { arrayPath: bare[1], index: Number(bare[2]), field: null };
  const fielded = OBJECT_ARRAY_INDEX_FIELD_RE.exec(str);
  if (fielded) return { arrayPath: fielded[1], index: Number(fielded[2]), field: fielded[3] };
  return null;
}

// The element's identity string (from the first present ARRAY_IDENTITY_KEYS
// key), or null if `value` isn't a plain object or carries no usable scalar
// identity at any of those keys.
function wholeElementIdentity(value) {
...
/**
 * PURE. See the block comment above. Returns a new diff with (1) matched
 * whole-object add/remove boundary pairs replaced by their true re-diffed
 * result, and (2) artifact `@_name`/`@_uuid` modified entries inside a
 * detected cascade run dropped. Returns the diff unchanged (normalized
 * shape) when nothing qualifies. `vendor` is passed through to the re-diff
 * in step 1 so its volatile-subtree filtering still applies, same as any
 * other diffValue() call site.
 */
function collapseObjectArrayIdentityCascades(diff, vendor) {
  const added = Array.isArray(diff.added) ? diff.added : [];
  const removed = Array.isArray(diff.removed) ? diff.removed : [];
  const modified = Array.isArray(diff.modified) ? diff.modified : [];

  // --- Step 1: boundary whole-object reconciliation ------------------------
  const wholeRemovedByPath = new Map(); // arrayPath -> Map(identity -> entry)
  const wholeAddedByPath = new Map();
  const indexWholeEntry = (map, entry, arrayPath, identity) => {
    let g = map.get(arrayPath);
    if (!g) {
      g = new Map();
      map.set(arrayPath, g);
    }
    if (!g.has(identity)) g.set(identity, entry); // first one wins -- duplicates aren't expected
  };

  for (const e of removed) {
    const parsed = parseArrayElementPath(e.path);
    if (!parsed || parsed.field !== null) continue;
    const identity = wholeElementIdentity(e.value);
    if (identity !== null) indexWholeEntry(wholeRemovedByPath, e, parsed.arrayPath, identity);
  }
  for (const e of added) {
    const parsed = parseArrayElementPath(e.path);
    if (!parsed || parsed.field !== null) continue;
    const identity = wholeElementIdentity(e.value);
    if (identity !== null) indexWholeEntry(wholeAddedByPath, e, parsed.arrayPath, identity);
  }

  const reconciledRemoved = new Set();
  const reconciledAdded = new Set();
  const extraAdded = [];
  const extraRemoved = [];
  const extraModified = [];

  for (const [arrayPath, removedById] of wholeRemovedByPath) {
    const addedById = wholeAddedByPath.get(arrayPath);
    if (!addedById) continue;
    for (const [identity, removedEntry] of removedById) {
      const addedEntry = addedById.get(identity);
      if (!addedEntry) continue;
      const acc = { added: [], removed: [], modified: [] };
      diffValue(removedEntry.value, addedEntry.value, addedEntry.path, acc, vendor);
      extraAdded.push(...acc.added);
      extraRemoved.push(...acc.removed);
      extraModified.push(...acc.modified);
      reconciledRemoved.add(removedEntry);
      reconciledAdded.add(addedEntry);
    }
  }

  const removedAfterStep1 = removed.filter((e) => !reconciledRemoved.has(e));
  const addedAfterStep1 = added.filter((e) => !reconciledAdded.has(e));

  // --- Step 2: identity-field artifact removal inside a cascade run --------
  const touchedIndicesByArrayPath = new Map(); // arrayPath -> Set(index)
  for (const e of modified) {
    const parsed = parseArrayElementPath(e.path);
    if (!parsed || parsed.field === null) continue;
    let s = touchedIndicesByArrayPath.get(parsed.arrayPath);
    if (!s) {
      s = new Set();
      touchedIndicesByArrayPath.set(parsed.arrayPath, s);
    }
    s.add(parsed.index);
  }

  const cascadeIndicesByArrayPath = new Map(); // arrayPath -> Set(index) inside a qualifying contiguous run
  for (const [arrayPath, touched] of touchedIndicesByArrayPath) {
    const indices = [...touched].sort((a, b) => a - b);
    const keep = new Set();
    let runStart = 0;
    for (let i = 1; i <= indices.length; i++) {
      const brokeRun = i === indices.length || indices[i] !== indices[i - 1] + 1;
      if (brokeRun) {
        if (i - runStart >= CASCADE_MIN_MODIFIED) {
          for (let k = runStart; k < i; k++) keep.add(indices[k]);
        }
        runStart = i;
      }
    }
    if (keep.size > 0) cascadeIndicesByArrayPath.set(arrayPath, keep);
  }

  const isArtifactIdentityEntry = (e) => {
    const parsed = parseArrayElementPath(e.path);
    if (!parsed || parsed.field === null || !IDENTITY_LEAF_FIELDS.has(parsed.field)) return false;
    const cascadeIndices = cascadeIndicesByArrayPath.get(parsed.arrayPath);
    return cascadeIndices ? cascadeIndices.has(parsed.index) : false;
  };

  const modifiedAfterStep2 = modified.filter((e) => !isArtifactIdentityEntry(e));

  return {
    added: [...addedAfterStep1, ...extraAdded],
    removed: [...removedAfterStep1, ...extraRemoved],
    modified: [...modifiedAfterStep2, ...extraModified],
  };
}

/**
 * One-time (safely re-runnable) retroactive fix: collapses primitive-array
 * shift cascades (collapsePrimitiveArrayShifts()) AND object-array identity
 * cascades (collapseObjectArrayIdentityCascades(), see its own header
 * comment above for why it's a separate, more conservative pass) in EXISTING
 * config_diffs rows, and re-derives change_summary from the result, so the
 * row header, summary text, and expanded detail all agree. Idempotent -- a
 * row with no cascade of either shape (including one already collapsed) is
 * left untouched. Joins `devices` for `vendor` (same pattern as
 * cleanupVolatileConfigDiffs()) so the object-array pass's boundary re-diff
 * still applies the correct per-vendor volatile-subtree filtering. Best-
 * effort/non-fatal, same shape as cleanupVolatileConfigDiffs()/
...
async function collapseHistoricalArrayShiftCascades(pool) {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      collapsed = collapseObjectArrayIdentityCascades(collapsed, row.vendor);
    } catch (_err) {
...
/**
 * Regenerate EVERY config_diffs.change_summary through the current
 * summarizeDiff().
 *
 * ⛔ A COSMETIC BACKFILL, WHICH IS WHY IT REWRITES ONLY THE SUMMARY. The
 * `diff` payload is never touched, so there is no way for this to lose a
 * recorded change -- the worst outcome is a row keeping the summary it already
 * had. Sibling repairs in this file rewrite diffs and carry far more care for
 * exactly that reason.
 *
 * It exists because the summary is STORED at write time. Improving
 * summarizeDiff() alone would leave every historical row reading
 * `12 added — e.g. devices.entry.vsys.entry.tag.entry[17], ...` for ever,
 * which is the whole complaint.
 *
 * ⛔ SKIPS A ROW WHOSE SUMMARY WOULD NOT CHANGE, so a re-run costs nothing and
 * the reported count means "rows actually improved" rather than "rows seen".
 */
async function regenerateAllChangeSummaries(pool) {
  const { rows } = await pool.query(
    'SELECT id, diff, change_summary FROM config_diffs WHERE diff IS NOT NULL'
  );
  let checked = 0;
  let updated = 0;
  let failed = 0;
  for (const row of rows) {
    ...
    let next;
    try {
      next = summarizeDiff(row.diff || {});
    } catch (_err) {
      // ⛔ One unsummarisable row must not abandon the rest, and must not
      // write a broken summary over a working one.
      failed += 1;
      continue;
    }
    if (!next || next === row.change_summary) continue;
    try {
      await pool.query('UPDATE config_diffs SET change_summary = $1 WHERE id = $2', [next, row.id]);
      updated += 1;
    } catch (_err) {
      failed += 1;
    }
...
module.exports = {
  ...
  collapseObjectArrayIdentityCascades,
