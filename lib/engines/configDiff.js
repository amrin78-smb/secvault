...
const SECTION_LABEL_ACRONYMS = new Set([
  'NAT', 'VPN', 'IPSEC', 'IKE', 'DNS', 'NTP', 'SSL', 'SSH', 'HA', 'PBF', 'URL', 'IPS', 'SNMP',
]);

function sectionNoun(label, count) {
  const other = /^Other \((.+)\)$/.exec(String(label || ''));
  let base = other ? other[1] : String(label || '').trim();
  if (!base) return null;
  // Strip a trailing plural the heading already carries, then re-apply it from
  // the count — otherwise `1 Address Objects`.
  base = base.replace(/s$/i, (m, off, str) => (/ss$/i.test(str) ? m : ''));
  // ⛔ AN ACRONYM IS NOT A WORD TO LOWERCASE. Blanket-lowercasing the label
  // turned "NAT Rules" into "2 nat rules", which reads as a typo on a page an
  // operator is judging the product by. Only the acronyms this product's own
  // section labels actually use are listed; anything else is an ordinary word.
  const noun = base
    .split(/(\s+)/)
    .map((w) => (SECTION_LABEL_ACRONYMS.has(w.toUpperCase()) ? w.toUpperCase() : w.toLowerCase()))
    .join('');
  if (count === 1) return `1 ${noun}`;
  return `${count} ${noun}${/(s|x|ch|sh)$/i.test(noun) ? 'es' : 's'}`;
}
...
// rule's own table.
const FIELD_AMBIGUOUS_SECTION_KEYS = new Set(['tag', 'profiles']);

function sectionLabelFor(path) {
  const segments = path.split('.').map(stripIndex).filter(Boolean);
  const underRulebase = segments.some((x) => FALLBACK_RULEBASE_LABELS.has(String(x).toLowerCase()));
  for (let i = 0; i < segments.length; i++) {
    const lower = segments[i].toLowerCase();
    if (FIELD_AMBIGUOUS_SECTION_KEYS.has(lower) && underRulebase && i === segments.length - 1) continue;
...
function friendlyDescriptionForIndexedNamedEntry(path, segments, changeType, values) {
  ...
  const noun = INDEXED_ENTRY_NOUNS.get(container)
    || (container
 ...
 */
function friendlyDescriptionForDeviceConfig(segments, changeType) {
  ...
  const words = tail.map((t) => humanizeFieldForSentence(t)).join(' ');
  return `${words.replace(/^./, (c) => c.toUpperCase())} ${verb}`;
}
...
function computeFriendlyDescription(path, changeType, values) {
  try {
    ...
    const deviceConfig = friendlyDescriptionForDeviceConfig(segments, changeType);
