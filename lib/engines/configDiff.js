 ...
 * Never throws -- a redaction error fails an entry CLOSED (a placeholder
 * string), never leaves the original value in place, matching the
 * fail-closed posture of every other redactor in this codebase.
 */
function redactSecretEntries(entries) {
  return (entries || []).map((e) => {
    try {
      if (!isSecretPath(e.path)) return e;
      const redacted = { ...e };
