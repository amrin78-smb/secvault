...
function resolveDevice(device, index, now = new Date()) {
  ...
  if (row.noDatePublished === true) {
    return {
      ...
      reason: `${row.vendor} has published no end-of-support date for ${model}`
        + (row.checkedAt ? ` as of ${String(row.checkedAt).slice(0, 10)}.` : '.'),
    };
  }
  ...
  return {
    ...
    reason: daysRemaining <= 0
      ? `${model} passed end-of-support on ${row.supportEndDate}.`
      : `${model} reaches end-of-support on ${row.supportEndDate} (${daysRemaining} days).`,
  };
}
