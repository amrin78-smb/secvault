 ...
 * @returns {'ok'|'running'|'degraded'|'error'|'missing'|'skipped'|'blocked'}
 */
function feedState(row, nowMs) {
  ...
  // ⛔ BLOCKED: the feed RAN and the publisher refused it. Checked here, before
  // the rank cascade, for the same reason as a skip -- it is not a verdict about
  // the advisory data. ⛔ It is NOT silent: it is named in every pill title and
  // rendered distinctly, because a feed that simply stops appearing is
  // indistinguishable from one that broke.
  // ⛔ THE LIMIT OF THIS CALL, stated so it can be re-examined: excluding it
  // from the verdict is only right while ANOTHER feed covers the same vendor.
  // Measured 2026-09-20 -- Fortinet sits at 282/287 advisories (98%) with usable
  // ranges via NVD and the central hub, and all 10 FortiOS CVEs NVD published in
  // the last 120 days are held and matched. If FortiGuard ever became the only
  // source for a vendor, a block WOULD be a coverage loss and green would be the
  // wrong colour.
...
// 'blocked' joins 'skipped' and 'missing' outside this table on purpose — see
// severityOf(), which ranks anything unlisted as WORSE than error, and the
// `rated` filter below, which removes all three before the reduction.
...
async function getSyncPillStatus(pool, { now = Date.now() } = {}) {
  ...
  const rated = feeds.filter(
    (f) => f.state !== 'missing' && f.state !== 'skipped' && f.state !== 'blocked'
  );
  const skippedFeeds = feeds.filter((f) => f.state === 'skipped');
  const blockedFeeds = feeds.filter((f) => f.state === 'blocked');
  ...
  if (rated.length === 0) {
    ...
    if (blockedFeeds.length > 0 && skippedFeeds.length === 0) {
      return {
        state: 'blocked',
        ok: false,
        label: 'FEEDS BLOCKED',
        title:
          `Every advisory feed was refused by its publisher on its most recent cycle (${blockedFeeds
            .map((f) => f.label)
            .join(', ')}), so nothing collected advisories. Each ran and was turned away — `
          + 'this is not a fault in SecVault, and it is not something it can retry its way out of.',
        feeds,
        lastSyncs,
      };
    }
    ...
    title:
      ...
      + (skippedFeeds.length > 0
        ? ` Deliberately skipped this cycle, with a recorded reason: ${skippedFeeds
          ...
          .join(', ')}.`
        : '')
      // ⛔ NAMED, ALWAYS. A blocked feed is excluded from the VERDICT, never
      // from the evidence — a green pill whose own explanation quietly omits a
      // feed invites the reader to assume the worst about the one that is absent.
      + (blockedFeeds.length > 0
        ? ` Refused by the publisher and contributing nothing this cycle: ${blockedFeeds
          .map((f) => f.label)
          .join(', ')}. Their advisories are supplied by the other feeds.`
        : ''),
    feeds,
    lastSyncs,
  };
