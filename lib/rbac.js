...
// appears. Eight is a deliberate ceiling, not an accident.
...
const ROLE_CAPABILITIES = {
  ...
  [ADMIN_ROLE]: [
    // ⛔ No MANAGE_USERS, no MANAGE_CREDENTIAL_PROFILES. Everything else.
    MANAGE_DEVICES,
    MANAGE_SETTINGS,
    RUN_UPDATE,
    OPERATE,
...
function capabilitiesOf(session) {
  const role = roleOf(session);
  const granted = (role && ROLE_CAPABILITIES[role]) || [];
  const out = {};
  for (const cap of ALL_CAPABILITIES) out[cap] = granted.includes(cap);
  return out;
}
