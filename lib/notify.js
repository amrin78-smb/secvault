...
async function sendEmail(channel, message) {
  ...
  await transporter.sendMail({
    ...
    // Optional (added 2026-08-02 for the compliance PDF report) -- nodemailer's
    // native attachments shape, passed straight through. Only meaningful for
    // this channel type; webhook senders below never read message.attachments
    // at all (a Slack/Teams/generic-webhook payload has no attachment concept).
    attachments: Array.isArray(message.attachments) ? message.attachments : undefined,
  });
}

// Single entry point — dispatches `message` ({ alertType, title, summary, url,
// deviceName, attachments? }) through `channel` ({ channelType, plaintext,
// config, ... }, an already-decrypted object from lib/notificationChannels.js).
// `attachments` (nodemailer shape: [{filename, content: Buffer, contentType}])
// is optional and only consumed by the email sender above. Throws on any
