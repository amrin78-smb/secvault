...
function buildApplicationsAnswer(result, pageError) {
  ...
  // ⛔ "THE FLOWS COULD NOT BE READ" AND "THERE ARE NO FLOWS" ARE DIFFERENT
  // FACTS, and only one of them is something the operator did. The engine
  // reports a failed `application_flows` read in `errors` and CARRIES ON with
  // an empty flow list, so without this branch the sentence below fired and
  // announced "none of them has a flow yet" — a measurement stated about a read
  // that never returned. `rollUp` has computed `flowsFailed` since the first
  // commit precisely for this, and nothing consumed it.
  if (r.flowsFailed) {
    return {
      lead: null,
      tone: 'unknown',
      sentence:
        `${r.apps} declared ${plural(r.apps, 'application is', 'applications are')} listed below, but `
        + 'their declared flows could not be read, so none of them has been tested against the rulebase.',
      coverage: 'This is a failure to read the flows, not an absence of them — nothing here says '
        + 'that no flow is declared.',
    };
  }
  if (r.apps === 0) {
    return {
