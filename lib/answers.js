...
function buildFleetAnswer(headline) {
  ...
  if (patchNow !== null && patchNow > 0) {
    return {
      lead: `${patchNow} ${plural(patchNow, 'vulnerability', 'vulnerabilities')}`,
      sentence: `need${patchNow === 1 ? 's' : ''} patching now — known-exploited, observed-reachable, or critical on an affected version.`,
      tone: 'critical',
      coverage,
    };
  }
