...
function buildSegmentationAnswer(result) {
  ...
  if (num(s.unmeasurable) > 0) {
    bits.push(num(s.unmeasurable) + ' of ' + total + ' '
      + plural(num(s.unmeasurable), 'path') + ' could not be measured');
  }
  if (num(result.rulesWithoutHitData) > 0) {
    bits.push(Number(result.rulesWithoutHitData).toLocaleString()
      + ' rules cannot report usage at all');
  }
  const coverage = joinClauses(bits);
  ...
  if (num(s.violations) > 0) {
    ...
    return {
      lead: n + ' segmentation ' + plural(n, 'violation'),
      sentence: plural(n, 'is', 'are') + ' permitted by a rule, with no traffic observed — standing holes rather than active breaches.',
      tone: 'critical',
      coverage,
    };
  }
...
function buildWorkQueueAnswer(summary) {
  const s = summary || {};
  const act = num(s.act_now) || 0;
  const scheduled = num(s.scheduled) || 0;
  const verify = num(s.verify) || 0;
