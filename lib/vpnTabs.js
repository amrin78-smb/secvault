...
const FLEET_VPN_TABS = [
  ...
  },
  {
    ...
    identity: true,
    label: 'User Activity',
    // ⛔ Measures HOURS IN WHICH A USER AUTHENTICATED, not connected time.
  ...
  },
  {
    ...
    identity: true,
    label: 'Detections',
    description: 'Named VPN threat detections with the evidence behind each',
  },
  {
    key: 'traffic',
    identity: true,
    label: 'User Traffic',
    description: 'Traffic attributed to named VPN users via their assigned address',
  },
  {
    key: 'tunnels',
    label: 'Tunnel Health',
    description: 'Site-to-site IPsec tunnel status, coverage and peering',
  },
];
...
/**
 * Tabs a session may see.
 *
 * ⛔ `identity: true` marks a tab that NAMES INDIVIDUAL PEOPLE — who logged
 * in, from where, at what hour, and what their traffic did. That is a
 * different kind of data from a session COUNT, and the Operator role does not
 * include it. Fleet Status, Log Activity and Tunnel Health carry no personal
 * identity and stay visible to everyone.
 *
 * ⛔ Filtering this list hides the tab; it does NOT stop the data being
 * rendered. The page must ALSO refuse to render an identity tab body — see
 * app/(dashboard)/vpn/page.js. A hidden tab whose URL still works is not a
 * permission boundary, it is a decoration.
 */
function visibleVpnTabs(tabs, canViewIdentity) {
  if (canViewIdentity) return tabs;
  return tabs.filter((t) => !t.identity);
}
