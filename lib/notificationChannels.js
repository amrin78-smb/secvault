...
// 'compliance_report' (added 2026-08-02) is structurally different from the
// other three -- it's schedule-driven (monthly), not event-driven, and only
// ever dispatched to channel_type='email' (a PDF attachment has no
// Slack/Teams/generic-webhook analog) -- see lib/engines/complianceReport.js
// and services/engine-worker.js's runComplianceReportJob(). The UI
// (components/settings/NotificationsPanel.js) gates that checkbox to email
// channels only for exactly this reason -- a Slack channel opted into it
// would be silently skipped by the job forever, with no error surfaced
// anywhere. Kept in this same array anyway (not a separate concept) because
// routing is still "which channel(s) get this class of thing", identical
// in shape to the other three.
// ⛔ THIS LIST IS THE GATE, AND IT SILENTLY OMITTED ingest_drop. The engine
// implemented that alert in full and the Settings UI offered the checkbox, but
// POST/PUT /api/notification-channels validate against THIS array -- so saving a
// channel subscribed to it returned 400 and the alert could never be delivered
// by anyone. Fully built, shown to the operator, structurally undeliverable.
//
// ⛔ THREE LISTS DEFINE THE ALERT VOCABULARY and nothing kept them in step:
// this one (what may be persisted), notificationDispatch.ALERT_TYPES (what the
// job dispatches) and NotificationsPanel.ALERT_TYPES (what is offered). They
// disagreed in both directions -- ingest_drop missing here, compliance_report
// missing there. tests/alertTypeRegistry.test.js now pins the relationship, so
// the next alert type cannot half-ship the same way.
const ALERT_TYPES = [
