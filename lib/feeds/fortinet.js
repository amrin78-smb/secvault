...
function parseAffectedEntry(str) {
  ...
  // Pattern 3: a bare exact version with no range operator and no "all versions" (seen in
  // known_not_affected, e.g. "FortiOS-7.6.4") — a single fixed release, not a range.
  const bareMatch = remainder.match(/^([\d.]+)\s*$/);
  if (bareMatch) {
    return { fixed: bareMatch[1] };
  }
...
function mergeVersionDataFromEntry(vulnEntry, acc) {
  ...
  for (const s of ps.known_affected || []) {
    const parsed = parseAffectedEntry(s);
    if (!parsed) continue;
...
function buildRecordsFromCsaf(csafJson, fgIrId) {
  ...
  const groups = new Map(); // cveId -> { ranges: [], fixedVersions: Set, cvss: {score,vector}, summary }
  ...
  for (const vulnEntry of (csafJson && csafJson.vulnerabilities) || []) {
    ...
    if (!group) {
      group = { ranges: [], fixedVersions: new Set(), cvss: { score: null, vector: null }, summary: null };
