...
// ⛔ TWO RSS SOURCES, filestore FIRST. These are the SAME DOCUMENT — verified
// live 2026-09-10, byte-identical at 38,086 bytes, same 50 items — because
// www.fortiguard.com/rss/ir.xml 302-redirects to the filestore copy and
// node-fetch follows it. So this is not a new data source; it is one less hop
// through the host that is currently the problem.
//
// The ordering is the whole point: every advisory PAGE on fortiguard.com is
// answering with an altcha bot-protection interstitial, and the RSS is the ONLY
// surviving Fortinet channel. Depending on that same host to serve the redirect
// puts the last working input behind the thing that broke the others.
// filestore.fortinet.com is a static file host and is NOT challenged.
//
// ⛔ This does NOT fix the real gap. The RSS yields a CVE id for only 5 of 50
// items and no affected-version data at all, and its <link> still points at the
// challenged host — so the 45 real FortiOS advisories remain unreachable. Do not
// let this change read as a fix for that.
const FORTIGUARD_RSS_URLS = [
...
/**
 * Fetch the PSIRT RSS, trying each source in order.
 *
 * ⛔ A source counts as FAILED if it throws OR if it comes back challenged or
 * unparseable — not merely on a network error. A bot challenge is served as a
 * perfectly good HTTP 200, which is exactly how this feed spent three weeks
 * reporting the wrong reason for its own failure.
 *
 * ⛔ If EVERY source fails, this THROWS with all of their reasons. It must never
 * return an empty item list: `[]` would mean "Fortinet published no advisories",
 * and the caller would dutifully report a clean run. Same rule as the adapter
 * contract's getRules().
 */
async function fetchRssItems() {
  const failures = [];
  for (const url of FORTIGUARD_RSS_URLS) {
    let xml;
    try {
    ...
    // Classify before parsing — the rule this whole file was rewritten around.
    const challenge = detectBotChallenge(xml);
    if (challenge) {
      // ⛔ detectBotChallenge() returns {markers, screenId}, NOT a string. Interpolating
      // it produced "[object Object]" — in the ONE message that is the sole evidence for
      // a run that got no input at all. classifyAdvisoryPage() unpacks it correctly;
      // this call site did not. Reporting nothing about a total outage is precisely the
      // failure this file was rewritten to stop.
      const markers = Array.isArray(challenge.markers) ? challenge.markers.join(', ') : '';
      const screen = challenge.screenId ? `; Screen ID ${challenge.screenId}` : '';
      failures.push(
        `${url}: served a BOT-PROTECTION INTERSTITIAL instead of the RSS`
        + `${markers ? ` (markers: ${markers})` : ''}${screen}`
      );
      continue;
    }
    let items;
    try {
      ...
      failures.push(`${url}: RSS did not parse as XML (${err.message})`);
      continue;
    }
    if (!items.length) {
      failures.push(`${url}: parsed but contained no <item> elements`);
      continue;
    }
    return items;
  }
  throw new Error(
    `every FortiGuard RSS source failed — ${failures.join(`; `)}`
  );
}
...
async function upsertAdvisory(pool, rec) {
    ...
    `INSERT INTO advisories (
     ...
     ON CONFLICT (cve_id) DO UPDATE SET
       ...
       affected_version_ranges = CASE
                    ...
                    -- ⛔ NEVER TRADE A FIX BOUNDARY FOR NONE. This feed runs
                    -- NINE SECONDS AFTER cve_hub and, when the CSAF resolves,
                    -- takes the ELSE branch below and overwrites whatever the
                    -- hub just repaired. Measured 2026-09-25: the hub repaired
                    -- 80 rows at 12:56:27 and not one survived the cycle.
                    --
                    -- The vendor is normally the better source and overwriting
                    -- is normally right -- but FortiGuard's CSAF is PROSE, and
                    -- parsing "7.4.0 through 7.4.10" yields an inclusive bound
                    -- with no fix version, while NVD's structured
                    -- versionEndExcluding gives 7.4.11, which IS the first
                    -- fixed release. On CVE-2026-24858 (KEV, CVSS 9.8, live on
                    -- three FortiGates) that difference is the whole
                    -- remediation target.
                    --
                    -- ⛔ ONE DIRECTION ONLY, and this is not a preference for
                    -- the hub: an incoming row that DOES carry a boundary still
                    -- wins, so the vendor keeps authority wherever it actually
                    -- has the better data. Only the strict downgrade is refused.
                    WHEN advisories.affected_version_ranges @> '[{"exclude_fixed": true}]'::jsonb
                         AND NOT (EXCLUDED.affected_version_ranges @> '[{"exclude_fixed": true}]'::jsonb)
                      THEN advisories.affected_version_ranges
  ...
  // ⛔ ZERO ROWS USED TO MEAN TWO DIFFERENT THINGS AND REPORTED ONE. The
  // ON CONFLICT ... WHERE declines the update either because ANOTHER VENDOR OWNS
  // this cve_id -- a Fortinet advisory permanently lost, because cve_id is UNIQUE
  // with one vendor -- or because this degraded record simply had no gap to fill,
  // which is routine. Both were returned as 'unchanged', so the loss was
  // invisible: the one case worth knowing about was filed under the one that
  // happens constantly.
  if (result.rows.length === 0) {
    const owner = await pool.query(
      'SELECT vendor FROM advisories WHERE cve_id = $1',
      [rec.cve_id]
    );
    const heldBy = owner.rows[0] && owner.rows[0].vendor;
    if (heldBy && heldBy !== rec.vendor) return { outcome: 'claimed', heldBy };
    return { outcome: 'unchanged' };
  }
  return { outcome: result.rows[0].inserted === true ? 'inserted' : 'updated' };
}
...
// ────────────────────────────────────────────────────────────────────────
/**
 * Is this run's ONLY problem that the publisher refused to serve us?
 *
 * ⛔ A PERMANENT AMBER CHIP FOR AN EXTERNAL, UNFIXABLE CONDITION TEACHES AN
 * OPERATOR TO IGNORE THE CHIP THAT MATTERS. FortiGuard answers every advisory
 * page with a Cloudflare JavaScript interstitial -- measured byte-identical
 * (19,751 bytes) from the SecVault box, an office connection and a Netlify
 * function, so it is not IP reputation and relocating the fetch cannot fix it.
 * The run therefore reported `partial` every six hours, for ever, for a system
 * behaving exactly as designed. That is the same trap the local NVD sync was in
 * before it started reporting `skipped` with a reason.
 *
 * ⛔ BUT IT IS NOT A SKIP, AND MUST NOT BORROW THAT WORD. `skipped` means
 * SecVault decided not to run the feed. This feed RAN: it fetched the RSS,
 * asked for every advisory, and was refused. Calling that "skipped" would be
 * this codebase's own failed-read-as-a-fact rule wearing a politer label.
 * `blocked` is its own status.
 *
 * ⛔ NARROW ON PURPOSE. It is true ONLY when every advisory-page failure was
 * the bot challenge, NOTHING was resolved, and no upsert failed. A run that got
 * even one advisory did useful work and stays `partial`; a run that also failed
 * to write what it did get has a real problem that must not be hidden behind
 * the publisher's. It is therefore SELF-CLEARING: the first page that resolves,
 * or the first upsert error, takes the feed straight back out of this state.
 */
function isPublisherBlocked(summary) {
  const reasons = Object.keys(summary.pageFailureReasons || {});
  if (reasons.length === 0) return false;
  // ⛔ PAGE_PROBE_SUSPENDED IS PART OF THE BLOCK, NOT A SECOND FAULT. The
  // circuit breaker stops probing after 3 consecutive challenges and files the
  // REMAINING items under its own reason -- so the live shape is
  // {bot_challenge: 3, page_probe_suspended: 47}, never bot_challenge alone.
  // The first version of this demanded a single reason and therefore NEVER
  // FIRED in production; the unit test agreed with it because the mixed reason
  // it invented (page_empty) was a genuinely different failure. Suspension is
  // the breaker doing exactly what the challenge built it to do.
  const ALLOWED = [REASON.BOT_CHALLENGE, REASON.PAGE_PROBE_SUSPENDED];
  if (!reasons.every((r) => ALLOWED.includes(r))) return false;
  // ⛔ ...but suspension ALONE is not evidence of a block. At least one real
  // challenge must have been observed, or an unrelated future cause of
  // suspension would silently inherit this state.
  if (!(summary.pageFailureReasons[REASON.BOT_CHALLENGE] > 0)) return false;
  if ((summary.resolvedFromPage || 0) > 0) return false;
  if ((summary.upsertErrors || []).length > 0) return false;
  // The RSS is a separate host and is NOT challenged. If even that failed this
  // is an ordinary outage, not a publisher refusing a crawler.
  return (summary.rssItemCount || 0) > 0;
}
...
async function fetchAndUpsertFortinetAdvisories(pool) {
  ...
  for (const item of rssItems) {
    ...
    for (const rec of records) {
      try {
        ...
        else if (res.outcome === 'claimed') {
          // A Fortinet advisory that cannot be stored because another vendor
          // reached this cve_id first. Counted, never silent.
          claimedByOtherVendor.push({ cve_id: rec.cve_id, wanted: rec.vendor, held_by: res.heldBy });
        } else unchanged += 1;
      } catch (e) {
  ...
  const errors = buildRunErrors(runSummary);
  // The detail stays in `errors` either way -- only the STATUS changes, so
  // nothing is hidden, it just stops being coloured as a fault of ours.
  const publisherBlocked = isPublisherBlocked(runSummary);
...
module.exports = {
  ...
  buildRunErrors,
  // Exported for tests/feedBlocked.test.js -- pure, takes a run summary.
  isPublisherBlocked,
  upsertAdvisory,
  extractFgIrId,
