...
// No previous timeout was set here — node-fetch@2 defaults to NO timeout at all, so a
// request that never gets a response (e.g. a firewall silently dropping packets instead
// of actively refusing the connection) hangs indefinitely rather than failing. Because
// runFullSync (lib/feeds/index.js) awaits the NVD sync THEN this KEV sync sequentially,
// an indefinitely-hanging KEV request stalls the entire feed-sync cycle forever — same
// failure mode as nvd.js's FETCH_TIMEOUT_MS (see that file's comment above fetchPage()),
// just previously missed in this sibling file. Same value, same node-fetch@2 `timeout`
// option (socket-inactivity timeout in ms, at connect OR during the response).
...
async function syncKev(pool) {
  ...
  let data;
  try {
    const res = await fetch(KEV_URL, { timeout: FETCH_TIMEOUT_MS });
    if (!res.ok) {
