    ...
    if (logId) {
      ...
      const errorsForLog = [
        ...(result.errors || []),
        {
          ...
          // ⛔ ADVISORIES THIS FEED COULD NOT STORE because another vendor
          // already owns that cve_id (advisories.cve_id is UNIQUE with ONE
          // vendor, so the first feed to ingest it owns it permanently). It was
          // previously indistinguishable from an uneventful update. Recorded
          // here rather than as an error: nothing is broken, a fact is being
          // lost, and the size of that loss is what decides whether the
          // identity should become (cve_id, vendor).
          claimed_by_other_vendor: (result.claimedByOtherVendor || []).length,
          claimed_examples: (result.claimedByOtherVendor || []).slice(0, 10),
        },
      ];
...
async function runFortinetPsirtSync(pool) {
  ...
  try {
    ...
    // ⛔ `blocked` IS ITS OWN STATUS, NOT A DRESSED-UP `partial` OR `skipped`.
    // The publisher answers every advisory page with a bot interstitial, which
    // no change on our side can fix -- so reporting `partial` every six hours
    // for ever was a permanent amber chip for correct behaviour. It is NOT
    // `skipped` either: that word means SecVault decided not to run the feed,
    // and this one ran and was refused. The errors payload is UNCHANGED, so
    // every detail stays readable; only the colour of the verdict moves.
    const status = result.publisherBlocked
      ? 'blocked'
      : (result.errors && result.errors.length > 0 ? 'partial' : 'success');
    if (logId) {
      // ⛔ Fortinet is the feed most exposed to this: it runs LAST of the three
      // discovery feeds and republishes third-party CVEs (OpenSSL and friends),
      // so it is the one that loses a cve_id another vendor already claimed.
      // Informational, not an error -- see the note in runNvdSync.
      const claimed = result.claimedByOtherVendor || [];
      const fortinetErrorsForLog = claimed.length === 0 ? result.errors : [
        ...(result.errors || []),
        {
          cve_id: null,
          message: 'advisories not stored: another vendor already owns the cve_id (informational, not an error)',
          claimed_by_other_vendor: claimed.length,
          claimed_examples: claimed.slice(0, 10),
        },
      ];
      await logSyncFinish(pool, logId, {
        status,
...
async function runFullSync(pool) {
  ...
  // ⛔ "THE FEED ARRIVED" IS NOT "THE FEED WAS APPLIED". The first version
  // required only a verified, non-empty feed and no errors — so a hub that
  // renamed a field and shipped 900 advisories this consumer could not read
  // would produce all-zero stats, an empty errors array, and hubDelivered TRUE.
  // NVD would be skipped every six hours for ever with a reason claiming the
  // hub supplied the corpus, CIRCL would never run, nothing would be ingested,
  // and checked_at would keep advancing because the HUB is healthy — so the
  // freshness check cannot catch it either. Green over nothing, which is the
  // precise failure this whole feed exists to prevent.
  //
  // ⛔ An ABSENT errors array is unknown, not clean: Array.isArray, not falsy.
  const hubApplied = cve_hub
    && Number(cve_hub.inserted || 0) + Number(cve_hub.repaired || 0)
       + Number(cve_hub.unchanged || 0) > 0;
  const hubDelivered = !!(
    cve_hub
    && !cve_hub.notRun
    && Array.isArray(cve_hub.errors) && cve_hub.errors.length === 0
    && Number(cve_hub.feed_rows) > 0
    && hubApplied
    // ⛔ And nothing in the feed was unreadable. One unusable row is a schema
    // drift warning; skipping local discovery on the strength of a feed we only
    // partly understood is not a trade worth making.
    && Number(cve_hub.skippedUnusable || 0) === 0
  );
...
async function getFeedStatusBySource(pool) {
  ...
  // ⛔ SEEDED FROM feedNames, NOT A HAND-WRITTEN LITERAL. The literal had
  // drifted: it omitted cve_hub (which now runs FIRST) and cloud_apps, so those
  // keys were `undefined` rather than `null` before their first run —
  // contradicting this function's own documented contract, and dropped entirely
  // by JSON.stringify, so any consumer enumerating the response lost them.
  const bySource = Object.fromEntries(feedNames.map((n) => [n, null]));
