...
const { syncEolFeed } = require('./eolFeed');
const {
...
/**
 * Pull the central hardware-EOL catalogue.
 *
 * ⛔ LOGGED AS A FEED ON PURPOSE. Hub reachability then appears in the same
 * dashboard strip as every other source and inherits syncBadgeColor — including
 * the skipped-renders-muted rule from v2.130.0 — with no new UI concept and no
 * second place to look.
 *
 * ⛔ BUT THIS MONITORS THE CUSTOMER'S VIEW, NOT OURS. A green row here means the
 * hub ANSWERED; it says nothing about whether the hub is still PUBLISHING. A hub
 * serving a three-month-old feed is up and useless, which is why
 * eolFeed.catalogueFreshness() exists as a separate signal and why a failed
 * publish is invisible to every customer until their own copy ages out. The hub
 * needs its own alerting; that is a nocvault-eol task.
 *
 * ⛔ NEVER FATAL. A catalogue nobody could fetch must not stop the CVE feeds
 * that run beside it — hardware lifecycle is useful, vulnerability discovery is
 * the product's main job.
 */
async function runEolFeedSync(pool, opts = {}) {
  const startedAt = Date.now();
  let logId = null;
  try {
    logId = await logSyncStart(pool, 'eol_catalogue');
    const result = await syncEolFeed(pool, opts);
    // ⛔ 'partial' when the feed landed but carried a warning (a normalizer
    // version this build does not implement, say) — that is a catalogue with a
    // hole in it, and the status is the only place the hole is visible.
    let status = 'failed';
    if (result.ok) status = (result.errors && result.errors.length > 0) ? 'partial' : 'success';
    if (logId) {
      await logSyncFinish(pool, logId, {
        status,
    ...
    return { ok: false, inserted: 0, updated: 0, errors: [{ message: err.message }] };
  }
}
...
async function runFullSync(pool) {
  ...
  // ⛔ LAST, with the cloud catalogue, and for the same reason: neither is a CVE
  // feed, and a slow third-party publisher must never delay advisory discovery.
  let eol_catalogue;
  try {
    eol_catalogue = await runEolFeedSync(pool);
  } catch (err) {
    eol_catalogue = { ok: false, inserted: 0, updated: 0, errors: [{ cve_id: null, message: err.message }] };
  }
  ...
  // ⛔ eol_catalogue IS RETURNED, not merely computed. A source that runs and is
  // then left out of the summary is invisible in the engine log — it would look
  // exactly like a source that never ran, which is the same failure the
  // vendor-PSIRT gate writes a `skipped` row to avoid.
  return { nvd, paloalto_psirt, fortinet_psirt, kev, cveorg, epss, cloud_apps, eol_catalogue };
}
...
async function getFeedStatusBySource(pool) {
  const feedNames = ['nvd', 'paloalto_psirt', 'fortinet_psirt', 'kev', 'cveorg', 'epss', 'cloud_apps'];
  const result = await pool.query(
