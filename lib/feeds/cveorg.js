...
// ⛔ RE-POINTED 2026-09-19 — THE OLD PREDICATE COULD NOT FIRE, AND HAD NOT
// SINCE v2.104.0. It asked for `cvss_score = 0 AND cvss_source IS NULL`, but the
// v2.104.0 writer fix stamped `cvss_source` on every row this feed produces, so
// every zero-score row now carries 'psirt'. Measured live 2026-09-19: the old
// predicate returns **0 rows and always will**; this file's own comment above
// promises "COUNTED AND REPORTED ... never skip silently", and it had been
// reporting an empty list for a fortnight. A guard that cannot fire is worse
// than no guard -- the code reads as handled and the summary reads as clean.
//
// ⛔ AND THE QUESTION IT ASKED WAS THE WRONG ONE. "A 0.0 with no source" was a
// proxy for "we fabricated this". Measured against the vendor's own live feed,
// nothing is fabricated: Palo Alto publishes `baseScore: 0` with
// `baseSeverity: "NONE"` on advisories titled "Informational: Impact of <third-
// party CVE>", and 45 of 47 declare PAN-OS `status: "unaffected"`. That 0 is the
// vendor's answer to the exact question SecVault asks -- does this affect THIS
// product -- and it is correct. Overwriting it with CVE.org's 9.8 would file the
// Spring Framework's severity against a firewall its vendor says is unaffected.
//
// The shape actually worth reporting is the one that can mis-band a REAL
// exposure: a severity of zero attached to a version range the advisory DOES
// claim. Live that is 3 rows (PAN-SA-2014-0004, -2016-0026, -2019-0011), whose
// ranges are all far below the current fleet, so none produces an assessment
// today -- but a future one would band `monitor` on a 0 while genuinely
// matching a device, and that is worth a human reading it.
const SUSPICIOUS_ZERO_SCORES_SQL = `
  SELECT cve_id
    FROM advisories
   WHERE cvss_score = 0
     AND jsonb_array_length(affected_version_ranges) > 0
   ORDER BY cve_id
   LIMIT 50`;
