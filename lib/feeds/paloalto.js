...
// normal sync.
//
// Re-runs the SAME, unchanged, already-correct extraction functions
// (matchingPanOsAffectedEntries/extractAffectedRanges/extractFixedVersions)
// against each row's own already-stored raw_data -- no re-fetch needed, and
// no new extraction logic to get wrong here. Scoped narrowly to
// vendor='paloalto' rows whose raw_data actually looks like a genuine CVE
// Record (containers.cna present) -- guards against ever touching a
// differently-shaped row (this table also holds NVD-API-2.0-native-shaped
// raw_data for some paloalto rows, which has no `containers` key at all and
// would trivially no-op through matchingPanOsAffectedEntries's own
// `cna.affected` access, but the explicit guard makes that skip intentional
// rather than incidental). Best-effort per row AND overall, mirroring every
// other backfill* function's non-fatal contract -- this is a noise/precision
// cleanup, not a feature that can leave the app broken if it fails partway.
async function backfillPaloAltoVersionRanges(pool) {
  ...
  for (const row of rows) {
    ...
    if (!rec || !rec.containers || !rec.containers.cna) continue; // not a CVE-Record-shaped row — skip
    checked++;

    try {
      const matchingEntries = matchingPanOsAffectedEntries(rec);
      const freshRanges = extractAffectedRanges(matchingEntries);
      const freshFixed = extractFixedVersions(matchingEntries);
