...
// ⛔ Bug fixed 2026-07-17, confirmed live against CVE-2020-2021: when a
// versions[] entry has NEITHER lessThan NOR lessThanOrEqual, that means "this
// entire named branch is affected with no further fix" (real shape, EOL
// branch) — e.g. {"version":"8.0.*","status":"affected"}, no upper-bound
// field at all. The previous code left `max: null` in that case, which
// isInRange() treats as UNBOUNDED — matching every future version forever.
// Live-confirmed impact: that exact CVE (whose OTHER branches correctly cap
// at 9.1.3/9.0.9/8.1.15) was flagging PAN-OS 11.1.13-h5 devices as
// "Patch Now", three major versions past anything the advisory actually
// names. Bound it to the end of the STATED branch instead
// (major[.minor].999), mirroring this app's existing "X.Y all versions" ->
// {min:"X.Y.0", max:"X.Y.999"} convention already used in
// lib/feeds/fortinet.js — still conservative/wide within the branch actually
// named (consistent with the KNOWN SIMPLIFICATION below), never unbounded.
// A fully-specified release with no lessThan (3+ real segments, e.g. a CVE
// affecting exactly one point release) bounds to itself — an exact match,
// not a widened branch.
