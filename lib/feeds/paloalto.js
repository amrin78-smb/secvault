...
//
// ⛔ THE VERSION IS REPORTED, NOT INFERRED — added 2026-09-11, and it is the
// same lesson lib/feeds/nvd.js's pickCvss() carries. This picker returned only
// {score, vector} and normalizePaloAltoRecord() wrote neither `cvss_source` nor
// `cvss_version`, so every row this feed has ever written carries a score whose
// SCALE and PROVENANCE are both NULL. Measured live before the fix: 286 scored
// paloalto rows with `cvss_source IS NULL` (and a further 67 mislabelled
// `circl`, below) out of 749 scored rows fleet-wide with `cvss_version IS NULL`
// on ALL of them. A v4.0 base score and a v3.1 base score are NOT the same
// quantity, and an unlabelled number cannot be told apart from the other.
//
// The version comes from the METRIC KEY the score itself was read out of — the
// record's own explicit declaration of which scale it published — never from
// the shape of the vector string, the size of the score, or a default. Score,
// vector and version leave this function as ONE triple, so nothing downstream
// can pair a score with a scale it did not come from.
...
function extractAffectedRanges(matchingEntries) {
  ...
  for (const entry of matchingEntries) {
    for (const v of entry.versions || []) {
      // ⛔ THIS IS THE ONLY THING SEPARATING "PAN-OS 12.2 IS VULNERABLE" FROM
      // "PAN-OS 12.2 IS EXPLICITLY FINE", on 261 live version entries.
      //
      // Measured against the live bulk endpoint 2026-09-19: 309 PAN-OS version
      // entries carry a NON-affected status, and **261 of them name a real
      // numeric version** -- e.g. CVE-2026-0308 lists `status: "unaffected",
      // version: "12.2.0"` beside three genuinely affected branches. For those
      // the entry is present, the product matches and the version parses, so
      // nothing else in this function would stop them: boundBranchEnd would
      // turn 12.2.0 into 12.2.999 and every device on that branch would be
      // filed as vulnerable BY THE ADVISORY THAT CLEARS IT.
      //
      // ⛔ AND NOTE WHICH GUARD DOES *NOT* CARRY THIS LOAD. The 45 informational
      // "Impact of <third-party CVE>" bulletins (scored `baseScore: 0` /
      // `baseSeverity: "NONE"`) write `version: "All"`, so looksLikeVersion()
      // below rejects them regardless of status. An earlier version of this
      // comment credited the status check for those, and a test written to
      // match it PASSED WITH THIS LINE DELETED -- the fixtures exercised the
      // other guard. Both guards are real; they cover different records. The
      // mixed-status fixture exists so the mutation fails.
      // Pinned by tests/cvssZeroScore.test.js.
...
function normalizePaloAltoRecord(rec, matchingEntries) {
  ...
  const { score, vector, version } = pickCvssFromPanOsRecord(rec);
  ...
  return {
    ...
    cvss_score: score,
    cvss_vector: vector,
    // ⛔ PROVENANCE TRAVELS WITH THE SCORE — the same contract lib/feeds/nvd.js
    // and lib/feeds/fortinet.js already write. `psirt` is the honest answer for
    // this feed: the number is read out of the VENDOR'S OWN CVE Record, served
    // by Palo Alto's PSIRT bulk endpoint — it is not NVD's analyst score and
    // not CIRCL's mirror of a CNA record. fortinet.js already uses that exact
    // literal for the same situation, and components/cve/CVETable.js's
    // CVSS_SOURCE_LABELS already renders it as "vendor PSIRT".
    //
    // ⛔ Live before this fix: 67 paloalto rows were labelled `circl` while
    // holding a score this feed had already overwritten — the upsert below has
    // always rewritten same-vendor score/vector but left the old source string
    // untouched, so the label named a source the number no longer came from.
    // Writing the source alongside the score is what stops that pair drifting.
    //
    // ⛔ Both are NULL when there is no score. A source and a scale describe a
    // number; attaching them to a row that has none would assert a measurement
    // that was never made — and `v—`/`—` is the correct rendering of a scale
    // nobody published.
    cvss_source: score === null ? null : 'psirt',
    cvss_version: score === null ? null : version,
...
async function upsertAdvisory(pool, rec) {
  const result = await pool.query(
    `INSERT INTO advisories (
       ...
       cwe_ids, vulnerability_category, cvss_source, cvss_version,
       updated_at
     ) VALUES (
       $1, $2, $3, $4, $5, $6,
       $7::timestamptz, $8::jsonb, $9::jsonb, $10, $11::jsonb,
       $12::text[], $13, $14::text, $15::text,
       now()
     )
     ON CONFLICT (cve_id) DO UPDATE SET
       ...
       -- ⛔ Source and scale move under the IDENTICAL guard as the score and
       -- vector, so the four are always the same measurement. Split them and a
       -- row can end up holding this feed's number under the previous source's
       -- label — which is exactly the state 67 live paloalto rows were in
       -- (score rewritten by this feed on every sync, cvss_source still
       -- reading 'circl' from whichever run first inserted the row).
       --
       -- ⛔ No gap-fill/precedence clause here, deliberately, and this changes
       -- no score. nvd.js guards its writes with "a CIRCL score may fill a gap
       -- but never overwrite NVD's" because those two sources answer at random
       -- depending on which one was reachable. This feed is the VENDOR'S OWN
       -- advisory for its own product, it has always overwritten same-vendor
       -- score/vector, and adding provenance must not quietly change WHICH
       -- score wins — only label the one that already does.
       cvss_source = CASE WHEN advisories.vendor = EXCLUDED.vendor
                    THEN EXCLUDED.cvss_source ELSE advisories.cvss_source END,
       cvss_version = CASE WHEN advisories.vendor = EXCLUDED.vendor
                    THEN EXCLUDED.cvss_version ELSE advisories.cvss_version END,
       ...
       updated_at = now()
     -- Returns the row as it now stands, i.e. the OWNER: on a conflict with
     -- another vendor's row every column above keeps its existing value.
...
// Palo Alto's own live-verified CPE prefix (lib/feeds/nvd.js's
// VENDOR_CPES.paloalto, trailing ':*' wildcards stripped to a prefix, exactly
// as nvd.js's own cpePrefixes() does). Duplicated here rather than imported
// from nvd.js, per this file's own established "duplicate small per-feed
// logic, don't share a module across feeds/*.js" convention (see the
// upsertAdvisory/extractCweIdsFromCveRecord comments above for the same
// choice made elsewhere in this file) — used only by the NVD-native-shape
// branch of backfillPaloAltoVersionRanges below.
const PALOALTO_NVD_CPE_PREFIX = 'cpe:2.3:o:paloaltonetworks:pan-os:';

function nvdCriteriaMatchesPaloAlto(criteria) {
  return typeof criteria === 'string' && criteria.startsWith(PALOALTO_NVD_CPE_PREFIX);
}

// The next 3 functions mirror lib/feeds/nvd.js's own extractVersionFromCriteria /
// branchRangeFromWildcardCriteria / extractAffectedRanges / extractFixedVersions
// EXACTLY (including nvd.js's own 2026-07-23 wildcard-CPE-version fix — see that
// file's header comment on extractVersionFromCriteria for the full story: a
// version segment like "10.0.*" used to be treated as one pinned exact version,
// collapsing a whole affected branch down to a single point and silently
// under-matching every other build in that branch). Duplicated, not imported,
// same per-feed-duplication convention as above — this copy exists ONLY to let
// the backfill below re-derive a NATIVE-NVD-shaped paloalto row's ranges using
// the same (now-fixed) logic nvd.js itself uses on a live sync, scoped to the
// single vendor prefix this file cares about.
function nvdExtractVersionFromCriteria(criteria) {
...
function nvdBranchRangeFromWildcardCriteria(criteria) {
  if (!criteria || typeof criteria !== 'string') return null;
...
function extractAffectedRangesFromNvdNative(configurations) {
  const ranges = [];
  for (const config of configurations || []) {
    for (const node of (config && config.nodes) || []) {
      for (const match of (node && node.cpeMatch) || []) {
        if (match && match.vulnerable === true && nvdCriteriaMatchesPaloAlto(match.criteria)) {
          const hasRangeField =
            match.versionStartIncluding != null ||
            match.versionEndIncluding != null ||
            match.versionEndExcluding != null;
          const pinnedVersion = hasRangeField ? null : nvdExtractVersionFromCriteria(match.criteria);
          const branchRange =
            hasRangeField || pinnedVersion ? null : nvdBranchRangeFromWildcardCriteria(match.criteria);
          if (!hasRangeField && !pinnedVersion && !branchRange) continue;
          ranges.push({
            min:
              pinnedVersion ||
...
function extractFixedVersionsFromNvdNative(configurations) {
  const versions = new Set();
  for (const config of configurations || []) {
    for (const node of (config && config.nodes) || []) {
      for (const match of (node && node.cpeMatch) || []) {
        if (match && match.vulnerable === false && nvdCriteriaMatchesPaloAlto(match.criteria)) {
          const v =
            match.versionStartIncluding ||
            match.versionEndIncluding ||
            match.versionEndExcluding ||
            nvdExtractVersionFromCriteria(match.criteria);
          if (v) versions.add(v);
        }
      }
    }
...
// Retroactive backfill, added 2026-07-23. Root cause: PAN-SA-2023-0004 (and a
// handful of other informational bulletins) use versions[] to describe
// CONFIGURATION SCENARIOS rather than firmware version ranges -- e.g. a
// `version` field literally reading "with GlobalProtect app on Windows,
// macOS, and Linux LocalNet: Configurations allowing local network access,
// ServerIP: Gateways with address set as an FQDN", or a bare "All". The
// 2026-07-17 looksLikeVersion() guard (see extractAffectedRanges/
// extractFixedVersions above) already stops this from happening on any FRESH
// sync -- but a fix to extraction logic does not retroactively repair a value
// already persisted before that fix landed, same as every other retroactive
// cleanup in this codebase (see lib/migrate.js's other backfill* functions).
// Confirmed live in production (2026-07-23, via engine.log): these
// already-corrupted ranges were still spamming
// "[versionComparator] Unparseable version segment" on every CVE match run,
// months after the extraction fix shipped -- almost certainly because these
// are old, informational (non-CVE-numbered) bulletins that Palo Alto's own
// live PSIRT bulk endpoint no longer returns, so upsertAdvisory() never gets
// a chance to naturally overwrite them with the now-fixed extraction on a
// normal sync.
//
// ⛔ Widened the SAME day, same commit: this backfill originally only handled
// CVE-Record-shaped rows (raw_data.containers.cna present), skipping every
// paloalto row sourced instead from lib/feeds/nvd.js's own NATIVE CPE-matching
// path (raw_data has a top-level `configurations` array, no `containers` key
// at all). That path has its OWN, separate, also-real bug fixed the same day —
// nvd.js's extractVersionFromCriteria/extractAffectedRanges silently collapsed
// a wildcarded CPE version segment ("10.0.*") to a single point instead of the
// whole named branch (see nvd.js's own header comment on that function). A
// stored paloalto advisory row in that native shape, upserted before THAT fix
// landed, would never be touched by this backfill even though it queries every
// vendor='paloalto' row regardless of shape — the per-row guard below used to
// silently `continue` past it. Now branches on the row's actual shape and
// re-derives ranges/fixed-versions with whichever (now-fixed) extraction logic
// matches it — matchingPanOsAffectedEntries/extractAffectedRanges/
// extractFixedVersions for a CVE Record, extractAffectedRangesFromNvdNative/
// extractFixedVersionsFromNvdNative (above) for the native-NVD shape. Only an
// UNRECOGNIZED third shape (neither) is skipped now.
// NOTE — scope: this only covers vendor='paloalto' rows. The identical
// wildcard-CPE bug can affect the other five VENDOR_CPES vendors' own
// native-NVD-shaped rows too (nvd.js's own extraction functions are shared
// across all six) — repairing THOSE rows is out of scope for this
// Palo-Alto-specific file and must live wherever nvd.js's own cross-vendor
// cleanup/backfill is added. Also note: this backfill only corrects the
// `advisories` table itself — any `device_cve_assessments` row already computed
// from a corrupted range is only refreshed on the next natural feed-sync/
// engine-worker CVE-match cycle (or an operator-triggered "Assess Now"), not by
// this migration directly.
//
// Best-effort per row AND overall, mirroring every other backfill* function's
// non-fatal contract -- this is a noise/precision cleanup, not a feature that
// can leave the app broken if it fails partway.
async function backfillPaloAltoVersionRanges(pool) {
  const { rows } = await pool.query(
    `SELECT id, affected_version_ranges, fixed_in_versions, raw_data
       FROM advisories
  ...
  for (const row of rows) {
    ...
    const isCveRecordShape = !!(rec.containers && rec.containers.cna);
    const isNvdNativeShape = !isCveRecordShape && Array.isArray(rec.configurations);
    if (!isCveRecordShape && !isNvdNativeShape) continue; // unrecognized shape — skip
    checked++;
    ...
    try {
      ...
      if (isCveRecordShape) {
        const matchingEntries = matchingPanOsAffectedEntries(rec);
        freshRanges = extractAffectedRanges(matchingEntries);
        freshFixed = extractFixedVersions(matchingEntries);
      } else {
        freshRanges = extractAffectedRangesFromNvdNative(rec.configurations);
        freshFixed = extractFixedVersionsFromNvdNative(rec.configurations);
      }
      ...
      console.warn(`[paloalto] Version-range backfill failed for advisory row ${row.id} (non-fatal): ${err.message}`);
    }
  }
...
module.exports = {
  fetchAndUpsertPaloAltoAdvisories,
  backfillPaloAltoVersionRanges,
  // Exported for tests/cvssVersionBackfill.test.js — the part worth pinning is
  // that the score, its vector and its SCALE leave the picker as one triple
  // read out of one metric key, which is what stops a v4.0 number being
  // labelled (or read) as a v3.1 one. No DB or network involved.
  pickCvssFromPanOsRecord,
  // Exported for tests/cvssZeroScore.test.js, which pins the two halves of the
  // vendor-zero-score decision by BEHAVIOUR rather than by source shape: a
  // vendor-published 0 survives into the row, and an `unaffected` version entry
  // produces no range. Pure -- no DB, no network.
