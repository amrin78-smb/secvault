  ...
  for (const [cveId, candidates] of byCve) {
    try {
      ...
      if (hubIsBetter(local, remote)) {
        const res = await pool.query(
          `UPDATE advisories
            ...
            WHERE cve_id = $1
              ...
              --
              -- ⛔ AND THE DOUBLING IS WHY v2.186.0 SILENTLY DID NOTHING.
              -- hubIsBetter() was widened to accept a remote row that states a
              -- fix boundary where we state none, and THIS CLAUSE was left on
              -- the old rule — so every widened repair passed the predicate and
              -- was then refused by the statement. Three deploys reported
              -- "repaired 80" while the database wrote nothing.
              --
              -- That is the cost of the doubling working exactly as designed:
              -- it made the unsafe outcome impossible rather than merely
              -- unreached, including the outcome I had decided was safe. Both
              -- halves express the SAME widened rule now, and a test pins them
              ...
              AND (affected_version_ranges IS NULL
                   ...
                   OR (
                     NOT (affected_version_ranges @> '[{"exclude_fixed": true}]'::jsonb)
                     AND $2::jsonb @> '[{"exclude_fixed": true}]'::jsonb
                     AND jsonb_array_length($2::jsonb)
                         >= jsonb_array_length(affected_version_ranges)
                   ))`,
          [
            cveId,
            JSON.stringify(remote.affected_version_ranges),
        ...
        // ⛔ COUNT WHAT THE DATABASE DID, NOT WHAT WE ASKED IT TO DO — the
        // rule this file already states forty lines up, for the INSERT, and
        // broke here. `stats.repaired++` fired whether or not the WHERE clause
        // matched, so `feed_sync_log` reported `repaired: 80` across three
        // consecutive deploys while the statement wrote NOTHING. That number
        // is what made the failure look like a different bug: every signal said
        // the repair was working, so the search moved on to what was undoing
        // it. A count of intent is worse than no count, because it is trusted.
        stats.repaired += Number(res && res.rowCount) || 0;
      } else if (
