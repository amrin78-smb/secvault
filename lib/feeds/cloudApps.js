...
async function storeProvider(pool, provider, rows, startedAt) {
  // ⛔ A PROVIDER WITH NO DECLARED FLOOR IS REFUSED, NOT DEFAULTED TO 1. This
  // read `MIN_PLAUSIBLE[provider] || 1`, which is no floor at all: a fifth feed
  // added to SOURCES without a matching entry here would ship with the one
  // protection that stops a bad response emptying its half of the catalogue
  // silently switched off. Failing closed turns the omission into a loud error
  // on its first run instead.
  const floor = MIN_PLAUSIBLE[provider];
  if (!Number.isFinite(floor) || floor < 1) {
    throw new Error(
      `${provider} has no declared plausibility floor in MIN_PLAUSIBLE; refusing to write or prune `
      + 'the catalogue for a source whose collapse could not be detected.'
    );
  }
  ...
  // ⛔ THE FLOOR IS TESTED ON THE DEDUPED COUNT, BECAUSE THAT IS WHAT SURVIVES
  // THE PRUNE. It was tested on `rows.length`, one step too early, which left
  // the guard bypassable by any response that was LONG but REPETITIVE — and
  // repetition is normal here, not hypothetical: AWS publishes the same prefix
  // once per service, so a degraded response of 2,000 rows collapsing to three
  // distinct keys cleared a floor of 2,000 and then let the DELETE below remove
  // every other AWS row in the table. The number that matters is how many rows
  // this run will actually refresh, never how many arrived.
  if (deduped.length < floor) {
    throw new Error(
      `${provider} returned ${deduped.length} distinct usable entries (from ${rows.length} rows), `
      + `below the plausibility floor of ${floor}; existing catalogue rows were left untouched.`
    );
  }
...
async function syncCloudApps(pool) {
  // ⛔ THE PRUNE'S CLOCK MUST BE THE DATABASE'S OWN. Rows are stamped with
  // Postgres `now()`, and the prune deletes anything older than this instant.
  // Taking it from the app host compares two machines' clocks: if the database
  // trails, `last_seen_at < startedAt` deletes rows THIS RUN JUST WROTE, and
  // the plausibility floor cannot catch it because the write genuinely
  // succeeded. Harmless on the reference deployment (app and database are the
  // same host) and free to get right.
  let startedAt;
  try {
    const { rows } = await pool.query('SELECT now() AS now');
    startedAt = rows[0] && rows[0].now ? new Date(rows[0].now) : new Date();
  } catch (_err) {
    startedAt = new Date();
  }
  const results = [];
 ...
 *
 * ⛔ `kind` IS SELECTED, not merely used in the WHERE. It was omitted at first —
 * the column separates the two result sets, so returning it looked redundant —
 * and every row arrived with `kind: undefined`. Any caller that CONCATENATES
 * hosts and ips and then re-splits on `r.kind` (derive.js does exactly that, to
 * answer "what does this service publish") silently got zero of each, and the
 * Declare control rendered a confident "no flows can be derived" for services
 * that publish plenty. A row that cannot say what it is, in a shape where both
 * kinds get merged, is a trap — so it says.
 */
async function loadCatalogue(pool) {
  const [{ rows: hosts }, { rows: ips }, { rows: meta }] = await Promise.all([
    pool.query(
      `SELECT provider, service, service_display, kind, value, category, source_version
         FROM cloud_app_ranges WHERE kind = 'host'`
    ),
    pool.query(
      `SELECT provider, service, service_display, kind, value, range_start, range_end,
