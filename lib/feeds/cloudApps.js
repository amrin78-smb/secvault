  ...
  for (const s of Array.isArray(sets) ? sets : []) {
    ...
    for (const u of Array.isArray(s.urls) ? s.urls : []) {
      out.push({
        ...
        range_start: null, range_end: null, category, source_version: version || null,
      });
    }
    for (const ip of Array.isArray(s.ips) ? s.ips : []) {
      ...
      out.push({
        ...
        range_start: b.start, range_end: b.end, category, source_version: version || null,
      });
    }
  }
  return out;
...
function parseAws(doc) {
  ...
  for (const p of (doc && Array.isArray(doc.prefixes)) ? doc.prefixes : []) {
    ...
    out.push({
      ...
      category: p.region || null, source_version: version,
    });
  }
  return out;
}
...
function parseGoogle(doc) {
  ...
  for (const p of (doc && Array.isArray(doc.prefixes)) ? doc.prefixes : []) {
    ...
    out.push({
      ...
      range_start: b.start, range_end: b.end,
      category: p.scope || null, source_version: version,
    });
  }
  return out;
}
...
function parseCloudflare(text) {
  ...
  for (const line of String(text || '').split(/\r?\n/)) {
    ...
    out.push({
      ...
      kind: 'ip', value: v,
      range_start: b.start, range_end: b.end,
      category: null, source_version: null,
    });
  }
  return out;
}
    ...
    const res = await pool.query(
      `INSERT INTO cloud_app_ranges
         (provider, service, service_display, kind, value, range_start, range_end,
          category, source_version, first_seen_at, last_seen_at)
       SELECT t.provider, t.service, t.service_display, t.kind, t.value,
              t.range_start, t.range_end, t.category, t.source_version, now(), now()
         FROM unnest(
                $1::text[], $2::text[], $3::text[], $4::text[], $5::text[],
                $6::bigint[], $7::bigint[], $8::text[], $9::text[]
              ) AS t(provider, service, service_display, kind, value,
                     range_start, range_end, category, source_version)
...
async function loadCatalogue(pool) {
  const [{ rows: hosts }, { rows: ips }, { rows: meta }] = await Promise.all([
    ...
    pool.query(
      `SELECT provider, service, service_display, value, range_start, range_end, category, source_version
