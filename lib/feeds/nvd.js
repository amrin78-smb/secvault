...
//
// ⛔ A CVE CLAIMED BY ANOTHER VENDOR IS COUNTED, NOT SILENTLY DROPPED (v2.151.0).
//
// `advisories.cve_id` is UNIQUE and carries exactly ONE vendor, so whichever feed
// ingests a CVE first owns it permanently. Every upsert in this pipeline already
// guards against clobbering another vendor's row -- and then said nothing, so a
// lost advisory was indistinguishable from an uneventful update. The risk is real
// and structural (CVE-2022-0778 is an OpenSSL bug Fortinet republishes; here it
// belongs to paloalto with 6 real version ranges), and it GROWS with every feed
// added: the central hub made this product's list of first-ingesters longer.
//
// ⛔ THE POINT IS TO MAKE THE DECISION MEASURABLE. Changing the identity to
// UNIQUE (cve_id, vendor) is a change to how this product counts vulnerabilities
// -- it touches the dashboard severity histogram, the /vulnerability tiles, the
// PDF headline figures, the notification dedupe key and every advisory-curation
// URL, none of which can currently even ADDRESS a specific vendor's row. Measured
// 2026-09-19 against NVD's own configurations for an 80-CVE spread of the corpus,
// **0 matched two or more supported vendors**, and 0 of Palo Alto's 291 published
// CVE ids are held here under another vendor. So the change is not justified by
// today's data. This counter is what will justify it, or keep refuting it.
...
const { jsonEquivalent } = require('../canonicalJson');
...
const VENDOR_CPES = {
  ...
  //
  // ⛔ Re-probed live 2026-09-16. 'flexedge_secure_sd-wan' returns **0** from both
  // NVD and CIRCL and is KEPT anyway: the rebrand is real and the first CVE filed
  // under it must not be missed because we stopped asking. A string that returns
  // nothing costs one request; a string we removed costs a missed advisory.
  // The management centre is included because SecVault's Forcepoint adapter talks
  // to the SMC, never to the engine.
  forcepoint: [
    'cpe:2.3:a:forcepoint:next_generation_firewall:*:*:*:*:*:*:*:*', // pre-7.1 — 3
    'cpe:2.3:a:forcepoint:flexedge_secure_sd-wan:*:*:*:*:*:*:*:*', // 7.1+ rebrand — 0 today
    'cpe:2.3:a:forcepoint:next_generation_firewall_security_management_center:*:*:*:*:*:*:*:*', // 1 — the SMC
    'cpe:2.3:a:forcepoint:security_manager:*:*:*:*:*:*:*:*', // 1
    'cpe:2.3:a:forcepoint:stonegate:*:*:*:*:*:*:*:*', // 1 — the pre-Forcepoint Stonesoft brand
  ],
  ...
  // ⛔ EXPANDED 2026-09-16 AFTER MEASURING HOW LITTLE THE OLD LIST CAUGHT.
  // The four strings below the fold were the whole list, and they returned SEVEN
  // advisories for the entire vendor — for a firewall with a thirty-year CVE
  // history. The cause was not a broken feed: NVD files Check Point's gateway
  // under ~15 different product names accumulated across three decades of
  // rebrands (FireWall-1 -> VPN-1 -> Security Gateway -> Quantum), and asking
  // for four of them gets four of them. A customer opening /vulnerability on a
  // Check Point fleet saw almost nothing and had no way to know why.
  //
  // ⛔ EVERY STRING BELOW WAS PROBED AGAINST THE LIVE NVD API AND THE totalResults
  // IT RETURNED IS RECORDED BESIDE IT. Anything returning 0 is not listed. This
  // is the same rule this codebase applies to a device parser: the source is
  // verified, not assumed. Measured total across the list: ~107 CVEs, against 7
  // collected before it.
  //
  // ⛔ BUT READ THE ERA BREAKDOWN BEFORE QUOTING THAT NUMBER, because ~107 is
  // true and misleading. Measured 2026-09-16 across the newly-added products:
  // 73 unique CVEs, of which **12 are from 2015 onward**. `firewall-1` stops at
  // 2006, `vpn-1` at 2011, `provider-1` at 2004. For a fleet running Gaia R80+
  // the real gain is roughly **7 -> 20**, not 7 -> 107. The legacy strings are
  // KEPT — version matching is what stops a 2004 advisory reaching a modern
  // device, so they cannot produce a false finding, and a customer on an old
  // appliance is the one person who needs them — but nobody should read this
  // list and conclude Check Point is now as well covered as Palo Alto. The
  // modern-era gaps that actually closed are gaia_portal, mobile_access,
  // ipsec_vpn, log_server, multi-domain and security_gateway: nine CVEs.
  //
  // ⛔ AND THE OMISSIONS ARE THE LOAD-BEARING PART. Check Point publishes ~129
  // more CVEs under 'cpe:2.3:a:checkpoint' that are NOT this device: ZoneAlarm
  // (consumer AV), Harmony (endpoint/SASE), Capsule, SmartConsole and the
  // identity/VPN CLIENTS. A vendor-level wildcard query was tested, works, and
  // was REJECTED for exactly that reason — it would file endpoint-agent CVEs
  // against a firewall, manufacturing urgent work that is not real. On a product
  // whose whole claim is that its findings are evidence-backed, over-claiming is
  // worse than under-claiming. Do not "simplify" this list into a wildcard.
  //
  // What IS included, and why: the gateway under every name it has had, plus the
  // MANAGEMENT plane (provider-1, multi-domain, management/log server) — because
  // SecVault's Check Point adapter connects to the management server, not the
  // gateway, so a management-server CVE is a CVE in something we are talking to.
  checkpoint: [
    // Current generation.
    'cpe:2.3:o:checkpoint:gaia_os:*:*:*:*:*:*:*:*', // 8
    'cpe:2.3:o:checkpoint:gaia_embedded:*:*:*:*:*:*:*:*', // 1
    'cpe:2.3:a:checkpoint:gaia_portal:*:*:*:*:*:*:*:*', // 2
    'cpe:2.3:o:checkpoint:quantum_security_gateway_firmware:*:*:*:*:*:*:*:*', // 7
    'cpe:2.3:o:checkpoint:quantum_spark_firmware:*:*:*:*:*:*:*:*', // 1
    'cpe:2.3:a:checkpoint:cloudguard_network_security:*:*:*:*:*:*:*:*', // 1
    // The same gateway under its previous names. Version matching is what keeps
    // a 2004 FireWall-1 NG advisory off an R81.20 device — these are included
    // because a customer running an older appliance is exactly the customer who
    // needs them, and excluding them would have been a silent coverage choice.
    'cpe:2.3:a:checkpoint:firewall-1:*:*:*:*:*:*:*:*', // 43 — the largest single bucket
    'cpe:2.3:a:checkpoint:vpn-1:*:*:*:*:*:*:*:*', // 14
    'cpe:2.3:a:checkpoint:vpn-1_firewall-1:*:*:*:*:*:*:*:*', // 2
    'cpe:2.3:a:checkpoint:vpn-1_firewall-1_vsx:*:*:*:*:*:*:*:*', // 1
    'cpe:2.3:a:checkpoint:security_gateway:*:*:*:*:*:*:*:*', // 8
    'cpe:2.3:a:checkpoint:connectra_ngx:*:*:*:*:*:*:*:*', // 4
    'cpe:2.3:o:checkpoint:ipso_os:*:*:*:*:*:*:*:*', // 1 — legacy Nokia IPSO appliances
    // Blades that run ON the gateway, not on an endpoint.
    'cpe:2.3:a:checkpoint:mobile_access:*:*:*:*:*:*:*:*', // 3
    'cpe:2.3:a:checkpoint:ipsec_vpn:*:*:*:*:*:*:*:*', // 1
    'cpe:2.3:a:checkpoint:web_intelligence:*:*:*:*:*:*:*:*', // 1
    // Management plane — what the adapter actually authenticates to.
    'cpe:2.3:a:checkpoint:provider-1:*:*:*:*:*:*:*:*', // 4
    'cpe:2.3:a:checkpoint:multi-domain_security_management:*:*:*:*:*:*:*:*', // 1
    'cpe:2.3:o:checkpoint:multi-domain_management_firmware:*:*:*:*:*:*:*:*', // 1
    'cpe:2.3:o:checkpoint:quantum_security_management_firmware:*:*:*:*:*:*:*:*', // 1
    'cpe:2.3:a:checkpoint:management_server:*:*:*:*:*:*:*:*', // 1
    'cpe:2.3:a:checkpoint:log_server:*:*:*:*:*:*:*:*', // 1
  ],

  // Dictionary (keyword "sangfor"): only 3 entries total —
  // a:sangfor:next-gen_application_firewall (5 CVEs live, the NGAF firewall) and
  // a:sangfor:vdi_client (not a firewall — excluded). Sangfor's NVD coverage is
  // genuinely sparse; expect few advisories, and that is the source, not a bug.
  //
  // ⛔ Re-probed 2026-09-16 alongside the Check Point expansion: 'next_gen_...'
  // and 'sangfor_ngaf' were tried as alternate spellings and BOTH returned 0, so
  // neither is listed. CIRCL additionally carries the same CVEs under a
  // misspelled 'net-gen application firewall', and Sangfor's own O&M product —
  // neither is the firewall, neither is queried.
  sangfor: ['cpe:2.3:a:sangfor:next-gen_application_firewall:*:*:*:*:*:*:*:*'], // 5
};
...
// ────────────────────────────────────────────────────────────────────────
// NVD REACHABILITY CIRCUIT BREAKER — per run, never persisted.
//
// ⛔ WHY. Measured on the reference deployment 2026-09-10: `services.nvd.nist.gov`
// resolves in DNS but TCP 443 never completes — it is blocked at that site's network
// edge, and only for NVD (CIRCL, FortiGuard, Palo Alto, CISA and MITRE all answer 200
// from the same host). `feed_sync_log` shows the consequence: 474 nvd runs, ZERO
// successes, every one 'partial', ~497s average. 94% of that is pure sleeping — each of
// the 11 CPE strings independently discovers the same blocked host by burning
// 20s (timeout) + 3s (retry delay) + 20s (retry timeout) = 43s before falling back to
// CIRCL. 11 × 43 = 473s of the measured 501s.
//
// So: once NVD has failed CONCLUSIVELY at the network level, stop re-discovering that
// fact for every remaining CPE string in the run and go straight to CIRCL.
//
// ⛔ WHAT "CONCLUSIVELY" MEANS, and what it deliberately does NOT mean:
//
//   • ONLY a network-level failure counts — `fetch()` itself threw, so there is no HTTP
//     status and no parsed body (`err.status == null && !err.nvdJsonParseError`). This is
//     the SAME condition that has always gated the CIRCL fallback (see CLAUDE.md's Feed
//     Sources section and .ai-codex/cve-pipeline.md stage 2) and it is preserved exactly.
//   • An HTTP error (403/429/5xx) and a JSON-parse failure on a 200 are NOT reachability
//     problems — NVD answered. They cannot trip this breaker; stronger, they call
//     recordReachable() and RESET the streak to zero, because they are positive evidence
//     that the host is up. There is no code path by which a value of `err.status` or the
//     `nvdJsonParseError` marker reaches recordNetworkFailure().
//   • The streak must be CONSECUTIVE. Any NVD response at all — a page of data, a 403, a
//     truncated body — re-arms the breaker from scratch.
//
// The threshold is 4 consecutive network-level failures. Under the existing retry shape
// (one failure, 3s, one retry) that is exactly TWO CPE strings' worth of complete
// failure — ~86 seconds of wall clock, four independent connection attempts to the same
// host, spread across two different query URLs. A single timeout is a blip and is already
// covered by the per-request retry that has always been here; four in a row with nothing
// in between is a blocked host. Costing ~86s to prove it (instead of ~473s) is the trade
// this constant encodes; lowering it to 2 would save another ~43s and would make a single
// unlucky CPE string speak for the whole run, which is not a trade worth taking on a
// security product's primary CVE source.
//
// ⛔ NEVER PERSISTED, NEVER CACHED ACROSS RUNS. The breaker is a local created inside
// fetchAndUpsertVendorCves() and dies with it — the block could be lifted at any time and
// SecVault must not keep itself blind after the network is fixed. Every run re-probes NVD
// from scratch and NVD stays the primary source.
// ────────────────────────────────────────────────────────────────────────
const NVD_UNREACHABLE_STREAK = 4;

// ⛔ THE ONE DEFINITION of "NVD could not be reached", used by every branch that consults
// CIRCL and by the breaker above, so the two can never disagree. Semantics are byte-identical
// to the condition that has always been written inline at those call sites
// (`err.status == null && !err.nvdJsonParseError`) and documented in CLAUDE.md's Feed Sources
// section / .ai-codex/cve-pipeline.md stage 2:
//   • `fetch()` itself threw → no `.status` → TRUE (timeout, DNS failure, refused/reset).
//   • Any HTTP response, including 403/429/5xx → `.status` is set → FALSE. NVD answered.
//   • HTTP 200 with an unparseable body → tagged `nvdJsonParseError` by fetchPage() → FALSE.
//     A SyntaxError has no `.status` either, which is exactly why that marker exists.
function isNetworkLevelFailure(err) {
  return Boolean(err) && err.status == null && !err.nvdJsonParseError;
}

function makeNvdCircuit(streakThreshold = NVD_UNREACHABLE_STREAK) {
  const threshold = Math.max(1, streakThreshold);
  let consecutiveNetworkFailures = 0;
  let totalNetworkFailures = 0;
  let open = false;
  let lastError = null;
  const failedCpeStrings = [];
  const skippedCpeStrings = [];
  let fallbackOk = 0;
  ...
  return {
    ...
    // ⛔ Called for EVERY outcome that proves NVD answered: a successful page, an HTTP
    // error response, a malformed body on a 200. Resetting on these is what makes it
    // structurally impossible for a non-reachability failure to accumulate toward the
    // threshold — it does not merely "not count", it actively clears the streak.
    recordReachable() {
      consecutiveNetworkFailures = 0;
    },

    // ⛔ Call sites MUST already have established `err.status == null &&
    // !err.nvdJsonParseError`. This is the only function that advances the streak.
    recordNetworkFailure(cpeString, message) {
      consecutiveNetworkFailures += 1;
      totalNetworkFailures += 1;
      if (message) lastError = message;
      if (cpeString && !failedCpeStrings.includes(cpeString)) failedCpeStrings.push(cpeString);
      if (!open && consecutiveNetworkFailures >= threshold) {
        open = true;
        console.warn(
          `[NVD unreachable] ${consecutiveNetworkFailures} consecutive network-level failures (no HTTP response at all) ` +
            `across ${failedCpeStrings.length} CPE string(s) — treating NVD as unreachable for the REST OF THIS RUN and ` +
            'going straight to the CIRCL fallback for every remaining CPE string. This is re-tested from scratch on the next run.'
        );
      }
      return open;
    },

    recordSkipped(cpeString) {
      if (cpeString) skippedCpeStrings.push(cpeString);
    },

    // Whether the CIRCL fallback actually answered, so the run summary can say "CIRCL
    // supplied the data instead" only when that is true. A fallback that also failed is
    // real data loss and must not be reported as a successful rescue.
    recordFallback(outcome) {
      if (outcome === 'ok') fallbackOk += 1;
      else if (outcome === 'failed') fallbackFailed += 1;
    },
    ...
    summary() {
      return {
        ...
        failedCpeStrings: failedCpeStrings.slice(),
        skippedCpeStrings: skippedCpeStrings.slice(),
        fallbackOk,
        fallbackFailed,
        lastError,
      };
...
//
// ⛔ DELIBERATELY NOT SHORTENED FOR THE FIRST PROBE OF A RUN (considered and rejected
// 2026-09-10, alongside the circuit breaker above). A shorter first probe would shave
// ~40s off the ~86s the breaker now spends proving NVD is blocked. It would also make a
// SLOW-BUT-REACHABLE NVD look unreachable, and the cost of that mistake is asymmetric:
// the run would silently switch to CIRCL, whose affected-range extraction is documented
// as wider/coarser (see extractAffectedRangesFromCveRecord's KNOWN SIMPLIFICATION) and
// whose CVSS scores are gap-fill-only (see upsertAdvisory). That degradation would land on
// deployments where NVD actually WORKS — i.e. everywhere nobody is watching this file —
// to save 40 seconds once every 6 hours. The circuit breaker already removed 94% of the
// waste; this last slice is not worth buying with a false negative on reachability.
...
async function upsertAdvisory(pool, rec) {
  const result = await pool.query(
    `INSERT INTO advisories (
     ...
     ON CONFLICT (cve_id) DO UPDATE SET
       ...
       updated_at = now()
     -- advisories.vendor is the row AS IT NOW STANDS: on a conflict with
     -- another vendor's row the SET list above deliberately keeps every existing
     -- value, so this returns the OWNER, not what we tried to write.
     RETURNING (xmax = 0) AS inserted, advisories.vendor AS owning_vendor`,
    [
      rec.cve_id,
      rec.vendor,
      rec.title,
  ...
  return {
    ...
    // null when we own it; otherwise the vendor that got here first.
    claimedBy: row.owning_vendor === rec.vendor ? null : row.owning_vendor,
  };
}
...
//
// Returns 'ok' when CIRCL answered (INCLUDING a legitimate zero-record answer — CIRCL replying
// "no CVEs for this product" is an answer, not a failure), 'failed' when it did not, and
// 'already-attempted' when this vendor/product pair was already covered earlier in this vendor's
// loop. The caller feeds that into the run's circuit summary so the operator-facing statement can
// only claim "CIRCL supplied the data instead" when CIRCL actually did.
async function tryCirclFallback(cpeString, vendorSlug, vendorPrefixes, cveMap, errors, attemptedPairs) {
  ...
  if (attemptedPairs.has(pairKey)) return 'already-attempted';
...
//
// `circuit` is the RUN-scoped NVD reachability breaker (see makeNvdCircuit) — shared across
// every vendor in the run for exactly the same reason `throttle` is: every CPE string of every
// vendor talks to the one host, so a conclusion about that host is a conclusion for all of them.
// It defaults to a fresh, private breaker so a caller that doesn't pass one simply gets the old
// per-call behaviour rather than a shared global.
async function fetchCvesForVendor(pool, vendorSlug, throttle, circuit = makeNvdCircuit()) {
  const errors = [];
  ...
  for (const cpeString of cpeStrings) {
    // ⛔ NVD has already been proven unreachable earlier in THIS run (see makeNvdCircuit).
    // Re-probing costs 43s per CPE string and cannot produce a different answer, so go
    // straight to CIRCL. NVD is not abandoned: the breaker is per-run and the next run
    // probes it from scratch. No per-CPE error entry is pushed here — one aggregate
    // statement for the whole run is pushed by fetchAndUpsertVendorCves(), naming every
    // skipped string, instead of 18 more copies of the same timeout.
    if (circuit.isOpen()) {
      circuit.recordSkipped(cpeString);
      circuit.recordFallback(
    ...
    while (startIndex < totalResults) {
      ...
      try {
        ...
        // NVD answered with a parseable body — positive proof of reachability, so the
        // consecutive-failure streak goes back to zero.
        circuit.recordReachable();
      } catch (err) {
        if (err.status === 429) {
          // Rate limited — back off 30s and retry once (see CLAUDE.md "NVD Rate Limiting").
          // An HTTP status means NVD ANSWERED: reachability is proven, streak reset.
          circuit.recordReachable();
          errors.push({
            cve_id: null,
          ...
          try {
            ...
            cpeSucceeded = false;
            if (isNetworkLevelFailure(err2)) {
              circuit.recordNetworkFailure(cpeString, err2.message);
              circuit.recordFallback(
                await tryCirclFallback(cpeString, vendorSlug, vendorPrefixes, cveMap, errors, circlAttempted)
              );
            } else {
          ...
          // NVD answered (403 is a response), so this proves reachability, never doubts it.
          circuit.recordReachable();
          errors.push({
            cve_id: null,
          ...
          cpeSucceeded = false;
          break;
        } else if (isNetworkLevelFailure(err)) {
          ...
          // fixed delay.
          //
          // This is also the ONLY branch that advances the run's reachability breaker (see
          // makeNvdCircuit) — the guard above is precisely the "genuine network-level
          // failure" condition, so an HTTP status or a parse marker can never get here.
          const openedNow = circuit.recordNetworkFailure(cpeString, err.message);
          if (openedNow) {
            // The streak has already proven this host is not answering. Skipping the 3s +
            // 20s retry here is the same judgement as skipping the remaining CPE strings,
            // applied to the request we are already inside.
            errors.push({
              cve_id: null,
              message: `NVD request failed (network error) for ${cpeString} startIndex=${startIndex}: ${err.message}; NVD now treated as unreachable for the rest of this run — going straight to CIRCL`,
            });
            cpeSucceeded = false;
            circuit.recordFallback(
              await tryCirclFallback(cpeString, vendorSlug, vendorPrefixes, cveMap, errors, circlAttempted)
          ...
          try {
            ...
            if (isNetworkLevelFailure(err2)) {
              ...
              // The retry got an HTTP response (or a parseable-but-corrupt body): NVD is
              // reachable after all, whatever else went wrong. Clear the streak.
              circuit.recordReachable();
            }
            break;
          }
          ...
          // never consulted for this case. It also RESETS the reachability breaker: a body
          // arrived, so the host is up — a corrupted response is an NVD-side data problem,
          // never evidence of a blocked network.
          circuit.recordReachable();
          errors.push({
            cve_id: null,
          ...
          // Reached only when err.status is set (a non-429/403 HTTP error) — the branches
          // above have already claimed every statusless case. NVD answered; streak reset.
          circuit.recordReachable();
          errors.push({
            cve_id: null,
  ...
  let inserted = 0;
  let updated = 0;
  // See the vendor-conflict note at the top of this file.
  const claimedByOtherVendor = [];
  for (const rec of cveMap.values()) {
    try {
      const res = await upsertAdvisory(pool, rec);
      if (res.claimedBy) {
        claimedByOtherVendor.push({ cve_id: rec.cve_id, wanted: rec.vendor, held_by: res.claimedBy });
  ...
  return { inserted, updated, errors, claimedByOtherVendor };
}
...
async function fetchAndUpsertVendorCves(pool) {
  ...
  // ⛔ Created HERE and nowhere else: one breaker per run, shared by every vendor (same
  // scope and same reason as `throttle` — one host, one verdict), and garbage-collected
  // with this call. Nothing about NVD's reachability is ever written to disk or the DB, so
  // a run that starts after the block is lifted probes NVD normally.
  const circuit = makeNvdCircuit();
  const errors = [];
  const byVendor = {};
  let inserted = 0;
  let updated = 0;
  ...
  for (const vendorSlug of Object.keys(VENDOR_CPES)) {
    try {
      const result = await fetchCvesForVendor(pool, vendorSlug, throttle, circuit);
  ...
  // ⛔ ONE STATEMENT, NOT 22 TIMEOUTS. 474 consecutive 'partial' runs each carrying ~22
  // near-identical "network timeout" lines is, at a glance, indistinguishable from a feed
  // that is partly working — which is how a permanently BLOCKED primary source went 474
  // runs without anyone naming it. This entry says the thing once, in words, and says what
  // answered instead.
  //
  // ⛔ It does NOT change feed_sync_log.status, and that is deliberate. lib/feeds/index.js
  // writes 'partial' whenever the errors array is non-empty, and this run's array is
  // non-empty regardless (tryCirclFallback pushes its own informational entry per pair —
  // summarizeCirclUsage depends on that prefix). 'partial' is also the HONEST label: the
  // primary source contributed nothing, and CIRCL's records are documented as coarser
  // (extractAffectedRangesFromCveRecord's KNOWN SIMPLIFICATION) with gap-fill-only CVSS
  // (upsertAdvisory). A successful fallback is not a healthy feed. Inventing a new status
  // value was considered and rejected: lib/formatDisplay.js's feedStatusRank() sorts an
  // UNRECOGNISED status down with the failures (correctly), components/onboarding/
  // FirstRun.js counts only status='success', and AdvisoriesTab.js paints anything that is
  // neither 'success' nor 'partial' RED — so a new value would read WORSE than today
  // everywhere it is consumed, while requiring edits in four files to mean anything.
  const circuitSummary = circuit.summary();
  if (circuitSummary.open) {
    const skipped = circuitSummary.skippedCpeStrings;
    // "cpe:2.3:o:fortinet:fortios:*:*:*:*:*:*:*:*" -> "o:fortinet:fortios". The wildcard tail
    // is identical on all 11 and turns the list into a wall of asterisks; the part:vendor:product
    // triple is what actually identifies which query was skipped.
    const shortCpe = (s) => (typeof s === 'string' ? s.replace(/^cpe:2\.3:/, '').replace(/(:\*)+$/, '') : String(s));
    const rescued =
      circuitSummary.fallbackFailed === 0
        ? `The CIRCL fallback answered for all ${circuitSummary.fallbackOk} vendor/product pair(s) it was asked for, and supplied this run's advisory data instead of NVD.`
        : `The CIRCL fallback answered for ${circuitSummary.fallbackOk} vendor/product pair(s) and FAILED for ${circuitSummary.fallbackFailed} — those products have no advisory data from this run at all.`;
    const message =
      `[NVD unreachable] services.nvd.nist.gov did not answer ${circuitSummary.networkFailures} request(s) at the ` +
      `network level (no HTTP response at all) across ${circuitSummary.failedCpeStrings.length} CPE string(s). After ` +
      `${circuitSummary.threshold} consecutive such failures NVD was treated as unreachable FOR THE REST OF THIS RUN, so ` +
      `${skipped.length} further CPE string(s) went straight to CIRCL instead of waiting out ~43s of timeouts each: ` +
      `${skipped.map(shortCpe).join(', ') || '(none)'}. ${rescued} This is a network-reachability problem at THIS deployment, not an ` +
      'NVD data problem, and nothing about it is remembered — the next run probes NVD from scratch. Last NVD error: ' +
      `${circuitSummary.lastError || 'n/a'}. (informational, not an error)`;
    console.warn(message);
    errors.push({
      cve_id: null,
      message,
      // Structured alongside the prose so a future status surface can read the fact
      // directly rather than re-parsing the sentence.
      nvd_unreachable: true,
      nvd_network_failures: circuitSummary.networkFailures,
      nvd_cpe_strings_skipped: skipped.length,
      circl_pairs_ok: circuitSummary.fallbackOk,
      circl_pairs_failed: circuitSummary.fallbackFailed,
    });
  }

  return { inserted, updated, errors, byVendor, nvdUnreachable: circuitSummary.open };
}
...
async function backfillNvdNativeVersionRanges(pool) {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      // ⛔ ORDER-INDEPENDENT COMPARISON, NOT JSON.stringify. jsonb does not
      // preserve key order (it sorts by key length, then bytes), so the value
      // read back from the database is a DIFFERENT STRING from the identical
      // value freshly built in JS. This guard used string equality and so could
      // never fire once: it rewrote the same 302 advisory rows with byte-identical
      // data on every deploy for months, reporting "cleaned up 302" each time and
      // leaving advisories at 18.6% dead tuples. The data was always correct; the
      // comparison was not. See lib/canonicalJson.js.
      if (jsonEquivalent(row.affected_version_ranges || [], freshRanges)
        && jsonEquivalent(row.fixed_in_versions || [], freshFixed)) {
...
module.exports = {
  ...
  // NVD reachability breaker — exported for tests/nvdUnreachable.test.js, which pins the
  // property that matters: ONLY a network-level failure may open it, an HTTP status or a
  // JSON-parse failure resets it, and it never survives a run. fetchPage/fetchCvesForVendor
  // are exported so that can be proven against the REAL error objects this file constructs
  // (with `fetch` stubbed), not against hand-written look-alikes.
  makeNvdCircuit,
