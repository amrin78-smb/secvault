    ...
    for (const v of entry.versions || []) {
      ...
      if (v.lessThan != null) {
        ...
        if (changesMax) {
          max = changesMax;
          excludeFixed = true;
          // Every named per-train fix point, not just the highest one used for
          // `max` above — see isInRange/isSafeOnMatchingTrain in
          // versionComparator.js for why the coarse {min,max} range alone isn't
          // enough (a device sitting exactly on an earlier-but-still-named fix
          // point is numerically below the overall max, so it still needs this
          // explicit checkpoint list to be recognized as safe).
          safeExactVersions = allCheckpointsFromChanges(v.changes);
        } else {
