  ...
  try {
    ...
    for (const rec of records) {
      try {
        ...
        cveMap.set(normalized.cve_id, normalized);
      } catch (e) {
        errors.push({
    ...
    while (startIndex < totalResults) {
      ...
      try {
        ...
        if (err.status === 429) {
          ...
          // an HTTP status (429/403/5xx) means NVD responded and is a different failure class.
          errors.push({
            cve_id: null,
            message: `NVD request failed (network error) for ${cpeString} startIndex=${startIndex}: ${err.message}`,
          });
          cpeSucceeded = false;
          await tryCirclFallback(cpeString, vendorSlug, vendorPrefixes, cveMap, errors, circlAttempted);
          break;
