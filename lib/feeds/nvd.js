...
function pickCvss(metrics) {
  ...
  const data =
    (metrics.cvssMetricV40 && metrics.cvssMetricV40[0] && metrics.cvssMetricV40[0].cvssData) ||
    ...
    for (const node of (config && config.nodes) || []) {
      for (const match of (node && node.cpeMatch) || []) {
        if (
          ...
          if (!hasRangeField && !pinnedVersion) {
          ...
          ranges.push({
            min: pinnedVersion || (match.versionStartIncluding != null ? match.versionStartIncluding : null),
            max:
              pinnedVersion ||
              (match.versionEndIncluding != null
...
// vendor/product fallback match below.
function vendorProductPairsFromPrefixes(vendorPrefixes) {
  return vendorPrefixes
    .map((p) => {
      ...
      return { vendor: normalizeIdentityToken(parts[3]), product: normalizeIdentityToken(parts[4]) };
    })
...
// matching entries ends up with empty affected_version_ranges/fixed_in_versions — see this file's
// callers (extractAffectedRangesFromCveRecord/extractFixedVersionsFromCveRecord) and
// normalizeCirclRecord, which (unlike lib/feeds/paloalto.js's equivalent zero-match skip) has no
// guard against upserting that empty-ranges advisory, silently making it unmatchable to any device.
function matchingAffectedEntriesFromCveRecord(rec, vendorPrefixes) {
  ...
  return affected.filter((entry) => {
    ...
    const entryVendor = normalizeIdentityToken(entry.vendor);
    const entryProduct = normalizeIdentityToken(entry.product);
    if (!entryVendor || !entryProduct) return false;
    return identityPairs.some((pair) => pair.vendor === entryVendor && pair.product === entryProduct);
  });
}
