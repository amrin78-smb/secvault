...
async function verifyForLogin(pool, userId, code) {
  ...
  if (result.valid) {
    ...
    // ⛔ THE DATABASE DECIDES THE WINNER, NOT THE READ ABOVE.
    //
    // The check above is a READ, and the write used to be unconditional. Two
    // requests carrying the SAME captured code, arriving together, both read
    // the stale `last_counter`, both passed the `<=` test, and both
    // authenticated — so single-use held only when logins happened to be
    // serial. A replay is not a serial event: it is by definition an attacker
    // firing the captured code as fast as they can, which is exactly the
    // concurrent case.
    //
    // The WHERE clause makes the counter advance atomically. The row is updated
    // only if `last_counter` is still what it was, so of two racing requests
    // exactly one gets rowCount 1 and the loser is rejected as a reuse. The
    // check above is kept as the cheap, common-case rejection; this is the one
    ...
    if (claimed.rowCount !== 1) return { ok: false, reason: 'code_reused' };
  ...
  if (normalised.length >= 12) {
    ...
    for (let i = 0; i < hashes.length; i += 1) {
      ...
      if (match) {
        // ⛔ REMOVE THE ONE ELEMENT IN SQL — NEVER WRITE BACK A COMPUTED ARRAY.
        //
        // This used to compute `remaining` in JS from the array it had read and
        // write the WHOLE array back. Two concurrent logins using two DIFFERENT
        // recovery codes each computed `remaining` from the same stale read, so
        // the second write RESTORED the code the first had just consumed — a
        // used, single-use recovery code silently became live again. That is
        // strictly worse than the TOTP race above, which merely admitted a
        // replay; this one manufactures a fresh credential.
        //
        // Deleting by VALUE inside one statement is atomic: the row lock
        // serialises the two updates and the second re-evaluates against the
        // already-updated array, so each removes only its own code. The `@>`
        // guard means a code already consumed by the winner yields rowCount 0
        // rather than a silent no-op that still authenticates.
        //
        // ⛔ COALESCE is required: jsonb_agg over zero rows is NULL, and the
        // column is NOT NULL — spending the LAST recovery code would otherwise
        // throw instead of leaving an empty list.
        const hash = hashes[i];
        // eslint-disable-next-line no-await-in-loop
        const consumed = await pool.query(
          `UPDATE user_mfa
              SET recovery_codes = COALESCE(
                    (SELECT jsonb_agg(e)
                       FROM jsonb_array_elements(recovery_codes) e
                      WHERE e <> to_jsonb($2::text)),
                    '[]'::jsonb
                  ),
                  updated_at = now()
            WHERE user_id = $1
              AND recovery_codes @> jsonb_build_array($2::text)`,
          [userId, hash]
        );
        if (consumed.rowCount !== 1) return { ok: false, reason: 'code_reused' };
        // ⛔ Derived from the array we READ minus the one we just proved we
        // consumed — not from a `remaining` computed before the write, which is
        // the stale value the old read-modify-write produced. This is only a
        // log/warning figure, but a count that is wrong under exactly the
        // condition being guarded against would be worse than none.
        return { ok: true, method: 'recovery', recoveryRemaining: Math.max(0, hashes.length - 1) };
      }
    }
  }
