...
/**
 * True when `description` actually CONTAINS this object's own name.
 *
 * ⛔ "A DESCRIPTION EXISTS" IS NOT "THE DESCRIPTION SAYS THE NAME", AND THE
 * DIFFERENCE DELETED THE NAME FROM THE PAGE. The caller was passing
 * `hasDescription`, so any recognised path shape dropped the `@_name` row —
 * including the two builders that are name-BLIND by design (a field under an
 * indexed parent, and a deviceconfig setting), whose sentences never contain
 * a name at all. The row was removed as a duplicate of something that was
 * never said, and the object's identity then appeared nowhere on the page:
 * not in the heading, not in the table, and the raw path only carries an
 * array index.
 *
 * The test is what it always should have been — is this exact text already
 * above the table.
 */
function headingNamesObject(description, value) {
  if (typeof description !== 'string' || description === '') return false;
  if (value === null || typeof value !== 'object' || Array.isArray(value)) return false;
  for (const key of NAME_KEYS_IN_HEADING) {
    const name = value[key];
    if (typeof name === 'string' && name.trim() !== '' && description.includes(name.trim())) return true;
  }
  return false;
}
