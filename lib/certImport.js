...
function pfxToPem(buf, passphrase) {
  ...
  try {
    ...
    const env = { ...process.env, SV_PFX_PASS: passphrase == null ? '' : String(passphrase) };
    ...
    try {
      ...
      try {
        ...
        const message = String((err2 && err2.stderr) || (err && err.stderr) || '');
        if (/mac verify failure|invalid password|wrong password/i.test(message)) {
