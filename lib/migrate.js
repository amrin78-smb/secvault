...
const { ensureLedger, isDone, markDone } = require('./backfillLedger');
const {
  backfillNvdNativeVersionRanges,
  // The LIVE classifier and the LIVE extractors, reused verbatim by
  // backfillAdvisoryMatchability() below so a retro-classified row can never mean something
  // different from a freshly-ingested one.
  VENDOR_CPES,
  cpePrefixes,
...
/**
 * Three-role RBAC migration (v2.110.0). Idempotent; safe on every run.
 *
 * ⛔ THIS EXISTS TO PREVENT AN IMMEDIATE LOCKOUT. Before the change there was
 * one role that could do everything, called `admin`. After it, `admin` can do
 * everything EXCEPT manage user accounts and credential profiles — and only
 * `super_admin` can. So on the first boot after deploying, an installation
 * whose only account is `admin` has NOBODY who can create a user, change a
 * role, or touch a credential profile, recoverable only by a direct database
 * edit. Promotion is not a convenience here; it is the difference between an
 * upgrade and a self-inflicted outage.
 *
 * ⛔ Promotes only when NO super_admin exists. Once one does, this is a no-op
 * forever — it must never re-promote an account a Super Admin deliberately
 * demoted to `admin` later.
 *
 * ⛔ Promotes the OLDEST admin, not all of them. On an installation with
 * several admins, granting every one of them user-management authority would
 * be a silent privilege escalation for accounts nobody reviewed. One account
 * regains what it had; the rest keep exactly what `admin` now means, and a
 * Super Admin can promote further deliberately.
 *
 * ⛔ `viewer` rows are NOT migrated to `operator`. Operator can write
 * (acknowledge findings, run analyses); viewer could not. Mapping one to the
 * other would hand write access to accounts created specifically without it.
 * A stray viewer row simply holds no capabilities — see lib/rbac.js — which
 * is read-only, exactly what it always meant. They are counted and reported
 * so an operator can see them and decide, rather than being silently changed.
 */
async function migrateRolesToThreeTier(pool) {
  const { rows: supers } = await pool.query(
    "SELECT id FROM users WHERE role = 'super_admin' LIMIT 1"
  );
  const { rows: legacyViewers } = await pool.query(
    "SELECT username FROM users WHERE role NOT IN ('super_admin','admin','operator')"
  );

  if (supers.length > 0) {
    return { promoted: null, alreadyHadSuperAdmin: true, legacyRoles: legacyViewers.map((r) => r.username) };
  }
    ...
    `UPDATE users SET role = 'super_admin', updated_at = now()
      WHERE id = (
        SELECT id FROM users WHERE role = 'admin'
         ORDER BY created_at ASC, username ASC LIMIT 1
      )
      RETURNING username`
  );

  return {
    promoted: promotedRows[0] ? promotedRows[0].username : null,
    alreadyHadSuperAdmin: false,
    legacyRoles: legacyViewers.map((r) => r.username),
  };
}
...
/**
 * Fill `advisories.cvss_version` from the CVSS vector the row ALREADY stores.
 *
 * ⛔ WHY THIS EXISTS. `cvss_source`/`cvss_version` were added (v2.90.3) because two feeds
 * scoring the same CVE on two different CVSS versions made the fleet severity histogram
 * alternate between two states on identical totals — see cve-pipeline.md's "CVSS provenance"
 * section. The column landed; the DATA never did. Measured live 2026-09-11: 1,004 advisories,
 * 749 of them scored, and `cvss_version IS NULL` on ALL 1,004. Every advisory therefore renders
 * the `v—` NOT-MEASURED marker, which says "SecVault does not know which scale this number is
 * on" — while the scale is sitting in the same row, in plain text, in `cvss_vector`.
 *
 * That is the failed-read-as-a-fact rule pointing the other way for once: not a fabricated
 * value, but a real, recorded fact displayed as unmeasured. Both are wrong for the same reason
 * — the screen stops matching what is known.
 *
 * ⛔ THE DERIVATION IS THE VECTOR'S OWN DECLARATION AND NOTHING ELSE. A CVSS v3.0/v3.1/v4.0
 * vector string begins with a literal `CVSS:<version>/` prefix — `CVSS:3.1/AV:N/AC:L/...` —
 * which IS the publisher stating the scale. This reads that prefix and copies it.
 * It must never infer a version from:
 *   - the SHAPE of the vector (a v2 vector `AV:N/AC:L/Au:N/C:P/I:P/A:P` has NO prefix and
 *     parses "successfully" as a set of metrics — it just never says 2.0, so the column stays
 *     NULL and `v—` is the CORRECT rendering for it),
 *   - the score, the vendor, the feed, or the era the row was ingested in,
 *   - a default of any kind. There is no safe default here: mislabelling a v4.0 score as v3.1
 *     is worse than admitting the scale is unknown, because a wrong label is believed.
 * Measured live: 0 of the 749 vectors lack the prefix, and the only prefixes present are
 * CVSS:3.1 (555), CVSS:4.0 (124) and CVSS:3.0 (70). The guard costs nothing today and is the
 * whole point the first time a v2 vector or a new scale shows up.
 *
 * ⛔ IT ONLY EVER FILLS A GAP. `cvss_version IS NULL` is in the WHERE clause, so a version a
 * feed wrote is never overwritten by a version derived here — a feed knows which METRIC BLOCK
 * its score came out of, which is better evidence than the vector text, and the two can only
 * disagree if the publisher's own record is internally inconsistent.
 *
 * ⛔ THE VERSION AND THE SCORE MUST DESCRIBE THE SAME MEASUREMENT, so a row with no score is
 * left alone (`cvss_score IS NOT NULL`). Score and vector are written together by every feed
 * (each is picked out of one metric block, and every upsert moves the pair under one guard), so
 * the vector on a scored row is the vector that score came from. Stamping a scale onto a row
 * that has no number would invent a property of a measurement nobody made. Live today this
 * excludes nothing — all 255 unscored rows have no vector either — and that is what makes it
 * free to be strict.
 *
 * Bounded (one statement, `advisories` only), parameter-free but fully literal — no string
 * interpolation — and idempotent: every row it touches stops matching its own WHERE clause, so
 * a second run updates 0 rows. `updated_at` is deliberately NOT bumped: the advisory's contents
 * did not change, only a label SecVault derived from what it already had, and churning
 * `updated_at` across 749 rows would misdate the vendor data for every consumer that reads it.
 */
async function backfillCvssVersionFromVector(pool) {
  const { rows } = await pool.query(
    `UPDATE advisories
        SET cvss_version = substring(cvss_vector from '^CVSS:([0-9]+[.][0-9]+)/')
      WHERE cvss_version IS NULL
        AND cvss_score IS NOT NULL
        AND cvss_vector ~ '^CVSS:[0-9]+[.][0-9]+/'
      RETURNING vendor, cvss_version`
  );

  const byVersion = {};
  const byVendor = {};
  for (const row of rows) {
    byVersion[row.cvss_version] = (byVersion[row.cvss_version] || 0) + 1;
    byVendor[row.vendor] = (byVendor[row.vendor] || 0) + 1;
  }

  // ⛔ What was LEFT NULL is reported too, split by reason. A backfill that prints only what it
  // changed reads as "done" while the remaining NULLs look like a bug in it — these three
  // numbers are the difference between "nothing left to derive" and "something is not being
  // parsed". Counted AFTER the update, so a re-run reports the same standing picture.
  const { rows: remainderRows } = await pool.query(
    `SELECT
       count(*) FILTER (WHERE cvss_score IS NULL)                                   AS unscored,
       count(*) FILTER (WHERE cvss_score IS NOT NULL AND cvss_vector IS NULL)        AS scored_no_vector,
       count(*) FILTER (WHERE cvss_score IS NOT NULL AND cvss_vector IS NOT NULL
                              AND cvss_vector !~ '^CVSS:[0-9]+[.][0-9]+/')           AS vector_declares_no_version
       FROM advisories
      WHERE cvss_version IS NULL`
  );
  const remainder = remainderRows[0] || {};
  ...
  return {
    ...
    byVendor,
    stillNull: {
      unscored: Number(remainder.unscored || 0),
      scoredNoVector: Number(remainder.scored_no_vector || 0),
      vectorDeclaresNoVersion: Number(remainder.vector_declares_no_version || 0),
    },
  };
}

/**
 * Retro-classify `advisories.matchability` for every already-stored row.
 *
 * ⛔ WHY THIS EXISTS. The 2026-09-09 fix stopped STORING an advisory whose version ranges
 * could not be extracted, because `affected_version_ranges = []` is read downstream as an
 * affirmative "this device is not affected" (see lib/engines/versionMatcher.js's
 * UNASSESSABLE_MATCHABILITY note). It did nothing for the rows already sitting in the table:
 * measured on the reference fleet, 283 cisco_asa, 95 fortinet, 52 paloalto, 7 checkpoint,
 * 5 sangfor and 2 forcepoint advisories carry an empty range list, and every one of them is
 * currently consumed as a clean bill of health. This writes down which is which.
 *
 * ⛔ NOTHING IS DELETED, EVER. A CVE record is evidence; the column is what makes its status
 * honest. An advisory we cannot evaluate still belongs in the table, still appears on the
 * Advisories page, and is still the thing an operator curates conditions for.
 *
 * ⛔ REUSES THE LIVE CLASSIFIER, never a second copy. Each row is re-classified by running its
 * OWN stored `raw_data` through the exact function the ingest path calls — CVE Record Format
 * 5.x through classifyCveRecordMatchability(), NVD-API-2.0-native through
 * classifyNvdNativeMatchability(). A backfill that reimplemented the rule would drift from it
 * on the first change, and the whole point is that the label means the same thing either way.
 *
 * ⛔ CLASSIFIED AGAINST WHAT IS STORED, not against what a re-extraction could produce. A row
 * whose ranges are empty in the DB is unassessable TODAY regardless of whether today's
 * extractor could do better — labelling it 'matched' while leaving the empty array in place
 * would re-create the exact bug. The one exception is deliberate and narrow, below.
 *
 * ⛔ THE ONE RANGE WRITE: a row with an EMPTY stored range list whose own raw_data now yields
 * real ranges is repaired first, then classified on the repaired value. Measured live, that is
 * exactly the 5 Sangfor advisories the curated `next-gen`/`Net-Gen` product alias rescued
 * (CVE-2023-30802 … CVE-2023-30806). Rows that ALREADY have ranges are never re-derived here:
 * paloalto's PSIRT rows were extracted by lib/feeds/paloalto.js's own independent copy of that
 * logic, and re-deriving them through nvd.js's copy would silently swap one extractor's output
 * for another's across 299 rows. An empty result is NEVER written over a non-empty one — that
 * direction is the bug, not the fix.
 *
 * Idempotent and safe to re-run: it writes only when the computed value differs from what is
 * stored, and best-effort per row, same non-fatal contract as every other backfill here.
 *
 * @param {import('pg').Pool} pool
 * @returns {Promise<{checked: number, labelled: number, rangesRepaired: number,
 *   byVendor: object, byStatus: object}>}
 */
async function backfillAdvisoryMatchability(pool) {
  const { rows } = await pool.query(
    `SELECT id, cve_id, vendor, matchability, affected_version_ranges, fixed_in_versions, raw_data
       FROM advisories
  ...
  for (const row of rows) {
    ...
    // No verified CPE strings for this vendor slug means there is nothing to classify
    // AGAINST. Leave the column NULL ("not yet classified") rather than guess a status —
    // NULL is a real state here, and versionMatcher deliberately treats it as assessable.
    if (cpeStrings.length === 0) continue;
    const vendorPrefixes = cpePrefixes(cpeStrings);

    const isCveRecord = !!(rec && rec.containers);
    const isNvdNative = !!(rec && Array.isArray(rec.configurations));
    if (!isCveRecord && !isNvdNative) continue; // unrecognised shape — same reasoning as above

    checked++;

    try {
      let ranges = Array.isArray(row.affected_version_ranges) ? row.affected_version_ranges : [];
      let fixedIn = Array.isArray(row.fixed_in_versions) ? row.fixed_in_versions : [];
      let repairRanges = false;

      // The one range write — see this function's header. Guarded to empty-in / non-empty-out
      // only, so it can only ever ADD information.
      if (ranges.length === 0) {
        const fresh = isCveRecord
          ? extractAffectedRangesFromCveRecord(rec, row.vendor, vendorPrefixes)
          : extractAffectedRanges(rec.configurations, vendorPrefixes);
        if (fresh.length > 0) {
          ranges = fresh;
          fixedIn = isCveRecord
            ? extractFixedVersionsFromCveRecord(rec, vendorPrefixes)
            : extractFixedVersions(rec.configurations, vendorPrefixes);
          repairRanges = true;
        }
      }

      const verdict = isCveRecord
        ? classifyCveRecordMatchability(rec, vendorPrefixes, ranges)
        : classifyNvdNativeMatchability(rec, vendorPrefixes, ranges);

      byVendor[row.vendor] = byVendor[row.vendor] || {};
      byVendor[row.vendor][verdict.status] = (byVendor[row.vendor][verdict.status] || 0) + 1;
      byStatus[verdict.status] = (byStatus[verdict.status] || 0) + 1;

      const needsLabel = row.matchability !== verdict.status;
      if (!needsLabel && !repairRanges) continue;

      if (repairRanges) {
        await pool.query(
          `UPDATE advisories
              SET affected_version_ranges = $1::jsonb,
                  fixed_in_versions = $2::jsonb,
                  matchability = $3,
                  updated_at = now()
            WHERE id = $4`,
          [JSON.stringify(ranges), JSON.stringify(fixedIn), verdict.status, row.id]
        );
        rangesRepaired++;
        console.log(
          `[migrate] advisory ${row.cve_id} (${row.vendor}): recovered ${ranges.length} version range(s) from its own raw_data; matchability -> ${verdict.status}`
        );
      } else {
        await pool.query(
          `UPDATE advisories SET matchability = $1, updated_at = now() WHERE id = $2`,
          [verdict.status, row.id]
        );
      }
      if (needsLabel) labelled++;
    } catch (err) {
      console.warn(
        `[migrate] matchability backfill failed for advisory ${row.cve_id} (${row.vendor}, non-fatal): ${err.message}`
      );
    }
  }

  return { checked, labelled, rangesRepaired, byVendor, byStatus };
}

/**
 * Repair the FortiOS IPsec-as-a-user-login misclassification (2026-09-10).
 *
 * ⛔ WHAT WENT WRONG. `tunnel-up` sat in FORTINET_SUCCESS_ACTIONS
 * unconditionally, but FortiOS emits that verb BOTH for a real SSL-VPN
 * tunnel-mode login and for a site-to-site IPsec tunnel completing phase 2.
 * On the IPsec form there is no person at all — `user=` holds the PEER
 * GATEWAY ADDRESS (which is why those rows also carry `xauthuser="N/A"`).
 * So gateways were being recorded as employees logging in successfully.
 *
 * ⛔ THE CODE FIX ONLY AFFECTS NEW EVENTS. auth_outcome and src_user are
 * computed at INGEST and stored, so every already-persisted row keeps the
 * wrong value — and TWO of the derived tables are PERMANENT
 * (syslog_vpn_auth_hourly, syslog_user_hourly), so without this repair the
 * fleet keeps a permanent record of logins that never happened.
 *
 * ⛔ THE PRECISE, NARROW PREDICATE MATTERS. Measured live before the repair:
 * 14 IPsec successes AND 5 GENUINE ones. Three of the genuine rollup rows
 * belong to a real person ("Panupong") who really did log in over SSL-VPN.
 * Deleting by action alone — or by "fortinet successes look wrong" — would
 * have erased a named employee’s real login history to remove fake gateways.
 * So every predicate below is POSITIVELY IPsec / positively an address, never
 * "not obviously SSL".
 *
 * Idempotent: a second run matches nothing. Best-effort per statement.
 */
async function repairFortinetIpsecAuthMisclassification(pool) {
  const out = { eventsReclassified: 0, eventsUserCleared: 0, vpnAuthRowsDeleted: 0, userRollupRowsDeleted: 0 };

  // 1. The events themselves. POSITIVELY ipsec, and only rows currently
  //    claiming a success — an IPsec row that already classified as null is
  //    already correct and is left alone.
  const rc = await pool.query(
    `UPDATE syslog_events
        SET auth_outcome = NULL
      WHERE log_class = 'vpn' AND vendor = 'fortinet'
        AND auth_outcome = 'success'
        AND message LIKE '%tunneltype=%ipsec%'`
  );
  out.eventsReclassified = rc.rowCount || 0;

  // 2. The peer address sitting in src_user. This must run over the whole
  //    retained window, NOT just the rollup lookback: syslog_user_hourly is
  //    re-aggregated from a 24h lookback, so deleting its rows in step 4
  //    without fixing the events would simply re-create them within a day.
  //    ⛔ An address is not a NULL-shaped placeholder — it is a real string
  //    carrying the wrong fact, which is why meaningful() never caught it.
  const uc = await pool.query(
    `UPDATE syslog_events
        SET src_user = NULL
      WHERE log_class = 'vpn' AND vendor = 'fortinet'
        AND src_user IS NOT NULL
        AND (src_user ~ '^[0-9]{1,3}([.][0-9]{1,3}){3}$' OR src_user ~ '^[0-9a-fA-F]*:[0-9a-fA-F:]*$')`
  );
  out.eventsUserCleared = uc.rowCount || 0;

  // 3. The PERMANENT VPN auth rollup. Delete only rows that contain NO real
  //    username at all — i.e. every name in the array is an address. A row
  //    holding even one genuine account is left completely alone.
  const vd = await pool.query(
    `DELETE FROM syslog_vpn_auth_hourly
      WHERE vendor = 'fortinet' AND auth_outcome = 'success'
        AND usernames IS NOT NULL AND cardinality(usernames) > 0
        AND NOT EXISTS (
          SELECT 1 FROM unnest(usernames) AS u WHERE u !~ '^[0-9]{1,3}([.][0-9]{1,3}){3}$'
        )`
  );
  out.vpnAuthRowsDeleted = vd.rowCount || 0;

  // 4. The PERMANENT per-user traffic rollup, where these addresses had
  //    become 13 distinct "users" carrying ~14,000 events. Same failure mode
  //    this codebase already documented for the literal "N/A" becoming the
  //    5th busiest user on the Top Users chart.
  const ud = await pool.query(
    `DELETE FROM syslog_user_hourly
      WHERE src_user IS NOT NULL
        AND (src_user ~ '^[0-9]{1,3}([.][0-9]{1,3}){3}$' OR src_user ~ '^[0-9a-fA-F]*:[0-9a-fA-F:]*$')`
  );
  out.userRollupRowsDeleted = ud.rowCount || 0;

  return out;
}

// ⛔ MODULE SCOPE, deliberately: the completion line that reads this lives in
// the require.main block BELOW main(), not inside it. Declared inside main() it
// passes `node --check` cleanly and then throws ReferenceError on the very last
// line of a SUCCESSFUL migration, turning a good deploy into a failed one.
let __migrateStarted = Date.now();

async function main() {
  // ⛔ TIME EVERY RUN. migrate was 601-823s of a 740-980s deploy — 84% — and
  // nothing printed a duration, so the cost was invisible to anyone reading
  // the update log. A step that dominates a deploy must say how long it took.
  __migrateStarted = Date.now();
  console.log('[migrate] Running schema migration...');
  await runSchema(pool);
  console.log('[migrate] Schema migration complete.');

  // ⛔ The ledger must exist before any gated backfill asks whether it ran.
  // ⛔ TOLERATED, NOT FATAL. lib/backfillLedger.js states the intent plainly:
  // if the ledger is missing, the right answer is "nothing is recorded, run
  // everything" — and isDone()/markDone() both already fail that way. This
  // call did not, so a transient connection reset here would reject main(),
  // exit 1, and stop Update-SecVault.ps1 from starting SecVault-App at all —
  // AFTER git pull and npm ci had already replaced the code. A bookkeeping
  // table must never be able to take the application down.
  try {
    await ensureLedger(pool);
  } catch (err) {
    console.warn(`[migrate] backfill ledger unavailable (${err.message}) — every repair will run this time.`);
  }
  ...
  try {
    // Three-role RBAC. Runs before anything else that could depend on a role.
    try {
      const roleMigration = await migrateRolesToThreeTier(pool);
      if (roleMigration.promoted) {
        console.log(`[migrate] RBAC: promoted '${roleMigration.promoted}' to super_admin — it was the oldest admin and no super_admin existed. Without this nobody could manage users or credential profiles.`);
      } else if (roleMigration.alreadyHadSuperAdmin) {
        console.log('[migrate] RBAC: a super_admin already exists — no promotion (deliberately never re-promotes a demoted account).');
      } else {
        console.log('[migrate] RBAC: no admin account found to promote.');
      }
      if (roleMigration.legacyRoles.length > 0) {
        console.log(`[migrate] RBAC: ${roleMigration.legacyRoles.length} account(s) hold a role outside the three-tier model and now have NO capabilities (read-only): ${roleMigration.legacyRoles.join(', ')}. Deliberately not auto-mapped to operator, which can write.`);
      }
    } catch (err) {
      console.log('[migrate] RBAC role migration failed: ' + err.message);
    }
  ...
  // ⛔ ONE-SHOT: gated by lib/backfillLedger.js. BUMP THE REVISION whenever
  // summarizeDiff()'s output format changes, or already-deployed servers keep
  // showing the old wording for every historical change for ever — the summary
  // is stored at write time, so improving the function alone fixes nothing
  // that already happened.
  if (!(await isDone(pool, 'change-summary-readability', 2))) {
    const __t = Date.now();
    let __ok = true;
    try {
      const { checked, updated, failed } = await regenerateAllChangeSummaries(pool);
      console.log(
        `[migrate] Change-summary readability: checked ${checked} row(s), rewrote ${updated}`
          + (failed > 0 ? `, ${failed} could not be summarised (left as they were)` : '')
          + '.'
      );
      if (failed > 0) __ok = false;
    } catch (err) {
      __ok = false;
      console.warn(`[migrate] Change-summary readability backfill failed (non-fatal): ${err.message}`);
    }
    // ⛔ markDone() takes (pool, name, revision, durationMs, summary). Omitting
    // the duration made it Math.round(undefined) = NaN, which Postgres refused
    // — so the repair ran, succeeded, and never recorded itself, and would
    // have re-run on every deploy for ever. The ledger caught it loudly rather
    // than silently marking a failed repair done, which is the direction that
    // file is built to fail in.
    if (__ok) await markDone(pool, 'change-summary-readability', 2, Date.now() - __t, null);
    console.log(`[migrate]   change-summary-readability took ${((Date.now() - __t) / 1000).toFixed(1)}s`);
  } else {
    console.log('[migrate] change-summary-readability: already done (rev 2) — skipped.');
  }

  // ⛔ ONE-SHOT: gated by lib/backfillLedger.js. The REVISION is the safety
  // mechanism — bump it in the same commit as any change to this repair’s
  // logic, or installs still holding the broken data never run the fix.
  if (!(await isDone(pool, 'palo-alto-config-redaction', 2))) {
    const __t = Date.now();
    let __ok = true;
    try {
    ...
    // ⛔ Only a CLEAN run is recorded. A failure retries next deploy; marking
    // a failed repair complete would be a failed read stored as an affirmative
    // fact, applied to this file's own maintenance.
    if (__ok) await markDone(pool, 'palo-alto-config-redaction', 2, Date.now() - __t, null);
  } else {
    console.log('[migrate] palo-alto-config-redaction: already done (rev 2) — skipped.');
  }
  ...
  // mechanism — bump it in the same commit as any change to this repair’s
  // logic, or installs still holding the broken data never run the fix.
  if (!(await isDone(pool, 'palo-alto-version-ranges', 2))) {
    const __t = Date.now();
    let __ok = true;
    try {
    ...
    // a failed repair complete would be a failed read stored as an affirmative
    // fact, applied to this file's own maintenance.
    if (__ok) await markDone(pool, 'palo-alto-version-ranges', 2, Date.now() - __t, null);
  } else {
    console.log('[migrate] palo-alto-version-ranges: already done (rev 2) — skipped.');
  }
  ...
  // mechanism — bump it in the same commit as any change to this repair’s
  // logic, or installs still holding the broken data never run the fix.
  if (!(await isDone(pool, 'nvd-native-version-ranges', 2))) {
    const __t = Date.now();
    let __ok = true;
    try {
    ...
    // a failed repair complete would be a failed read stored as an affirmative
    // fact, applied to this file's own maintenance.
    if (__ok) await markDone(pool, 'nvd-native-version-ranges', 2, Date.now() - __t, null);
  } else {
    console.log('[migrate] nvd-native-version-ranges: already done (rev 2) — skipped.');
  }


  // Retro-classify advisories.matchability from each row's own stored raw_data -- see
  // backfillAdvisoryMatchability()'s own comment. ⛔ Runs AFTER both version-range backfills
  // above on purpose: those repair ranges, and a row's matchability is classified against the
  // ranges it actually holds, so classifying first would label a row 'unmatchable' that the
  // very next step was about to give real ranges to. Best-effort and idempotent like every
  // other backfill here.
  // ⛔ NOT GATED, DELIBERATELY — and it WAS gated for a few hours today, which
  // was a mistake worth recording. This is not a one-shot repair: it is the
  // ONLY thing that ever writes advisories.matchability for the paloalto
  // vendor. lib/feeds/nvd.js and lib/feeds/fortinet.js both set the column at
  // ingest; lib/feeds/paloalto.js does not mention it at all. Gating this meant
  // every new PSIRT advisory (~3/month) would land with matchability NULL
  // forever, versionMatcher would treat it as assessable, and a
  // declared-affected-but-unextractable advisory would be silently assessed as
  // "not affected" instead of reaching the curation worklist.
  //
  // It also cost 0.2s of the 320s first run, so gating it saved nothing. The
  // forward fix is to write matchability in paloalto.js's upsert; until that
  // exists, this pass runs every deploy and that is correct.
  try {
    const { checked, labelled, rangesRepaired, byStatus, byVendor } =
      await backfillAdvisoryMatchability(pool);
    console.log(
      `[migrate] Advisory matchability backfill: classified ${checked} advisory row(s), `
        + `${labelled} label(s) written, ${rangesRepaired} row(s) had ranges recovered. `
        + `By status: ${JSON.stringify(byStatus)}. By vendor: ${JSON.stringify(byVendor)}.`
    );
  } catch (err) {
    console.warn(`[migrate] Advisory matchability backfill failed (non-fatal): ${err.message}`);
  }

  // Derive advisories.cvss_version from each row's own stored cvss_vector -- see
  // backfillCvssVersionFromVector()'s own comment. Every advisory currently renders the `v—`
  // NOT-MEASURED marker for a scale that is sitting in the same row in plain text. Best-effort
  // and idempotent like every other backfill here; it only ever fills a NULL, and a vector that
  // does not declare its own version is deliberately left NULL rather than guessed at.
  // ⛔ ONE-SHOT: gated by lib/backfillLedger.js. The REVISION is the safety
  // mechanism — bump it in the same commit as any change to this repair’s
  // logic, or installs still holding the broken data never run the fix.
  if (!(await isDone(pool, 'cvss-version-from-vector', 1))) {
    const __t = Date.now();
    let __ok = true;
    try {
      const { updated, byVersion, byVendor, stillNull } = await backfillCvssVersionFromVector(pool);
      console.log(
        `[migrate] CVSS scale backfill: derived cvss_version for ${updated} advisory row(s) from their `
          + `own vector. By version: ${JSON.stringify(byVersion)}. By vendor: ${JSON.stringify(byVendor)}. `
          + `Still NULL (correctly): ${JSON.stringify(stillNull)}.`
      );
    } catch (err) {
      __ok = false;
      console.warn(`[migrate] CVSS scale backfill failed (non-fatal): ${err.message}`);
    }
    // ⛔ Only a CLEAN run is recorded. A failure retries next deploy; marking
    // a failed repair complete would be a failed read stored as an affirmative
    // fact, applied to this file's own maintenance.
    if (__ok) await markDone(pool, 'cvss-version-from-vector', 1, Date.now() - __t, null);
  } else {
    console.log('[migrate] cvss-version-from-vector: already done (rev 1) — skipped.');
  }
  ...
  // mechanism — bump it in the same commit as any change to this repair’s
  // logic, or installs still holding the broken data never run the fix.
  if (!(await isDone(pool, 'unmeasurable-facts', 1))) {
    const __t = Date.now();
    let __ok = true;
    try {
    ...
    // a failed repair complete would be a failed read stored as an affirmative
    // fact, applied to this file's own maintenance.
    if (__ok) await markDone(pool, 'unmeasurable-facts', 1, Date.now() - __t, null);
  } else {
    console.log('[migrate] unmeasurable-facts: already done (rev 1) — skipped.');
  }


  // FortiOS IPsec-as-a-login repair. Best-effort like the others, but note
  // this one corrects PERMANENT rollups: until it runs, the fleet holds a
  // permanent record of successful VPN logins that never happened.
  // ⛔ ONE-SHOT: gated by lib/backfillLedger.js. The REVISION is the safety
  // mechanism — bump it in the same commit as any change to this repair’s
  // logic, or installs still holding the broken data never run the fix.
  if (!(await isDone(pool, 'fortinet-ipsec-auth-repair', 1))) {
    const __t = Date.now();
    let __ok = true;
    try {
      const r = await repairFortinetIpsecAuthMisclassification(pool);
      console.log(
        `[migrate] FortiOS IPsec auth repair: reclassified ${r.eventsReclassified} event(s) that were `
          + `counting a site-to-site tunnel as a user login, cleared ${r.eventsUserCleared} peer address(es) `
          + `from src_user, and removed ${r.vpnAuthRowsDeleted} fabricated VPN-auth rollup row(s) and `
          + `${r.userRollupRowsDeleted} fabricated per-user rollup row(s). Genuine SSL-VPN logins are `
          + `left untouched.`
      );
    } catch (err) {
      __ok = false;
      console.warn(`[migrate] FortiOS IPsec auth repair failed (non-fatal): ${err.message}`);
    }
    // ⛔ Only a CLEAN run is recorded. A failure retries next deploy; marking
    // a failed repair complete would be a failed read stored as an affirmative
    // fact, applied to this file's own maintenance.
    if (__ok) await markDone(pool, 'fortinet-ipsec-auth-repair', 1, Date.now() - __t, null);
  } else {
    console.log('[migrate] fortinet-ipsec-auth-repair: already done (rev 1) — skipped.');
  }
...
if (require.main === module) {
  main()
    .then(() => {
      console.log(`[migrate] Done in ${((Date.now() - __migrateStarted) / 1000).toFixed(1)}s.`);
      process.exit(0);
    })
    .catch((err) => {
...
module.exports = {
  ...
  // Exported for tests/cveMatchability.test.js — it takes a stub pool and the interesting part
  // is which rows it refuses to touch, not the DB plumbing.
  backfillAdvisoryMatchability,
  // Exported for tests/cvssVersionBackfill.test.js — same reason: with a stub pool, what is
  // worth pinning is the SQL's guards (fills a NULL only, needs a score, needs the vector to
  // DECLARE its version) and the fact that the derived value comes from that same declaration.
  backfillCvssVersionFromVector,
};
