...
async function migrateZoneClassificationsToPerDevice(pool) {
  ...
  await pool.query(`
    ...
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'zone_classifications_device_zone_key'
      ) THEN
        ALTER TABLE zone_classifications
          ADD CONSTRAINT zone_classifications_device_zone_key UNIQUE (device_id, zone_name);
      END IF;
    END $$;
  `);
