...
async function backfillPaloAltoConfigRedaction(pool) {
  ...
  for (const table of ['device_configs', 'config_backups']) {
    ...
    const { rows } = await pool.query(
      `SELECT c.id
        ...
        WHERE d.vendor = 'paloalto'
          AND (c.config_raw ~ $1 OR c.config_raw ~ $2)
        ORDER BY c.id`,
    ...
    for (const { id } of rows) {
      checked += 1;
      const cur = await pool.query(`SELECT config_raw FROM ${table} WHERE id = $1`, [id]);
      ...
      if (table === 'device_configs') {
        ...
        await pool.query(
          `UPDATE device_configs
              SET config_raw = $2,
                  content_hash = encode(
                    sha256(convert_to(coalesce($2, '') || chr(10) || coalesce(config_parsed::text, ''), 'UTF8')),
                    'hex')
            WHERE id = $1`,
          [id, res.text]
        );
      } else {
        await pool.query('UPDATE config_backups SET config_raw = $2 WHERE id = $1', [id, res.text]);
      }
      updated += 1;
  ...
  return { checked, updated, secretsRedacted, leftContainers, cliRows };
}
...
async function main() {
  ...
  try {
    const { checked, updated, secretsRedacted, leftContainers, cliRows } =
