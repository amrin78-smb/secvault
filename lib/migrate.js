...
async function main() {
  ...
  if (!(await isDone(pool, 'change-summary-readability', 1))) {
    const __t = Date.now();
    let __ok = true;
    try {
    ...
    if (__ok) await markDone(pool, 'change-summary-readability', 1, Date.now() - __t, null);
    console.log(`[migrate]   change-summary-readability took ${((Date.now() - __t) / 1000).toFixed(1)}s`);
  } else {
    console.log('[migrate] change-summary-readability: already done (rev 1) — skipped.');
  }
