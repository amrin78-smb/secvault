...
async function main() {
  ...
  // mechanism — bump it in the same commit as any change to this repair’s
  // logic, or installs still holding the broken data never run the fix.
  if (!(await isDone(pool, 'palo-alto-version-ranges', 1))) {
    const __t = Date.now();
    let __ok = true;
    try {
    ...
    // a failed repair complete would be a failed read stored as an affirmative
    // fact, applied to this file's own maintenance.
    if (__ok) await markDone(pool, 'palo-alto-version-ranges', 1, Date.now() - __t, null);
  } else {
    console.log('[migrate] palo-alto-version-ranges: already done (rev 1) — skipped.');
  }
  ...
  // mechanism — bump it in the same commit as any change to this repair’s
  // logic, or installs still holding the broken data never run the fix.
  if (!(await isDone(pool, 'nvd-native-version-ranges', 1))) {
    const __t = Date.now();
    let __ok = true;
    try {
    ...
    // a failed repair complete would be a failed read stored as an affirmative
    // fact, applied to this file's own maintenance.
    if (__ok) await markDone(pool, 'nvd-native-version-ranges', 1, Date.now() - __t, null);
  } else {
    console.log('[migrate] nvd-native-version-ranges: already done (rev 1) — skipped.');
  }
  ...
  // mechanism — bump it in the same commit as any change to this repair’s
  // logic, or installs still holding the broken data never run the fix.
  if (!(await isDone(pool, 'advisory-matchability', 1))) {
    const __t = Date.now();
    let __ok = true;
    try {
    ...
    // a failed repair complete would be a failed read stored as an affirmative
    // fact, applied to this file's own maintenance.
    if (__ok) await markDone(pool, 'advisory-matchability', 1, Date.now() - __t, null);
  } else {
    console.log('[migrate] advisory-matchability: already done (rev 1) — skipped.');
  }
