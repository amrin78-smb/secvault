...
async function getDatabaseSize(pool) {
  const out = { totalBytes: null, tables: [], deadTuples: [], error: null };
  try {
  ...
  } catch { /* the total above is the headline; a missing breakdown is not fatal */ }
  try {
...
async function getSyslogRetention(pool) {
  try {
    ...
    return { partitions: null, oldestDay: null, newestDay: null, bytes: null, error: err.message };
  }
}
...
async function getIngestHealth(pool, minutes = 15) {
  const m = Number.isFinite(Number(minutes)) ? Math.min(Math.max(Number(minutes), 1), 1440) : 15;
  try {
    const { rows } = await pool.query(
    ...
    return {
      ...
      dropped: r.dropped === null ? null : Number(r.dropped),
    ...
    return { windowMinutes: m, received: null, dropped: null, backlog: null, error: err.message };
  }
}
...
function diskState(vol) {
  if (!vol || vol.freeBytes === null || vol.freeBytes === undefined) return null;
