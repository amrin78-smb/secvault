...
const FEED_LABELS = {
  // ⛔ The label says CENTRAL, not "hub": on a site whose egress cannot reach
  // NVD this is the only source of usable version ranges, and an operator
  // reading the banner needs to know which line is carrying the product.
  cve_hub: 'Central CVE feed',
  nvd: 'NVD',
  kev: 'CISA KEV',
...
// ⛔ 'skipped' MUST BE LISTED. Without an entry it fell through to the
// unknown-status rank (1) and rendered DEGRADED — the exact opposite of what a
// deliberate skip means, and of what this product documents. The bug was latent
// only because nothing had ever been skipped on the reference deployment: both
// PSIRT vendors are in the inventory, so planVendorPsirts never skipped either.
// It sorts AFTER success because a skip is not a result at all: it is the system
// correctly choosing not to run, and it must never colour a status pill.
// ⛔ 'blocked' MUST BE LISTED, for the same reason 'skipped' must (see below):
// an unlisted status falls through to rank 1 and sorts with the FAILURES, which
// is the permanent-amber outcome this state exists to remove. It ranks ABOVE
// success because, like a skip, it is not a verdict about our data at all --
// the publisher refused us, and whether the corpus is complete depends on the
// other feeds.
const FEED_STATUS_ORDER = {
  failed: 0, error: 0, partial: 1, running: 2, success: 3, skipped: 4, blocked: 5,
};
...
// Compliance standard keys are DB values (audit_checks.standards is a TEXT[]
// of these), and they surface raw in badge form on the finding detail page and
// the zone banner. These are the names the standards actually go by.
//
// ⛔ An unrecognised key falls through to titleCase, never to a guess. A new
// standard added to the seed library shows as "Nist Csf" until someone adds it
// here — visibly unpolished, which is the prompt to add it, rather than
...
function standardLabel(key) {
  ...
  return STANDARD_LABELS[key.toUpperCase()] || titleCase(key);
}
