...
function filtersFor(finding) {
  ...
  switch (f.kind) {
    ...
    // A username seen from a country absent from its own history. Both halves
    // are single values, so both are filtered.
    case 'new_country_for_user':
      if (isNonEmpty(f.username)) out.srcUser = f.username.trim();
      if (isNonEmpty(f.country)) out.srcCountry = f.country.trim();
      break;
