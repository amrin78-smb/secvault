...
function sshCommandOverride(repoRoot) {
  const keyPath = path.join(repoRoot, 'installer', 'dependencies', 'secvault_deploy');
  if (!fs.existsSync(keyPath)) return null;
  // IdentitiesOnly: only offer this key, matching Install-SecVault.ps1's own
  // config entry's intent. StrictHostKeyChecking=accept-new: same
  // trust-on-first-use-then-verify posture Install-SecVault.ps1 already
  // uses, not a weaker one. BatchMode=yes: never hang on an interactive
  // prompt if something unexpected happens (host key mismatch, etc.) — fail
  // fast instead, consistent with GIT_TERMINAL_PROMPT=0 above.
  return `ssh -i "${keyPath}" -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new -o BatchMode=yes`;
}
