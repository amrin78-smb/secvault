...
//
// ⛔ `start` DEFAULTS, AND THAT DEFAULT IS A BUG FIX, NOT TIDYING. Two routes
// called this as `findGitRoot() || process.cwd()`, which reads as handled and
// is not: with `start` undefined, `path.join(undefined, '.git')` THROWS, so the
// `|| process.cwd()` fallback can never be reached — a guard that cannot fire.
// Measured on the live server 2026-09-22: GET /api/system/console-url answered
// HTTP 500 with an empty body, so Settings → Certificate → Console address
// (whose entire purpose is that setting it "no longer means editing a file over
// RDP") could only be set by editing a file over RDP. The idle-timeout PUT was
// dead for the same reason, while its GET worked — because only the write path
// resolves the env file, which is exactly why nothing noticed.
//
// Defaulting here rather than at the two call sites fixes every future caller
// too, including one written by someone copying the same line again.
// ⛔ AND THE NOT-FOUND PATH RETURNED `start`, NOT THE NORMALISED VALUE — so
// the whole fix above held only while a `.git` was found. `findGitRoot(null)`
// returned NULL and `findGitRoot('')` returned '', i.e. exactly the values the
// default and the `|| process.cwd()` above exist to rule out, on the one
// deployment shape the comments here explicitly anticipate: "a non-git on-prem
// deploy". The regression test passed anyway, because this checkout HAS a
// .git, so the loop returned early and the broken line never ran — a guard
// tested only on the path where it cannot fire. The fallback is resolved ONCE,
// into `root`, and both exits return it.
function findGitRoot(start = process.cwd()) {
  const root = start || process.cwd();
  let dir = root;
  for (let i = 0; i < 6; i++) {
...
function sshCommandOverride(repoRoot) {
  ...
  // ⛔ SECOND root cause found 2026-07-21 (see installer/Update-SecVault.ps1's
  // identical fix for the full story): `core.sshCommand`'s value is always
  // interpreted by git's own bundled MSYS2 shell, which treats backslash as
  // an escape character in an unquoted word -- silently stripping every `\`
  // from a native Windows path before ssh ever sees it. The double-quoting
  // already used below happens to shield THIS file from the worst of it
  // (backslash isn't special to a POSIX shell inside double quotes unless
  // followed by $, `, ", \, or a newline -- none of which occur in these
  // paths), but converting to forward slashes here too removes any doubt and
  // matches the same fix applied to the PowerShell script that builds the
  // identical kind of command line, rather than relying on that quoting
  // subtlety holding forever.
  const sshBinarySlash = sshBinary.replace(/\\/g, '/');
  const keyPathSlash = keyPath.replace(/\\/g, '/');
  const knownHostsPathSlash = knownHostsPath.replace(/\\/g, '/');
  return (
    `"${sshBinarySlash}" -i "${keyPathSlash}" -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new ` +
    `-o UserKnownHostsFile="${knownHostsPathSlash}" -o BatchMode=yes`
  );
}
