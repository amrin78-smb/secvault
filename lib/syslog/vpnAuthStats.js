...
// ── Presentation grouping: flagged sources, folded into countries ─────────
//
// ⛔ THIS CHANGES NOTHING ABOUT WHAT IS FLAGGED. findUsernameSprayers() above
// is the only rule and it is untouched, as is findFailureOnlyCountries()'s
// per-vendor gate. Everything below only decides how the SAME set of flagged
// sources is arranged on screen.
//
// It exists because the flat list reached 247 stanzas on the live fleet — one
// per flagged address — which no operator can read. Folding them into countries
// turns that into 11 rows.
//
// ⛔ THE FOLD MUST NOT HIDE THE WORST OFFENDER. Measured live: ONE Bulgarian
// address accounts for 1,680 failures against 834 distinct usernames, while the
// United States contributes 172 separate addresses totalling 3,051. A plain
// "biggest country first" list with everything behind a click would bury the
// single most dangerous address on the page two levels down — a worse product
// than the scrolling it replaces. So every country carries its WORST SOURCE as
// a preview, rendered with no interaction at all.

// How many addresses one country lists before it says "showing N of M".
// ⛔ Never a silent cut — `hiddenSources` is returned so the caller must say
// what it left out. The United States alone has 172 flagged addresses here.
const SOURCES_PER_COUNTRY = 25;

/**
 * Relative heat band for a country row's bar.
 *
 * ⛔ RELATIVE, NOT ABSOLUTE. `critical` here means "at least half the failure
 * volume of the worst country in this window", NOT `critical` in the sense
 * rule_analysis_results.severity uses. It is a comparison inside one window,
 * and the row prints its own raw numbers beside the bar so the colour never
 * has to be read as a standalone claim.
 *
 * ⛔ Returns null — not 'low' — when there is nothing to compare against. A
 * zero maximum means there is no denominator, and drawing the bottom of a ramp
 * would render "no basis for comparison" as "lowest risk", which is this
 * codebase's failed-read-as-a-fact bug in paint.
 *
 * @returns {'critical'|'high'|'medium'|'low'|null}
 */
function heatBand(failures, maxFailures) {
  const f = Number(failures);
  const max = Number(maxFailures);
  if (!Number.isFinite(f) || !Number.isFinite(max) || max <= 0 || f <= 0) return null;
  const share = f / max;
...
/**
 * Fold a list of flagged sources into country groups.
 *
 * @param {Array} sources  the OUTPUT of findUsernameSprayers(), unchanged
 * @param {{perCountry?: number}} [opts]
 * @returns {{countries, totalCountries, totalSources, totalFailures, maxFailures}}
 *
 * Each country row: { country, located, failure, sourceCount, worst, sources,
 *                     hiddenSources, band }
 *
 * ⛔ GROUPS ON THE NORMALISED COUNTRY. `getVpnLoginLocations()` already folded
 * ISO codes to names, so Palo Alto's "US" and FortiOS's "United States" arrive
 * here as one value. Grouping on the raw vendor field splits the United States
 * into two rows and under-states both — the exact bug the ISO map at the top of
 * this file was written for.
 *
 * ⛔ A SOURCE WITH NO RESOLVABLE COUNTRY GETS ITS OWN GROUP (`located:false`),
 * never a real country's, and never a made-up "Unknown" country that would sit
 * in the ranking looking like a place. Its failures ARE measured, so it is
 * still ranked by them; only its location is unknown, and the caller draws that
 * group with no hue.
 */
function groupSourcesByCountry(sources, opts = {}) {
  const perCountry = Math.max(1, Math.trunc(Number(opts.perCountry) || SOURCES_PER_COUNTRY));
  const list = Array.isArray(sources) ? sources : [];

  const byCountry = new Map();
  for (const s of list) {
    const located = typeof s.country === 'string' && s.country.trim() !== '';
    // ⛔ ONE key for every unlocated source, and it is `null`, not a string —
    // a null key cannot collide with a vendor that literally writes "Unknown"
    // or "N/A" into the country field.
    const key = located ? s.country : null;
    const g = byCountry.get(key) || {
      country: located ? s.country : null,
      located,
      failure: 0,
      sourceCount: 0,
      sources: [],
    };
    g.failure += Number(s.failure) || 0;
    g.sourceCount += 1;
    g.sources.push(s);
    byCountry.set(key, g);
  }
  ...
  const countries = [...byCountry.values()]
    .map((g) => {
      // Worst-first inside the country, ordered on the same quantity the flag
      // itself turns on.
      const sorted = g.sources
        .slice()
        .sort(
          (a, b) =>
            (b.failure - a.failure) ||
            (b.usernames - a.usernames) ||
            String(a.srcIp).localeCompare(String(b.srcIp))
        );
      return {
        country: g.country,
        located: g.located,
        ...
        // The preview. Never null for a group that exists — a group only
        // exists because at least one source created it.
        worst: sorted[0] || null,
        sources: sorted.slice(0, perCountry),
        hiddenSources: Math.max(0, sorted.length - perCountry),
      };
    })
    // ⛔ The unlocated group is ranked with everything else, NOT pinned to the
    // bottom. Sinking it would let a large unattributed burst sit below eleven
    // small countries purely because the firewall did not name its geography.
    .sort(
      (a, b) =>
        (b.failure - a.failure) ||
  ...
  const maxFailures = countries.reduce((m, c) => Math.max(m, c.failure), 0);
  for (const c of countries) c.band = heatBand(c.failure, maxFailures);

  return {
    countries,
    totalCountries: countries.length,
    totalSources: list.length,
    totalFailures: countries.reduce((n, c) => n + c.failure, 0),
    maxFailures,
  };
}
