...
const INBOUND_INSERT = `
  ...
  SELECT s.bucket_hour, s.device_id, s.dst_ip, s.dst_port, s.protocol,
         CASE
           WHEN lower(s.action) IN ('allow','accept','permit','start','close',
                                    'client-rst','server-rst') THEN true
           WHEN lower(s.action) IN ('deny','drop','block','blocked','denied',
                                    'block-url','block-ip','block-continue',
                                    'block-override','reset-both','reset-client',
                                    'reset-server','timeout') THEN false
           ELSE NULL
         END,
