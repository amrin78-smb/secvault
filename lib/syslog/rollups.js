...
const USER_INSERT = `
  INSERT INTO syslog_user_hourly
    (bucket_hour, device_id, src_user, event_count, denied_count, bytes_sent, bytes_received)
  SELECT bucket_hour, device_id, src_user,
         count(*),
         count(*) FILTER (WHERE action IN ('deny','drop','reset-both','block')),
         sum(bytes_sent) FILTER (WHERE bytes_summable),
         sum(bytes_received) FILTER (WHERE bytes_summable)
...
async function recomputeWindow(pool, from, to) {
  ...
  const result = {
    ...
    countryRows: 0, userRows: 0, urlCatRows: 0,
    ms: 0, error: null,
  };
