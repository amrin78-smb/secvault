...
const HOURLY_INSERT = `
  ...
  SELECT date_trunc('hour', received_at), source_ip, device_id, vendor, action, severity,
         count(*), sum(bytes_sent), sum(bytes_received), now()
    FROM syslog_events
   WHERE received_at >= $1 AND received_at < $2
   GROUP BY 1, 2, 3, 4, 5, 6`;

const RULE_DELETE = `
  DELETE FROM syslog_rule_hits_daily
...
const RULE_INSERT = `
  INSERT INTO syslog_rule_hits_daily
  ...
  SELECT received_at::date, device_id, source_ip, vendor, rule_id, rule_uuid, rule_name, action,
         count(*), sum(bytes_sent), sum(bytes_received), min(received_at), max(received_at)
    FROM syslog_events
   WHERE received_at >= $1 AND received_at < $2
