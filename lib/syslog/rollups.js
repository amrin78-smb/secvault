...
const TALKER_INSERT = `
  ...
  SELECT date_trunc('hour', received_at), device_id, src_ip,
         count(*),
...
const APP_INSERT = `
  ...
  SELECT date_trunc('hour', received_at), device_id, application, protocol,
         count(*),
         sum(bytes_sent) FILTER (WHERE bytes_summable),
         sum(bytes_received) FILTER (WHERE bytes_summable)
    FROM syslog_events
   WHERE received_at >= $1 AND received_at < $2
     AND (application IS NOT NULL OR protocol IS NOT NULL)
   GROUP BY 1, 2, 3, 4`;
...
const BLOCKED_INSERT = `
  ...
  SELECT date_trunc('hour', received_at), device_id, dst_ip, dst_port, protocol, count(*)
    FROM syslog_events
   WHERE received_at >= $1 AND received_at < $2
...
async function recomputeWindow(pool, from, to) {
  ...
  try {
    ...
    // The three detail rollups. Each gets its own transaction for the same
    // reason as the two above: a reader must never see a bucket mid-rebuild.
    for (const [name, del, ins, key] of [
      ['talker', TALKER_DELETE, TALKER_INSERT, 'talkerRows'],
      ['app', APP_DELETE, APP_INSERT, 'appRows'],
      ['blocked', BLOCKED_DELETE, BLOCKED_INSERT, 'blockedRows'],
    ]) {
      try {
        await client.query('BEGIN');
        await client.query(del, [from, to]);
        const res = await client.query(ins, [from, to]);
