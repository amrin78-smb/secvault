 ...
 */
// ⛔ syslog_ingest_stats HAD NO RETENTION ANYWHERE -- not in the list below,
// not a partition drop, not a DELETE in the whole repo. One row per flush on a
// 2-second cycle is ~43,000 rows a day, for ever. It is small per row, which is
// exactly why nothing noticed; it is also the table the ingest_drop alert and
// the dashboard Server tab both read, so it is the last place a slow scan
// should be allowed to appear.
//
// ⛔ IT NEEDS ITS OWN STATEMENT, not a row in the list below: that loop keys on
// `bucket_hour` and this table records `recorded_at`. Adding it there would
// have thrown on an undefined column -- loudly, which is the one mercy.
//
// ⛔ THE 7-DAY FLOOR IS NOT A TUNING KNOB. fetchOpenIngestDrop looks back 48
// hours and getIngestHealth accepts a window up to 24; trimming below that
// would leave an alert unable to see the incident it exists to report.
const INGEST_STATS_MIN_DAYS = 7;

async function trimIngestStats(pool, days) {
  const keepDays = Math.max(Number(days) || INGEST_STATS_MIN_DAYS, INGEST_STATS_MIN_DAYS);
  try {
    const r = await pool.query(
      `DELETE FROM syslog_ingest_stats WHERE recorded_at < now() - ($1::int * interval '1 day')`,
      [keepDays]
    );
    return { table: 'syslog_ingest_stats', deleted: r.rowCount, keepDays };
  } catch (err) {
    // Housekeeping must never fail the maintenance pass around it.
    return { table: 'syslog_ingest_stats', deleted: null, keepDays, error: err.message };
  }
}
...
async function trimDetailRollups(pool, retentionDays) {
  ...
  // succeeded.
  //
  // syslog_threat_hourly was exactly that omission: measured on the live fleet
  // it had reached 119,404 rows / 70 MB in 27 hours (~22 GB/year) with nothing
  // anywhere ever deleting a row from it, while this function logged a
  // successful trim every cycle.
  //
  // ⛔ minDays is a FLOOR, not a second knob. Every reader of a table listed
  // here clamps its own window, and the trim must never delete a bucket a UI
  // can still ask for — that would answer a legitimate "last 7 days" question
  // with a partial series and no indication it was partial. lib/syslog/
  // threatStats.js clamps every query to 24*8 hours, so the threat rollup keeps
  // at least 8 days no matter how low SYSLOG_DETAIL_RETENTION_DAYS is set.
  // Re-check the reader before lowering a floor.
  for (const { table: t, minDays } of [
    { table: 'syslog_talker_hourly', minDays: 1 },
    { table: 'syslog_app_hourly', minDays: 1 },
    { table: 'syslog_blocked_dst_hourly', minDays: 1 },
    { table: 'syslog_country_hourly', minDays: 1 },
    { table: 'syslog_user_hourly', minDays: 1 },
    { table: 'syslog_urlcat_hourly', minDays: 1 },
    { table: 'syslog_device_inbound_hourly', minDays: 1 },
    { table: 'syslog_vpn_auth_hourly', minDays: 1 },
    { table: 'syslog_threat_hourly', minDays: 8 },
  ]) {
    const tableDays = Math.max(days, minDays);
    try {
