...
const THREAT_INSERT = `
  INSERT INTO syslog_threat_hourly
    (bucket_hour, device_id, src_ip, dst_ip, threat_name, threat_severity,
     src_country, event_count, first_seen_at, last_seen_at)
...
async function recomputeWindow(pool, from, to) {
  ...
  try {
    ...
    try {
      ...
      // The 4th element says whether the INSERT needs [from, to]. Every pass
      // reading rollup_src is already bounded by the temp table and takes no
      // parameters; the threat pass reads syslog_events directly and does.
      for (const [del, ins, key, insNeedsWindow] of [
        ...
        [THREAT_DELETE, THREAT_INSERT, 'threatRows', true],
      ]) {
        await client.query(del, [from, to]);
        const res = insNeedsWindow
          ? await client.query(ins, [from, to])
