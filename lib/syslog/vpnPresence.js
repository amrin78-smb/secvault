...
async function getVpnUserPresence(pool, opts = {}) {
  ...
  return {
    ...
    // ⛔ `durationGap` is USER-FACING TEXT ABOUT ANOTHER SUBSYSTEM, so it goes
    // stale without anything here changing. It claimed session history "is not
    // collected today" for three hours after vpn_sessions shipped and was
    // already holding rows — a static check cannot catch that, and only a
    // browser render surfaced it. If VPN session retention changes, correct
    ...
    notes: {
      ...
      durationGap:
        ...
        + 're-authenticates every 30 minutes counts as several. True connected duration is '
        + 'measured separately, from VPN session history (vpn_sessions), which began '
        + 'collecting on 2026-09-10.',
    },
  };
}
