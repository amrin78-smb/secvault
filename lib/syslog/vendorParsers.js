...
function parseFortinet(message) {
  ...
  return {
    ...
    srcUser: meaningful(f.user) || null,
    ...
    // traffic. Measured live over 12h on this fleet: 2,037 ssl-login-fail
    // against ~4 successes — its success logids are effectively absent, which
    // is a DEVICE-SIDE logging setting, not a gap SecVault can close. Any UI
    // must render that as "not reported", never as 0.
    authOutcome: logClass === 'vpn' ? classifyAuthOutcome('fortinet', null, null, f.action) : null,
    // ⛔ VPN rows carry the remote peer as `remip=`, NOT `srcip=` — which is why
