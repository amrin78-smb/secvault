...
async function getTopRules(pool, days = 1, limit = 10, deviceId = null) {
  ...
  const { rows } = await pool.query(
    `SELECT coalesce(h.rule_name, h.rule_id) AS rule,
      ...
      WHERE h.bucket_hour >= date_trunc('day', now()) - ($1::int - 1) * interval '1 day'${deviceClauseAliased(deviceId, 'h', 3)}
      GROUP BY 1, 2, 3, 4
      ORDER BY hits DESC
      LIMIT $2`,
...
async function getDeviceInboundHits(pool, deviceId, hours = 24, limit = 10) {
  ...
  const { rows } = await pool.query(
    `SELECT i.dst_ip::text AS dst_ip, i.dst_port, i.protocol,
            ...
            sum(i.distinct_sources)::bigint AS sources,
      ...
      ORDER BY (i.public_source AND i.allowed) DESC, events DESC
      LIMIT $2`,
    [h, lim, deviceId]
  );
  return rows.map((r) => ({
    ...
    events: Number(r.events),
    sources: r.sources === null ? null : Number(r.sources),
    lastSeen: r.last_seen || null,
  }));
}
...
async function getDeviceSyslogCoverage(pool, deviceId, hours = 24) {
  ...
  const { rows } = await pool.query(
    `SELECT
       (SELECT count(*) FROM syslog_rollup_hourly WHERE device_id = $1::uuid LIMIT 1) > 0 AS ever_sent,
