...
async function exportEvents(pool, filters, now, opts = {}) {
  ...
  while (cursor > asked.from) {
    ...
    if (rows.length > max) {
      // The probe row proves at least one more match exists than the ceiling
      // allows. Drop it and stop; the file covers [cursor, asked.to] fully.
      rows.length = max;
  ...
  // ⛔ THE ONLY REFUSAL LEFT IS "NOTHING AT ALL COULD BE READ". Anything
  // else is a complete file for a stated window. A refusal here is honest
  // because there is no window to name: not one slice came back.
  if (stopReason === 'timed_out' && rows.length === 0) {
    return {
      ok: false,
      reason: 'timed_out',
      detail:
  ...
  const shortened = coveredFrom.getTime() > asked.from.getTime();

  return {
    ok: true,
    csv: renderEventsCsv(rows, { deviceNames }),
    filename: exportFilename({ from: coveredFrom, to: asked.to }, rest, { shortened }),
