...
function resolveTlsConfig(env, fsImpl) {
  ...
  const io = fsImpl || fs;
  // Injectable alongside fsImpl so a test can exercise the failure path without
  // real key material; defaults to the real TLS stack, which is the whole point.
  const makeContext = (fsImpl && fsImpl.createSecureContext) || tls.createSecureContext;
  ...
  try {
    ...
    // ⛔ ACTUALLY LOAD THE MATERIAL. Everything above proves only that two
    // non-empty files exist, and `active` used to mean exactly that — while
    // https.createServer() builds its SecureContext SYNCHRONOUSLY and THROWS on
    // anything OpenSSL cannot parse. server.js's main().catch then exits 1 and
    // NSSM restarts it, forever.
    //
    // So the one corruption class that degraded safely was the one that cannot
    // happen quietly (an empty file), and the classes that actually happen in
    // the field — a DER .cer saved over the .crt, a truncated download, a
    // restored .bak pair whose halves do not match — took the console down
    // permanently, on a reboot, with no rollback in the path because a reboot
    // never runs Update-SecVault.ps1.
    //
    // That inverted this product's stated rule: a broken certificate must
    // DEGRADE to HTTP, and degrading is only safe because `failed` is loud.
    // Building the context here is what makes `active` mean "TLS will load".
    try {
      makeContext({ cert, key });
    } catch (err) {
      return {
        ...base,
        status: 'failed',
        // ⛔ The OpenSSL code is the useful half (ERR_OSSL_PEM_NO_START_LINE =
        // not PEM at all; ERR_OSSL_X509_KEY_VALUES_MISMATCH = wrong key), so it
        // is kept rather than flattened into "certificate invalid".
        error: `The certificate and key could not be loaded (${err.code || 'unknown error'}): ${err.message}`,
      };
    }
