...
// net-snmp defaults `backoff` to 1.0 (no growth) when unset, as it is here,
// so its own worst-case natural duration before it calls back with a
// RequestTimedOutError is timeoutMs * (DEFAULT_RETRIES + 1) — the margin
// MUST exceed the retries' share of that (timeoutMs * DEFAULT_RETRIES) or
// this outer race fires mid-retry on every ordinary timeout, not just the
// rare "callback never fires" case the paragraph above describes. Extra
// flat 3000ms on top is the actual backstop buffer.
function hardTimeoutMs(timeoutMs) {
  return timeoutMs + timeoutMs * DEFAULT_RETRIES + 3000;
}
...
function withHardTimeout(promise, timeoutMs, label, host) {
  let timer;
  const totalMs = hardTimeoutMs(timeoutMs);
  const timeoutPromise = new Promise((_, reject) => {
    timer = setTimeout(() => {
      reject(
        new Error(
          `SNMP ${label} to ${host} timed out after ${totalMs}ms — no response. ` +
