...
const { VALID_AUTH_PROTOCOLS, VALID_PRIV_PROTOCOLS } = require('./adapters/snmpCredential');
...
function buildProfilePlaintext(
  ...
  if (credentialType === 'snmp') {
    if (snmpVersion === 'v3') {
      ...
      // Mirrors app/api/devices/[id]/snmp/route.js's PUT handler exactly:
      // authProtocol only ever gets set (defaulting to 'SHA') when an
      // authPassword is actually supplied, validated against the allowed
      // enum; privProtocol/privPassword are only ever set when BOTH
      // authPassword AND privPassword are present, enforcing SNMPv3's
      // auth-before-priv requirement.
      const hasPriv = Boolean(authPassword && privPassword);
      return JSON.stringify({
        version: 'v3',
        username,
        authProtocol: authPassword
          ? (VALID_AUTH_PROTOCOLS.includes(authProtocol) ? authProtocol : 'SHA')
          : null,
        authPassword: authPassword || null,
        privProtocol: hasPriv
          ? (VALID_PRIV_PROTOCOLS.includes(privProtocol) ? privProtocol : 'AES')
          : null,
        privPassword: hasPriv ? privPassword : null,
      });
    }
