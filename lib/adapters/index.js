...
const { verifyRequestsForDevice } = require('../engines/ruleChangeRequests');
...
async function collectAndStore(device, pool) {
  ...
  const result = {
    ...
    // Set only when a rules pull succeeded and rule-cleanup verification
    // actually ran: {checked, removed, stillPresent, unverifiable, error}.
    // Stays null when the rules pull failed — "we did not look" is not "nothing
    // was found".
    ruleChangeVerification: null,
    errors: [],
  };
  ...
  // ⛔ devices.last_rules_collected_at — the ONLY place it is ever written, and
  // written ONLY on the success path of the rules pull above (the transaction
  // COMMITted, so firewall_rules now holds THIS pull's ruleset).
  //
  // It exists because devices.last_collected_at cannot answer the question the
  // rule-cleanup loop turns on: last_collected_at is stamped when ANY
  // capability succeeded (version, config, interfaces...), so a device whose
  // RULE pull has been failing for a week still looks freshly collected.
  //
  // ⛔ NEVER stamp it when getRules() threw. The adapter contract is that
  // getRules() THROWS on a retrieval failure and never returns [] — precisely
  // so a failed pull cannot be mistaken for an empty ruleset. Stamping on
  // failure would resurrect that bug one level up: verifyRequestsForDevice()
  // reads this column to decide whether "the rule is absent" means anything,
  // and a stamp written after a failed pull would report every requested rule
  // as removed. That turns a collection outage into a fabricated success.
  //
  // Deliberately a separate statement AFTER the transaction rather than part of
  // it: a failure to stamp must never roll back a successfully collected
  // ruleset. If this UPDATE fails the column simply stays older, and
  // verification degrades to 'unverifiable' — the safe direction.
  if (rulesCollected) {
    try {
      await pool.query(
        `UPDATE devices
            SET last_rules_collected_at = now(),
                updated_at = now()
          WHERE id = $1`,
        [device.id]
      );
    } catch (err) {
      result.errors.push(`rules collected stamp: ${err.message}`);
    }
  }
  ...
  // Rule-cleanup verification (the verify half of the change-request loop —
  // listing removable rules is what ManageEngine Firewall Analyzer already
  // does; stating whether they actually went is what SecVault can do because it
  // re-collects the ruleset).
  //
  // ⛔ ORDERING IS LOAD-BEARING, in two ways, and both point the same way:
  //   1. It runs AFTER the last_rules_collected_at stamp above. That column is
  //      the entire basis on which verifyRequestsForDevice() decides an absence
  //      is meaningful — it requires a rules pull that succeeded STRICTLY after
  //      the request was submitted. Reading it before writing it would judge
  //      this pull against the previous pull's timestamp.
  //   2. It runs AFTER the ruleset was reinserted. firewall_rules is
  //      DELETE+reinserted per successful pull, so "absent" only means anything
  //      once this pull's rows are in place.
  // Gated on rulesCollected for the same reason the Phase 5 gate exists: after
  // a FAILED pull, firewall_rules still holds the PREVIOUS ruleset and the
  // stamp is (correctly) stale, so there is no new evidence to weigh —
  // re-reading it would only churn item outcomes without learning anything.
  //
  // ⛔ A bookkeeping problem here must never break a collection run or lose a
  // ruleset. verifyRequestsForDevice() already never throws (it returns its
  // error in the summary); the try/catch is belt-and-braces for that contract.
  if (rulesCollected) {
    try {
      const verification = await verifyRequestsForDevice(pool, device.id);
      result.ruleChangeVerification = verification;
      if (verification && verification.error) {
        result.errors.push(`rule change verification: ${verification.error}`);
      }
    } catch (err) {
      result.errors.push(`rule change verification: ${err.message}`);
    }
  }
  ...
  // getObjects() runs.
  // ⛔ THE USAGE ANALYSIS DOES NOT RUN HERE. It runs AFTER the topology block
  // below, because `nat_rules` became a reference surface for it on
  // 2026-09-25 and that block is what collects them. See the gate there.
  ...
  // ── Object usage analysis — DELIBERATELY AFTER the topology block ────────
  //
  // ⛔ BUG FIXED 2026-07-18: usage analysis used to run unconditionally, even
  // when object collection threw and storeObjects() never ran — recomputing
  // object_analysis_results from a STALE network_objects catalog against this
  // pull's FRESH firewall_rules. Mismatched-freshness inputs produce actively
  // WRONG verdicts, not merely stale ones (an object renamed on the device:
  // the stale catalog still has the old name, current rules reference the new
  // one, so the old-named object gets a fresh "unused" verdict that
  // misrepresents a rename as an abandonment).
  //
  // ⛔ MOVED HERE 2026-09-25, AND THE GATE GREW A THIRD INPUT. `nat_rules` is
  // now a reference surface for this analysis, but it is collected by the
  // topology block ABOVE — so while this ran inside the getObjects block it
  // was reading the PREVIOUS pull's NAT against this pull's objects and rules.
  // That is the same mismatched-freshness defect the 2026-07-18 comment
  // describes, one table over, and its dangerous direction is specific:
  //
  //   • a NAT rule DELETED this cycle merely keeps an object OFF the unused
  //     list — we under-report, which is safe;
  //   • a NAT rule ADDED this cycle is invisible, so the object it references
  //     is reported UNUSED — a deletion suggestion for an object NAT depends
  //     on. That is the direction that costs something.
  //
  // So the analysis needs all three inputs from THIS cycle. A device whose
  // adapter has no getNatRules() has no NAT to be inconsistent with, which is
  // a genuine "nothing to collect", not a failure — and is why the gate tests
  // the CAPABILITY rather than assuming every device reports NAT.
  if (typeof adapter.getObjects === 'function') {
    const natCapable = typeof adapter.getNatRules === 'function';
    const natConsistent = !natCapable || result.natRulesCollected === true;

    if (objectsCollected && natConsistent) {
      try {
    ...
    } else if (!objectsCollected) {
      result.errors.push(
        'object usage analysis: skipped — object collection failed; previous objects and findings left untouched'
      );
    } else {
      // ⛔ Named separately from the objects case: the operator's next step is
      // different, and a shared message would send them to look at the wrong
      // collection.
      result.errors.push(
        'object usage analysis: skipped — NAT rule collection failed, and NAT is a reference '
        + 'surface for it; analysing this pull\'s objects against the previous pull\'s NAT could '
        + 'report an in-use object as unused. Previous findings left untouched'
      );
    }
  }
