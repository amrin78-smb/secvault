...
async function collectAndStore(device, pool) {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO device_versions (device_id, version_string, version_tuple, build, model, serial)
  ...
  if (typeof adapter.getObjects === 'function') {
    ...
    // Usage analysis runs whenever ANY object collection was attempted this
    // pull (even a partial/failed one) so object_analysis_results never sits
    // stale relative to whatever network_objects currently holds — mirrors
    // runObjectUsageAnalysisForDevice()'s own "zero objects is a legitimate
    // state, clears stale findings" contract, see lib/engines/objectUsage.js.
    try {
