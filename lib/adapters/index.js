...
async function collectAndStore(device, pool) {
  ...
  // method failing must never block the others (unlike getRules()'s
  // fail-loud DELETE+reinsert contract — these tables have no
  // destructive-empty risk, see interface.js's header comment).
  ...
  if (typeof adapter.getLicenses === 'function') {
    try {
      ...
      await storeDeviceLicenses(device.id, licenses, pool);
  ...
  if (typeof adapter.getDiskUsage === 'function') {
    try {
      const { filesystems } = await adapter.getDiskUsage();
      await storeDeviceDiskUsage(device.id, filesystems, pool);
  ...
  if (result.version && Array.isArray(result.version.contentVersions)) {
    try {
