...
//
// ⚠️ DOC-DERIVED, RESPONSE SHAPE STILL UNCONFIRMED. ⛔ The original wording
// here ("this codebase has no live PAN-OS device") is now FALSE — there are 11
// live Palo Altos in the fleet. What is still true is narrower and more
// specific: hit-count enrichment reports 0 on every rule, and nobody has yet
// read the `[PaloAlto Debug] rule-hit-count raw response` line to find out
// whether the command is failing, the parse is missing, or the devices
// genuinely report zeroes. Treat "unverified" as an OPEN QUESTION with a
// known way to answer it, not as "unverifiable". `show rule-hit-count` is a real, documented PAN-OS
