...
// EVERY error string leaving this module must pass through this — the key travels
// as a `key=` query param, so a raw error message is a credential disclosure into
// engine.log and into the /api/devices/[id]/test HTTP response body.
function redactKey(text, apiKey) {
  if (!text) return text;
  let out = String(text);
  if (apiKey) {
    // 1. The bare key (non-URL contexts).
    out = out.split(apiKey).join('***');
    // 2. The key exactly as URLSearchParams encoded it into the request URL below.
    //    URLSearchParams uses application/x-www-form-urlencoded, which is NOT the
    //    same as encodeURIComponent (space -> '+' not '%20'; !'()~ get escaped).
    //    PAN-OS keys are base64 so the two forms usually coincide — but redaction
    //    must not silently depend on that assumption holding.
    out = out.split(new URLSearchParams({ k: apiKey }).toString().slice(2)).join('***');
    // 3. The percent-encoded form, for any path that used encodeURIComponent.
...
async function panRequest(conn, params) {
  ...
  // needs its own redaction. node-fetch@2 embeds the full request URL — API key and
  // all — in body-read errors:
  //   "Response timeout while trying to fetch <url> (over 15000ms)"  (body-timeout;
  //      the `timeout` option covers the body read too, and `show config running`
  //      returns a multi-MB config that can genuinely exceed 15s on a slow WAN)
  ...
  try {
    ...
    throw new Error(
      `PAN-OS API response body could not be read (${endpointLabel}): ${redactKey(err.message, apiKey)}`
    );
  }
