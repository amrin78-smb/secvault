...
const xmlParser = new XMLParser({
  ...
  // ⛔ parseTagValue:false is LOAD-BEARING — do not remove it as a "cleanup".
  // fast-xml-parser defaults it to TRUE, which silently coerces any
  // numeric-looking element text to a JS number and DESTROYS DATA:
  //   <serial>007051000188962</serial>  ->  7051000188962   (leading zeros gone)
  //   <url-version>20260803.20149</url-version> -> 20260803.20149 (float)
  // Found live 2026-08-03 after devices were switched from the SSH transport to
  // this one: the stored serial changed, and every affected device raised a
  // bogus "serial was changed" config diff. PAN-OS identifiers are STRINGS that
  // happen to look numeric (serials, content-version pairs, licence auth codes),
  // and the SSH transport has always treated them as such — this keeps both
  // transports agreeing and stops the config differ seeing phantom changes.
  // Every consumer already goes through scalarText()/String(), so nothing
  // downstream depends on receiving a number here.
  parseTagValue: false,
});
...
// op: show rule-hit-count vsys <vsys-name> rule-base security rules all.
//
// ✅ LIVE-VERIFIED 2026-08-25 against all 10 API-transport Palo Altos.
//
// ⛔ It was BROKEN for its entire existence, and the way it was broken is the
// point. The command was missing the <vsys> wrapper, so PAN-OS rejected it
// with a parse error on EVERY call, on EVERY device. index.js's
// _enrichHitCounts() catches that (correctly — a hit count is additive, and
// must never fail a rule pull) and every rule then kept its default
// hit_count of 0. So a total, permanent failure to read hit counts was
// stored as the affirmative claim "this rule has had zero hits" — which
// ruleAnalysis.js then reported as an `unused` finding. Every Palo Alto
// unused-rule finding SecVault has ever produced was fabricated from a
// failed read. This is the same failed-read-as-affirmative-empty class as
// getRules() returning [] on a failure; see CLAUDE.md's Adapter contract.
//
// With the wrapper corrected the response matches 100% of stored rules on
// every device (1,503 rules, 1,094 with a nonzero count), the two extra
// entries per device being PAN-OS's implicit intrazone/interzone defaults
// which are not part of the config rulebase. `show rule-hit-count` is a real, documented PAN-OS
// operational command; the op-command XML form used here follows the
// documented CLI-to-XML-API translation convention this file already uses
// for every other `<show>...</show>` op command. index.js logs the FULL raw
// response the first time this runs (`[PaloAlto Debug] rule-hit-count raw
// response:`) so a future live connection can confirm/correct
// parser.parseRuleHitCount()'s extraction logic.
//
// This is an ADDITIVE enrichment, not part of getRules()'s core contract —
// callers must treat any failure here as non-fatal (see index.js's
// _enrichHitCounts()): a missing hit-count is fine, a missing ruleset is not.
function buildRuleHitCountCmd(vsysName) {
  // ⛔ The <vsys> wrapper is REQUIRED and its absence is not a soft failure.
  // Without it PAN-OS rejects the command outright:
  //   "show -> rule-hit-count -> vsys-name unexpected here"
  // Confirmed live 2026-08-25 against IDC FW and SMT. Do not remove it.
  return (
    '<show><rule-hit-count><vsys><vsys-name>' +
    `<entry name='${vsysName}'><rule-base><entry name='security'>` +
    '<rules><all/></rules></entry></rule-base></entry>' +
    '</vsys-name></vsys></rule-hit-count></show>'
  );
}

async function getRuleHitCount(conn, vsysName) {
  const { result } = await panRequest(conn, {
    type: 'op',
    cmd: buildRuleHitCountCmd(vsysName),
  });
  return result;
}

// op: show running security-policy — the MERGED/effective security policy
// (local config + Panorama pre/post-rulebase combined). Fallback for a fully
// Panorama-managed device whose LOCAL config tree has no rulebase content at
// all (every rule is Panorama-pushed) — same underlying PAN-OS concept as
// the SSH transport's identical fallback (sshParser.js's "Effective/running
// security policy" section, which IS live-verified against a real device).
//
// The XML op-command form follows this file's own established CLI-to-XML
// translation convention exactly (see showSystemInfo/showRunningConfig
// above: each CLI token becomes a nested tag pair in document order) —
// "show running security-policy" -> <show><running><security-policy/>
// </running></show>. That translation convention itself is proven (used
// successfully for every other op command in this file); what is NOT yet
// verified is the RESPONSE shape this particular command returns — see
// parser.parseEffectiveSecurityPolicy()'s own header comment for the full
// caveat and how that risk is bounded until a live device confirms it.
const EFFECTIVE_POLICY_CMD = '<show><running><security-policy></security-policy></running></show>';

async function getEffectiveSecurityPolicy(conn) {
  const { raw, result } = await panRequest(conn, {
    type: 'op',
    cmd: EFFECTIVE_POLICY_CMD,
  });
  return { raw, result };
}

// op: show global-protect-gateway current-user — currently active
// GlobalProtect (SSL VPN / remote-access) sessions, across EVERY configured
// gateway (no `gateway "<name>"` filter is passed, so this matches every
// gateway on the device in one call). Backs
// PaloaltoAdapter.getVpnSessionSummary() below — see that method's own
// header comment for the full scope-decision rationale (GlobalProtect vs.
// IPsec site-to-site tunnels) and the citation for this response shape.
//
// XML translation follows this file's own established CLI-to-XML-API
// convention exactly (see EFFECTIVE_POLICY_CMD above): each CLI token
// becomes a nested tag pair in document order — "show
// global-protect-gateway current-user" -> <show><global-protect-gateway>
// <current-user></current-user></global-protect-gateway></show>. That
// translation convention itself is proven (used successfully for every
// other op command in this file); UNLIKE most of this file's other
// op-command response shapes, this one IS independently confirmed against
// Palo Alto's own published XML API documentation (see
// getVpnSessionSummary()'s citation) rather than being purely doc-derived
// guesswork — still not run against a real device from this codebase,
// though, so treat it as doc-derived-but-officially-documented, not
// live-verified.
const GLOBALPROTECT_CURRENT_USER_CMD =
  '<show><global-protect-gateway><current-user></current-user></global-protect-gateway></show>';

async function getGlobalProtectCurrentUsers(conn) {
  const { result } = await panRequest(conn, {
    type: 'op',
    cmd: GLOBALPROTECT_CURRENT_USER_CMD,
  });
  return result;
}

// op: show vpn ipsec-sa — active IPSec site-to-site tunnel Phase-2 SAs. Same
// CLI-token-to-nested-tag translation convention as every other op command in
// this file. DOC-DERIVED response shape (normalized by
// parser.normalizeIpsecTunnels) — not live-verified; the raw response is
// logged as [PaloAlto Debug] in index.js:getVpnTunnels() for validation.
const IPSEC_SA_CMD = '<show><vpn><ipsec-sa></ipsec-sa></vpn></show>';

async function getIpsecTunnels(conn) {
  const { result } = await panRequest(conn, { type: 'op', cmd: IPSEC_SA_CMD });
  return result;
}

// ── Topology op commands (added 2026-08-03, live-verified against ITC-SLY —
// the one live PAN-OS API-transport device in this fleet) ─────────────────
//
// ⛔ `show interface all`'s CLI-to-XML mapping does NOT follow this file's
// usual "every token becomes a nested tag" convention — confirmed live that
// `<show><interface><all/></interface></show>` is REJECTED ("show ->
// interface -> all needs to have non NULL value"); the working form is
// `<show><interface>all</interface></show>` (the final CLI token becomes
// the VALUE of the previous tag, not a further nested tag, when PAN-OS
// treats it as a parameter rather than a sub-command). Do not "fix" this
// to match the other op commands' nesting style.
async function showInterfacesAll(conn) {
  const { result } = await panRequest(conn, { type: 'op', cmd: '<show><interface>all</interface></show>' });
  return result;
}

// `show routing route` — standard nested-tag form, confirmed live. Returns
// clean structured <entry> elements (destination/nexthop/metric/flags/
// interface), unlike the SSH transport's padded-column text — no positional
// parsing needed at all.
async function showRoutingRoute(conn) {
  const { result } = await panRequest(conn, { type: 'op', cmd: '<show><routing><route/></routing></show>' });
  return result;
}

// `show running nat-policy` — confirmed live: the XML response's
// <result><member> text is BYTE-IDENTICAL in format to the SSH transport's
// plain-text `show running nat-policy` output (the same brace-delimited
// `"<name>; index: N" { ... }` blocks) — NOT a structured XML breakdown per
// rule. `sshParser.parseNatPolicyOutput()` is reused directly on this text
// rather than writing a second parser for data that's already the same
// shape (see paloalto/index.js's getNatRules()).
async function showRunningNatPolicy(conn) {
  const { result } = await panRequest(conn, { type: 'op', cmd: '<show><running><nat-policy/></running></show>' });
  return result;
}

// `show config pushed-shared-policy` — LIVE-VERIFIED 2026-08-25 against
// PAKFood, the fleet's only Panorama-managed device.
//
// A Panorama-managed firewall keeps NO local rulebase: its config-get for
// the vsys rulebase xpath returns an empty `<result/>`, and the rules live
// in the policy Panorama pushes to it. This command returns that pushed
// policy as CLEAN STRUCTURED XML (118,734 bytes on the live device), under
//   result.policy.panorama["pre-rulebase"].security.rules.entry   (33 rules)
//   result.policy.panorama["post-rulebase"]                       (defaults)
//
// parser.parseRulesDeep() extracts it UNMODIFIED — its shape-agnostic deep
// walk already handles the {"#text":…, "@_loc":…} member shape Panorama uses
// to tag which device-group a value came from.
//
// Preferred over `show running security-policy` because it preserves real
// OBJECT NAMES (VLAN4-10.248.4.0_24) and complete application lists, where
// the running-policy text gives resolved IPs and truncates long application
// lists with a literal "...".
async function showPushedSharedPolicy(conn) {
  const { result } = await panRequest(conn, {
    type: 'op',
...
// `request license info` — live-verified 2026-08-03 against ITC-SLY. Returns
// clean structured XML: <result><licenses><entry> with <feature>/<description>/
// <serial>/<issued>/<expires>/<expired>/<authcode> per entitlement.
//
// ⚠️ Note this is `type: 'op'` with a <request> root, NOT a <show> root — PAN-OS
// treats `request license info` as an operational command even though it isn't a
// `show`. The value-form `<request><license>info</license></request>` is REJECTED
// live ("request -> license is unexpected", code 17), so the nested form here is
// load-bearing — the opposite of showInterfacesAll() above, which needs the value
// form. There is no general rule; each command was verified individually.
async function requestLicenseInfo(conn) {
  const { result } = await panRequest(conn, {
    type: 'op',
...
// `show high-availability state` — live-verified 2026-08-03 across 11 devices
// (6 in real HA pairs, 5 standalone). Unlike most op commands this returns
// PLAIN TEXT in <result>, not structured XML, on BOTH transports — the same
// indented `Local Information:` / `Peer Information:` block format the SSH CLI
// prints. A standalone device returns the literal string `HA not enabled`
// (SSH) / an <enabled>no</enabled> element (API).
async function showHighAvailabilityState(conn) {
  const { result } = await panRequest(conn, {
    type: 'op',
...
// `show system disk-space` — live-verified 2026-08-03. Returns raw `df -h`
// text in a CDATA block, BYTE-IDENTICAL in format to the SSH transport's own
// output, so sshParser.parseDiskSpaceOutput() is reused directly for both
// transports rather than writing a second parser (same situation as
// showRunningNatPolicy() above).
async function showSystemDiskSpace(conn) {
  const { result } = await panRequest(conn, {
    type: 'op',
...
// `show system resources` — returns raw `top` text in a CDATA block, the same
// format the SSH transport prints, so sshParser.parsePerformanceResourcesOutput()
// serves both. Live-verified 2026-08-04 on HRIS and IDC FW.
async function showSystemResources(conn) {
  const { result } = await panRequest(conn, {
    type: 'op',
...
// `show session info` — clean STRUCTURED XML (unlike most op commands here):
// num-active, num-max, cps, pps, kbps, num-tcp, num-udp. Live-verified
// 2026-08-04. num-active together with num-max gives session-table SATURATION,
// which SNMP cannot express at all — a device at 8678/1048574 and one at
// 550/200000 are very different risks despite both being "some sessions".
async function showSessionInfo(conn) {
  const { result } = await panRequest(conn, {
    type: 'op',
...
module.exports = {
  ...
  getGlobalProtectCurrentUsers,
  getIpsecTunnels,
  showInterfacesAll,
  showRoutingRoute,
  showRunningNatPolicy,
  DEFAULT_VSYS,
