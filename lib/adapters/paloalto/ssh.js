...
// ⚠️ ROOT CAUSE CONFIRMED LIVE (2026-07-16, PAN-OS 11.1.13-h5, PA-440, standalone):
// a real device returned a 93,435-char `show config running` dump starting with
// `config { mgt-config { users { ... } } }` — the classic curly-brace tree, NOT
// the flat `set ...` lines this adapter is built on. RULE_LINE_REGEX (which
// requires a line starting with `set `) never matched a single line — 0 rules,
// with the `skipped` counter also at 0 (proving no rule line was even ATTEMPTED,
// not that parsing failed on real `set` lines). getConfig()'s parseConfigFromSet()
// was silently broken the same way, since both share this one dump.
//
// The original design issued `set cli config-output-format set` as an
// operational-mode init command ahead of `show config running`. That does not
// work: `show config running` is a hard-coded operational command that ALWAYS
// renders the brace tree regardless of the `cli config-output-format`
// preference — that preference only applies to `show`/`show <path>` run FROM
// INSIDE configuration mode. The standard, documented way to get a flat `set`
// dump is:
//   1. `configure`                        — enter configuration mode (`>` → `#`)
//   2. `set cli config-output-format set`  — THEN, inside config mode, `show` honours it
//   3. `show`                              — bare, dumps the entire tree from root
//
// This 3-step sequence is the FIX for the confirmed-live root cause above, but
// the fix itself is NOT YET independently verified against this device — the
// next live Collect Now will show a fresh [PaloAlto SSH Debug] preview; confirm
// it now starts with `set ` before trusting rule counts from this adapter.
//
...
// falls back to the broken brace-format path.
...
class PaloaltoSshAdapter extends FirewallAdapter {
  ...
  async _getConfigText() {
    ...
    if (!loggedFirstConfig) {
      ...
      // Bounded preview: a running config is tens of thousands of lines. 8000 chars
      // (was 2000): a real 93KB config's first 2000 chars covered only the
      // mgt-config/users section, never reaching the rulebase -- that gap cost a
      // full extra round of log collection to diagnose the brace-vs-set-format bug.
      const preview = sshParser.redactConfig(output).slice(0, 8000);
      console.log(
        `[PaloAlto SSH Debug] Config via \`${RUNNING_CONFIG_COMMAND}\` in set format ` +
          `(${output.length} chars, secrets redacted). First 8000 chars:\n${preview}`
      );
  ...
  async getRules() {
    ...
    const rules = sshParser.parseRulesFromSetConfig(configText);

    if (rules.length === 0) {
      // The device answered with a real config that simply has no rule lines this
      // parser recognizes. [] is honest here — but say so loudly, because the other
      // possibility is that this firmware's `set` grammar differs from the one
      // sshParser.js assumes (which is NOT verified against live hardware).
      console.warn(
        `[PaloAlto SSH] Config retrieved for device ${this.device.id} but no security rules were ` +
          'parsed from it. Either the rulebase is genuinely empty, or the `set` output format ' +
          'differs on this firmware — check the [PaloAlto SSH Debug] preview and ' +
  ...
  async getConfig() {
    ...
    return {
      raw: sshParser.redactConfig(configText),
      parsed: sshParser.parseConfigFromSet(configText, systemInfo),
    };
  }
}
