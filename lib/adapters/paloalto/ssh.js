...
class PaloaltoSshAdapter extends FirewallAdapter {
  ...
  // Session count is deliberately omitted here: `show session info`'s SSH
  // output is a different (text) shape from the API's structured XML and has
  // not been captured from a live device, so it is left null rather than
  // parsed speculatively. CPU/memory/uptime come from `show system resources`,
  // whose `top` output IS identical across both transports.
  async getPerformanceMetrics() {
    ...
    return {
      ...
      raw: { source: 'show system resources' },
      lowConfidence: false,
