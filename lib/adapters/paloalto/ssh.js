...
class PaloaltoSshAdapter extends FirewallAdapter {
  ...
  async getVpnSessionSummary() {
    ...
    if (!loggedFirstGlobalProtectUsers) {
      // No redaction needed — usernames/computer names/IPs here are
      // operational session metadata, not credentials, the same "safe to
      // log unredacted" call this file already makes for rule-hit-count and
      // effective-policy output.
      console.log(
        `[PaloAlto SSH Debug] \`${GLOBALPROTECT_CURRENT_USER_COMMAND}\` raw output:\n${output}`
      );
