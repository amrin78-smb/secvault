...
const { FirewallAdapter, CapabilityUnavailableError } = require('../interface');
const credStore = require('../../credStore');
const { runCommands, parseJsonCredential } = require('../sshClient');
const sshParser = require('./sshParser');
// One pure helper borrowed from the XML transport's parser: extractContentVersions()
// is transport-agnostic by construction (it reads the same `show system info` field
// names off either the XML tree or sshParser's flat key/value object), so
// duplicating it here would mean two copies of the same field list drifting apart.
// This mirrors the already-established reverse exception — ./index.js (the XML
// adapter) requires ./sshParser for parseNatPolicyOutput. Nothing else is shared.
const { extractContentVersions } = require('./parser');
...
const IPSEC_SA_COMMAND = 'show vpn ipsec-sa';

// Topology commands (added 2026-08-02) — confirmed live against a real
// device (HRIS, this deployment) before this parser/method pair was
// written, per CLAUDE.md's "verify against live responses" rule. See
// sshParser.js's "Topology parsers" section for the exact output shapes.
let loggedFirstInterfaces = false;
let loggedFirstRoutingTable = false;
let loggedFirstNatPolicy = false;
const INTERFACES_COMMAND = 'show interface all';
const ROUTING_TABLE_COMMAND = 'show routing route';
const NAT_POLICY_COMMAND = 'show running nat-policy';

// Device lifecycle & health (added 2026-08-03), all three live-verified
// against the real fleet before any parser was written.
let loggedFirstLicenses = false;
let loggedFirstHaState = false;
let loggedFirstDiskSpace = false;
let loggedFirstPerformance = false;

// ⛔ "No address" is NOT an address (2026-09-09). PAN-OS prints the literal
// string "N/A" for an interface with no IP configured, and that string was
// being stored verbatim into device_interfaces.ip_address — a TEXT column, so
// nothing errored, and every `ip_address IS NOT NULL` test then counted those
// interfaces as ADDRESSED. Same class as hit_count's old DEFAULT 0: the
// vendor's "I have none" recorded as if it were a measurement. NULL is the only
// honest value. Duplicated per adapter file, per this codebase's existing
// per-transport duplication convention.
...
const LICENSE_COMMAND = 'request license info';
// `all` rather than `state`: it is a strict superset (adds peer Model/Serial
// and the HA1/HA2 link detail) in the identical block format, and a
// standalone device returns the same literal `HA not enabled` for both.
const HA_STATE_COMMAND = 'show high-availability all';
const DISK_SPACE_COMMAND = 'show system disk-space';
...
// Operational-mode command for GlobalProtect current-user count — see
// getVpnSessionSummary() below and sshParser.js's "GlobalProtect
// current-user count" section for the command choice, output format, and
// its one known accepted gap. A plain `show` command, no `configure`/
// set-format step needed (same class of command as EFFECTIVE_POLICY_COMMAND
// above).
const GLOBALPROTECT_CURRENT_USER_COMMAND = 'show global-protect-gateway current-user';
...
class PaloaltoSshAdapter extends FirewallAdapter {
  ...
  async getVersion() {
    ...
    return {
      ...
      // Content/signature versions (added 2026-08-03). Same class of gap as
      // serial/hostname above: `show system info` ALREADY carries app/av/
      // threat/wildfire/url-filtering versions and their release dates, and
      // this adapter already fetches it — they were simply never extracted.
      // No additional device command is issued. `info.fields` is
      // parseSystemInfoOutput()'s flat key/value map of that same output.
      contentVersions: extractContentVersions(info.fields || {}),
    };
  }
    ...
    try {
      ...
      console.warn(
        `[PaloAlto SSH] Hit-count enrichment failed unexpectedly for device ${this.device.id} — ` +
          `hit_count left NULL (NOT MEASURED) for all rules: ${err.message}`
      );
    }
    ...
    if (containersFound !== 1) {
      console.warn(
        `[PaloAlto SSH] Device ${this.device.id}: ${containersFound} rulebase container(s) found — ` +
          ...
          'hit_count left NULL (NOT MEASURED) for all rules.'
      );
      return;
    }
    ...
    if (vsysNames.length > 1) {
      ...
      console.warn(
        `[PaloAlto SSH] Device ${this.device.id}: ${vsysNames.length} vsys found ` +
          ...
          'cross-vsys rule-name collisions. hit_count left NULL (NOT MEASURED) for all rules.'
      );
      return;
    }
  ...
  // OPTIONAL — FirewallAdapter's getVpnSessionSummary() (interface.js). See
  // ./index.js's PaloaltoAdapter.getVpnSessionSummary() for the FULL
  ...
  // Panorama-managed-device gap this file documents at length for
  // getRules() does NOT apply here) — not re-quoted, same
  // duplicate-comment-per-transport convention this file's SNMP monitoring
  // section already uses. See sshParser.js's "GlobalProtect current-user
  // count" section for the CLI output format this parses and its one known,
  // accepted gap (a device with no GlobalProtect gateway configured at
  // all).
  async getVpnSessionSummary() {
    const results = await this._run([GLOBALPROTECT_CURRENT_USER_COMMAND]);
    const output = results[0] ? results[0].output : '';

    if (sshParser.looksLikeCliError(output)) {
      // ⛔ CapabilityUnavailableError, not a bare Error (2026-09-09). this._run()
      // above RESOLVED: the SSH session connected, authenticated and got a
      // prompt back, and PAN-OS then answered the op-command with a rejection.
      // That rejection describes the account's operational-command access, not
      // the firewall's reachability — a connect/login/timeout would have thrown
      // out of _run() and never arrived here. Treating it as unreachable is the
      // FortiGate SSL-VPN bug (see interface.js) on the other vendor.
      throw new CapabilityUnavailableError(
        `PAN-OS \`${GLOBALPROTECT_CURRENT_USER_COMMAND}\` on device ${this.device.id} was rejected — ` +
          "check the SSH account's operational-command access.",
        { capability: 'vpn_session_summary' }
      );
    }

    const count = sshParser.countActiveGlobalProtectUsers(output);
    if (count === null) {
      // No `GlobalProtect Gateway: <name> (<N> users)` header AT ALL. A gateway
      // that IS configured always prints a header (even the confirmed "(0
      // users)" form, which sums to a real 0), so a TOTAL absence means this
      // device has no GlobalProtect gateway configured — a normal state for a
      // firewall that doesn't do remote-access VPN, NOT an error. Report a clean
      // 0 rather than throwing, which previously logged a WARN on this device
      // EVERY poll (real report: device "TFM-RN"). `looksLikeCliError()` above
      // already caught a genuinely rejected command, so reaching here means
      // benign no-gateway output. `no_globalprotect_gateway` is recorded in the
      // snapshot's raw so this stays visible/diagnosable without log spam.
      return {
        active_session_count: 0,
        sessions: [],
        raw: { source_command: GLOBALPROTECT_CURRENT_USER_COMMAND, no_globalprotect_gateway: true },
      };
    }

    // One-time raw-output dump for live field-mapping validation. Fires on the
    // first device that actually HAS users (count > 0) — NOT merely the first
    // device polled, which may be an empty/no-gateway box whose output shows no
    // real fields. Tagged [GP-SAMPLE] + device id so a single log search finds
    // exactly the right line. No redaction needed — usernames/computers/IPs are
    // operational session metadata, not credentials.
    if (!loggedFirstGlobalProtectUsers && count > 0) {
      console.log(
        `[PaloAlto SSH Debug][GP-SAMPLE] device ${this.device.id} \`${GLOBALPROTECT_CURRENT_USER_COMMAND}\` raw output (${count} users):\n${output}`
      );
      loggedFirstGlobalProtectUsers = true;
    }

    // Per-user detail is ADDITIVE and best-effort — a parse failure here must
    // never disturb the authoritative count above (see
    // sshParser.parseGlobalProtectSessions()'s header comment).
    let sessions = [];
    try {
      sessions = sshParser.parseGlobalProtectSessions(output);
    } catch (_err) {
      sessions = [];
    }
    ...
    return {
      ...
      sessions,
      raw: { source_command: GLOBALPROTECT_CURRENT_USER_COMMAND },
    };
  }

  // OPTIONAL — IPSec site-to-site tunnel status via `show vpn ipsec-sa`
  // (parseIpsecTunnels()'s header comment covers the output shape and its
  // doc-derived standing). Throws only on a rejected command; a parse that
  // finds nothing returns [] (a device with no IPSec tunnels — legitimate,
  // NOT an error), same "[] means genuinely none" honesty the count uses.
  async getVpnTunnels() {
    const results = await this._run([IPSEC_SA_COMMAND]);
    const output = results[0] ? results[0].output : '';

    if (sshParser.looksLikeCliError(output)) {
      throw new Error(
        `PAN-OS \`${IPSEC_SA_COMMAND}\` on device ${this.device.id} was rejected — ` +
          "check the SSH account's operational-command access."
      );
    }

    if (!loggedFirstIpsecTunnels) {
      console.log(`[PaloAlto SSH Debug] \`${IPSEC_SA_COMMAND}\` raw output:\n${output}`);
      loggedFirstIpsecTunnels = true;
    }

    try {
      return sshParser.parseIpsecTunnels(output);
    } catch (_err) {
      return [];
    }
  }
  ...
  // exact contract). Never throws on a CLI rejection — same "degrade to
  // empty, don't fail the whole collectAndStore pull" treatment as
  // getObjects()/getVpnTunnels(), since topology data has no destructive
  // wipe-on-empty risk the way getRules() does.
  async getInterfaces() {
    let output;
    try {
      const results = await this._run([INTERFACES_COMMAND]);
      output = results[0] ? results[0].output : '';
    } catch (err) {
      console.warn(`[PaloAlto SSH] getInterfaces() failed for device ${this.device.id}: ${err.message}`);
    ...
    if (!loggedFirstInterfaces) {
      console.log(`[PaloAlto SSH Debug] \`${INTERFACES_COMMAND}\` raw output:\n${output}`);
      loggedFirstInterfaces = true;
    }

    try {
      // normalizeInterfaceList: PAN-OS's literal "N/A" for an unaddressed
      // interface becomes NULL here, at the collection point. See the helper.
      return { interfaces: normalizeInterfaceList(sshParser.parseInterfacesOutput(output)) };
    } catch (_err) {
      // ⛔ RETHROW, never an empty set. Returning [] here made
      // collectAndStore's per-method try/catch unreachable: it would have
      // left this device's rows untouched on a failure, but the adapter
  ...
  async getRoutingTable() {
    let output;
    try {
      const results = await this._run([ROUTING_TABLE_COMMAND]);
      output = results[0] ? results[0].output : '';
    } catch (err) {
      console.warn(`[PaloAlto SSH] getRoutingTable() failed for device ${this.device.id}: ${err.message}`);
      // ⛔ RETHROW, never an empty set. Returning [] here made
      // collectAndStore's per-method try/catch unreachable: it would have
      // left this device's rows untouched on a failure, but the adapter
      // reported success, so storeDevice*() ran its DELETE and reinserted
    ...
    if (!loggedFirstRoutingTable) {
      console.log(`[PaloAlto SSH Debug] \`${ROUTING_TABLE_COMMAND}\` raw output:\n${output}`);
      loggedFirstRoutingTable = true;
    }

    try {
      return { routes: sshParser.parseRoutingTableOutput(output) };
    } catch (_err) {
      // ⛔ RETHROW, never an empty set. Returning [] here made
      // collectAndStore's per-method try/catch unreachable: it would have
      // left this device's rows untouched on a failure, but the adapter
  ...
  // OPTIONAL — FirewallAdapter's getNatRules(). `show running nat-policy`
  // shows the COMPILED policy (multi-vsys devices would need a per-vsys
  // command variant — not yet handled; this device/every device in this
  // fleet's Phase 1 scope is single-vsys, confirmed via `show interface
  // all`'s vsys column above always reading `1`).
  async getNatRules() {
    let output;
    try {
      const results = await this._run([NAT_POLICY_COMMAND]);
      output = results[0] ? results[0].output : '';
    } catch (err) {
      console.warn(`[PaloAlto SSH] getNatRules() failed for device ${this.device.id}: ${err.message}`);
      // ⛔ RETHROW, never an empty set. Returning [] here made
      // collectAndStore's per-method try/catch unreachable: it would have
      // left this device's rows untouched on a failure, but the adapter
      // reported success, so storeDevice*() ran its DELETE and reinserted
    ...
    if (!loggedFirstNatPolicy) {
      console.log(`[PaloAlto SSH Debug] \`${NAT_POLICY_COMMAND}\` raw output:\n${output}`);
      loggedFirstNatPolicy = true;
    }

    try {
      return { rules: sshParser.parseNatPolicyOutput(output) };
    } catch (_err) {
      // ⛔ RETHROW, never an empty set. Returning [] here made
      // collectAndStore's per-method try/catch unreachable: it would have
      // left this device's rows untouched on a failure, but the adapter
  ...
  // OPTIONAL — getPerformanceMetrics() (added 2026-08-04). See the API
  // transport's equivalent for why this is preferred over SNMP on PAN-OS.
  // CPU/memory/uptime come from `show system resources`, whose `top` output is
  // identical across both transports.
  //
  // Session count: added 2026-08-05 (v2.55.0). It was previously null here
  // because `show session info`'s SSH output had never been captured and this
  // codebase does not write parsers against guessed formats. It has now been
  // captured live from TUG — see parseSessionInfoOutput's own note for the
  // confirmed shape and for why the "allocated" line is the right figure.
  //
  // ⛔ Session info is BEST-EFFORT: CPU and memory are the primary readings and
  // must not be lost because a secondary command misbehaved.
  //
  // The first cut ran both commands in ONE _run() and claimed best-effort in a
  // comment — but runCommands() rejects the WHOLE batch if any command times
  // out or the session drops, so a stalled 'show session info' took CPU and
  // memory down with it. A command PAN-OS merely rejects still returns a
  // prompt, so that case was always survivable; a timeout was not.
  //
  // Happy path is still a single session with both commands. Only if that
  // throws do we retry with resources alone, so the fallback costs a second
  // session exactly when the first one already failed.
  async getPerformanceMetrics() {
    let output;
    let sessionOutput = '';
    try {
      const results = await this._run(['show system resources', 'show session info']);
      output = results[0] ? results[0].output : '';
      sessionOutput = results[1] ? results[1].output : '';
    } catch (bothErr) {
      try {
        const results = await this._run(['show system resources']);
        output = results[0] ? results[0].output : '';
        console.warn(
          `[PaloAlto SSH] session-info read failed for device ${this.device.id} ` +
            `(${bothErr.message}); CPU/memory collected without a session count.`
        );
      } catch (err) {
    ...
    if (!loggedFirstPerformance) {
      console.log(`[PaloAlto SSH Debug] \`show system resources\` raw output:\n${output.slice(0, 2000)}`);
      loggedFirstPerformance = true;
    }

    const m = sshParser.parsePerformanceResourcesOutput(output);
    // Never throws; returns nulls on an unexpected/empty shape.
    const sess = sshParser.parseSessionInfoOutput(sessionOutput);
    return {
      ...
      raw: {
        source: 'show system resources + show session info',
        sessionsSupported: sess.sessionsSupported,
        sessionTableUtilizationPercent: sess.tableUtilizationPercent,
      },
      lowConfidence: false,
  ...
  // against HRIS (which carries a genuinely expired SD-WAN entitlement).
  // Same never-throw/degrade-to-empty treatment as the topology methods.
  async getLicenses() {
    let output;
    try {
      const results = await this._run([LICENSE_COMMAND]);
      output = results[0] ? results[0].output : '';
    } catch (err) {
      console.warn(`[PaloAlto SSH] getLicenses() failed for device ${this.device.id}: ${err.message}`);
      return null; // null = "could not determine", never an empty set — see collectAndStore
    }

    if (!loggedFirstLicenses) {
      // Licence output carries feature names, dates and auth codes — no
      // credentials or key material, so it is safe to log unredacted.
      console.log(`[PaloAlto SSH Debug] \`${LICENSE_COMMAND}\` raw output:\n${output.slice(0, 4000)}`);
      loggedFirstLicenses = true;
    }
    // ⛔ A rejected command does NOT throw on a shell channel — runCommands
    // returns the rejection text as ordinary output, which the parser then
    // reads as "this device has zero licences". collectAndStore's
    // Array.isArray gate passes, the DELETE runs, and the device's real
    // licences are wiped while the collect reports success.
    if (sshParser.looksLikeCliError(output)) return null;


    try {
      return sshParser.parseLicenseInfoOutput(output);
    } catch (_err) {
      return null; // null = "could not determine", never an empty set — see collectAndStore
    }
  }
  ...
  // across 11 devices (6 real Active-Passive pairs, 5 standalone).
  //
  // Returns { enabled: false } for a standalone device — a POSITIVE fact
  // worth storing, not an absence. Returns null only when the command could
  // not be run at all, so the caller can tell "standalone" apart from
  // "never collected" rather than conflating them.
  async getHaStatus() {
    let output;
    try {
      const results = await this._run([HA_STATE_COMMAND]);
      output = results[0] ? results[0].output : '';
    } catch (err) {
      console.warn(`[PaloAlto SSH] getHaStatus() failed for device ${this.device.id}: ${err.message}`);
      return null;
    }

    if (!loggedFirstHaState) {
      console.log(`[PaloAlto SSH Debug] \`${HA_STATE_COMMAND}\` raw output:\n${output.slice(0, 4000)}`);
      loggedFirstHaState = true;
    }

    try {
      const ha = sshParser.parseHaStateOutput(output);
      // enabled === null means the parser could not tell — don't store a guess.
  ...
  async getDiskUsage() {
    let output;
    try {
      const results = await this._run([DISK_SPACE_COMMAND]);
      output = results[0] ? results[0].output : '';
    } catch (err) {
      console.warn(`[PaloAlto SSH] getDiskUsage() failed for device ${this.device.id}: ${err.message}`);
      return null; // null = "could not determine", never an empty set — see collectAndStore
    }

    if (!loggedFirstDiskSpace) {
      console.log(`[PaloAlto SSH Debug] \`${DISK_SPACE_COMMAND}\` raw output:\n${output.slice(0, 2000)}`);
      loggedFirstDiskSpace = true;
    }
    // ⛔ A rejected command does NOT throw on a shell channel — runCommands
    // returns the rejection text as ordinary output, which the parser then
    // reads as "this device has zero filesystems". collectAndStore's
    // Array.isArray gate passes, the DELETE runs, and the device's real
    // filesystems are wiped while the collect reports success.
    if (sshParser.looksLikeCliError(output)) return null;


    try {
      return sshParser.parseDiskSpaceOutput(output);
    } catch (_err) {
      return null; // null = "could not determine", never an empty set — see collectAndStore
    }
  }
