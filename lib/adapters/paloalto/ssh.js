...
class PaloaltoSshAdapter extends FirewallAdapter {
  ...
  async _getConfigText() {
    ...
    if (!loggedFirstConfig) {
      ...
      // Bounded preview: a running config is tens of thousands of lines.
      const preview = sshParser.redactConfig(output).slice(0, 2000);
      console.log(
        `[PaloAlto SSH Debug] Config via \`${RUNNING_CONFIG_COMMAND}\` in set format ` +
          `(${output.length} chars, secrets redacted). First 2000 chars:\n${preview}`
      );
