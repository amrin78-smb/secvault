...
class PaloaltoSshAdapter extends FirewallAdapter {
  ...
  async getRules() {
    ...
    if (containersFound === 0) {
      ...
      // tree. Before treating this as a structural retrieval failure, try the
      // Panorama-managed fallback — see sshParser.js's "Effective/running
      // security policy" section for the full rationale, and CLAUDE.md's
      // "Palo Alto SSH — OPEN CASE" note for the real device that surfaced
      // this: a wholly Panorama-managed device's local config tree genuinely
      // has no rulebase.security.rules content at all, because every rule is
      // Panorama-pushed and only appears in the MERGED effective policy, not
      // the local config. `show running security-policy` is a completely
      // different command/format and needs no `configure`/set-format step.
      ...
      if (effectiveRules !== null) {
        console.log(
          `[PaloAlto SSH] Device ${this.device.id}: no rulebase found in the local config tree, but ` +
            `\`${EFFECTIVE_POLICY_COMMAND}\` returned ${effectiveRules.length} rule(s) — using the ` +
