...
// as a prompt and the command returns SILENTLY TRUNCATED. `show config running` on
...
// Flat `set ...` output instead of the default brace tree. See the header of
// sshParser.js for why this format was chosen. Scoped to the config session only:
// `show system info` does not need it, and not setting it there keeps the version
// path independent of this command being supported.
const SET_FORMAT = 'set cli config-output-format set';

const SYSTEM_INFO_COMMAND = 'show system info';
const RUNNING_CONFIG_COMMAND = 'show config running';
...
class PaloaltoSshAdapter extends FirewallAdapter {
  ...
  async _getSession(extraInitCommands = []) {
    ...
    return {
      ...
      options: {
        ...
        // there is deliberately no enablePassword here. (A read-only PAN-OS admin
        // still runs `show config running`; it returns the config it can see.)
        enablePassword: null,
      },
    };
  }
  ...
  async _getConfigText() {
    ...
    if (sshParser.looksLikeCliError(output)) {
      throw new Error(
        `PAN-OS rejected \`${RUNNING_CONFIG_COMMAND}\` on device ${this.device.id}. ` +
          'This usually means the SSH account lacks config-read permission — give it a ' +
          'role with configuration access. Refusing to store a partial/empty result.'
      );
    }
