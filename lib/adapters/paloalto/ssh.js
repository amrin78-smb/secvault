...
class PaloaltoSshAdapter extends FirewallAdapter {
  ...
  async getVpnSessionSummary() {
    ...
    if (count === null) {
      throw new Error(
        `PAN-OS \`${GLOBALPROTECT_CURRENT_USER_COMMAND}\` on device ${this.device.id} returned output ` +
          'that did not contain a recognized "GlobalProtect Gateway: <name> (<N> users)" header line ' +
          '— refusing to guess an active session count. See the [PaloAlto SSH Debug] log above and ' +
          "sshParser.js's countActiveGlobalProtectUsers() for the one known, accepted gap this can " +
          'hit (a device with no GlobalProtect gateway configured at all).'
      );
    }
