...
const SECRET_TOKENS = new Set([
  ...
  'api-key',
  'auth-code',
  // Added 2026-09-09, in step with parser.js's SECRET_TAGS (the two lists are
  // deliberately separate files for separate formats, but a secret leaf NAME is
  // a property of PAN-OS, not of the transport — so a name added to one belongs
  // in the other unless it genuinely cannot occur in that format).
  //
  // ⛔ Matching here is WHOLE-TOKEN and EXACT, so — exactly as in parser.js — a
  // compound name is not covered by its own suffix: `wmi-password` was never
  // matched by `password`. Both of these are real leaves in the brace tree
  // (`user-id-collector setting wmi-password …`), so they leak on this transport
  // for the same reason, independent of the XML attribute bug.
  'wmi-password', // User-ID agentless WMI probing account password.
  'agent-user-override-key', // GlobalProtect agent config override key.
  // `community` is the SNMP trap-server community string (a credential). Its
  // BGP homonym — a route-tagging attribute list — is a brace SECTION here
  // (`community {`), and redactLine already skips a keyword whose remainder is
  // just `{`, so the container survives and only a real `community <value>;`
  // leaf is redacted. See parser.js's SECRET_LEAF_TAGS for the full split.
  'community',
]);
...
function ruleFromBraceEntry(name, attrs, sequenceNumber) {
  ...
  return {
    ...
    // The SSH transport does not run it, so this is genuinely UNMEASURED --
    // NULL, never 0 (see lib/schema.sql).
    hit_count: null,
    raw_rule: a,
  };
}
...
function effectivePolicyRuleToNormalizedRule(rule) {
  ...
  return {
    ...
    log_enabled: true,
    comment: null,
    // This fallback output carries no hit-count column at all -- NULL, not 0.
    hit_count: null,
    raw_rule: a,
  };
}
...
function parseNatPolicyOutput(text) {
  ...
  while ((match = NAT_BLOCK_RE.exec(text)) !== null) {
    ...
    // ⛔ Live-confirmed on ITC-SLY (API transport, same text format as this
    // SSH parser -- see api.js's showRunningNatPolicy()): a
    // "dynamic-ip-and-port" (PAT/overload) rule's translate-to string
    // includes the EGRESS INTERFACE NAME between "src:" and the IP --
    // `"src: ethernet1/2 118.174.183.36(*) (dynamic-ip-and-port) ..."` --
    // unlike the "static-ip" shape (`"src: 27.254.123.18 (static-ip) ..."`)
    // this file was originally verified against. A regex requiring the IP
    // to immediately follow "src:" silently produced ZERO rows for every
    // PAT/overload NAT rule (the single most common real-world NAT type)
    // until this fix -- `.*?` skips any interface-name token lazily,
    // matching the first dotted-quad found after "src:"/"dst:" either way.
    const srcTranslated = /src:.*?(\d{1,3}(?:\.\d{1,3}){3})/.exec(translateTo);
    const dstTranslated = /dst:.*?(\d{1,3}(?:\.\d{1,3}){3})/.exec(translateTo);
...
// ---------------------------------------------------------------------------
// parseHaStateOutput() and parseDiskSpaceOutput() are used by BOTH transports:
// PAN-OS returns these two op-commands as plain text on the XML API as well
// (inside <result>, CDATA for disk), byte-identical in format to the CLI. The
// API adapter reuses them directly rather than duplicating a second parser —
// the same documented cross-transport reuse already established for
// parseNatPolicyOutput(). parseLicenseInfoOutput() is NOT shared: the API
// returns structured XML for licences, which parser.js handles separately.

// Duplicated (not imported) from parser.js's equivalent, per this file's
// header rule that it shares nothing with parser.js. Small, pure, and stable.
// Returns 'YYYY-MM-DD' or null — "Never" (a genuinely perpetual licence) and
// anything unrecognized both yield null, and the caller keeps the raw string
// so the two cases stay distinguishable downstream.
const SSH_LICENSE_MONTHS = {
  january: 1, february: 2, march: 3, april: 4, may: 5, june: 6,
  july: 7, august: 8, september: 9, october: 10, november: 11, december: 12,
};

function parseLicenseDateText(raw) {
  if (typeof raw !== 'string') return null;
  const m = raw.trim().match(/^([A-Za-z]+)\s+(\d{1,2}),\s*(\d{4})$/);
  if (!m) return null;
  const month = SSH_LICENSE_MONTHS[m[1].toLowerCase()];
...
// `request license info` CLI output → { licenses: [...] }.
// Live-verified 2026-08-03 against HRIS. Format is repeated blocks:
//   License entry:
//   Feature: Threat Prevention
//   Description: Threat Prevention
//   Serial: 007051000188962
//   Authcode:
//   Issued: July 14, 2024
//   Expires: June 20, 2027
//   Expired?: no
//   Base license: PA-VM
// Note the literal question mark in "Expired?:". Never throws.
function parseLicenseInfoOutput(text) {
  if (typeof text !== 'string' || text.trim() === '') return { licenses: [] };
  const licenses = [];
  // Split on the block header; the first chunk is whatever preceded it.
  const blocks = text.split(/^\s*License entry:\s*$/m).slice(1);
  for (const block of blocks) {
    const fields = {};
    for (const line of block.split('\n')) {
      const m = line.match(/^\s*([A-Za-z][A-Za-z0-9 ?_/-]*?)\s*:\s*(.*?)\s*$/);
      if (!m) continue;
      fields[m[1].trim().toLowerCase()] = m[2];
    }
    const feature = (fields['feature'] || '').trim();
    if (!feature) continue; // no feature name → not usable data
    const expiresRaw = typeof fields['expires'] === 'string' ? fields['expires'].trim() : null;
    licenses.push({
      feature,
      description: (fields['description'] || '').trim() || null,
      serial: (fields['serial'] || '').trim() || null,
      issuedAt: parseLicenseDateText(fields['issued']),
      expiresAt: parseLicenseDateText(fields['expires']),
      expiresRaw: expiresRaw || null,
      expired: typeof fields['expired?'] === 'string' ? /^yes$/i.test(fields['expired?'].trim()) : null,
      authcode: (fields['authcode'] || '').trim() || null,
    });
  }
  return { licenses };
}

// `show system disk-space` → { filesystems: [...] }. Plain `df -h` output on
// both transports. Sizes stay as the device's own human strings; only the
// percentage is parsed to a number (see the device_disk_usage schema comment
// for why byte-parsing df -h is deliberately avoided). Never throws.
function parseDiskSpaceOutput(text) {
  if (typeof text !== 'string' || text.trim() === '') return { filesystems: [] };
  const filesystems = [];
  for (const line of text.split('\n')) {
    const trimmed = line.trim();
    if (!trimmed) continue;
    if (/^Filesystem\b/i.test(trimmed)) continue; // header row
    // filesystem size used avail use% mounted-on  (mount point may contain spaces)
    const m = trimmed.match(/^(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\d+)%\s+(.+)$/);
    if (!m) continue; // not a df row — skip quietly, never throw
    const usePercent = Number(m[5]);
    filesystems.push({
      filesystem: m[1],
      sizeRaw: m[2],
      usedRaw: m[3],
...
// Strips PAN-OS's "(last N days)" suffix from a state value, returning the
// bare state plus the duration text. `State: active (last 56 days)` →
// { state: 'active', duration: 'last 56 days' }. The duration is the ONLY
// signal that distinguishes a long-stable pair from one that failed over
// recently (live: every healthy pair reported 43-57 days, while TUM(TUTH1)
// and ITC-SK reported 4 and 2 days alongside a "Link down" reason), so it is
// preserved in `raw` rather than discarded.
function splitHaState(value) {
  if (typeof value !== 'string') return { state: null, duration: null };
  const m = value.trim().match(/^(\S+)\s*\((.+)\)\s*$/);
  if (m) return { state: m[1].toLowerCase(), duration: m[2].trim() };
  const bare = value.trim();
  return { state: bare ? bare.toLowerCase() : null, duration: null };
}

// `show high-availability all` (or `state`) → normalized HA status.
// Live-verified 2026-08-03 across 11 Palo Alto devices: 6 real Active-Passive
// pairs and 5 standalone.
//
// A standalone device returns the literal text `HA not enabled` — that is a
// POSITIVE fact ("this device is standalone"), not a failure, and is returned
// as { enabled: false } so the caller can store it rather than treating it as
// missing data.
//
// ⚠️ Do NOT read state from the group header line. Live samples show three
// different suffixes for the same healthy condition — `Group 33: Master`,
// `Group 48: Active`, and `Group 63: ` (empty). The authoritative value is
// `Local Information: → State:`.
//
// Never throws; any field it cannot find stays null rather than being guessed.
function parseHaStateOutput(text) {
  if (typeof text !== 'string' || text.trim() === '') {
    return { enabled: null, raw: null };
  }
  if (/HA not enabled/i.test(text)) {
    return {
      enabled: false, mode: null, groupId: null,
      localState: null, peerState: null, peerMgmtIp: null, peerSerial: null,
      peerConnectionStatus: null, configSyncState: null,
      lastNonfunctionalReason: null, versionCompatOk: null, versionCompat: null,
      raw: { source: 'ha-not-enabled' },
    };
  }

  // Indent-aware walk. Section headers sit at 2 spaces ("Local Information:"),
  // their fields at 4, and nested sub-blocks ("Device Information:",
  // "Version Compatibility:") at 4 with fields at 6. Tracking the (section,
  // subsection) pair is what keeps Local's and Peer's identically-named
  // `State:`/`Device Information:` fields from overwriting each other.
  let section = null;
  let subsection = null;
  const get = {};
  const versionCompat = {};
  ...
  for (const line of text.split('\n')) {
    ...
    if (indent <= 2) {
      // Top-level: either a section header (empty value) or a bare field.
      if (value === '') { section = key; subsection = null; continue; }
      section = null; subsection = null;
      if (/^Mode$/i.test(key)) get.mode = value;
      continue;
    }
    if (indent <= 4) {
      if (value === '') { subsection = key; continue; }
      subsection = null;
      if (section === 'Local Information') {
        if (/^State$/i.test(key)) get.localStateRaw = value;
      } else if (section === 'Peer Information') {
        if (/^State$/i.test(key)) get.peerStateRaw = value;
        else if (/^Connection status$/i.test(key)) get.peerConnectionStatus = value.toLowerCase();
        // Two distinct labels, deliberately treated differently:
        // "Last non-functional state reason" is a FAULT signal (live: "Link
        // down"); "Last suspended state reason: User requested" is a
        // deliberate admin action on an otherwise healthy pair and must NOT
        // be reported as a fault. Only the former populates the column.
        else if (/^Last non-functional state reason$/i.test(key)) get.lastNonfunctionalReason = value;
        else if (/^Last suspended state reason$/i.test(key)) raw.lastSuspendedReason = value;
      } else if (section === 'Configuration Synchronization') {
        if (/^Running Configuration$/i.test(key)) get.configSyncState = value.toLowerCase();
      }
      continue;
    }
    // indent >= 6 — inside a sub-block.
    if (section === 'Peer Information' && subsection === 'Device Information') {
      if (/^Management IPv4 Address$/i.test(key)) get.peerMgmtIp = value.split('/')[0].trim() || null;
      else if (/^Serial$/i.test(key)) get.peerSerial = value;
    } else if (section === 'Local Information' && subsection === 'Version Compatibility') {
      // ⛔ Skip empty values. The Version Compatibility block contains a nested
      // `Plugin Information:` SUB-HEADER (empty value, with `DLP-compat: Match`
      // indented beneath it) — capturing that header as if it were a verdict
      // made an all-Match pair evaluate to versionCompatOk=false and render as
      // DEGRADED. Found live on SMT 2026-08-03, whose every component reads
      // Match; only a real value can be a verdict.
      if (value !== '') versionCompat[key] = value;
    }
  }

  const local = splitHaState(get.localStateRaw);
  const peer = splitHaState(get.peerStateRaw);
  if (local.duration) raw.localStateDuration = local.duration;
  if (peer.duration) raw.peerStateDuration = peer.duration;

  // Tri-state: null when the device reported no Version Compatibility block at
  // all. Never defaulted to true — that would silently claim a pair is
  // version-consistent when it was simply never checked.
  const compatKeys = Object.keys(versionCompat);
  const versionCompatOk =
    compatKeys.length === 0 ? null : compatKeys.every((k) => /^match$/i.test(String(versionCompat[k]).trim()));
  ...
  return {
    ...
    peerMgmtIp: get.peerMgmtIp || null,
    peerSerial: get.peerSerial || null,
    peerConnectionStatus: get.peerConnectionStatus || null,
    configSyncState: get.configSyncState || null,
    lastNonfunctionalReason: get.lastNonfunctionalReason || null,
    versionCompatOk,
...
// ---------------------------------------------------------------------------
// `show system resources` returns raw `top` output in BOTH transports (CDATA on
// the XML API), so one parser serves both — same situation as
// parseDiskSpaceOutput/parseNatPolicyOutput. Live-verified on HRIS and IDC FW.
//
//   top - 21:26:29 up 58 days,  3:14,  0 users,  load average: 0.32, ...
//   %Cpu(s):  8.9 us,  3.0 sy,  0.0 ni, 88.1 id,  0.0 wa, ...
//   MiB Mem :  15633.0 total,    340.8 free,   5359.2 used,   9932.9 buff/cache
//
// This replaces SNMP as Palo Alto's CPU/memory source. PAN-OS exposes no clean
// single "CPU percent" OID, which is why the SNMP path is hardcoded
// lowConfidence:true for this vendor — these figures come from the device's own
// `top`, so they carry no such caveat.
//
// ⛔ CPU is 100 - idle, NOT us+sy. Summing named states silently drops any
// category this build reports that we don't enumerate (ni/wa/hi/si/st all
// appear here), which would understate load.
// ⛔ Memory EXCLUDES buff/cache — `used` is read directly rather than derived
// from total-free, because on these devices free is tiny while buff/cache is
// large (live: 340MB free but 9.9GB buff/cache), so total-free would report
// ~98% and look like a device about to fall over when it is fine.
function parsePerformanceResourcesOutput(text) {
  const out = { cpuPercent: null, memoryPercent: null, uptimeSeconds: null };
  if (typeof text !== 'string' || text.trim() === '') return out;

  const idle = text.match(/%Cpu\(s\):.*?(\d+(?:\.\d+)?)\s*id\b/i);
  if (idle) {
    const v = 100 - Number(idle[1]);
    if (Number.isFinite(v) && v >= 0 && v <= 100) out.cpuPercent = Math.round(v * 100) / 100;
  }

  const mem = text.match(/MiB Mem\s*:\s*([\d.]+)\s*total,\s*([\d.]+)\s*free,\s*([\d.]+)\s*used/i);
  if (mem) {
    const total = Number(mem[1]);
    const used = Number(mem[3]);
    if (Number.isFinite(total) && total > 0 && Number.isFinite(used)) {
      out.memoryPercent = Math.round((used / total) * 10000) / 100;
    }
  }

  // 'up 58 days,  3:14' or 'up 44 days, 19:38' or 'up 3:14' (no days).
  const up = text.match(/\bup\s+(?:(\d+)\s*days?,\s*)?(?:(\d+):(\d+))?/i);
  if (up && (up[1] || up[2])) {
    out.uptimeSeconds =
      Number(up[1] || 0) * 86400 + Number(up[2] || 0) * 3600 + Number(up[3] || 0) * 60;
  }
...
// PAN-OS `show session info` (SSH/CLI transport) -> {sessionCount, ...}.
//
// LIVE-VERIFIED 2026-08-05 against TUG (PAN-OS, mgmt_method='ssh'), per
// CLAUDE.md's "verify every field name against real output before writing a
// parser" rule. Until now this was left unparsed and sessionCount was returned
// as null on the SSH transport, because the output shape had never been seen.
// Confirmed real format (label, arbitrary whitespace, value):
//
//   Number of sessions supported:                    199998
//   Number of allocated sessions:                    6058
//   Number of active TCP sessions:                   3680
//   Number of active UDP sessions:                   2351
//   Session table utilization:                       3%
//
// ⛔ "allocated" is the ACTIVE SESSION COUNT — the figure the API transport
// also reports and the one the dashboard trends. It is NOT the sum of the
// per-protocol "active X sessions" lines (TCP+UDP+ICMP = 6052 vs allocated
// 6058 on the live capture); those omit several session classes, so summing
// them would under-report. Read the allocated line directly.
//
// Never throws — returns nulls for anything absent, same contract as every
// other parser in this file.
function parseSessionInfoOutput(text) {
  const out = { sessionCount: null, sessionsSupported: null, tableUtilizationPercent: null };
  if (typeof text !== 'string' || text.length === 0) return out;
  ...
  out.sessionCount = num('Number of allocated sessions');
  out.sessionsSupported = num('Number of sessions supported');
  out.tableUtilizationPercent = num('Session table utilization');
  return out;
}

module.exports = {
  parsePerformanceResourcesOutput,
  parseSessionInfoOutput,
  parseSystemInfoOutput,
  parseLicenseInfoOutput,
