...
//
// ── Why `set` format ─────────────────────────────────────────────────────────
// ssh.js enters configuration mode (`configure`), runs `set cli config-output-format
// set` THERE, then runs a bare `show` — that sequence is required; `show config
// running` from operational mode ignores the format preference entirely and
// always returns the brace tree (confirmed live 2026-07-16 against a real PA-440,
// see the CONFIGURE_MODE comment in ssh.js). Once in flat `set ...` form, this
// parser is built on it for these reasons:
