...
function parseBraceBlock(tokens, pos) {
  ...
  while (pos < tokens.length && tokens[pos].kind !== '}') {
    ...
    if (next.kind === '{') {
      pos += 1;
      const [child, afterChild] = parseBraceBlock(tokens, pos);
      pos = afterChild;
...
// Parses a brace-format config dump → NormalizedRule[]. Collects EVERY
// `rulebase/security/rules` container found anywhere in the tree (see
// findSecurityRulesContainers) and concatenates them in document order —
...
// yields zero containers found — an honest [], not a crash.
