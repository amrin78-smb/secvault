...
// real, legitimate 0). Per this file's "refuse to guess" rule (mirroring
// Fortinet's cliParser.countActiveVpnSessions()'s identical
// null-on-unrecognized-format contract), that unconfigured-gateway case
// returns null here, and ssh.js's caller throws — meaning this feature will
// simply fail (logged, skipped by the engine-worker poll job, never fatal)
// on every PAN-OS device that doesn't run GlobalProtect, until a real
// device confirms what that output actually looks like.
  ...
  try {
    ...
    for (const sec of sections) {
      for (const block of sec.body.split(/\n\s*\n/)) {
        ...
        if (!any) continue;
        const username = f.domainusername || f.username || f.primaryusername || null;
        ...
        out.push({
          ...
          source_ip: usableIp(f.publicip) || usableIp(f.clientsourceip) || usableIp(f.sourceip) || usableIp(f.publicipv6) || null,
          assigned_ip: usableIp(f.virtualip) || usableIp(f.assignip) || null,
