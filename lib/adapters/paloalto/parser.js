...
//   2. Tolerant, multi-candidate FIELD extraction per rule. The CONFIRMED
//      LIVE CLI/text version of this exact command (sshParser.js) uses a
//      DIFFERENT field vocabulary than the plain rulebase config-get shape
//      parseRuleEntry() above expects — most notably ONE combined
//      "application/service" field (e.g. "0:any/any/any/any") instead of
//      separate application/service fields. Since it's unknown whether
//      PAN-OS's XML rendering of this same command keeps that combined shape
//      or splits it into two elements, extractEffectiveAppsAndServices()
//      below tries the combined shape FIRST (matching the one real,
//      confirmed data point this codebase has), falling back to the
//      separate-fields shape rather than silently dropping applications/
//      services if the guess is wrong.
//
...
// "N:appname/proto/srcport/dstport" -> { app, proto, dstPort }. Same parsing
// convention as sshParser.js's parseAppServiceToken() for the identical
// field on the CLI/text transport — the leading "N:" is a display index, not
// meaningful data.
function parseAppServiceToken(rawToken) {
  const text = scalarText(rawToken);
  if (!text) return { app: null, proto: null, dstPort: null };
...
// See this section's header comment for why both shapes are tried.
function extractEffectiveAppsAndServices(entry) {
  const combined = entry['application/service'];
  if (combined !== undefined && combined !== null) {
    const members = memberStrings(combined);
    const tokens = members.length > 0 ? members : [scalarText(combined)].filter((t) => t !== null);
    const applications = [];
    const services = [];
  ...
  // Fallback shape: separate fields, config-get-style — untested for this
  // specific command, kept so a wrong guess about combined-vs-separate can't
  // silently drop every application/service instead of just picking wrong.
  return {
    applications: memberStrings(entry.application),
    services: memberStrings(entry.service),
  };
