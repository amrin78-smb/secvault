...
function parseRuleEntry(entry, idx) {
  if (!entry || typeof entry !== 'object') {
    return {
      ...
      log_enabled: true,
      comment: null,
      // NULL, not 0: this shape carries no hit-count data at all, and
      // "not measured" must never be stored as "zero hits" (schema.sql).
      hit_count: null,
  ...
  return {
    ...
    // they come from the op command `show rule-hit-count`, which index.js's
    // _enrichHitCounts() merges in AFTER this parse. NULL is the correct value
    // here: at this point the count is genuinely unknown, and if enrichment
    // fails it must STAY unknown rather than becoming a claim of zero hits.
    hit_count: null,
    raw_rule: entry,
  };
}
...
//
// ⛔ SUPERSEDED, AND THE GUESS WAS WRONG. Live capture from PAKFood on
// 2026-08-25 (PAN-OS 11.1.13-h5, the fleet's only Panorama-managed firewall)
// proves `show running security-policy` does NOT return structured XML at all:
// it returns the CLI's plain brace text wrapped in a single <member> element.
// The deep-walk below looks for objects carrying both '@_name' and an 'action'
// sibling; the parsed response has exactly one key ("member", holding a
// string), so it matched nothing and returned null on EVERY device. On the
// API transport this function was effectively dead code from the day it was
// written -- it could never succeed for anyone.
//
// It is KEPT, deliberately, as index.js's THIRD and last resort, in case a
// future PAN-OS release does return the documented shape. What actually
// collects rules from a Panorama-managed device today, in order:
//   1. api.showPushedSharedPolicy() -> parseRulesDeep()  (33 rules live)
//   2. the <member> CLI text -> sshParser.parseEffectiveSecurityPolicy()
//      (35 rules live), the same "the API returns the CLI text verbatim"
//      precedent getNatRules() already relies on
//
// The lesson is CLAUDE.md's own: the two design choices below were meant to
// bound the risk of guessing a response shape, and they did bound it (it
// failed closed to null rather than inventing rules) -- but they could not
// make the guess correct. Only a live device could, and the format it
// returned was not among the shapes considered plausible here.
//
...
function effectivePolicyEntryToNormalizedRule(entry, idx) {
  ...
  return {
    ...
    // same accepted gap as the SSH transport's identical fallback. NULL, so
    // that accepted gap reads as "unknown" and not as "zero hits".
    hit_count: null,
    raw_rule: entry,
  };
}
...
function normalizeGlobalProtectUsers(entries) {
  ...
  for (const e of list) {
    ...
    out.push({
      ...
      // ⛔ NOT `lifetime`. That is the CONFIGURED session lifetime, not elapsed
      // time, and picking it first made every row report the same number:
      // measured live, 137 of 144 sessions showed 2,592,000s (exactly 30 days)
      // and the other 7 showed 86,400s (exactly 24 hours), rendered in the UI
      // as "720h 0m" while the same rows' login_time said a few hours ago.
      // A Duration column that is identical for every user is a fabricated
      // measurement presented with total confidence — the hit_count rule, in a
      // column nobody thought to check because the number looked plausible.
      // If no elapsed figure is reported, store NULL and let the UI show "—".
      duration_seconds: gpIntOrNull(pickGpField(e, ['login-duration', 'session-duration'])),
      bytes_in: null,
      bytes_out: null,
...
function normalizeIpsecTunnels(result) {
  ...
  for (const e of entries) {
    ...
    // ⛔ DEFAULTING TO 'up' IS CORRECT HERE, AND ONLY HERE, AND ONLY BECAUSE OF
    // THE COMMAND. `show vpn ipsec-sa` lists ESTABLISHED Phase-2 SAs only, so an
    // entry existing IS the evidence — the same reasoning already documented for
    // cisco_asa's `show vpn-sessiondb l2l`. Measured live 2026-09-10: all 141
    // Palo Alto tunnel entries carry NEITHER a `state` nor a `status` field (the
    // payload is SA data — i_spi/o_spi/enc/life), so this branch decides every
    // one of them.
    //
    // ⛔ THE CONSEQUENCE IS THE PART THAT MATTERS: a DOWN tunnel does not appear
    // in this output AT ALL. So SecVault can never observe a down Palo Alto
    // tunnel, "0 down" is NOT a measurement, and the row count is
    // CURRENTLY-ESTABLISHED tunnels, not CONFIGURED ones. That limit is carried
    // as `downObservable` in lib/engines/vpnTunnelHealth.js so the UI states it
    // instead of rendering a reassuring zero — do not "fix" this to null without
    // reading that.
    let status = 'up';
    if (stateRaw) {
...
const SECRET_TAGS = [
  ...
  'api-key',
  'auth-code',
  // Added 2026-09-09 after a live audit found these leaking on the fleet.
  // ⛔ Matching in this file is EXACT TAG NAME, never substring, so a compound
  // name is NOT covered by its own suffix: `wmi-password` was never matched by
  // `password`. (gotchas.md's "universal keyword pattern" IS substring-based;
  // this Palo Alto copy has always been exact-name and has drifted from that
  // convention — so any compound secret name must be listed here in full.)
  'wmi-password', // User-ID agentless WMI probing account password. Stored right
  //                 beside a cleartext `<wmi-account>` holding a Windows DOMAIN
  //                 service account, so the blast radius reaches past the firewall.
  'agent-user-override-key', // GlobalProtect agent config override key.
];

// Tags redacted ONLY when the element is a TEXT LEAF — its body contains no
// child element at all.
//
// ⛔ `community` is the entire reason this second list exists, and it is not a
// softening of the fail-closed posture above. PAN-OS reuses this one tag name
// for two unrelated things; both confirmed present on the live fleet:
//   - `…snmp-setting…<version><v2c><server><entry><manager>…<community>` — an
//     SNMP community string, i.e. a credential. Always a text leaf.
//   - `…<bgp>…<import><rules><entry><action><allow><update><community>` — a BGP
//     community route-tagging ATTRIBUTE list. Always a CONTAINER (it holds
//     `<type>`/`<member>`/`<extended-community>` children).
// Listing `community` in SECRET_TAGS would replace that whole BGP subtree with
// `<redacted>`, destroying real routing policy in the stored snapshot for zero
// security benefit. The leaf/container split is NOT context cleverness about
// where the tag sits (which this file's header rightly warns against) — it is a
// property of the element itself, and it cannot hide a secret: a container's
// children each carry their own tag name and are matched on their own merits.
//
// `<extended-community>` is a DIFFERENT tag and is never matched by `community`,
// because every matcher below anchors on the `<` immediately preceding the name.
const SECRET_LEAF_TAGS = ['community'];
...
// ⛔ THE OPENING TAG MAY CARRY ATTRIBUTES. Fixed 2026-09-09 — this matcher used
// to be `(<${tag}>)…`, requiring a BARE opening tag. PAN-OS stamps `ptpl="…"`
// (and `src="tmpl"`) on every node pushed down from a Panorama TEMPLATE, so
// `<private-key ptpl="PA-220-Ranode">` and `<bind-password ptpl="…">` never
// matched and were persisted verbatim into device_configs.config_raw /
// config_backups.config_raw — both GRANT SELECT'd to claude_readonly /
// nocvault_readonly. The tag LIST was right; the matcher was wrong, which is why
// this read as working: the bare-tag occurrences on the same device redacted
// fine, so a spot check of `<phash>` looked clean.
//
// Why `(?:\s[^>]*)?` cannot over-match: the group is optional, and when present
// it must begin with WHITESPACE and, because `[^>]*` can never cross a `>`, it
// cannot run past the end of this one tag. So it matches `<key>` and
// `<key ptpl="x">` but NOT `<keyxchg-algo-rsa>`, `<key-something>` or
// `<public-key>` — a longer tag name has a non-space character where the group
// demands a space or a `>`. Verified against the live fleet, where
// `keyxchg-algo-dhe`/`-ecdhe`/`-rsa`, `public-key`, `password-label`,
// `password-complexity` and `extended-community` all sit next to the real
// secrets and must all survive.
const TAG_OPEN_ATTRS = '(?:\\s[^>]*)?';

function redactConfigXml(text) {
  if (typeof text !== 'string' || text.length === 0) return '';
  let result = text;
  for (const tag of SECRET_TAGS) {
    const elementRe = new RegExp(`(<${tag}${TAG_OPEN_ATTRS}>)([\\s\\S]*?)(</${tag}>)`, 'gi');
  ...
  // Leaf-only tags (see SECRET_LEAF_TAGS). `[^<]*` is what makes it leaf-only:
  // the body may not contain a child element. It still covers every real secret
  // body — an `-AQ=`-prefixed PAN-OS blob is base64 (A-Za-z0-9+/=) and a literal
  // '<' inside XML text would be escaped as `&lt;` — so nothing is missed by it.
  for (const tag of SECRET_LEAF_TAGS) {
    const leafRe = new RegExp(`(<${tag}${TAG_OPEN_ATTRS}>)([^<]*)(</${tag}>)`, 'gi');
    result = result.replace(leafRe, `$1${REDACTED_XML}$3`);
    // ⛔ `(?<![\w-])`, NOT `\b`, as the left boundary. `\b` matches between a
    // '-' and a letter, so `\bcommunity="` also fires inside
    // `extended-community="` — the exact over-redaction the leaf/container split
    // above exists to avoid. (The SECRET_TAGS attribute matcher above still uses
    // `\b`; that is its long-standing behaviour and narrowing an EXISTING
    // redaction is how you introduce a miss, so it is deliberately left alone.)
    const attrRe = new RegExp(`((?<![\\w-])${tag}=")([^"]*)(")`, 'gi');
    result = result.replace(attrRe, `$1${REDACTED_XML}$3`);
  }
  return result;
}
...
function redactConfigTree(node) {
  ...
  if (node && typeof node === 'object') {
    ...
    for (const [key, value] of Object.entries(node)) {
      ...
      // SECRET_LEAF_TAGS mirrors redactConfigXml's leaf-only rule in tree form:
      // a primitive value is the text leaf (the SNMP community string), an
      // object/array is the container (BGP's community attribute list) and is
      // walked normally so its own children are still matched on their merits.
      const leafOnly = SECRET_LEAF_TAGS.includes(bareKey);
      const isLeaf = value === null || typeof value !== 'object';
      const redact = SECRET_TAGS.includes(bareKey) || (leafOnly && isLeaf);
      out[key] = redact ? REDACTED_XML : redactConfigTree(value);
    }
    return out;
  }
  return node;
...
function parseHaStateXml(result) {
  ...
  // Capture the ACTUAL version values behind each compat verdict, both sides.
  // Without these, a degraded pair can only say "versions do not match", which
  // is not actionable — with them the UI can show
  // "URL Database: local 20260803.20149 vs peer 0000.00.00.000", which points
  // straight at the cause (live: a lapsed URL-filtering licence stopping the
  // passive peer's database updating).
  //
  // Every `<x>-compat` verdict has a matching `<x>-version` value on each side,
  // EXCEPT build-compat whose value field is `build-rel`.
  const versionFieldFor = (compatKey) =>
    compatKey === 'build-compat' ? 'build-rel' : compatKey.replace(/-compat$/, '-version');
  const versions = {};
  for (const compatKey of compatKeys) {
    const field = versionFieldFor(compatKey);
    const localVal = scalarText(local[field]);
    const peerVal = scalarText(peer[field]);
    if (localVal || peerVal) {
      versions[compatKey] = { local: localVal || null, peer: peerVal || null };
    }
  }

  const mgmtIp = scalarText(peer['mgmt-ip']);
  const raw = { source: 'api-xml' };
  if (Object.keys(versions).length > 0) raw.versions = versions;
...
module.exports = {
  ...
  redactConfigXml,
  redactConfigTree,
  // Exported 2026-09-09 for lib/migrate.js's backfillPaloAltoConfigRedaction(),
  // which re-redacts rows already persisted before the attribute-matching fix
  // above. It deliberately does NOT reuse redactConfigXml() itself (that one
  // redacts a whole subtree, which cannot be done to stored history without
  // risking real content) — but it MUST read the tag names from here, so a tag
  // added to either list is covered retroactively as well as going forward.
  SECRET_TAGS,
  SECRET_LEAF_TAGS,
  extractObjects,
  parseInterfacesXml,
  ...
  mapAction,
  scalarText,
  // Exported 2026-08-04 for components/config/DiffViewer.js, which needs to turn a
  // whole-rule add/remove entry from an XML/API config diff back into a
  // NormalizedRule so it can render a rule table instead of a raw key/value dump.
  // Same reuse precedent as sshParser's `ruleFromBraceEntry`, which that component
  // already imports for the SSH-transport equivalent. Pure and dependency-light
  // (this file's only require is the dep-free versionComparator), so pulling it
  // into a client bundle costs nothing.
  parseRuleEntry,
};
