...
class PaloaltoAdapter extends FirewallAdapter {
  ...
  async getVpnSessionSummary() {
    ...
    if (entries === null) {
      // Genuinely unrecognized shape (an object with neither a scannable
      // 'entry' key nor PAN-OS's own confirmed-empty <result/> shape) —
      // refuse to guess, the same "no confident-looking wrong answer"
      // discipline Fortinet's cliParser.countActiveVpnSessions() applies to
      // its own unrecognized-output case.
      throw new Error(
        `PAN-OS \`show global-protect-gateway current-user\` op command on device ${this.device.id} ` +
          'returned a result shape with no recognizable <entry> element(s) and no confirmed-empty ' +
          '<result/> either — refusing to guess an active session count. Check the [PaloAlto Debug] ' +
          "log above against parser.js's parseGlobalProtectCurrentUsers()."
      );
    }
