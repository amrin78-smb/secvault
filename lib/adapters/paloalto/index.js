...
class PaloaltoAdapter extends FirewallAdapter {
  ...
  async getRules() {
    ...
    if (rules.length > 0) {
      ...
      // Only reached on the primary single-vsys path, where the vsys queried
      // for rules (api.DEFAULT_VSYS) is unambiguous. Runs AFTER the real
      // ruleset is already built, so a failure here can never affect what
      // getRules() returns for the rules themselves.
