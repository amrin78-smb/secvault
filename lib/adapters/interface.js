...
/**
 * Thrown when SecVault REACHED the device and the device answered, but the
 * requested capability could not be read — the feature is not configured, or
 * the output did not match any format this adapter recognises.
 *
 * ⛔ THIS IS NOT A CONNECTIVITY FAILURE, and conflating the two is what this
 * class exists to stop. On 2026-09-09 a FortiGate with no SSL-VPN configured
 * returned unrecognised output from `get vpn ssl monitor`; the adapter
 * correctly refused to guess a session count and threw — and the poller
 * recorded that as the DEVICE BEING UNREACHABLE. The device list then said
 * "Failing 0% of polls succeeding" about a firewall that was answering every
 * other poll, had just been collected in full, and was reachable by SSH.
 *
 * ⛔ "This feature is not present" recorded as "this device is down" is the
 * same failed-read-as-a-fact rule CLAUDE.md documents for hit_count, one layer
 * up: an absent capability turned into a negative fact about the device.
 *
 * Throw this ONLY when the transport genuinely succeeded. If the SSH connect,
 * the login or the API call failed, throw a normal Error — that IS reachability
 * evidence and must keep counting against the device.
 */
class CapabilityUnavailableError extends Error {
  constructor(message, { capability = null, cause = null } = {}) {
    super(message);
    this.name = 'CapabilityUnavailableError';
    this.capability = capability;
    // Flag as well as instanceof: adapters and engines are loaded through
    // several paths here, and an instanceof across two module instances of the
    // same file silently returns false. Callers check the flag.
    this.deviceWasReached = true;
    if (cause) this.cause = cause;
  }
}

/** True for an error that must NOT count against device reachability. */
function isCapabilityUnavailable(err) {
  return Boolean(err && err.deviceWasReached === true);
}

module.exports = { FirewallAdapter, CapabilityUnavailableError, isCapabilityUnavailable };
