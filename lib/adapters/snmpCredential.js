...
function parseSnmpCredential(plaintext) {
  ...
  if (parsed.version === 'v3') {
    ...
    const authPasswordProvided = typeof parsed.authPassword === 'string' && parsed.authPassword !== '';
    const privPasswordProvided = typeof parsed.privPassword === 'string' && parsed.privPassword !== '';
    if (authPasswordProvided && !VALID_AUTH_PROTOCOLS.includes(parsed.authProtocol)) {
      throw new Error('Stored SNMPv3 credential has an authentication password but no valid "authProtocol" (MD5/SHA). Re-save it under the device SNMP tab.');
    }
    if (privPasswordProvided && !authPasswordProvided) {
      throw new Error('Stored SNMPv3 credential has a privacy password but no authentication password — SNMPv3 requires authentication before privacy. Re-save it under the device SNMP tab.');
    }
    if (privPasswordProvided && !VALID_PRIV_PROTOCOLS.includes(parsed.privProtocol)) {
      throw new Error('Stored SNMPv3 credential has a privacy password but no valid "privProtocol" (DES/AES). Re-save it under the device SNMP tab.');
    }
    return {
      version: 'v3',
      username: parsed.username,
      authProtocol: authPasswordProvided ? parsed.authProtocol : null,
      authPassword: authPasswordProvided ? parsed.authPassword : null,
      privProtocol: privPasswordProvided ? parsed.privProtocol : null,
      privPassword: privPasswordProvided ? parsed.privPassword : null,
    };
  }
