...
// Returns NULL, not 0, when the rule carries no hits block. The Mgmt API only
// includes `hits` when the caller asked for it AND the rulebase tracks it;
// absent means UNKNOWN, and reporting unknown as "zero hits" is what made
// ruleAnalysis fabricate `unused` findings (see lib/schema.sql).
...
function parseRulebasePages(pages) {
  ...
  // ⛔ SECOND HALF OF THE SAME BUG, fixed 2026-09-09. Two defects remained:
  //
  //   1. A null / non-object page was `continue`d WITHOUT being counted, so
  //      `malformedPages` stayed 0 and the warning below never fired for the
  //      most likely real-world shape of all: api.js's request() returns
  //      `parsed = null` for ANY HTTP-200 whose body is empty or not JSON — a
  //      corporate proxy, a captive portal or a load balancer answering with
  //      HTML. That page vanished silently.
  //   2. Even the counted case only WARNED and still returned the surviving
  //      rules (or []). collectAndStore() DELETEs this device's firewall_rules
  //      before reinserting whatever comes back, so a warning in engine.log is
  //      not a safeguard: a partial page set silently truncates a real ruleset
  //      and an all-malformed page set wipes it outright, both reported as a
  //      successful collection.
  //
  // A malformed page is a FAILED READ, so it throws — CLAUDE.md's "getRules()
  // must THROW on a retrieval failure, never return []", applied to the
  // partial case as well as the empty one (same reasoning as Fortinet's
  // "collect every VDOM or fail"). A page whose `rulebase` is a genuinely
  // EMPTY array is not malformed and still yields an honest [].
  const flat = [];
  let malformedPages = 0;
  for (const page of pageList) {
    if (!page || typeof page !== 'object' || !Array.isArray(page.rulebase)) {
      malformedPages += 1;
      continue;
    }
  ...
  if (pageList.length === 0) {
    throw new Error(
      'CheckPoint parseRulebasePages: no show-access-rulebase pages were supplied at all — there ' +
        'is no evidence about this layer\'s rulebase either way. Refusing to report zero rules.'
    );
  }

  if (malformedPages > 0) {
    throw new Error(
      `CheckPoint rule collection failed: ${malformedPages} of ${pageList.length} ` +
        'show-access-rulebase page(s) had no usable "rulebase" array (the page was null/not an ' +
        'object — api.js returns null for any HTTP-200 with an empty or non-JSON body, e.g. a proxy ' +
        'or captive-portal response — or "rulebase" was missing or the wrong type on this Mgmt API ' +
        `version). ${flat.length} rule(s) were parsed from the remaining page(s), but a partial ` +
        'rulebase is not a rulebase: collectAndStore() DELETEs this device\'s firewall_rules before ' +
        'reinserting, so returning what survived would silently truncate or wipe the real ruleset. ' +
        'Check the [CheckPoint Debug] raw response and this file\'s field mappings.'
    );
  }
...
// Entry point for getRules() when the policy package has MORE THAN ONE access
// layer (R80+ "Ordered Layers", a normal configuration — not an edge case).
//
// ⛔ index.js used to take `layers[0]` and store that one layer as if it were
// the device's whole rulebase — the same class of silent wrong answer as the
// already-fixed `packages[0]` bug, one level down: every rule in layers 2..n
// simply did not exist as far as SecVault was concerned, so shadow analysis,
// unused-rule findings and CVE applicability all reasoned over a rulebase that
// was never the device's. Positional picking is banned here for the same
// reason it is banned for policy packages.
//
// `layerGroups`: [{ layerUid, layerName, pages }] in the package's OWN declared
// order. Every layer's pages are parsed with parseRulebasePages() above — so a
// malformed page in ANY layer throws, exactly like a partial single-layer read.
//
// Sequence numbers: Check Point restarts `rule-number` at 1 inside each layer,
// so keeping the per-layer numbers would give this device several rules at
// position 1 and make ordering analysis meaningless. Across multiple layers the
// sequence is therefore renumbered continuously in layer order, and the
// originating layer is preserved in raw_rule so provenance is not lost. The
// SINGLE-layer case (the overwhelmingly common one) is passed straight through
// to parseRulebasePages() unchanged, so nothing about existing collections
// moves.
function parseLayeredRulebasePages(layerGroups) {
  const groups = Array.isArray(layerGroups) ? layerGroups : [];

  if (groups.length === 0) {
    throw new Error(
      'CheckPoint parseLayeredRulebasePages: no access layers supplied — refusing to report a ' +
        'device as having zero rules on no evidence.'
    );
  }

  if (groups.length === 1) {
    return parseRulebasePages(groups[0] && groups[0].pages);
  }

  const out = [];
  for (const group of groups) {
    const layerLabel = (group && (group.layerName || group.layerUid)) || null;
    const layerRules = parseRulebasePages(group && group.pages);
    for (const rule of layerRules) {
      const rawRule =
        rule.raw_rule && typeof rule.raw_rule === 'object' && !Array.isArray(rule.raw_rule)
          ? { ...rule.raw_rule, _secvault_access_layer: layerLabel }
          : rule.raw_rule;
      out.push({ ...rule, sequence_number: out.length + 1, raw_rule: rawRule });
    }
  }
...
//
// ⛔ Bug fixed 2026-07-19: this had no obj.type check at all, so on a
// distributed deployment (device.mgmt_ip == the MANAGEMENT SERVER's own IP —
// see the comment above) the ipv4-address half of the match could hit the
// Security Management Server's own 'server' object (show-gateways-and-servers
// is documented to return checkpoint-host/server objects alongside
// simple-gateway/simple-cluster ones, not gateways exclusively) before the
// real gateway's name-based match was ever reached, since list order from the
// API is not guaranteed. isGatewayLikeType() now requires the candidate's own
// `type` to actually look like a gateway/cluster object — matching this same
// file's existing type.includes('cluster') convention (see index.js's
// _showGatewayElement) — so a same-IP management/log-server object is never
// mistaken for the gateway.
function isGatewayLikeType(type) {
  return typeof type === 'string' && /gateway|cluster/i.test(type);
}
...
// Widened 2026-07-30 to match the canonical form used everywhere else in this
// codebase (lib/adapters/forcepoint/parser.js's SECRET_KEY_PATTERN and
// lib/engines/configDiff.js's SECRET_PATH_PATTERN — see .ai-codex/gotchas.md's
// Redaction rules section, which flagged this file's copy as the one
// remaining narrower, out-of-step form): added `phash`, `pre[-_]?shared`, and
// `keytab`, none of which the previous pattern matched.
...
// show-administrators → normalized admin/local-user account list, feeding
// getConfig()'s config_parsed.administrators.
//
// ⚠️ DOC-DERIVED, NOT LIVE-VERIFIED — no Check Point management server
// exists in this deployment (same caveat as every other field mapping in
// this file). Field names below are cross-referenced from TWO independent
// public sources that agree with each other: (1) real JSON examples posted
// on Check Point's own CheckMates community forum showing an `objects[]`
// array of administrator entries with hyphenated keys (name,
// authentication-method, permissions-profile.name, must-change-password),
// and (2) the Ansible check_point.mgmt.cp_mgmt_administrator module, which
// is generated from Check Point's own Management API schema and documents
// the same field set (name, authentication_method, email, phone_number,
// permissions_profile, must_change_password, expiration_date, comments) —
// underscored there because Ansible module parameters are always
// snake_case, but the live API itself uses this codebase's established
// kebab-case convention (see e.g. 'access-policy-name', 'ipv4-address'
// elsewhere in this file), which is why the lookups below use hyphenated
// keys. Never throws — a missing/renamed field degrades to null, matching
// this file's own header rule ("pure mapping functions... must NEVER
// throw"). `permissions-profile` can arrive as an inline object (expected
// when the caller requests 'details-level': 'full', same assumption
// getObjects()'s group-member resolution already makes and documents as
// unverified) or a bare uid string — extractMemberName() already handles
// both shapes, reused here rather than duplicating that fallback logic.
function parseAdministratorObjects(objects) {
  const list = Array.isArray(objects) ? objects : [];
  return list
    .filter((obj) => obj && typeof obj === 'object' && typeof obj.name === 'string')
    .map((obj) => ({
      name: obj.name,
      uid: obj.uid || null,
      authentication_method: obj['authentication-method'] || null,
      email: obj.email || null,
      phone_number: obj['phone-number'] || null,
      must_change_password:
        obj['must-change-password'] === undefined ? null : Boolean(obj['must-change-password']),
      expiration_date:
        typeof obj['expiration-date'] === 'string' || typeof obj['expiration-date'] === 'number'
          ? obj['expiration-date']
          : null,
      permissions_profile: extractMemberName(obj['permissions-profile']),
