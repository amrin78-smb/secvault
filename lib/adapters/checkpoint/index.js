...
let loggedFirstAdministratorsPage = false;
let loggedFirstSnmp = false;

// SNMP OIDs for getSnmpMetrics() — deliberately STANDARD MIB-II / HOST-
// RESOURCES-MIB ONLY, same scope decision as lib/adapters/sangfor/index.js's
// getSnmpMetrics() (see that file's own comment block for the full
// reasoning). Check Point publishes its own CHECKPOINT-MIB (fwNumConn,
// multi-proc CPU/memory tables, etc.) but this codebase has no live Check
// Point management server OR gateway to confirm those enterprise OIDs
// against (CLAUDE.md "External API Integrations": never assume field names/
// OIDs from docs alone) — inventing a plausible-looking CHECKPOINT-MIB OID
// set here would be exactly the "guessing ungrounded syntax" CLAUDE.md's
// live-verification rule warns against. Only RFC 1213 (MIB-II) and RFC 2790
// (HOST-RESOURCES-MIB) OIDs are used, which essentially every SNMP agent
// (including Gaia OS, which is Linux-based) implements regardless of vendor.
// lowConfidence is unconditionally true for the same reason Sangfor's is:
// even a clean response says nothing about whether Check Point's agent maps
// these OIDs the way the MIB intends.
...
class CheckpointAdapter extends FirewallAdapter {
  ...
  async getRules() {
    ...
    return api.withSession(conn, async (session) => {
      ...
      // ⛔ NEVER pick an access layer positionally — fixed 2026-09-09. This used
      // to warn and then collect `layers[0]` only, storing one layer as if it
      // were the device's entire rulebase: the same class of silent wrong
      // answer as the already-fixed `packages[0]` bug, and R80+ Ordered Layers
      // are a normal configuration, not an edge case. Every layer is now
      // collected, in the package's own declared order — and if ANY layer
      // cannot be identified or read, the whole collection fails rather than
      // storing a partial rulebase (same rule as Fortinet's "collect every VDOM
      // or fail"): collectAndStore() DELETEs firewall_rules before reinserting,
      // so a partial result is destructive, not merely incomplete.
      if (layers.length > 1) {
        console.warn(
          `[CheckPoint] Package "${pkg.name || pkg.uid}" has ${layers.length} access layers ` +
            '(R80+ Ordered Layers) — collecting ALL of them in the package\'s declared order and ' +
            'renumbering sequence_number continuously across them (Check Point restarts rule-number ' +
            'at 1 in each layer). The originating layer is preserved on each rule\'s raw_rule.'
        );
      }
      ...
      for (const layer of layers) {
        ...
        const layerUid = isObj ? layer.uid : layer;
        const layerName = isObj ? layer.name || null : null;
        if (!layerUid) {
          throw new Error(
            `Access layer ${layerGroups.length + 1} of ${layers.length} on package ` +
              `"${pkg.name || pkg.uid}" has no uid — cannot query its rulebase. Refusing to collect ` +
              'the remaining layers on their own: a partial rulebase would overwrite this device\'s ' +
              'real one.'
          );
        }
        const pages = await this._fetchAccessRulebasePages(session, layerUid);
        layerGroups.push({ layerUid, layerName, pages });
      }

      return parser.parseLayeredRulebasePages(layerGroups);
    });
  }

  // → { raw: string, parsed: object }
  // parsed = { gateway, api_versions, administrators } — feeds the Phase 6
  // dot-path predicate engine, which walks parsed config by dot-path.
  async getConfig() {
    const conn = await this._getConn();
    return api.withSession(conn, async (session) => {
      const gateway = await this._findGateway(session);
      ...
      // Admin/local-user account listing — server-wide, like getObjects()'s
      // catalog calls, not gateway-scoped (Check Point management-server
      // "administrator" objects are management-plane users, not Gaia OS
      // accounts on the gateway itself — there is no API surface for the
      // latter from the management server). `show-administrators` is a real,
      // documented Management API command (confirmed via the public API
      // reference index and independently corroborated field names from
      // Check Point's own community forum JSON examples and the Ansible
      // check_point.mgmt.cp_mgmt_administrator module, which is generated
      // from the same API schema) — NOT fabricated, but still UNVERIFIED
      // against a real management server (no live Check Point server exists
      // in this deployment — see CLAUDE.md "External API Integrations").
      // Independently try/caught, like api_versions above: a permissions
      // failure here (the API user may not have rights to read admin
      // accounts) must not fail the whole config collection.
      let administrators = null;
      try {
        const adminObjects = await this._fetchAllPages(session, 'show-administrators', {});
        if (!loggedFirstAdministratorsPage) {
          loggedFirstAdministratorsPage = true;
          console.log(
            '[CheckPoint Debug] show-administrators raw objects (redacted):',
            JSON.stringify(parser.redactSecrets(adminObjects), null, 2)
          );
        }
        administrators = parser.parseAdministratorObjects(adminObjects);
      } catch (err) {
        console.warn(`[CheckPoint] show-administrators failed during getConfig: ${err.message}`);
      }
      ...
      // sibling adapter already applies. `administrators` goes through the
      // same redactSecrets() pass — parseAdministratorObjects() never carries
      // a raw password/password-hash field through on purpose, but redacting
      // again here is defence-in-depth, matching how every other field in
      // this method is handled.
      const parsed = {
        gateway: parser.redactSecrets(gateway),
        api_versions: parser.redactSecrets(apiVersions),
        administrators: parser.redactSecrets(administrators),
      };
  ...
  // (credential_type='snmp') from this adapter's own Management API
  // credential — never gated on / mixed with testConnectivity()/getRules()'s
  // auth, and does NOT go through _getConn()/api.js/_findGateway() at all.
  //
  // ⛔ CRITICAL, same exception class as Forcepoint's (see
  // lib/adapters/forcepoint/index.js's own getSnmpMetrics() comment and
  // CLAUDE.md's Forcepoint SNMP exception): this.device.mgmt_ip is the
  // Check Point SECURITY MANAGEMENT SERVER's address, never the gateway's —
  // every other method in this file (testConnectivity/getVersion/getRules/
  // getConfig/getObjects) is correct to talk to mgmt_ip because they all go
  // through the Management API, but an SNMP agent lives on the GATEWAY
  // appliance, not the management server. Falling back to mgmt_ip here (the
  // way most other vendors' getSnmpMetrics() do) would silently SNMP-poll
  // the wrong box — the management server, not the firewall — producing
  // misleading or nonsense data with no signal anything was wrong. snmp_host
  // is therefore REQUIRED, not an optional override, and this throws a
  // clear, actionable error rather than guessing when it's unset.
  //
  ...
  async getSnmpMetrics() {
    ...
    if (!targetHost) {
      throw new Error(
        `Device ${this.device.id} ("${this.device.name}") has no snmp_host configured. Check Point SNMP ` +
          'polling talks directly to the gateway appliance, not the Security Management Server — set ' +
          "snmp_host to the gateway's own management IP under the device SNMP tab (this device's " +
          'mgmt_ip is the management server, not the gateway, and is deliberately never used as a ' +
          'fallback here).'
      );
    }
    ...
    try {
      ...
      if (!loggedFirstSnmp) {
        ...
        console.log(
          '[CheckPoint SNMP Debug] raw responses:',
      ...
      return {
        ...
        // SNMP representation, only vendor-proprietary MIBs (Check Point's
        // own CHECKPOINT-MIB has fwNumConn, but per this method's own header
        // comment, no live device/gateway exists to confirm that OID against,
        // so it is deliberately not attempted here). sessionCount is always
