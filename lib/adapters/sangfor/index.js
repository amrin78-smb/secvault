...
class SangforAdapter extends FirewallAdapter {
  ...
  // { command, output, connected, lastError, rejections }:
  //   command/output — the first command that produced usable output (or nulls)
  //   connected      — true if at least one runCommands call completed (SSH worked)
  //   lastError      — the last thrown error, for diagnostics
  //   rejections     — commands the DEVICE answered with a CLI rejection banner
  //
  // ⛔ NON-EMPTY IS NOT SUCCESS (fixed 2026-09-09). This used to accept the
  // first output with any characters in it. A rejection banner (`% Invalid
  // input detected at '^' marker.`) is non-empty, so an unsupported command
  // "won", the banner was cached as this device's config, and the remaining
  // dialect fallbacks were never tried — defeating the exact mechanism this
  // three-command list exists to provide. See parser.detectCliRejection() for
  // why this is a NEGATIVE guard only and never a positive parse.
  async _tryCommands(conn, options, commands) {
    ...
    for (const command of commands) {
      try {
        ...
        if (output.trim().length > 0) {
          const rejection = parser.detectCliRejection(output);
          if (rejection) {
            rejections.push(`\`${command}\`: ${rejection}`);
            console.warn(
              `[Sangfor] \`${command}\` was REJECTED by the CLI on ${conn.host} ` +
                `(${rejection}) — not treating the banner as output; trying next fallback.`
            );
            continue;
          }
          return { command, output, connected, lastError, rejections };
        }
    ...
    return { command: null, output: null, connected, lastError, rejections };
  }
  ...
  async _getConfigText() {
    ...
    return {
      ...
      // Carried through so getRules()/getConfig() can say WHICH failure this
      // was: "the device refused every command we know" reads very differently
      // from "SSH never connected", and the operator needs the banner text to
      // add this firmware's dialect to CONFIG_COMMANDS.
    ...
    if (result.text === null) {
      throw new Error(
        `Sangfor rule collection failed: all of ${CONFIG_COMMANDS.map((c) => `\`${c}\``).join(', ')} ` +
          `failed, returned no output, or were rejected by the device's CLI${
            result.lastError ? ` (last error: ${result.lastError.message})` : ''
          }${
            result.rejections && result.rejections.length > 0
              ? ` (CLI rejections: ${result.rejections.join(' | ')} — this firmware's dialect is ` +
                'not in CONFIG_COMMANDS; a rejection banner is NOT a config)'
              : ''
      ...
      throw new Error(
        `Sangfor config collection failed: all of ${CONFIG_COMMANDS.map((c) => `\`${c}\``).join(', ')} ` +
          `failed, returned no output, or were rejected by the device's CLI${
            ...
            result.rejections && result.rejections.length > 0
              ? ` (CLI rejections: ${result.rejections.join(' | ')})`
              : ''
          }. Check SSH reachability/credentials and the [Sangfor Debug] logs. ` +
          'Storing a rejection banner as this device\'s config would make hasUsableConfig() true ' +
          'and score every compliance predicate `fail` instead of `na` — SecVault\'s own blind ' +
          'spot recorded as a fault on the firewall.'
      );
    }
  ...
  // ⛔ getVpnSessionSummary() — DELIBERATELY NOT IMPLEMENTED (evaluated
  // 2026-07-30, researched live via web search/fetch, not skipped on a
  // hunch). Same "won't guess ungrounded syntax" discipline as getObjects()
  // above, applied to a second capability.
  //
  // What was checked before deciding to skip: the Sangfor community forum
  // (community.sangfor.com — including its "NGAF CLI" thread and its
  // console-port-access thread) and the NGAF User Manuals for v8.0.6,
  // v8.0.35, and v8.0.47 (the versions with any public English-language
  // documentation at all). None of them publish a CLI command for querying
  // active SSL-VPN/VPN sessions. The one concrete CLI command surfaced by
  // any of this research is unrelated (`show ip lease`, for the DHCP lease
  // table). More importantly, the v8.0.47 manual states outright: "The
  // CONSOLE interface is for development, test, and debugging only.
  // End-users need to access the device by the networking interface and
  // logging in to the console" — i.e. Sangfor's own documentation frames
  // VPN monitoring ("Online Users") as a WEB-GUI feature, not a published
  // CLI surface, unlike FortiOS's `get vpn ssl monitor` (public CLI
  // reference, live-confirmed in lib/adapters/fortinet/ssh.js) or a Cisco
  // ASA `show vpn-sessiondb` equivalent.
  //
  // Any command this method could send (`show vpn session`, `display vpn
  // ssl monitor`, or similar) would be invented by analogy to OTHER
  // vendors' conventions, not grounded in anything Sangfor-specific — the
  // same category of risk CLAUDE.md's live-verification rule and this
  // file's own getObjects() comment both reject. A wrong guess here is
  // worse than no feature: services/engine-worker.js's VPN session poller
  // only calls this method when `typeof adapter.getVpnSessionSummary ===
  // 'function'`, so simply not defining it is a clean, honest "not
  // supported for this vendor" — never a fabricated 0 or a fabricated
  // count from a misparsed error page.
  //
  // Revisit only once a real NGAF is reachable and a `[Sangfor Debug]`-style
  // raw capture of an actual VPN-monitoring CLI command exists to parse
  // against — do not add this method from documentation alone.

  // ⛔ Local admin/user account listing — DELIBERATELY NOT ADDED to
  // getConfig()'s `parsed.sections` (evaluated 2026-07-30, same research
  // pass as getVpnSessionSummary() above). lib/engines/adminAccountSummary.js
  // (read, not owned by this adapter folder) already documents, in its own
  // dispatch-table comment, that Sangfor collects no admin/user account data
  // today and deliberately falls through to `supported: false` — this
  // decision keeps that true rather than adding an inert or, worse,
  // fabricated field.
  //
  // The blocker is identical in kind to getObjects()'s: recognizing a local
  // admin/user definition needs an unverified block-header keyword (Fortinet
  // uses `config system admin` / `edit "<name>"`; Palo Alto uses
  // `mgt-config users`; Sangfor's own equivalent — if `show running-config`/
  // `display current-configuration` even includes local admin accounts
  // inline rather than storing them outside the running-config entirely —
  // has no captured sample anywhere in this codebase and no public manual
  // section documenting it). Guessing a keyword like `local-user` or
  // `admin-user` to grep the config dump this adapter already has in hand
  // would fabricate an admin-account listing (and therefore a superuser
  // count) exactly as confidently as a real one, with zero way for a reader
  // of the resulting dashboard to tell the two apart — worse than simply not
  // reporting it, per this codebase's applicability tri-state discipline
  // (CLAUDE.md: never a confident-looking wrong answer).
  //
  // Revisit once a real NGAF's `[Sangfor Debug]` config-text preview
  // (already logged by _getConfigText() above) has actually been inspected
  // for whatever local-admin-account syntax that firmware really uses, and
  // — separately, outside this file's ownership — a `sangfor` entry is
  // added to adminAccountSummary.js's SUMMARIZERS dispatch to read it.
}
