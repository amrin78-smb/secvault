...
// A line that gives version-number context (vs. an incidental dotted number
// elsewhere in the banner, e.g. a management URL/IP or a build-date string).
const VERSION_CONTEXT_LINE_RE = /\b(version|firmware|release|sys[\s-]?ver)\b/i;
...
// Finds the dotted version-number token most likely to be the real device
// version, rather than the first dotted number anywhere in a multi-line
// banner (which could be a management URL/IP or a build-date string that
// happens to appear earlier in the output).
//
// Preference order:
//   1. First dotted-number match on a line that also carries version context
//      (a "version"/"firmware"/"release"/"sysver" keyword).
//   2. Fallback — first dotted-number match anywhere in the text (the
//      original behavior), so any input that parsed successfully before
//      this fix keeps parsing successfully now.
// Never throws.
function findVersionToken(text) {
  try {
    const lines = text.split(/\r?\n/);
    for (const line of lines) {
      if (VERSION_CONTEXT_LINE_RE.test(line)) {
  ...
  const fallback = text.match(VERSION_RE);
...
function parseVersionOutput(text) {
  ...
  try {
    const versionToken = findVersionToken(text);
    if (versionToken) result.version_string = versionToken;
...
// ---------------------------------------------------------------------------
// detectCliRejection(output) → string|null   (added 2026-09-09)
//
// ⛔ NON-EMPTY OUTPUT IS NOT SUCCESS. index.js's _tryCommands() accepted the
// first command whose output had any characters in it at all. A CLI rejection
// banner — `% Invalid input detected at '^' marker.`, `Error: Unrecognized
// command found at '^' position.` — is non-empty, so it was:
//   1. cached as this device's CONFIG and stored in device_configs;
//   2. short-circuiting the remaining CONFIG_COMMANDS, which DEFEATS the whole
//      dialect-fallback mechanism this adapter is built around (Sangfor NGAF is
//      Cisco-flavoured on some firmware lines and Huawei-flavoured on others,
//      which is exactly why there are three candidate commands);
//   3. downstream, hasUsableConfig() returns TRUE for the object built from
//      that banner, so lib/engines/applicability.js scores every compliance
//      predicate `fail` instead of `na` — SecVault's own inability to read the
//      device, recorded as a negative fact ABOUT the device.
//
// ⛔ This is deliberately a NEGATIVE guard only. It detects a rejection; it
// does NOT try to positively confirm that some output IS a valid Sangfor
// config. CLAUDE.md forbids writing a parser for hardware that cannot be
// verified against, and no live NGAF has ever been connected to this codebase
// (see "Live Validation Status") — a positive-anchor check would be exactly the
// guess that rule exists to prevent. A rejection banner, by contrast, is a
// vendor-independent CLI convention with a stable, tiny surface.
//
// Returns the offending line (a short reason string) or null. Pure, never
// throws.
// ---------------------------------------------------------------------------
const CLI_REJECTION_PATTERNS = [
  /^%\s*\S/, // Cisco-flavoured: "% Invalid input detected at '^' marker."
  /^\s*\^\s*$/, // the bare caret marker line that accompanies it
  /\berror\s*:/i, // Huawei-flavoured: "Error: Unrecognized command found ..."
  /\binvalid\s+(input|command|parameter)\b/i,
  /\bunrecognized\s+command\b/i,
  /\bunknown\s+command\b/i,
  /\bincomplete\s+command\b/i,
  /\bambiguous\s+command\b/i,
  /\bbad\s+command\b/i,
  /\bcommand\s+not\s+found\b/i,
  /\bsyntax\s+error\b/i,
  /\bpermission\s+denied\b/i,
  /\b(insufficient|inadequate)\s+(privilege|permission)/i,
  /\bnot\s+(supported|authorized)\b/i,
  /\baccess\s+denied\b/i,
];

// A real running-config is long and can legitimately contain any of the words
// above inside an object name or a comment, so a bare keyword search over a
// whole dump would produce false rejections — and a false rejection is itself a
// discarded real config. Two bounded shapes only, both matching how a CLI
// actually answers a command it refused:
//   - the FIRST non-empty line is a rejection (the banner comes immediately
//     after the echoed command), or
//   - the entire response is SHORT (<= 6 non-empty lines) and any line matches.
const CLI_REJECTION_MAX_LINES = 6;

function detectCliRejection(output) {
  ...
  const matches = (line) => CLI_REJECTION_PATTERNS.some((re) => re.test(line));

  if (matches(lines[0])) return lines[0];

  if (lines.length <= CLI_REJECTION_MAX_LINES) {
    const hit = lines.find(matches);
    if (hit) return hit;
  }
  ...
  for (const block of blocks) {
    try {
      ...
      rules.push({
        ...
        comment: acc.comment,
        // Sangfor's CLI exposes no per-rule hit counter at all -- NULL, not 0.
        hit_count: null,
        // SECURITY: raw_rule is persisted verbatim into firewall_rules.raw_rule
        // (jsonb), which is whole-table GRANT SELECT'd to the claude_readonly /
        // nocvault_readonly diagnostic roles with no column exclusion — unlike
        // device_configs, nothing downstream redacts this. HEADER_RE is broad
        // enough that a block matched as a "policy"/"rule" block could still
        // contain an embedded secret line (e.g. a VPN/crypto block using
        // "policy" framing with a PSK). Redact only at this storage point —
        // field extraction above operates on the original unredacted
        // block.lines so legitimate tokens (e.g. "key-chain") are never
        // mangled by the fail-closed keyword redaction.
        raw_rule: { source: 'sangfor-cli-config', text: redactConfig(blockText) },
      });
    } catch (err) {
...
// ---------------------------------------------------------------------------
// parseConfigSections(text) → { hostname?, version?, interfaces?, ssl_vpn? }
// Best-effort structural hints from the raw config. Fields are simply absent when
// nothing recognizable is found. Never throws.
// Exception: ssl_vpn is always present ({ enabled: null|true|false }) once this
// function reaches that point — null means "could not determine", not absence —
// see the low-confidence caveat at its assignment site below.
// ---------------------------------------------------------------------------
function parseConfigSections(text) {
  ...
  try {
    ...
    // ⛔ LOW CONFIDENCE, added 2026-07-19: Sangfor is this codebase's least-verified
    // adapter (see CLAUDE.md "Live Validation Status") — no live device has ever
    // confirmed ANY Sangfor CLI field name, including this one. This regex is a
    // best-effort guess at a plausible SSL VPN status line based on general Sangfor
    // CLI documentation, not confirmed syntax. Treat sections.ssl_vpn.enabled === null
    // as the expected, common case (undetected), not a sign of failure. Correct this
    // against real Sangfor CLI output the first time a live device is available, per
    // this file's own established "written without live hardware" posture.
    sections.ssl_vpn = { enabled: null };
    const sslVpnMatch = text.match(/^\s*(?:ssl[\s-]?vpn)\s+(enable|disable)/im);
    if (sslVpnMatch) {
      sections.ssl_vpn.enabled = sslVpnMatch[1].toLowerCase() === 'enable';
    }
  } catch (err) {
