...
let vpnDebugLogged = false;
let vpnTunnelsDebugLogged = false;
...
class CiscoAsaAdapter extends FirewallAdapter {
  ...
  async getRules() {
    ...
    // hit_count NULL (UNMEASURED), never 0. A rule whose ACE simply did not
    // appear in the hit-count output also stays NULL rather than being
    // reported as having had no traffic.
    try {
      ...
      console.warn(
        `[CiscoASA] Hit count collection failed for device ${this.device.id} (hit_count left UNMEASURED/null for all rules): ${err.message}`
      );
    }
  ...
  async getSnmpMetrics() {
    ...
    try {
      ...
      // Each table walk gets its own try/catch — a failed/timed-out walk must
      // degrade only that metric to null, never discard the scalar results
      // (uptime, session count) already obtained above. Matches the pattern
      // lib/adapters/paloalto/index.js's getSnmpMetrics() already uses for its
      // own CPU/memory walks.
      let cpuRows = [];
      try {
        cpuRows = await walkSubtree(session, SNMP_CPU_TABLE_BASE, undefined, targetHost);
      } catch (err) {
        console.warn(`[CiscoASA] SNMP CPU table walk failed for device ${this.device.id}: ${err.message}`);
      }

      let memUsedRows = [];
      try {
        memUsedRows = await walkSubtree(session, SNMP_MEM_USED_TABLE_BASE, undefined, targetHost);
      } catch (err) {
        console.warn(`[CiscoASA] SNMP memory-used table walk failed for device ${this.device.id}: ${err.message}`);
      }

      let memFreeRows = [];
      try {
        memFreeRows = await walkSubtree(session, SNMP_MEM_FREE_TABLE_BASE, undefined, targetHost);
      } catch (err) {
        console.warn(`[CiscoASA] SNMP memory-free table walk failed for device ${this.device.id}: ${err.message}`);
      }
  ...
  // → { active_session_count, raw } — OPTIONAL, see lib/adapters/interface.js's
  // getVpnSessionSummary() contract comment (mirrors
  // lib/adapters/fortinet/{index.js,ssh.js}'s own getVpnSessionSummary() —
  // same optional-capability pattern, same "throw rather than guess a wrong
  // count" discipline — see services/engine-worker.js's vpn-session-poll job,
  // which checks `typeof adapter.getVpnSessionSummary === 'function'` before
  // calling this and treats a thrown error exactly like any other per-device
  // polling failure: logged and skipped, never fatal to the job).
  //
  // DOC-DERIVED, NOT LIVE-VERIFIED — no live ASA device exists anywhere in
  // this project's history (same caveat as getVersion()/getRules() above).
  // `show vpn-sessiondb summary` is the well-known ASA CLI command for a
  // whole-device, de-duplicated VPN session count (see
  // parser.js:parseVpnSessionSummary()'s own header comment for the full
  // parsing reasoning and its documented risk if a real device's table turns
  // out not to use colon-separated columns). Reuses the same
  // looksLikeCliError()/_privilegeErrorMessage() guard getRules()/getConfig()
  // already use for privileged `show` commands on this vendor.
  async getVpnSessionSummary() {
    // Both commands run in ONE SSH shell session (reusing the existing
    // command-running path — no second connection). `show vpn-sessiondb
    // summary` is the AUTHORITATIVE count source, unchanged; `show
    // vpn-sessiondb anyconnect` is the ADDITIVE per-user DETAIL source, whose
    // parsing is strictly best-effort and can never affect the count (see the
    // sessions block below and parser.js:parseVpnSessiondbDetail()).
    const results = await this._run([
      'show vpn-sessiondb summary',
      'show vpn-sessiondb anyconnect',
    ]);
    const output = results[0] ? results[0].output : '';
    const detailOutput = results[1] ? results[1].output : '';

    if (parser.looksLikeCliError(output)) {
      // ⛔ CapabilityUnavailableError, not a bare Error (2026-09-09). this._run()
      // above RESOLVED — the SSH session connected, authenticated, reached an
      // exec prompt and the ASA itself printed this rejection. It describes the
      // account's privilege level (or an ASA with no VPN licence at all, the
      // exact analogue of the FortiGate with no SSL-VPN that this error class
      // was written for), never whether the appliance is up. A connect, login
      // or timeout failure throws out of _run() and never gets this far.
      throw new CapabilityUnavailableError(
        this._privilegeErrorMessage('show vpn-sessiondb summary'),
        { capability: 'vpn_session_summary' }
      );
    }

    // First-integration debug logging (same pattern as getVersion()'s
    // [CiscoASA Debug] log above) — no live device was available during this
    // build, so the raw output is logged once to verify/adjust
    // parseVpnSessionSummary()'s (and parseVpnSessiondbDetail()'s) column-shape
    // assumptions on first real connect.
    if (!vpnDebugLogged) {
      vpnDebugLogged = true;
      console.log('[CiscoASA Debug] show vpn-sessiondb summary raw output:', output);
      console.log('[CiscoASA Debug] show vpn-sessiondb anyconnect raw output:', detailOutput);
    }

    const summary = parser.parseVpnSessionSummary(output);
    if (!summary) {
      // ⛔ CapabilityUnavailableError, not a bare Error (2026-09-09). Same
      // reasoning as the rejection branch above: the command RAN and the ASA
      // answered — this output just isn't a session-summary table. On an ASA
      // with no VPN licence that is the normal answer, and on an unrecognised
      // firmware it is a parser gap. Both are facts about the capability, and
      // neither is evidence the device is down. Refusing to guess the count
      // stays right; the reachability boolean must not absorb the refusal.
      throw new CapabilityUnavailableError(
        `Cisco ASA \`show vpn-sessiondb summary\` on device ${this.device.id} returned output that ` +
          'did not match the expected VPN Session Summary table (no recognizable per-type row or ' +
          'column header) — check the [CiscoASA Debug] log above and update ' +
          'parser.js:parseVpnSessionSummary() for this firmware. Refusing to guess a session count.',
        { capability: 'vpn_session_summary' }
      );
    }

    // Per-user session DETAIL is ADDITIVE and strictly BEST-EFFORT: any doubt
    // (the detail command was rejected, or parsing hits trouble) resolves to an
    // empty array, NEVER a thrown error and NEVER a change to the authoritative
    // count computed above. Mirrors the Palo Alto adapter's sessions handling.
    let sessions = [];
    try {
      if (detailOutput && !parser.looksLikeCliError(detailOutput)) {
        sessions = parser.parseVpnSessiondbDetail(detailOutput);
      }
    } catch (_err) {
      sessions = [];
    }
    ...
    return {
      ...
      sessions,
      raw: {
        source_command: 'show vpn-sessiondb summary',
        detail_command: 'show vpn-sessiondb anyconnect',
        session_detail_count: sessions.length,
        by_category: summary.by_category,
        // Device-reported totals, kept for cross-checking only — NOT used as
        // active_session_count (see parseVpnSessionSummary()'s header
        // comment for why "Active and Inactive" would overstate the true
        // active count).
        total_active_and_inactive: summary.total_active_and_inactive,
        total_cumulative: summary.total_cumulative,
      },
    };
  }

  // → NormalizedTunnel[] — OPTIONAL, and SEPARATE from getVpnSessionSummary()
  // above. Where that method reports a whole-device active SESSION count, this
  // one enumerates established IPSec site-to-site (LAN-to-LAN) TUNNELS from
  // `show vpn-sessiondb l2l`. The engine-worker's tunnel-poll job checks
  // `typeof adapter.getVpnTunnels === 'function'` before calling and treats a
  // thrown error like any other per-device polling failure (logged, skipped).
  //
  // Each element (any field may be null):
  //   { name, peer, status, ike_version, bytes_in, bytes_out, raw }
  //
  // Contract: returns [] (NOT an error) when there are genuinely no L2L
  // tunnels up. MAY throw only when the CLI REJECTS the command (never reached
  // privileged EXEC / unsupported) — a parse that simply finds nothing, or any
  // internal parse error, degrades to [] and never throws. Reuses the same
  // command-running path and looksLikeCliError()/_privilegeErrorMessage()
  // guard the other privileged `show` methods use.
  //
  // DOC-DERIVED, NOT LIVE-VERIFIED (no live ASA exists in this project's
  // history — same caveat as getVersion()/getVpnSessionSummary()). See
  // parser.js:parseVpnSessiondbL2l()'s header for the parsing reasoning.
  async getVpnTunnels() {
    let output = '';
    try {
      const results = await this._run(['show vpn-sessiondb l2l']);
      output = results[0] ? results[0].output : '';
    } catch (err) {
      // A genuine command rejection (privilege/unsupported) is allowed to
      // propagate so the engine-worker records it as a polling failure — the
      // SAME discipline getRules()/getVpnSessionSummary() use for a rejected
      // privileged command. A transport failure surfaces here too; re-throw it
      // rather than silently returning [] (which would look like "no tunnels").
      throw err;
    }

    // A rejected command must THROW (see the contract note above) — do NOT let
    // it fall through to parseVpnSessiondbL2l() and read as "zero tunnels".
    if (parser.looksLikeCliError(output)) {
      throw new Error(this._privilegeErrorMessage('show vpn-sessiondb l2l'));
    }

    // First-integration debug logging (same [CiscoASA Debug] pattern as
    // getVersion()/getVpnSessionSummary()) — logged once to verify/adjust
    // parseVpnSessiondbL2l()'s block-shape assumptions on first real connect.
    if (!vpnTunnelsDebugLogged) {
      vpnTunnelsDebugLogged = true;
      console.log('[CiscoASA Debug] show vpn-sessiondb l2l raw output:', output);
    }

    // Parsing is strictly best-effort from here: an empty result or any parse
    // trouble resolves to [], NEVER a thrown error (the throw cases are only
    // the command-rejection guards above). parseVpnSessiondbL2l() itself also
    // fails closed to [].
    let tunnels = [];
    try {
      tunnels = parser.parseVpnSessiondbL2l(output);
    } catch (_err) {
      tunnels = [];
    }
