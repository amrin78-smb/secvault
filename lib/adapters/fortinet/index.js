    ...
    return this._withSession(async (conn) => {
      ...
      if (!vdoms || vdoms.length <= 1) {
        ...
        if (!Array.isArray(results)) {
          // ⛔ CapabilityUnavailableError, not a bare Error (2026-09-09) — the
          // direct REST twin of FortinetSshAdapter._getVpnSessionSummarySingleVdom()'s
          // already-fixed parse-failure branch. _withSession() has authenticated,
          // _discoverVdoms() has answered, and api.getSslVpnMonitor() RETURNED a
          // response (a transport/auth failure would have rejected before this
          // line). The device was reached; the response simply carried no
          // 'results' array, which on FortiOS almost always means SSL-VPN is not
          // configured. Refusing to guess the count stays right; recording the
          // firewall as unreachable for it does not.
      ...
      if (!anySucceeded) {
        // ⛔ DELIBERATELY a plain Error, unlike the ssh.js twin's identical-looking
        // check. There, every VDOM's command rode in ONE already-resolved SSH
        // batch, so a failure could only be a rejection or a parse miss. Here each
        // VDOM is its OWN HTTP call inside the loop above, so "every VDOM failed"
        // is exactly what a firewall that went away mid-poll looks like. That IS
