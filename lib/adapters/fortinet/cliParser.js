...
function parseFortinetNatRulesOutput(policyOutput, vipOutput, ippoolOutput, interfaceIpByName) {
  ...
  const vipByName = new Map(vipEntries.map((v) => [v.name, v]));

  const ippoolEntries = entriesFromConfigText(ippoolOutput, 'firewall ippool') || [];
  const ippoolByName = new Map(ippoolEntries.map((p) => [p.name, p]));
  ...
  for (const policy of policies) {
    ...
    for (const dstName of dstNames || []) {
      const vip = vipByName.get(dstName);
      if (!vip) continue;
    ...
    if (scalarToken(policy.nat) === 'enable') {
      ...
      if (scalarToken(policy.ippool) === 'enable') {
        ...
        const pool = poolNames.length > 0 ? ippoolByName.get(poolNames[0]) : null;
