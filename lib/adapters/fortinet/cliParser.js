...
// `get router info routing-table all` — per CLAUDE.md's "verify against
// live responses before writing any parser" rule. No FortiGate NAT parser
// here (deliberately): unlike Palo Alto's separate ordered NAT rulebase,
// FortiOS models NAT as a per-policy flag (`set nat enable`, source NAT
// only, usually to the egress interface's own address) plus separate VIP
// objects for destination NAT referenced from a policy's `dstaddr` — a
// structurally different shape that needs its own live verification
// (`show firewall vip` was not captured this session) before writing a
// parser. FortinetSshAdapter simply does not implement getNatRules() (the
// optional-capability convention, same as several vendors already omit
// getVpnSessionSummary() entirely) — a Fortinet hop's NAT is unresolved,
// not silently wrong, until this is built.
