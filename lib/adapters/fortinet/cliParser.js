...
const CLI_ERROR_REGEX =
  /(?:^|\n)\s*(?:Unknown action \d+|command parse error|Command fail\b|-1: Permission denied|node_check_object fail|The CLI command is not|Invalid VDOM name)/i;
...
// → { line, opensMultiline } — opensMultiline is true when the secret's value opened a
//   quote it did not close (PEM private keys span many lines); the caller must then
//   suppress every following line up to the closing quote.
function redactSetLine(rawLine, blockPath) {
  const m = rawLine.match(/^(\s*set\s+(\S+)\s+)([\s\S]*)$/i);
  if (!m) return { line: rawLine, opensMultiline: false };
  ...
  return { line: rawLine, opensMultiline: false };
}
...
function redactConfig(text) {
  ...
  for (const rawLine of text.replace(/\r/g, '').split('\n')) {
    try {
      if (inMultilineSecret) {
        // Inside a secret's multi-line value: emit NOTHING until its closing quote.
        if (countUnescapedQuotes(rawLine) >= 1) inMultilineSecret = false;
        continue;
      }
      ...
      if (/^set\s+/i.test(line)) {
        ...
        if (redacted.opensMultiline) inMultilineSecret = true;
        continue;
      }
 ...
 *
 * The keys ({global, interfaces, ssl_vpn, snmp, admins}) and the FortiOS field names
