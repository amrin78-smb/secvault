...
// GET /api/v2/cmdb/firewall/address?vdom=<name> — named address objects (the
// address book). Feeds index.js's getObjects() (an OPTIONAL adapter capability —
// see lib/adapters/interface.js's contract comment) for
// lib/engines/objectUsage.js's unused/duplicate-object detection. Distinct from
// getFirewallPolicies() above, which returns RESOLVED rule field values, not the
// named objects themselves.
async function getFirewallAddresses(conn, vdom = null) {
  return fortiRequest(conn, '/api/v2/cmdb/firewall/address', { vdom });
}

// GET /api/v2/cmdb/firewall/addrgrp?vdom=<name> — named address GROUP objects.
async function getFirewallAddrgrp(conn, vdom = null) {
  return fortiRequest(conn, '/api/v2/cmdb/firewall/addrgrp', { vdom });
}

// GET /api/v2/cmdb/firewall.service/custom?vdom=<name> — named custom service
// objects. Note the `.service` path segment (a distinct FortiOS cmdb API group
// from plain `firewall/...`) — this is FortiOS's own documented endpoint shape,
// not a typo.
async function getFirewallServiceCustom(conn, vdom = null) {
  return fortiRequest(conn, '/api/v2/cmdb/firewall.service/custom', { vdom });
}

// GET /api/v2/cmdb/firewall.service/group?vdom=<name> — named service GROUP
// objects.
async function getFirewallServiceGroup(conn, vdom = null) {
  return fortiRequest(conn, '/api/v2/cmdb/firewall.service/group', { vdom });
}
...
// GET /api/v2/monitor/vpn/ssl?vdom=<name> — active SSL-VPN session list (a
// MONITOR endpoint, not cmdb — operational/live state, not stored config).
// Added 2026-07-19, doc-derived/NOT yet live-verified (see CLAUDE.md's Live
// Validation Status) — the path itself is a well-documented FortiOS
// monitor endpoint, but the exact per-session field shape is not confirmed;
// getVpnSessionSummary() in index.js only counts entries, sidestepping that
// uncertainty.
//
// ⛔ Bug fixed 2026-07-19, found in the same bug sweep that shipped this
// endpoint originally: omitting `vdom` here returns only the admin token's
// DEFAULT VDOM's sessions on a multi-VDOM box — the identical "silent
// under-count" class of bug CLAUDE.md's own VDOM rule for getFirewallPolicies
// (immediately above) already documents and getRules() already handles via
// per-VDOM iteration. Added the same optional `vdom` param here; index.js's
// getVpnSessionSummary() now enumerates VDOMs the same way getRules() does.
async function getSslVpnMonitor(conn, vdom = null) {
  return fortiRequest(conn, '/api/v2/monitor/vpn/ssl', { vdom });
}

// GET /api/v2/monitor/vpn/ipsec?vdom=<name> — IPSec (site-to-site) tunnel runtime
// state: phase1 `name`, remote gateway (`rgwy`), and per-phase2 `proxyid` entries
// carrying `p2name`/`status`/`incoming_bytes`/`outgoing_bytes`. A MONITOR endpoint
// (live state, not stored config), distinct from getSslVpnMonitor() above which lists
// SSL-VPN user sessions. Doc-derived / NOT yet live-verified (see CLAUDE.md's Live
// Validation Status) — index.js's getVpnTunnels() logs the raw response once via
// '[Fortinet Debug]' and normalizes every field defensively. Same per-VDOM `?vdom=`
// convention as the other monitor/cmdb calls in this file.
async function getIpsecTunnels(conn, vdom = null) {
  return fortiRequest(conn, '/api/v2/monitor/vpn/ipsec', { vdom });
}
...
// GET /api/v2/cmdb/system/ntp — NTP server + sync settings.
async function getNtp(conn) {
  return fortiRequest(conn, '/api/v2/cmdb/system/ntp');
}

// GET /api/v2/cmdb/system/dns — DNS resolver settings.
async function getDns(conn) {
  return fortiRequest(conn, '/api/v2/cmdb/system/dns');
}

// GET /api/v2/cmdb/log/syslogd/setting — remote syslog server settings.
async function getLogSyslogdSetting(conn) {
  return fortiRequest(conn, '/api/v2/cmdb/log/syslogd/setting');
}

// GET /api/v2/cmdb/system/password-policy — admin password complexity policy.
async function getPasswordPolicy(conn) {
  return fortiRequest(conn, '/api/v2/cmdb/system/password-policy');
}

// GET /api/v2/cmdb/system/fortiguard — FortiGuard update/subscription settings.
async function getFortiguard(conn) {
  return fortiRequest(conn, '/api/v2/cmdb/system/fortiguard');
}

// GET /api/v2/cmdb/system/autoupdate/schedule — FortiGuard auto-update schedule.
async function getAutoupdateSchedule(conn) {
  return fortiRequest(conn, '/api/v2/cmdb/system/autoupdate/schedule');
}
...
module.exports = {
  ...
  getNtp,
  getDns,
  getLogSyslogdSetting,
  getPasswordPolicy,
  getFortiguard,
  getAutoupdateSchedule,
