...
function parsePolicies(policies, statsResults, options = {}) {
  ...
  return policies.map((rule, idx) => {
    ...
    // FortiOS policy names are optional. On a multi-VDOM box fall back to the policy
    // id so the prefixed name still identifies the rule rather than reading "[vd1] ".
    const baseName = rule.name || (prefix ? `policy ${policyId !== null ? policyId : '?'}` : null);

    return {
      rule_name: baseName === null ? null : `${prefix}${baseName}`,
