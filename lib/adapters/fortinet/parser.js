...
function parsePolicies(policies, statsResults, options = {}) {
  ...
  return policies.map((rule, idx) => {
    ...
    return {
      ...
      // NULL when the policy-stats fetch produced nothing for this policy --
      // no stats means UNMEASURED, not zero traffic (schema.sql).
      hit_count: stats ? stats.hit_count : null,
      raw_rule: withVdomRaw(rule, vdom),
      vdom,
    };
  });
