...
class FortinetSshAdapter extends FirewallAdapter {
  ...
  _rulesFromPolicyOutput(output, { vdom, prefixRuleName, sequenceStart }) {
    ...
    if (!loggedFirstPolicyOutput) {
      // Bounded preview: a real policy dump is thousands of lines.
      console.log(
        `[Fortinet Debug] SSH \`show firewall policy\`${where} (${output.length} chars). First 2000 chars:\n` +
          output.slice(0, 2000)
      );
  ...
  async _getConfigText() {
    ...
    if (!loggedFirstConfigPreview) {
      ...
      const preview = cliParser.redactConfig(output).slice(0, 2000);
      console.log(
