...
// ⛔ "No address" is NOT an address (2026-09-09). A vendor's placeholder for an
// interface with no IP configured ("N/A", "none", an empty string) was being
// stored verbatim into device_interfaces.ip_address — a TEXT column, so nothing
// errored, and every `ip_address IS NOT NULL` test then counted those
// interfaces as ADDRESSED. Measured on the live fleet the offenders were all
// PAN-OS's "N/A", but the same guard belongs on every collection point rather
// than only the one caught misbehaving. Same class as hit_count's old
// DEFAULT 0: the vendor's "I have none" recorded as if it were a measurement.
// Duplicated per adapter file, per this codebase's per-transport convention.
...
class FortinetSshAdapter extends FirewallAdapter {
  ...
  async _getVpnSessionSummarySingleVdom() {
    ...
    if (cliParser.looksLikeCliError(output)) {
      // ⛔ CapabilityUnavailableError, not a bare Error (2026-09-09). The SSH
      // session CONNECTED, LOGGED IN and RAN the command — this rejection is
      // the device's own answer, printed at its own prompt, so it is evidence
      // about the admin account's access profile and NOT about whether the
      // firewall is reachable. Recording it as `reachable: false` is the exact
      // "an unreadable optional capability is not an unreachable device" bug
      // the parse-failure branch below was already fixed for.
    ...
    if (count === null) {
      ...
      // ⛔ CapabilityUnavailableError, not a bare Error: the SSH session
      // connected, logged in and ran the command, so this says nothing about
      // whether the device is reachable — the most likely cause is simply that
      // SSL-VPN is not configured on it. Thrown as a plain Error until
      // 2026-09-09, which made the VPN poller record the device as UNREACHABLE
      // and the device list report "Failing 0% of polls succeeding" about a
      // firewall answering every other poll. Refusing to guess the count is
      // still right; blaming the device's reachability for it was not.
      throw new CapabilityUnavailableError(
  ...
  async getVpnSessionSummaryMultiVdom(status) {
    ...
    if (vdoms === null) {
      // ⛔ CapabilityUnavailableError (2026-09-09). _discoverVdomsForVpnPoll()
      // only returns null when `show system vdom` ANSWERED and the answer
      // carried no VDOM block; if the SSH batch itself had failed, its _run()
      // would have thrown a plain Error straight past this point. So the
      // transport succeeded and what we are missing is scope on the admin
      // account. Refusing to guess the total is still right — blaming the
      // device's reachability for an access-profile limit is not.
      throw new CapabilityUnavailableError(
    ...
    if (unsafe.length > 0) {
      // ⛔ CapabilityUnavailableError (2026-09-09). We got here by successfully
      // reading the device's own VDOM list, so the transport is proven. This is
      // a refusal to send names we don't trust to the CLI — a fact about the
      // names, not about the firewall answering.
      throw new CapabilityUnavailableError(
    ...
    if (!anySucceeded) {
      // ⛔ CapabilityUnavailableError (2026-09-09), and it is safe here for a
      // reason specific to the SSH transport: every VDOM's command went out in
      // the SINGLE `this._run(commands)` batch above, which has already
      // RESOLVED. A connect/login/timeout failure would have rejected that call
      // and never reached this line. So each per-VDOM failure counted here is a
      // CLI rejection or an unrecognised output shape — the device answered for
      // all of them. (The REST twin in ./index.js issues one HTTP call PER VDOM,
      // so its identical-looking `!anySucceeded` can genuinely be a network
      // failure and deliberately stays a plain Error.)
      throw new CapabilityUnavailableError(
  ...
  async getInterfaces() {
    ...
    try {
      // normalizeInterfaceList: a vendor placeholder for "no address" becomes
      // NULL here, at the collection point. See the helper's own note.
      return { interfaces: normalizeInterfaceList(cliParser.parseFortinetInterfacesOutput(output)) };
    } catch (_err) {
      // ⛔ RETHROW, never an empty set. Returning [] here made
      // collectAndStore's per-method try/catch unreachable: it would have
      // left this device's rows untouched on a failure, but the adapter
  ...
  async getPerformanceMetrics() {
    ...
    if (m.cpuPercent === null && m.memoryPercent === null && m.sessionCount === null && m.uptimeSeconds === null) {
      // ⛔ CapabilityUnavailableError, not a bare Error (2026-09-09). This
      // branch is reachable ONLY after _run() RESOLVED — the SSH session
      // connected, authenticated and ran the command, and `output` is the
      // device's own reply. The transport succeeded; only the METRIC READING
      // was unreadable. The connect/login/timeout failure is the separate
      // catch above, which stays a plain Error precisely because that one IS
      // reachability evidence and must keep counting against the device.
      //
      // This is the fleet's DENSEST heartbeat (every SNMP_POLL_INTERVAL_MINUTES),
      // so filing an unreadable capability here as `reachable: false` outweighs
      // every other connectivity source in worstRate — the exact bug that had
      // the device list reporting "Failing" about a FortiGate answering every
      // other poll. services/engine-worker.js's snmp-poll branches on
      // isCapabilityUnavailable(err) (the FLAG, never instanceof) and records
      // reachable:true with the gap kept in the message.
      throw new CapabilityUnavailableError(
  ...
  _rulesFromPolicyOutput(output, { vdom, prefixRuleName, sequenceStart }) {
    ...
    // debug format. Rules therefore carry hit_count NULL (UNMEASURED). Until
    // 2026-08-25 they carried 0, which the Phase 5 engine read as "unused" --
    // so every policy on every SSH-transport FortiGate produced a fabricated
    // unused-rule finding. The engine now skips `unused` entirely when the
    // count is unmeasured. Documented limitation of the SSH transport; use the
    // REST API transport if unused-rule findings matter for this device.
