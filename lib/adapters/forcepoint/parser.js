...
function classifyNetworkElement(el) {
  ...
  if (type === 'host' || (!type && el.address !== undefined)) return 'host';
  if (
    type === 'network' ||
    (!type && (el.ipv4_network !== undefined || el.ipv6_network !== undefined))
  ) {
    return 'network';
  }
  if (type.includes('range') || (!type && el.ip_range !== undefined)) return 'address_range';
  return 'other';
}
