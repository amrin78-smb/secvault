...
function parsePolicy(policyElement, networkElements, serviceElements) {
  // ⛔ Was a console.warn + `return []` until 2026-09-09 — the failed-read-as-a-
  // fact bug in its purest form. "No policy element provided" is the ABSENCE of
  // an answer, and [] is the affirmative answer "this firewall has no rules";
  // collectAndStore() DELETEs the device's firewall_rules before reinserting,
  // so returning [] here wipes the real ruleset and reports success. index.js
  // now guards its own call site as well — both, deliberately: this function is
  // the last line of defence for any future caller.
  if (!policyElement || typeof policyElement !== 'object' || Array.isArray(policyElement)) {
    throw new Error(
      'Forcepoint parsePolicy: no usable policy element was provided (got ' +
        `${policyElement === null ? 'null' : Array.isArray(policyElement) ? 'an array' : typeof policyElement}` +
        '). This is a failed read, not an empty rulebase — refusing to return [] and let ' +
        "collectAndStore() overwrite the device's stored rules with nothing."
    );
  }
  ...
  return rawRules.map((rule, idx) => {
    if (!rule || typeof rule !== 'object') {
      return {
        ...
        log_enabled: true,
        comment: null,
        // NULL, not 0 -- "not measured" is not "zero hits" (schema.sql).
        hit_count: null,
    ...
    return {
      ...
      // NULL when SMC returned no hit-count field at all -- absent means the
      // SMC did not report it, not that the rule saw no traffic (schema.sql).
      hit_count:
        typeof rule.hit_count === 'number'
          ? rule.hit_count
          : (rule.hits && typeof rule.hits.hit_count === 'number' ? rule.hits.hit_count : null),
      raw_rule: rule,
    };
  });
}
...
// ---------------------------------------------------------------------------
// VPN gateway presence (Task 2, added 2026-07-30, doc-derived)
//
// SMC exposes an engine's VPN identity via an `internal_gateway` href field
// on the engine element itself (per the smc-python client's own
// `engine.internal_gateway` accessor) — this is NOT live-verified against a
// real SMC server (same standing caveat as every other field in this file;
// see CLAUDE.md "Live Validation Status" / the [SMC Debug] "Engine element"
// log in smc.js:getEngines(), which already captures this field's real shape
// on first live connect for confirmation).
//
// Tri-state, mirroring lib/adapters/sangfor/parser.js's `ssl_vpn.enabled`
// precedent (true/false/null, null = "undetected" — the expected, common
// state until this is confirmed live, NOT a failure signal):
//   true  — a recognized VPN-gateway field is present and truthy
//   false — the field is present but empty/falsy
//   null  — no recognized field name found at all. This does NOT mean "no
//           VPN configured" — it means this field-name guess didn't match
//           what this SMC version actually returns. Never treat null as a
//           confirmed negative (same tri-state discipline CLAUDE.md requires
//           for config_applies/compliance predicates).
function detectVpnGatewayConfigured(engineElement) {
  if (!engineElement || typeof engineElement !== 'object') return null;
  if (!Object.prototype.hasOwnProperty.call(engineElement, 'internal_gateway')) return null;

  const value = engineElement.internal_gateway;
...
// ---------------------------------------------------------------------------
// SMC-managed administrator accounts (Task 3, added 2026-07-30, doc-derived)
//
// SMC centrally manages who can administer the SMC/engines — there is no
// separate "local admin account" concept on an individual Forcepoint engine
// the way cisco_asa's `usernames[]` or fortinet's `admins` config section
// capture device-local accounts. This is deliberately SERVER-WIDE, matching
// getObjects()'s network/service element scope (not per-engine like
// getVersion()/getRules()/getConfig()'s engine-identity resolution) — SMC's
// data model has no "which engine does this admin belong to" concept to
// resolve against.
//
// Field names (`name`, `superuser`, `enabled`) are DOC-DERIVED from the
// smc-python client library's AdminUser wrapper, NOT live-verified — same
// standing caveat as every other field mapping in this file. Never throws:
// an unrecognized/missing field degrades that one field to null rather than
// dropping the whole account entry.
//
// ⚠️ MUST be called with ALREADY-REDACTED elements (see redactEngineElement)
// — mirrors parseConfig()'s own contract. Admin/user elements are exactly
// the kind of object CLAUDE.md's redaction rule exists for: any password
// hash / RADIUS shared secret / one-time-password field on a real admin_user
// element would match SECRET_KEY_PATTERN and come back as '<redacted>'
// already, so this function itself never sees (and can't leak) raw secret
// values, whatever the real field name for one turns out to be.
function summarizeAdministrators(redactedAdminElements) {
  const elements = safeArray(redactedAdminElements);
  return elements
    .filter((el) => el && typeof el === 'object' && typeof el.name === 'string')
    .map((el) => ({
      name: el.name,
      superuser: typeof el.superuser === 'boolean' ? el.superuser : null,
...
module.exports = {
  ...
  detectVpnGatewayConfigured,
  summarizeAdministrators,
