...
class ForcepointAdapter extends FirewallAdapter {
  ...
  async getRules() {
    ...
    if (!policyHref) {
      throw new Error(
        `Forcepoint rule collection failed for device ${this.device.id} ("${this.device.name}") — ` +
          ...
          'SMC version (see the [SMC Debug] entries in engine.log) and update ' +
    ...
    // ⛔ Only the THROW branch was guarded (fixed 2026-09-09). smc.getPolicy()
    // does not have to throw to fail: it returns null for an HTTP-200 whose
    // body is empty or not JSON — a proxy, a captive portal, an SMC that
    // answered the href with something unexpected. That null flowed straight
    // into parser.parsePolicy(null, ...), which warned and returned [], and
    // collectAndStore() DELETEs this device's firewall_rules before reinserting
    // whatever getRules() returns. The comment directly above named this exact
    // failure and then guarded only the catch. A resolved-but-unusable response
    // is a failed READ, identical in consequence to a thrown one, so it throws
    // here too.
    if (!policyElement || typeof policyElement !== 'object' || Array.isArray(policyElement)) {
      throw new Error(
        `Forcepoint rule collection failed for device ${this.device.id} ("${this.device.name}") — ` +
          `the assigned policy href resolved but returned no usable policy element (got ` +
          `${policyElement === null ? 'null' : Array.isArray(policyElement) ? 'an array' : typeof policyElement}` +
          '; smc.js returns null for any HTTP-200 with an empty or non-JSON body). Refusing to ' +
          'treat that as "this device has zero rules" — the stored ruleset is left untouched. ' +
          'Check the [SMC Debug] log and SMC reachability.'
      );
    }
  ...
  async getConfig() {
    ...
    const { raw, parsed } = parser.parseConfig(redactedElement);

    // Task 2 (2026-07-30, doc-derived — see parser.js:detectVpnGatewayConfigured):
    // tri-state "does this engine have an SMC VPN gateway identity configured"
    // signal, computed from data already in hand (no extra SMC call). Reads
    // from the already-redacted element — internal_gateway is a href/id field,
    // not secret-shaped, so redaction never removes it.
    const vpnGatewayConfigured = parser.detectVpnGatewayConfigured(redactedElement);

    // Task 3 (2026-07-30, doc-derived — see smc.js:getAdministrators /
    // parser.js:summarizeAdministrators): SMC-managed administrator account
    // listing. Server-wide, not per-engine (see summarizeAdministrators'
    // comment). Best-effort: a failure here must never break config
    // collection — getConfig()'s overall success still gates Phase 6
    // diff/backup and Phase 7 compliance, neither of which depends on this
    // field existing. Redacted through the SAME redactEngineElement pass as
    // the engine element itself before any field is read off it, per
    // CLAUDE.md's "any new raw element data added to config_parsed must flow
    // through the same redaction path" rule.
    let smcAdministrators = null;
    try {
      const adminElements = await smc.getAdministrators(conn);
      const redactedAdminElements = parser.redactEngineElement(adminElements);
      smcAdministrators = parser.summarizeAdministrators(redactedAdminElements);
    } catch (err) {
      console.warn(
        `[Forcepoint] getConfig(): failed to fetch SMC administrator accounts for device ${this.device.id}: ${err.message}`
      );
      smcAdministrators = null;
    }

    // `raw` stays exactly what parser.parseConfig() built (JSON of the
    // redacted engine element only) — the two fields below are SecVault-
    // computed/separately-fetched additions that don't exist as part of the
    // engine element itself, so folding them into `raw` would misrepresent
    // it as "the engine's own raw data". Spread into a NEW object (never
    // mutate `parsed`/`redactedElement` in place) and use `smc_`-prefixed
    // key names specifically to avoid any chance of colliding with a real,
    // unprefixed SMC field name already present on the spread engine element.
    return {
      raw,
      parsed: {
        ...parsed,
        smc_vpn_gateway_configured: vpnGatewayConfigured,
        smc_administrators: smcAdministrators,
      },
    };
  }

  // getVpnSessionSummary() — INTENTIONALLY NOT IMPLEMENTED (Task 1, researched
  // 2026-07-30).
  //
  // FirewallAdapter's optional getVpnSessionSummary() contract (see
  // lib/adapters/interface.js and lib/adapters/fortinet/index.js's own
  // implementation for the reference shape: { active_session_count, raw })
  // needs a real-time "how many VPN/SSL-VPN sessions are active right now"
  // count. Every OTHER capability in this file reaches the SMC through the
  // SAME simple, generic pattern: GET a /api/elements/<type> listing or
  // follow an element's own `href` — exercised five times over in this file
  // (engines, fw_policy, network_elements, service_elements, admin_user).
  //
  // SMC's actual mechanism for anything resembling live session/connection
  // state is a fundamentally DIFFERENT API surface — the Monitoring/Log
  // Server query API, an async "submit a query definition, then poll a fetch
  // endpoint for results" flow, not a simple GET. Its exact schema (query
  // field IDs, filter syntax, endpoint path, and whether it even models "VPN
  // session" as its own queryable entity distinct from raw connection/log
  // records) is not something this build could confirm with reasonable
  // confidence without a live SMC instance or its official API reference —
  // inventing that schema would be exactly the guessed-vendor-endpoint risk
  // CLAUDE.md's "verify all field names against live responses before
  // writing any parser" rule warns against. Unlike every other doc-derived
  // field in this file (which degrades safely to null/a warning on a wrong
  // guess), a fabricated query-submission endpoint would either silently
  // 404 on every poll or — worse — appear to partially "work" against some
  // unrelated endpoint that happens to accept the payload shape.
  //
  // Skipped. If a live SMC instance or its official (non-public) Monitoring
  // API reference ever becomes available, re-research this against
  // Forcepoint's actual documented VPN SA / session monitoring query type
  // before attempting an implementation — do not guess the schema from this
  // comment alone.
  ...
  async getSnmpMetrics() {
    ...
    try {
      ...
      // Each table walk is independently try/catched — these OIDs are
      // doc-derived and NOT yet live-verified (see the class comment above),
      // so one table failing/timing out on a real engine is an expected
      // first-contact outcome, not a hypothetical. A single failed subtree
      // must only null that one field, never discard the scalar/other
      // subtree results already obtained in this same poll.
      let cpuRows = [];
      let cpuRowsError = null;
      try {
        cpuRows = await walkSubtree(session, FW_CPU_TOTAL_TABLE_BASE, undefined, targetHost);
      } catch (err) {
        cpuRowsError = err.message;
        console.warn(
          `[Forcepoint] getSnmpMetrics(): CPU table walk failed for device ${this.device.id}: ${err.message}`
        );
      }

      let memTotalRows = [];
      let memTotalRowsError = null;
      try {
        memTotalRows = await walkSubtree(session, FW_MEM_TOTAL_BASE, undefined, targetHost);
      } catch (err) {
        memTotalRowsError = err.message;
        console.warn(
          `[Forcepoint] getSnmpMetrics(): memory-total table walk failed for device ${this.device.id}: ${err.message}`
        );
      }

      let memUsedRows = [];
      let memUsedRowsError = null;
      try {
        memUsedRows = await walkSubtree(session, FW_MEM_USED_BASE, undefined, targetHost);
      } catch (err) {
        memUsedRowsError = err.message;
        console.warn(
          `[Forcepoint] getSnmpMetrics(): memory-used table walk failed for device ${this.device.id}: ${err.message}`
        );
      }

      let connRows = [];
      let connRowsError = null;
      try {
        connRows = await walkSubtree(session, FW_CONN_NUMBER_BASE, undefined, targetHost);
      } catch (err) {
        connRowsError = err.message;
        console.warn(
          `[Forcepoint] getSnmpMetrics(): connection-count table walk failed for device ${this.device.id}: ${err.message}`
        );
      }
