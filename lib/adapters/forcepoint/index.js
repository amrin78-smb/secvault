...
async function collectAndStore(device, pool) {
  ...
  // Phase 5: rule hygiene analysis runs after every rule pull (arch doc:
  // "Rule analysis engine — after each rule pull"). Findings are rewritten
  // per device, so running it here keeps them consistent with the freshly
  // reinserted firewall_rules rows (whose old findings just cascade-deleted).
  try {
