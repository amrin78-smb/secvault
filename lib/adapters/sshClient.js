...
// NO `m` flag, deliberately. A shell prompt only ever appears at the very END
// of the received buffer; with `m`, `$` matches at every line end, so ANY line
// of command output that happens to end in > # $ or % looks like a prompt and
// the read returns early with a SILENTLY TRUNCATED result. That is not
// hypothetical for firewall configs: `#` is Cisco ASA's conventional `banner
// motd` delimiter, so `banner motd ####` mid-config would end the read.
// Verified: with `m` a partial ASA running-config containing a banner matches;
// without it, that same buffer correctly does not, while every real prompt
// shape (`asa> `, `asa#`, `fw/pri/act# ` failover, `fw(config)# `, trailing
// newline) still matches, because `\s*$` still absorbs trailing whitespace.
//
// Residual risk (accepted, needs a live device to tune): if a network chunk
// boundary lands exactly at the end of a `#`-terminated config line, that
// buffer transiently ends in `#` and still matches. Any prompt heuristic on a
// raw shell channel has this failure mode; removing `m` shrinks the window
// from "every banner line, always" to "an exact chunk-boundary coincidence".
// Adapters can pass their own promptRegex if a device needs a stricter shape.
      ...
      client.on('ready', () => {
        ...
        client.shell(ptyOptions, (err, stream) => {
          ...
          (async () => {
            ...
            if (enablePassword) {
              // Command-injection guard — found in a follow-up bug sweep
              // (2026-07-17). The initial login password never reaches this
              // code path (it's authenticated at the SSH protocol level by
              // ssh2, not written to the shell stream) but enablePassword
              // IS written verbatim via send() below. An embedded \r/\n in a
              // stored credential (corrupted/malformed entry, not attacker
              // input, but still not validated anywhere upstream) would
              // inject additional lines as separate commands into a
              // privileged root shell. Refuse rather than silently strip —
              // stripping could send a subtly wrong password and produce a
              // confusing auth failure instead of a clear one.
              if (/[\r\n]/.test(enablePassword)) {
                throw new Error(
                  'enablePassword contains an embedded newline/carriage-return — refusing to write it to an interactive privileged shell. Check the stored SSH credential for this device.'
                );
              }
              buffer = '';
              send('enable');
